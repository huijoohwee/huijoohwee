...
export function buildRichMediaPreviewSemanticKey(parts) {
...
export function normalizeRemoteVideoFrameSeconds(value) {
...
function formatRemoteVideoFrameSecondsForFileName(value) {
    return String(normalizeRemoteVideoFrameSeconds(value) ?? 0).replace(/\./g, '_');
}
export function normalizeRemoteVideoFrameFormat(value) {
...
export function buildRemoteVideoTimestampFramePreviewDescriptor(args) {
    ...
    if (!sourceUrl || startSeconds == null)
        return null;
...
const normalizeYouTubeIdLikeValue = (value) => {
    const raw = String(value || '').trim();
    if (!raw)
        return null;
    return /^[A-Za-z0-9_-]{6,128}$/.test(raw) ? raw : null;
};
const readYouTubeIdFromUrl = (href) => {
    try {
        ...
        if (host === 'youtube.com' ||
            ...
            if (v)
                return normalizeYouTubeIdLikeValue(v);
            const parts = url.pathname.split('/').filter(Boolean);
            const head = parts[0] || '';
            const id = parts[1] || '';
            if ((head === 'embed' || head === 'shorts' || head === 'live') && id)
                return normalizeYouTubeIdLikeValue(id);
            if (head === 'watch') {
...
export function stripYouTubeUrlTrailingPunctuation(value) {
...
export function parseYouTubeStartSeconds(href) {
...
export function formatMediaTimestampSeconds(seconds) {
...
export function buildYouTubeEmbedUrl(href, opts) {
...
export function buildYouTubeThumbnailPreviewDescriptor(value) {
...
export function buildYouTubeTimestampPreviewDescriptor(href, opts) {
    ...
    if (!id || startSeconds == null || !Number.isFinite(startSeconds))
        return null;
...
export function buildYouTubeTimestampFramePreviewDescriptor(href, opts) {
    const sourceUrl = stripYouTubeUrlTrailingPunctuation(href);
    const id = getYouTubeId(sourceUrl);
    const startSeconds = parseYouTubeStartSeconds(sourceUrl);
    if (!id || startSeconds == null || !Number.isFinite(startSeconds))
...
export function getTwitterStatusId(href) {
    try {
...
export function buildTwitterEmbedUrl(href) {
    const id = getTwitterStatusId(href);
    if (!id)
        return null;
    return `https://platform.twitter.com/embed/Tweet.html?id=${id}`;
}
export function getVimeoId(href) {
    try {
        const url = new URL(String(href || '').trim());
        if (!String(url.hostname || '').toLowerCase().endsWith('vimeo.com'))
            return null;
        const parts = url.pathname.split('/').filter(Boolean);
...
export function buildVimeoEmbedUrl(href) {
    const id = getVimeoId(href);
    if (!id)
        return null;
    return `https://player.vimeo.com/video/${id}`;
}
export function getBilibiliVideoId(href) {
    try {
        const url = new URL(String(href || '').trim());
        const host = String(url.hostname || '').toLowerCase();
        if (!(host === 'www.bilibili.com' || host.endsWith('.bilibili.com')))
            return null;
...
export function buildBilibiliEmbedUrl(href) {
