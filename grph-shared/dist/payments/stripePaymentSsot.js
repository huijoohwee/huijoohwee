...
export const STRIPE_CHECKOUT_MODES = ['payment', 'subscription'];
...
export const STRIPE_PAYMENT_D1_MIGRATION_APPLY_COMMAND_TEMPLATE = 'npx wrangler d1 migrations apply <DATABASE> --remote --config <WRANGLER_CONFIG>';
...
export const readStripePaymentServerKey = (env) => {
...
export const readStripeWebhookSigningSecret = (env) => {
    const secret = readEnvString(env, STRIPE_PAYMENT_ENV_KEYS.webhookSecret);
    return secret.startsWith('whsec_') ? secret : '';
};
export const normalizeStripeCheckoutReturnOrigin = (value) => String(value || '').trim().replace(/\/+$/g, '');
export const readStripeCheckoutReturnOrigin = (env) => normalizeStripeCheckoutReturnOrigin(readEnvString(env, STRIPE_PAYMENT_ENV_KEYS.checkoutReturnOrigin));
export const validateStripeCheckoutReturnOrigin = (value) => {
...
export const readStripeCheckoutRequestUrlOrigin = (requestUrl) => {
    try {
...
const readStripeCheckoutMode = (env) => {
    const rawMode = readEnvString(env, STRIPE_PAYMENT_ENV_KEYS.checkoutMode);
    if (!rawMode)
        return 'payment';
    return STRIPE_CHECKOUT_MODES.includes(rawMode) ? rawMode : null;
};
export function resolveStripeCheckoutServerConfig(env) {
...
export const normalizeStripeCheckoutQuantity = (value) => {
    const raw = typeof value === 'number' ? value : Number(value);
    if (!Number.isFinite(raw))
        return 1;
    return Math.max(1, Math.min(STRIPE_CHECKOUT_MAX_QUANTITY, Math.floor(raw)));
};
const normalizeStripeCheckoutMetadataValue = (value) => String(value || '').trim();
export const readStripeCheckoutWorkspaceId = (payload) => normalizeStripeCheckoutMetadataValue(payload.workspaceId);
export const readStripeCheckoutAgenticCommerceSessionId = (payload) => normalizeStripeCheckoutMetadataValue(payload.agenticCommerceSessionId);
export const readStripeCheckoutClientReferenceId = (payload) => readStripeCheckoutAgenticCommerceSessionId(payload) || readStripeCheckoutWorkspaceId(payload);
export const readStripeCheckoutStripeIdempotencyKey = (payload) => readStripeCheckoutAgenticCommerceSessionId(payload);
export const readStripeCheckoutReadinessSmoke = (payload) => payload.readinessSmoke === true;
export const readStripeCheckoutExpectedSessionTotal = (payload) => {
    ...
    if (!Number.isFinite(amount) || amountTotal <= 0 || !/^[a-z]{3}$/.test(currency))
        return null;
    return { amountTotal, currency };
};
export const validateStripeCheckoutSessionCreatePayload = (payload) => {
...
export const validateStripeCheckoutExpectedTotalForConfig = (payload, config) => {
...
export function normalizeStripeCheckoutSuccessUrl(rawUrl) {
    const trimmed = String(rawUrl || '').trim();
    if (!trimmed || trimmed.includes(STRIPE_CHECKOUT_SESSION_ID_TOKEN))
        return trimmed;
    try {
        new URL(trimmed);
    }
...
export function isStripeCheckoutReturnUrlAllowed(rawUrl, serverOrigin, configuredOrigin) {
    try {
...
export function buildStripeCheckoutSessionCreateForm(payload, config) {
...
export function buildStripeCheckoutSessionStatusUrl(sessionId) {
