...
import { AGENTIC_COMMERCE_SOLANA_PAY_KEY, AGENTIC_COMMERCE_SOLANA_PAY_ENV_KEYS, AGENTIC_COMMERCE_SOLANA_PAY_SETTLE_PATH, } from './agenticCommerceSolanaPaySsot.js';
...
export const AGENTIC_COMMERCE_ACP_DISCOVERY_SERVICES = ['checkout'];
export const AGENTIC_COMMERCE_ACP_DISCOVERY_TRANSPORTS = ['rest'];
...
export const readAgenticCommerceSellerId = (env, requestUrl) => {
...
export const isAgenticCommerceWeb3Enabled = (env) => {
...
export const normalizeAgenticCommerceCurrency = (value) => {
    const currency = String(value || '').trim().toLowerCase();
    return /^[a-z][a-z0-9]{1,11}$/.test(currency) ? currency : '';
};
export const normalizeAgenticCommerceAmount = (value) => {
    const amount = typeof value === 'number' ? value : Number(value);
    return Number.isFinite(amount) ? Math.max(0, Math.floor(amount)) : 0;
};
const normalizeAgenticCommerceBaseUrl = (value) => String(value || '').trim().replace(/\/+$/g, '');
const buildAgenticCommerceUrl = (baseUrl, path) => `${normalizeAgenticCommerceBaseUrl(baseUrl)}${path}`;
const buildAgenticCommerceMainPanelReadinessRow = (sectionId, id, label, value, path = value.startsWith('/') ? value : null) => ({
    id,
    label,
    value,
    path,
...
const buildAgenticCommerceMainPanelReadinessSection = (id, title, rows) => ({
    id,
    title,
    rows,
});
export const buildAgenticCommerceMainPanelReadiness = () => {
    const sections = [
        buildAgenticCommerceMainPanelReadinessSection('overview', 'Overview', [
            buildAgenticCommerceMainPanelReadinessRow('overview', 'acp-discovery', 'ACP discovery', `GET ${AGENTIC_COMMERCE_ROUTE_PATHS.acpDiscovery}`, AGENTIC_COMMERCE_ROUTE_PATHS.acpDiscovery),
            buildAgenticCommerceMainPanelReadinessRow('overview', 'acp-config', 'ACP config', `GET ${AGENTIC_COMMERCE_ROUTE_PATHS.acpConfig}`, AGENTIC_COMMERCE_ROUTE_PATHS.acpConfig),
            buildAgenticCommerceMainPanelReadinessRow('overview', 'api-version', 'API version', AGENTIC_COMMERCE_API_VERSION, null),
        ]),
        buildAgenticCommerceMainPanelReadinessSection('discovery', 'Discovery', [
            buildAgenticCommerceMainPanelReadinessRow('discovery', 'ucp-profile', 'UCP profile', AGENTIC_COMMERCE_ROUTE_PATHS.ucpProfile),
            buildAgenticCommerceMainPanelReadinessRow('discovery', 'mpp-openapi', 'MPP OpenAPI', AGENTIC_COMMERCE_ROUTE_PATHS.mppOpenApi),
            buildAgenticCommerceMainPanelReadinessRow('discovery', 'x402-payment-required', 'x402 payment required', AGENTIC_COMMERCE_ROUTE_PATHS.x402PaymentRequired),
            buildAgenticCommerceMainPanelReadinessRow('discovery', 'x402-api-root', 'x402 API root', AGENTIC_COMMERCE_ROUTE_PATHS.x402ApiRoot),
        ]),
        buildAgenticCommerceMainPanelReadinessSection('sessions', 'Sessions', [
            buildAgenticCommerceMainPanelReadinessRow('sessions', 'checkout-sessions', 'Checkout sessions', AGENTIC_COMMERCE_ROUTE_PATHS.checkoutSessions),
            buildAgenticCommerceMainPanelReadinessRow('sessions', 'stripe-webhook', 'Stripe webhook', STRIPE_PAYMENT_ROUTE_PATHS.webhook),
        ]),
        ...
        buildAgenticCommerceMainPanelReadinessSection('governance', 'Governance', [
            buildAgenticCommerceMainPanelReadinessRow('governance', 'openbox-ingest', 'OpenBOX ingest', AGENTIC_COMMERCE_ROUTE_PATHS.openboxIngest),
            buildAgenticCommerceMainPanelReadinessRow('governance', 'risk-source', 'Risk source', 'OpenBOX risk signal', null),
        ]),
        buildAgenticCommerceMainPanelReadinessSection('proofs', 'Proofs', [
            buildAgenticCommerceMainPanelReadinessRow('proofs', 'harness-proof', 'Harness proof', AGENTIC_COMMERCE_ROUTE_PATHS.commerceProofArtifact),
            buildAgenticCommerceMainPanelReadinessRow('proofs', 'trace-artifact', 'Trace artifact', AGENTIC_COMMERCE_ROUTE_PATHS.commerceTraceArtifact),
        ]),
    ];
...
export const AGENTIC_COMMERCE_MAIN_PANEL_READINESS = buildAgenticCommerceMainPanelReadiness();
export const buildAgenticCommerceDepositAddress = (env, sessionId) => {
...
export const readAgenticCommerceX402Asset = (env) => (readEnvString(env, AGENTIC_COMMERCE_ENV_KEYS.x402Asset) || AGENTIC_COMMERCE_DEFAULT_X402_ASSET);
export const readAgenticCommerceX402Amount = (env) => {
...
export const readAgenticCommerceX402Price = (env) => (readEnvString(env, AGENTIC_COMMERCE_ENV_KEYS.x402Price) || AGENTIC_COMMERCE_DEFAULT_X402_PRICE);
export const readAgenticCommerceX402FacilitatorUrl = (env) => {
