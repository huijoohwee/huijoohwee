...
version_control:
  ...
  surfaces:
    main_panel: "history"
    bottom_panel: "gitGraph-chart"
    floating_panel: "gitGraph-list"
  restore_actions: ["undo", "redo", "select-version"]
  generated_mermaid_is_projection: true
  branch_merge_supported: false
  static_commit_fixtures_forbidden: true
credential_policy:
...
kgParserRoutingContract:
  ...
  surfaces:
    ...
    - "MainPanel: History"
    - "BottomPanel: GitGraph Chart"
    - "FloatingPanel: GitGraph List"
...
This starter supports runtime-derived linear Version History without storing sample commits in frontmatter.

| Surface | Projection | Action |
|---|---|---|
| MainPanel History | Ordered runtime entries | Inspect or restore a selected version |
| BottomPanel GitGraph Chart | Derived Mermaid GitGraph | Select a version visually |
| FloatingPanel GitGraph List | Ordered version rows | Select and restore a version |
| Toolbar and panel controls | Shared history index | Undo or Redo |

Version commits are semantic-change-only, parent-linked, and bounded to `100` local-session entries. Each entry owns the graph, graph-field settings, active Markdown, and active source-file snapshot needed for atomic restoration. The GitGraph chart is projection-only; forbid static commit fixtures, duplicate history arrays, whole-workspace source snapshots, and renderer-local restore state.

This contract does not claim reload durability, collaboration, branches, merges, conflict resolution, or Git compatibility. Add those capabilities only through a new runtime-backed contract with focused proof.
