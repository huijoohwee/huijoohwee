...
const DEFAULT_BOARD_FILE = "singabldr.board.json";
...
function createCameraRig(camera, domEl) {
  const state = {
  ...
  domEl.addEventListener("wheel", (ev) => {
    ev.preventDefault();
...
async function buildLandInstancedMesh({
  ...
  const cols = Math.max(1, Math.floor(gridWidth / voxelSize));
  const rows = Math.max(1, Math.floor(gridHeight / voxelSize));

  const isLand = makePolygonTester(polygons);
  const proj = makeLonLatProjector(bounds, cols, rows);
  ...
  // Upper-bound allocation; actual visible count is set after build.
  const max = cols * rows;
  ...
  for (let iy = 0; iy < rows; iy++) {
    ...
    for (let ix = 0; ix < cols; ix++) {
      ...
      const { lon, lat } = proj.gridToLonLat(ix, iy);
      if (!isLand(lon, lat)) {
        processed++;
        continue;
      }

      const x = (ix - cols / 2) * voxelSize;
      const z = (iy - rows / 2) * voxelSize;
...
function buildPoiMarkers(features, proj, voxelSize) {
...
async function main() {
  ...
  function onProgress(p) {
    ...
    setStatus(
      "Generating Voxels…",
      `${p.phase} • ${pct}% • visible=${p.visible ?? 0} • ${p.elapsedMs ?? 0}ms`
    );
  }
  ...
  try {
    ...
    const polygons = board?.scene?.polygons || [];
    const grid = getGridFromBoard(board);

    // Build COARSE first (fast start), then FINE in the background.
    setStatus("Generating Voxels…", "coarse (fast start)");
    const coarseVoxel = grid.voxelSize * COARSE_MULTIPLIER;

    const coarseBuild = await buildLandInstancedMesh({
      name: "coarse",
      bounds,
      polygons,
      gridWidth: grid.width,
      gridHeight: grid.height,
      voxelSize: coarseVoxel,
      color: 0x00cec9,
      onProgress,
      signal,
    });
    ...
    coarse = {
      voxelSize: coarseVoxel,
      proj: coarseBuild.proj,
      group: new THREE.Group(),
    };
    coarse.group.add(coarseBuild.mesh);
    scene.add(coarse.group);

    // POIs on coarse projector (good enough for markers).
    poi = buildPoiMarkers(board?.features || [], coarseBuild.proj, coarseVoxel);
    scene.add(poi);

    // Center camera on Singapore-ish centroid (lon/lat midpoint).
    ...
    // Kick off fine build (async, chunked). This is the expensive part; it should never freeze.
    setStatus("Generating Voxels…", "fine (building in background)");
    const fineVoxel = grid.voxelSize;

    // Coalesce in case something triggers a rebuild burst.
    coalesce("build:fine", async () => {
      try {
        const fineBuild = await buildLandInstancedMesh({
          name: "fine",
          bounds,
          polygons,
          gridWidth: grid.width,
