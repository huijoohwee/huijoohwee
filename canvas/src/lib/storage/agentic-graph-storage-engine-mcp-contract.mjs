export const AGENTIC_OS_STORAGE_GIT_INVOCATION_PREFIX = Object.freeze([
  '/git.run',
...
export const AGENTIC_OS_FILE_SYNC_INVOCATION_PREFIX = Object.freeze([
  '/file.sync',
  '@persisted-cache',
  '@file-sync-provider',
...
export const AGENTIC_OS_STORAGE_BROWSER_TOOL_IDS = Object.freeze({
...
export const AGENTIC_OS_STORAGE_LOCAL_TOOL_NAMES = Object.freeze({
  gitRun: 'agentic-graph.git.run',
  fileSyncRun: 'agentic-graph.file.sync',
})

export const AGENTIC_OS_STORAGE_GIT_OPERATIONS = Object.freeze([
  'clone',
  'fetch',
  'commit',
  'push',
...
export const AGENTIC_OS_FILE_SYNC_DIRECTIONS = Object.freeze(['pull', 'push'])
...
export const AGENTIC_OS_STORAGE_GIT_CONTROL_INPUT_SCHEMA = Object.freeze({
  type: 'object',
  additionalProperties: false,
  properties: {
    invocation: INVOCATION_SCHEMA,
    operation: { type: 'string', enum: AGENTIC_OS_STORAGE_GIT_OPERATIONS },
    remoteId: ID_SCHEMA,
    canonicalPathScope: CANONICAL_PATH_SCHEMA,
    baseRef: GIT_REF_SCHEMA,
    message: { type: 'string', minLength: 1, maxLength: 500, pattern: '\\S' },
...
export const AGENTIC_OS_FILE_SYNC_CONTROL_INPUT_SCHEMA = Object.freeze({
  type: 'object',
  additionalProperties: false,
  properties: {
    invocation: INVOCATION_SCHEMA,
    direction: { type: 'string', enum: AGENTIC_OS_FILE_SYNC_DIRECTIONS },
...
export const parseAgenticGraphGitInvocation = invocation => {
  ...
  requirePrefix(tokens, AGENTIC_OS_STORAGE_GIT_INVOCATION_PREFIX)
  const values = parseKeyValues(
    tokens.slice(AGENTIC_OS_STORAGE_GIT_INVOCATION_PREFIX.length),
  ...
  if (!AGENTIC_OS_STORAGE_GIT_OPERATIONS.includes(operation)) {
...
export const parseAgenticGraphFileSyncInvocation = invocation => {
  const tokens = normalize(invocation).split(/\s+/).filter(Boolean)
  requirePrefix(tokens, AGENTIC_OS_FILE_SYNC_INVOCATION_PREFIX)
  const values = parseKeyValues(
    tokens.slice(AGENTIC_OS_FILE_SYNC_INVOCATION_PREFIX.length),
  ...
  if (!AGENTIC_OS_FILE_SYNC_DIRECTIONS.includes(direction)) {
...
export const normalizeAgenticGraphGitControlInput = input => {
  ...
  if (!AGENTIC_OS_STORAGE_GIT_OPERATIONS.includes(operation)) throw new Error('Git operation is invalid.')
  if (!isSafeId(remoteId)) throw new Error('Git remote id is invalid.')
  if (!isSafeCanonicalPath(canonicalPathScope)) throw new Error('Git canonical path scope is invalid.')
  if (!isSafeGitRef(baseRef)) throw new Error('Git base ref is invalid.')
...
export const normalizeAgenticGraphFileSyncControlInput = input => {
  ...
  if (!AGENTIC_OS_FILE_SYNC_DIRECTIONS.includes(direction)) throw new Error('File-sync direction is invalid.')
  if (!isSafeId(providerId)) throw new Error('File-sync provider id is invalid.')
  if (!isSafeCanonicalPath(prefix)) throw new Error('File-sync prefix is invalid.')
  return { direction, providerId, prefix }
}
