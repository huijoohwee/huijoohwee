export const AGENTIC_OS_AGENT_READY_PROMPT_NAMES = Object.freeze({
...
const PROMPT_CONTRACTS = Object.freeze([
  Object.freeze({
    name: AGENTIC_OS_AGENT_READY_PROMPT_NAMES.researchSourceFiles,
    title: 'Research agentic-graph Source Files',
    description: 'Guide an MCP host through read-only agentic-graph Source Files research using search and fetch with citation-ready URLs.',
  ...
  Object.freeze({
    name: AGENTIC_OS_AGENT_READY_PROMPT_NAMES.inspectAgentSurface,
    title: 'Inspect agentic-graph Agent Surface',
    description: 'Guide an MCP host through read-only inspection of agentic-graph agent, MCP, and MCP Apps readiness metadata.',
    arguments: Object.freeze([
      Object.freeze({
        name: 'focus',
...
const buildSourceFilesResearchPromptText = (args = {}) => {
  ...
  return [
    `Research agentic-graph Source Files for: ${query}`,
    '',
...
const buildAgentSurfaceInspectionPromptText = (args = {}) => {
  const focus = readPromptArg(args, 'focus')
  return [
    'Inspect the agentic-graph agent-ready surface through the read-only inspect_agent_surface tool.',
    '',
...
export const getAgenticGraphAgentReadyPrompt = (name, args = {}) => {
  ...
  if (!contract) {
    throw new Error(`Unknown agentic-graph MCP prompt: ${normalizeString(name)}`)
  }
  if (contract.name === AGENTIC_OS_AGENT_READY_PROMPT_NAMES.researchSourceFiles) {
    return {
      description: contract.description,
      messages: [buildPromptMessage(buildSourceFilesResearchPromptText(args))],
    }
  }
  if (contract.name === AGENTIC_OS_AGENT_READY_PROMPT_NAMES.inspectAgentSurface) {
    return {
      description: contract.description,
      messages: [buildPromptMessage(buildAgentSurfaceInspectionPromptText(args))],
    }
  }
  throw new Error(`Unhandled agentic-graph MCP prompt: ${contract.name}`)
}
