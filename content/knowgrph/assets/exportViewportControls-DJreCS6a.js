import{cb as Ke,cc as Ze,az as $e,a9 as Je,aa as Qe,cd as et,ce as tt,cf as rt,cg as ge,ch as ue}from"./Canvas-CUdICYaC.js";import{L as ve,e as nt,r as Q,bF as at,m as it,o as ot,l as dt,j as st,aE as lt,n as ct}from"./index-mXzroygG.js";const gt=[`(function(){
...
`),e=x(e,"var kgResolveMediaSrc = function(url, kind){",`var kgIsWeChatHotlinkProtectedAssetUrl = function(absUrl){
    ...
    try {
      var kgPanelMode = '';
      try { kgPanelMode = String((window && window.localStorage) ? (window.localStorage.getItem('${ve.renderRichMediaPanelMode}') || '') : ''); } catch (e0) { kgPanelMode = ''; }
...
</html>`}const Et=t=>{const r=String(t||"").toLowerCase();return r==="href"||r==="xlink:href"||r==="src"||r==="poster"},Mt=t=>String(t||"").toLowerCase()==="srcset",Bt=t=>{const r=[],a=t.attributes;for(let e=0;e<a.length;e+=1){const o=a.item(e);if(!o||!Et(o.name)&&!Mt(o.name)&&String(o.name||"").toLowerCase()!=="style")continue;const d=String(o.value||"").trim();d&&r.push({name:o.name,value:d})}return r},Nt=async(t,r)=>{const a=String(t||"").trim();if(!a)return"";const e=a.split(",").map(d=>d.trim()).filter(Boolean);if(e.length===0)return a;const o=[];for(let d=0;d<e.length;d+=1){const i=e[d].split(/\s+/g).filter(Boolean);if(i.length===0)continue;const s=i[0],c=await r(s),b=i.slice(1).join(" ");o.push(b?`${c} ${b}`:c)}return o.join(", ")},At=async(t,r)=>{const a=String(t||"");if(!a.trim())return a;const e=/url\(\s*(['"]?)([^'"\)]+)\1\s*\)/g;let o="",d=0;for(;;){const n=e.exec(a);if(!n)break;const i=n[0],s=String(n[2]||"").trim();o+=a.slice(d,n.index),d=n.index+i.length;const c=s&&await r(s),b=n[1]||"";o+=`url(${b}${c}${b})`}return o+=a.slice(d),o};async function Tt(t){const r=String(t.svgMarkup||"");if(!r.trim())return"";if(typeof DOMParser!="function"||typeof XMLSerializer!="function")return r;let a=null;try{a=new DOMParser().parseFromString(r,"image/svg+xml")}catch{a=null}if(!a)return r;const e=a.documentElement;if(!e||String(e.tagName||"").toLowerCase()!=="svg")return r;const o=Array.from(e.querySelectorAll("*"));if(o.length===0)return r;const d=typeof t.maxInlineRepoBytes=="number"&&Number.isFinite(t.maxInlineRepoBytes)?Math.max(0,Math.floor(t.maxInlineRepoBytes)):24e5,n=async i=>{const s=String(i||"").trim();if(!s)return"";const c=pe(s);return await fe(c,{maxBytes:d})||c};for(let i=0;i<o.length;i+=1){const s=o[i],c=Bt(s);if(c.length!==0)for(let b=0;b<c.length;b+=1){const u=c[b],l=u.value,p=String(u.name||"").toLowerCase();if(p==="srcset"){const N=await Nt(l,n);if(!N||N===l)continue;try{s.setAttribute(u.name,N)}catch{}continue}if(p==="style"){const N=await At(l,n);if(!N||N===l)continue;try{s.setAttribute(u.name,N)}catch{}continue}const S=await n(l);if(!(!S||S===l))try{s.setAttribute(u.name,S)}catch{}}}try{return new XMLSerializer().serializeToString(e)||r}catch{return r}}function zt(t){const r=t.graphData,a=t.schema;if(!r||!a)return null;const e=tt({graphData:r,edges:null}),o=it(t.collapsedGroupIds),d=ot(a),n=dt(e),i=st({graphData:e,graphDataRevision:typeof t.graphDataRevision=="number"&&Number.isFinite(t.graphDataRevision)?Math.floor(t.graphDataRevision):0}),s=lt(a),c=String(t.documentSemanticModeKey||"document"),b=ct({schemaLayoutEngineJson:d,frontmatterModeEnabled:t.frontmatterModeEnabled,documentSemanticMode:c,graphMetaKey:n,renderMediaAsNodes:t.renderMediaAsNodes,mediaPanelDensity:String(t.mediaPanelDensity),collapsedGroupIdsKey:o}),u=String(t.canvas2dRenderer||"d3"),S=rt({datasetKey:i,mode:s,frontmatterMode:t.frontmatterModeEnabled,semanticMode:c,renderMode:"2d",renderVariant:u,layoutVariant:"",viewKey:b,prevViewKey:null,prevDatasetKey:null,prevMode:null,prevFrontmatterMode:null,prevSemanticMode:null,prevRenderMode:null,prevRenderVariant:null,prevLayoutVariant:null,nodes:Array.isArray(e.nodes)?e.nodes:[],layoutPositionCacheByMode:t.layoutPositionCacheByMode??null}).layoutPositionsForMode;return!S||Object.keys(S).length===0?null:S}const me=t=>{const r=t;if(!(!r||!r.style))try{r.style.removeProperty("transform"),r.style.removeProperty("transform-origin"),r.style.removeProperty("translate"),r.style.removeProperty("left"),r.style.removeProperty("top"),r.style.removeProperty("right"),r.style.removeProperty("bottom"),r.style.removeProperty("inset"),r.style.removeProperty("position"),r.style.removeProperty("width"),r.style.removeProperty("height"),r.style.removeProperty("z-index")}catch{}},he=t=>{const r=t;if(!(!r||!r.style))try{r.style.display="none"}catch{}},_t=(t,r)=>{const a=t.querySelectorAll("img,video,source,iframe"),e=r.querySelectorAll("img,video,source,iframe"),o=Math.min(a.length,e.length);for(let d=0;d<o;d+=1){const n=a[d],i=e[d],s=String(n.tagName||"").toLowerCase();if(s==="img"){const c=n,b=i,u=String(c.currentSrc||c.src||b.getAttribute("src")||"").trim();if(u)try{b.setAttribute("src",u)}catch{}continue}if(s==="video"||s==="source"){const c=String(n.currentSrc||n.src||i.getAttribute("src")||"").trim();if(c)try{i.setAttribute("src",c)}catch{}continue}if(s==="iframe"){const c=n,b=i,u=typeof c.srcdoc=="string"?c.srcdoc:"";if(u){try{b.removeAttribute("src")}catch{}try{b.setAttribute("srcdoc",u)}catch{}continue}const l=String(c.src||i.getAttribute("src")||"").trim();if(l)try{b.setAttribute("src",l)}catch{}}}},Ft=t=>{if(typeof document>"u")return[];const r=[],a=new Set;for(let e=0;e<t.length;e+=1){const o=String(t[e]||"").trim();if(!o)continue;const d=Array.from(document.querySelectorAll(o));for(let n=0;n<d.length;n+=1){const i=d[n];!i||a.has(i)||(a.add(i),r.push(i))}}return r},xe=(t,r)=>t?[t]:Ft(r),ce=t=>{try{const r=t,a=getComputedStyle(r);if(a.display==="none"||a.visibility==="hidden")return 0;const e=Number(a.opacity),o=r.getClientRects().length;return o>0&&Number.isFinite(e)&&e>.01?3:o>0?2:1}catch{return 1}},be=(t,r,a)=>{if(!r)return;const e=t.get(r);if(!e){t.set(r,a);return}ce(a)>=ce(e)&&t.set(r,a)},Dt=t=>{var r;try{const a=xe(t.overlayRootEl,['section[aria-label="D3 rich media overlay"]','section[aria-label="Flow media overlay"]','section[aria-label="Design media overlay"]','section[aria-label="3D media overlay"]',"#kg-overlay",'[data-kg-rich-media-overlay="1"]','[data-kg-rich-media-panel="1"]']);if(a.length===0)return"";const e=new Map;for(let n=0;n<a.length;n+=1){const i=a[n],s=(r=i.matches)!=null&&r.call(i,'[data-kg-rich-media-panel="1"][data-node-id]')?[i]:Array.from(i.querySelectorAll('[data-kg-rich-media-panel="1"][data-node-id]'));for(let c=0;c<s.length;c+=1){const b=s[c],u=String(b.getAttribute("data-node-id")||"").trim();u&&be(e,u,b)}}const o=Array.from(e.values());if(o.length===0)return"";const d=document.createElement("div");for(let n=0;n<o.length;n+=1){const i=o[n],s=i.cloneNode(!0);ge(i,s,ue),me(s),he(s);try{_t(i,s)}catch{}d.appendChild(s)}return d.innerHTML}catch{return""}},Ct=t=>{var r;try{const a=xe(t.overlayRootEl,['section[aria-label="Design markdown overlay"]','section[aria-label="Flow media overlay"]',"#kg-overlay",'[data-kg-markdown-design-overlay="1"]',"[data-kg-markdown-design-block]"]);if(a.length===0)return"";const e=new Map;for(let n=0;n<a.length;n+=1){const i=a[n],s=(r=i.matches)!=null&&r.call(i,"[data-kg-markdown-design-block]")?[i]:Array.from(i.querySelectorAll("[data-kg-markdown-design-block]"));for(let c=0;c<s.length;c+=1){const b=s[c],u=String(b.getAttribute("data-kg-markdown-design-block")||"").trim();u&&be(e,u,b)}}const o=Array.from(e.values());if(o.length===0)return"";const d=document.createElement("div");for(let n=0;n<o.length;n+=1){const i=o[n],s=i.cloneNode(!0);ge(i,s,ue),me(s),he(s);try{const c=String(i.getAttribute("data-kg-markdown-design-block")||"").trim();c&&s.setAttribute("data-md-id",c)}catch{}d.appendChild(s)}return d.innerHTML}catch{return""}},Wt=()=>{try{const t=Dt({overlayRootEl:null}),r=Ct({overlayRootEl:null});return`${t}${r}`}catch{return""}},Ht=()=>{var t;try{const r=String(((t=globalThis==null?void 0:globalThis.localStorage)==null?void 0:t.getItem(ve.viewportControlsPreset))||"").trim();return r==="map"||r==="design"?r:null}catch{return null}};export{ht as a,Lt as b,Ht as c,Wt as d,ie as e,zt as p,Tt as r};
