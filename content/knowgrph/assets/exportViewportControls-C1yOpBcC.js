import{l as et}from"./panelFrame-_TxIn4p9.js";import{cU as we,hz as tt,h_ as rt,ot as $,im as at,n8 as nt,hI as it,jZ as ot,f2 as dt,fQ as st,fm as lt,hb as ct,nE as ut,fu as gt,k2 as ke,cy as Se}from"./index-QyPU4ngJ.js";import{r as pe,e as pt,s as vt}from"./overlaySizing2d-4HKh6TnP.js";import{f as yt}from"./groupVisibility-7_qcxsx3.js";import{f as ft}from"./markdownPanelOverlayPool-DI5N0Xya.js";import{g as mt}from"./displayFilter-DNO_8D0D.js";import{d as ht}from"./positioning-EdTba5ij.js";const bt=[`(function(){
...
`],xt=()=>bt.join(""),W=(e,r,a)=>!r||!e.includes(r)?e:e.split(r).join(a),Z=(e,r,a,t)=>{if(!r)return e;const i=a,d=e.indexOf(r);return d<0?e:e.slice(0,d)+i+e.slice(d+r.length)},wt=e=>{const r=String(e||"");if(!r)return"";const a=r.replace(/`/g,"\\`").replace(/\$\{/g,"\\${");try{return Function("return `"+a+"`")()}catch{return r}};function kt(e){const r=(()=>{const i=String(e.markdownBlocksJson||"").trim();if(!i)return"[]";try{const d=JSON.parse(i);return Array.isArray(d)?i:"[]"}catch{return"[]"}})(),a=wt(xt());if(!a)return"";let t=a;return t=W(t,`var UI_IGNORE_SELECTOR = '[data-kg-canvas-wheel-ignore="true"], [data-kg-canvas-pointer-ignore="true"]';`,"var UI_IGNORE_SELECTOR = '#kg-hud, #kg-hud *';"),t=W(t,"var pointerMode = 'select';","var pointerMode = 'pan';"),t=W(t,`var src = String(edgeEl.getAttribute('data-source-id') || edgeEl.getAttribute('data-source') || '').trim();
      ...
      var tgt = __kgResolveNodeId(String(edgeEl.getAttribute('data-target-id') || edgeEl.getAttribute('data-target') || '').trim());`),t=W(t,"var src = String(ee.getAttribute('data-source-id') || ee.getAttribute('data-source') || '').trim();","var src = __kgResolveNodeId(String(ee.getAttribute('data-source-id') || ee.getAttribute('data-source') || '').trim());"),t=W(t,"var tgt = String(ee.getAttribute('data-target-id') || ee.getAttribute('data-target') || '').trim();","var tgt = __kgResolveNodeId(String(ee.getAttribute('data-target-id') || ee.getAttribute('data-target') || '').trim());"),t=W(t,"__KG_CFG__",e.interactionCfgJson),t=W(t,"__KG_MEDIA_NODES__",e.mediaNodesJson),t=W(t,"__KG_MD_BLOCKS__",r),t=W(t,"__KG_NODE_META__",e.nodeLabelByIdJson),t=W(t,"__KG_EDGE_META__",e.edgeMetaByIdJson),t=W(t,"__KG_NODE_POS__",e.nodePosByIdJson),t=W(t,"__KG_GROUP_MEMBERS__",e.groupMembersByIdJson),t=W(t,"__KG_FRONTMATTER_VIS__",e.frontmatterVisibilityJson),t=W(t,"__KG_INITIAL_FRONTMATTER_ENABLED__",e.initialFrontmatterEnabled===!0?"true":"false"),t=W(t,"__KG_RICH_MEDIA_PANEL_MODE_LS_KEY__",JSON.stringify(we.renderRichMediaPanelMode)),t=W(t,"__KG_DENSITY__",JSON.stringify(e.density)),t=W(t,"__KG_WIDTH_RATIO_DEFAULT__",String(e.widthRatioDefault)),t=W(t,"__KG_WIDTH_RATIO_COMPACT__",String(e.widthRatioCompact)),t=W(t,"__KG_WIDTH_MIN_DEFAULT__",String(e.widthMinDefault)),t=W(t,"__KG_WIDTH_MIN_COMPACT__",String(e.widthMinCompact)),t=W(t,"__KG_WIDTH_MAX_DEFAULT__",String(e.widthMaxDefault)),t=W(t,"__KG_WIDTH_MAX_COMPACT__",String(e.widthMaxCompact)),t=Z(t,"var tip = document.getElementById('kg-tooltip');",`var tip = document.getElementById('kg-tooltip');
    ...
    }`),t=Z(t,`if (fitBtn) fitBtn.addEventListener('click', function(){ fitToCenter(); });
    if (resetBtn) resetBtn.addEventListener('click', function(){ resetView(); });
    ...
    if (resetBtn) resetBtn.addEventListener('click', function(){ resetView(); });
    if (mediaBtn) mediaBtn.addEventListener('click', function(){ setMediaInteractive(!mediaInteractive); });`),t=Z(t,`function applyMediaPointerEvents(){
      var pe = (mediaInteractive && pointerMode !== 'pan' && !panHeld && !headerDrag) ? 'auto' : 'none';
      try {
        if (pe !== lastMediaPointerEvents) {
          lastMediaPointerEvents = pe;
    ...
    }`),t=Z(t,"var mediaInteractive = false;","var mediaInteractive = true;"),t=Z(t,`overlay.__kgMediaById[id] = panel;
      }
    }
    ...
    var overlayRaf = null;`),t=Z(t,`ensureMediaDom();
      ...
      if ((!mediaNodes || mediaNodes.length === 0) && (!markdownBlocks || markdownBlocks.length === 0)) return;`),t=Z(t,"if (!markdownBlocks || markdownBlocks.length === 0) return;",`if (!markdownBlocks || markdownBlocks.length === 0) return;
      try {
      ...
      } catch (eSkip) {}`),t=Z(t,`lastBoxById[id] = { left: left, top: top, w: panelW, h: panelH, display: 'block' };
        }
      }
    }
    ...
    function onWheel(e){`),t=Z(t,`} else {
        ...
        }`),t=Z(t,"function fitToCenter(){",`function getContentCentroid(){
      try {
        if (nodePosById) {
          var sx = 0;
          var sy = 0;
    ...
    function fitToCenter(){`),t=Z(t,`var cx = bb.x + bb.width / 2;
      var cy = bb.y + bb.height / 2;`,`var c = getContentCentroid();
      var cx = (c && isFinite(c.x)) ? c.x : (bb.x + bb.width / 2);
      var cy = (c && isFinite(c.y)) ? c.y : (bb.y + bb.height / 2);`),t=Z(t,"if (t && t.closest && (t.closest('[data-node-id]') || t.closest('[data-edge-id]') || t.closest('[data-kg-group-id]') || t.closest('.kg-media'))) return;","if (t && t.closest && t.closest('[data-edge-id]')) return;"),t=Z(t,`map[nodeId] = { x: ox + dx, y: oy + dy };
          return;`,`map[nodeId] = { x: ox + dx, y: oy + dy };
          try { scheduleEdgeGeometryUpdateForNode(nodeId); } catch (e0) {}
          return;`),t=Z(t,"map[id] = { x: ox + dx, y: oy + dy };",`map[id] = { x: ox + dx, y: oy + dy };
              try { scheduleEdgeGeometryUpdateForNode(id); } catch (e0) {}`),t=Z(t,"var allEdgeEls = svg ? svg.querySelectorAll('line[data-edge-id],path[data-edge-id],polyline[data-edge-id]') : null;",`try {
        if (svg && edgeMetaById && nodePosById) {
      ...
      var allEdgeEls = svg ? svg.querySelectorAll('line[data-edge-id],path[data-edge-id],polyline[data-edge-id]') : null;`),t=W(t,"__KG_PROXY_ORIGIN__",JSON.stringify(String(e.proxyOrigin||""))),t}function j(e){const r=Number(e);return Number.isFinite(r)?r:NaN}function St(e){const r=String(e||"").trim();if(!r)return null;const a=r.match(/translate\(\s*([-0-9.]+)\s*(?:[, ]\s*([-0-9.]+)\s*)?\)/i);if(a){const i=Number(a[1]),d=Number(a[2]);if(Number.isFinite(i)&&Number.isFinite(d))return{x:i,y:d};if(Number.isFinite(i))return{x:i,y:0}}const t=r.match(/matrix\(\s*([-0-9.]+)\s*[ ,]\s*([-0-9.]+)\s*[ ,]\s*([-0-9.]+)\s*[ ,]\s*([-0-9.]+)\s*[ ,]\s*([-0-9.]+)\s*[ ,]\s*([-0-9.]+)\s*\)/i);if(t){const i=Number(t[5]),d=Number(t[6]);if(Number.isFinite(i)&&Number.isFinite(d))return{x:i,y:d}}return null}function It(e){if(!e||!e.getAttribute)return null;const r=String(e.tagName||"").toLowerCase();if(r==="circle"){const t=j(e.getAttribute("cx")),i=j(e.getAttribute("cy"));return Number.isFinite(t)&&Number.isFinite(i)?{x:t,y:i}:null}if(r==="rect"){const t=j(e.getAttribute("x")),i=j(e.getAttribute("y")),d=j(e.getAttribute("width")),n=j(e.getAttribute("height"));return Number.isFinite(t)&&Number.isFinite(i)&&Number.isFinite(d)&&Number.isFinite(n)&&d>0&&n>0?{x:t+d/2,y:i+n/2}:null}const a=St(String(e.getAttribute("transform")||""));if(r==="g"&&e.querySelector){let t=null;try{const i=e.querySelector('circle[data-role="node-circle"], circle[cx][cy]');if(i){const d=j(i.getAttribute("cx")),n=j(i.getAttribute("cy"));Number.isFinite(d)&&Number.isFinite(n)&&(t={x:d,y:n})}}catch{}if(!t)try{const i=e.querySelector("rect[x][y][width][height]");if(i){const d=j(i.getAttribute("x")),n=j(i.getAttribute("y")),o=j(i.getAttribute("width")),s=j(i.getAttribute("height"));Number.isFinite(d)&&Number.isFinite(n)&&Number.isFinite(o)&&Number.isFinite(s)&&o>0&&s>0&&(t={x:d+o/2,y:n+s/2})}}catch{}return t&&a?{x:t.x+a.x,y:t.y+a.y}:t||a||null}return a||null}function Et(e){try{return!!(e.hasAttribute("data-port-key")||e.hasAttribute("data-port-dir"))}catch{return!1}}function ce(e){const r=String(e||"").trim();if(!r)return{};const a=r.replace(/^<\?xml[^>]*>\s*/i,"");try{const i=new DOMParser().parseFromString(a,"image/svg+xml").querySelector("svg");if(!i)return{};const d={},n=i.querySelectorAll("[data-node-id]");for(let s=0;s<n.length;s+=1){const l=n[s];if(Et(l))continue;const m=String(l.getAttribute("data-node-id")||"").trim();if(!m||d[m])continue;const u=It(l);if(u){d[m]=u;continue}}const o=i.querySelectorAll("line[data-source-id][data-target-id]");for(let s=0;s<o.length;s+=1){const l=o[s],m=String(l.getAttribute("data-source-id")||"").trim(),u=String(l.getAttribute("data-target-id")||"").trim();if(!m&&!u)continue;const c=j(l.getAttribute("x1")),p=j(l.getAttribute("y1")),h=j(l.getAttribute("x2")),B=j(l.getAttribute("y2"));m&&!d[m]&&Number.isFinite(c)&&Number.isFinite(p)&&(d[m]={x:c,y:p}),u&&!d[u]&&Number.isFinite(h)&&Number.isFinite(B)&&(d[u]={x:h,y:B})}return d}catch{return{}}}const ie=e=>typeof e=="number"&&Number.isFinite(e);function Mt(e){var a,t,i;const r=String(e.svgMarkup||"").trim();if(!r||typeof DOMParser>"u"||typeof XMLSerializer>"u")return r;try{const d=new DOMParser().parseFromString(r.replace(/^<\?xml[^>]*>\s*/i,""),"image/svg+xml"),n=d.querySelector("svg");if(!n)return r;const o=n.querySelector('[data-kg-layer="links"]');if(!o||o.querySelector("line,path,polyline"))return r;const s=Array.isArray((a=e.graphData)==null?void 0:a.nodes)?e.graphData.nodes:[],l=Array.isArray((t=e.graphData)==null?void 0:t.edges)?e.graphData.edges:[];if(l.length===0)return r;const m=new Set,u={};for(let C=0;C<s.length;C+=1){const E=String(((i=s[C])==null?void 0:i.id)||"").trim();if(!E)continue;m.add(E);const X=E.split("::").pop()||"";X&&!u[X]&&(u[X]=E)}const c=C=>{const E=String(C||"").trim();if(!E)return"";if(m.has(E))return E;const X=E.split("::").pop()||"";return X&&u[X]?u[X]:E},h={...ce(r),...e.nodePosById||{}},B=n.namespaceURI||"http://www.w3.org/2000/svg";for(let C=0;C<l.length;C+=1){const E=l[C],X=String((E==null?void 0:E.id)||"").trim()||`e${C}`,T=String((E==null?void 0:E.sourceId)||(E==null?void 0:E.source)||"").trim(),z=String((E==null?void 0:E.targetId)||(E==null?void 0:E.target)||"").trim(),P=c(T),R=c(z);if(!X||!P||!R)continue;const U=h[P],q=h[R];if(!U||!q||!ie(U.x)||!ie(U.y)||!ie(q.x)||!ie(q.y))continue;const O=d.createElementNS(B,"line");O.setAttribute("data-edge-id",X),O.setAttribute("data-source-id",P),O.setAttribute("data-target-id",R),O.setAttribute("x1",String(U.x)),O.setAttribute("y1",String(U.y)),O.setAttribute("x2",String(q.x)),O.setAttribute("y2",String(q.y)),O.setAttribute("stroke","var(--kg-canvas-edge-stroke)"),O.setAttribute("stroke-opacity","1"),O.setAttribute("stroke-width","2"),O.setAttribute("stroke-linecap","round"),O.setAttribute("fill","none"),o.appendChild(O)}const G=new XMLSerializer().serializeToString(n);return String(G||"").trim()||r}catch{return r}}const ve=e=>/^https?:\/\//i.test(String(e||"").trim()),Ie=e=>{const r=String(e||"").trim();if(!r)return"";if(/^(data:|blob:|mailto:|tel:|javascript:)/i.test(r))return r;const a=n=>{try{return decodeURIComponent(n)}catch{return n}},i=(n=>{var o;try{if(/^https?:\/\//i.test(n))return new URL(n);if(typeof window>"u")return null;const s=(o=window.location)==null?void 0:o.origin;return!s||!/^https?:\/\//i.test(String(s))?null:new URL(n,s)}catch{return null}})(r);if(!i)return r;const d=i.pathname||"";if(d==="/__fetch_remote"||d==="/__webpage_proxy"||d==="/__webpage_asset_proxy"){const n=i.searchParams.get("url");if(!n)return r;const o=a(n);return ve(o)?o:o||r}if(d.startsWith("/__webpage_asset_path/")){const o=d.slice(22).split("/").filter(Boolean);if(o.length>=1){const s=o[0]||"",l=a(s);if(ve(l)){const m="/"+o.slice(1).join("/"),u=i.search||"";return l.replace(/\/+$/,"")+m+u}}}return r},Ee=e=>tt(e),Bt=e=>{const r=String(e||"").trim(),a=r.lastIndexOf("."),t=a>=0?r.slice(a+1).toLowerCase():"";return t==="svg"?"image/svg+xml":t==="png"?"image/png":t==="jpg"||t==="jpeg"?"image/jpeg":t==="gif"?"image/gif":t==="webp"?"image/webp":t==="mp4"?"video/mp4":t==="webm"?"video/webm":t==="json"?"application/json":t==="html"||t==="htm"?"text/html":"application/octet-stream"},At=e=>{const r=globalThis.Buffer;if(r)return r.from(e).toString("base64");let a="";const t=32768;for(let d=0;d<e.length;d+=t){const n=e.subarray(d,Math.min(e.length,d+t));a+=String.fromCharCode(...n)}const i=globalThis.btoa;return i?i(a):""},Nt=async e=>{try{if(typeof fetch!="function")return null;const r=await fetch(e);if(!r||!r.ok)return null;const a=await r.arrayBuffer();return new Uint8Array(a)}catch{return null}},Me=async(e,r)=>{const a=String(e||"").trim(),t=Ee(a);if(!t)return null;const i=typeof(r==null?void 0:r.maxBytes)=="number"&&Number.isFinite(r.maxBytes)?Math.max(0,Math.floor(r.maxBytes)):9e5,d=(()=>{const l=String((r==null?void 0:r.origin)||"").trim();if(l&&/^https?:\/\//i.test(l))return l.replace(/\/+$/,"")+a;try{if(typeof window<"u"&&window.location&&/^https?:$/.test(String(window.location.protocol||"")))return String(window.location.origin||"").replace(/\/+$/,"")+a}catch{}return a})(),n=await Nt(d);if(!n||n.length===0||n.length>i)return null;const o=Bt(t),s=At(n);return s?`data:${o};base64,${s}`:null},J=e=>typeof e=="number"&&Number.isFinite(e),V=e=>String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),oe=(e,r)=>{const a=String(e.getAttribute(r)||"").trim();if(!a)return null;const t=Number(a);return Number.isFinite(t)?t:null},ye=e=>{const r=String(e.getAttribute("data-kg-title")||"").trim();if(r)return r;const a=e.querySelector("a[aria-label]"),t=String((a==null?void 0:a.getAttribute("aria-label"))||"").trim();if(t)return t;const i=e.querySelector("iframe[title]"),d=String((i==null?void 0:i.getAttribute("title"))||"").trim();if(d)return d;const n=e.querySelector("video[title],img[alt]"),o=String((n==null?void 0:n.getAttribute("title"))||(n==null?void 0:n.getAttribute("alt"))||"").trim();return o||""},fe=e=>String(e.getAttribute("data-md-id")||"").trim(),me=e=>String(e.getAttribute("data-kg-anchor-node-id")||"").trim(),Ft=(e,r)=>{try{e.setAttribute("data-md-id",r.id)}catch{}try{r.anchorNodeId?e.setAttribute("data-kg-anchor-node-id",r.anchorNodeId):e.removeAttribute("data-kg-anchor-node-id")}catch{}},he=e=>{const r=ot(e);if(r==="image"||r==="svg"||r==="video")return r;const a=String(e||"").trim().toLowerCase();return a&&(a.includes("wx_fmt=png")||a.includes("wx_fmt=jpg")||a.includes("wx_fmt=jpeg")||a.includes("wx_fmt=gif")||a.includes("wx_fmt=webp"))?"image":"iframe"},_t=(e,r)=>{try{return typeof document>"u"?r:String(getComputedStyle(document.documentElement).getPropertyValue(e)||"").trim()||r}catch{return r}},Dt=e=>{var u;const r=e.graph,a=String(e.svgMarkup||""),t=Array.isArray(r==null?void 0:r.nodes)?r.nodes:[],i=new Set;for(let c=0;c<t.length;c+=1){const p=String(((u=t[c])==null?void 0:u.id)||"").trim();p&&i.add(p)}if(a.trim()){const c=/\bdata-node-id\s*=\s*(?:"([^"]+)"|'([^']+)')/gi;let p=null;for(;p=c.exec(a);){const h=String(p[1]||p[2]||"").trim();h&&i.add(h)}}const d={};for(const c of i){const p=c.split("::").pop()||"";p&&!d[p]&&(d[p]=c)}const n=c=>{const p=String(c||"").trim();if(!p)return!0;const h=p.toLowerCase();return!!(h.includes("::blk:")||h.includes("::block:")||h.startsWith("blk:"))},o=c=>{const p=String(c||"").trim();if(!p)return"";if(i.has(p)||n(p))return p;const h=p.split("::").pop()||"";return h&&d[h]||p},s=Array.isArray(r==null?void 0:r.edges)?r.edges:[],l=new Set,m=c=>{if(typeof c=="string")return String(c||"").trim();if(c&&typeof c=="object"){const p=c.id;if(typeof p=="string")return String(p||"").trim()}return String(c||"").trim()};for(let c=0;c<s.length;c+=1){const p=s[c],h=p.source,B=p.target,G=p.sourceId??p.source_id,Y=p.targetId??p.target_id,C=o(m(h??G)),E=o(m(B??Y));C&&i.has(C)&&l.add(C),E&&i.has(E)&&l.add(E)}if(a.trim()){const c=/\bdata-(?:source-id|source|target-id|target)\s*=\s*(?:"([^"]+)"|'([^']+)')/gi;let p=null;for(;p=c.exec(a);){const h=String(p[1]||p[2]||"").trim();if(!h)continue;const B=o(h);B&&i.has(B)&&l.add(B)}}return{nodeIdSet:i,edgeLinkedNodeIdSet:l,resolveNodeId:o}},Ct=e=>{const r=String(e.overlayHtml||"");if(!r.trim())return"";if(!e.hasGraphNodeIdentity)return r;const a=d=>{const n=String(d.anchorNorm||d.anchorRaw||"").trim();if(n)return`md:${n}`;const o=String(d.idNorm||d.idRaw||"").trim();return o?`md:${o}`:""},t=d=>{const n=String(d||"").toLowerCase();if(!n)return 0;if(/(?:^|;)\s*display\s*:\s*none\b/i.test(n))return-200;if(/(?:^|;)\s*visibility\s*:\s*hidden\b/i.test(n))return-180;if(/(?:^|;)\s*opacity\s*:\s*0(?:\D|$)/i.test(n))return-160;const o=/(?:^|;)\s*position\s*:\s*(?:fixed|sticky)\b/i.test(n),s=/(?:^|;)\s*(?:left|top|right|bottom|inset)\s*:/i.test(n);return o&&s?-40:o?-20:0},i=d=>{var E,X;const n=String(d||"");if(!n.trim())return"";const o=(T,z,P)=>P?String(T||"").replace(new RegExp(`\\s${z}=(?:"[^"]*"|'[^']*')`,"i"),"").replace(/>$/,` ${z}="${P}">`):T,s=(T,z)=>String(T||"").replace(new RegExp(`\\s${z}=(?:"[^"]*"|'[^']*')`,"ig"),""),l=(T,z)=>{const P=String(T||""),R=`${z}="`,U=`${z}='`,q=P.indexOf(R);if(q>=0){const K=q+R.length,Q=P.indexOf('"',K);return Q>K?P.slice(K,Q).trim():""}const O=P.indexOf(U);if(O>=0){const K=O+U.length,Q=P.indexOf("'",K);return Q>K?P.slice(K,Q).trim():""}return""},m=(T,z,P)=>{let R=T;return R=s(R,"data-kg-anchor-node-id"),R=o(R,"data-md-id",z),P&&(R=o(R,"data-kg-anchor-node-id",P)),R},u=[];let c=!1;const p=new Map,h=new Map,B=/<article\b[^>]*data-kg-rich-media-panel=(?:"1"|'1')[^>]*>[\s\S]*?<\/article>/gi;let G=null;for(;G=B.exec(n);){c=!0;const T=G[0]||"",z=((E=T.match(/<article\b[^>]*>/i))==null?void 0:E[0])||"",P=e.resolveOverlayNodeId(l(z,"data-node-id")),R=P?`media:${P}`:"";if(!P||!R||!e.shouldKeepOverlayLinkedToGraph({nodeId:P}))continue;const U=T.replace(z,o(z,"data-node-id",P)),q=t(l(z,"style")),O=p.get(R);(!O||q>O.score)&&p.set(R,{chunk:U,score:q})}const Y=/<article\b[^>]*data-md-id[^>]*>[\s\S]*?<\/article>/gi;let C=null;for(;C=Y.exec(n);){c=!0;const T=C[0]||"",z=((X=T.match(/<article\b[^>]*>/i))==null?void 0:X[0])||"",P=l(z,"data-md-id"),R=l(z,"data-kg-anchor-node-id"),U=e.resolveOverlayNodeId(P),q=e.resolveOverlayNodeId(R),O=a({idRaw:P,idNorm:U,anchorRaw:R,anchorNorm:q});if(!O||!e.shouldKeepOverlayLinkedToGraph({nodeId:U||P,anchorNodeId:q||R}))continue;const K=m(z,U||P,q||R),Q=T.replace(z,K),ee=t(l(z,"style")),te=h.get(O);(!te||ee>te.score)&&h.set(O,{chunk:Q,score:ee})}for(const T of p.values())T.score<=-100||u.push(T.chunk);for(const T of h.values())T.score<=-100||u.push(T.chunk);return u.length<=0?c?"":n:u.join("")};try{if(typeof DOMParser>"u")return i(r);const n=new DOMParser().parseFromString(`<!doctype html><html><body><div id="kg-overlay-filter-root">${r}</div></body></html>`,"text/html").querySelector("#kg-overlay-filter-root");if(!n)return r;const o=new Map,s=n.querySelectorAll('[data-kg-rich-media-panel="1"][data-kg-rich-media-render-surface="1"][data-node-id]');for(let u=0;u<s.length;u+=1){const c=s[u],p=String(c.getAttribute("data-node-id")||"").trim(),h=e.resolveOverlayNodeId(p),B=h?`media:${h}`:"";if(!(!!h&&e.shouldKeepOverlayLinkedToGraph({nodeId:h}))||!B){c.remove();continue}const Y=t(String(c.getAttribute("style")||"").trim()),C=o.get(B);(!C||Y>C.score)&&o.set(B,{el:c,score:Y});try{c.setAttribute("data-node-id",h)}catch{}}for(let u=0;u<s.length;u+=1){const c=s[u];if(!c||!c.isConnected)continue;const p=e.resolveOverlayNodeId(String(c.getAttribute("data-node-id")||"").trim()),h=p?`media:${p}`:"",B=h?o.get(h):null,G=(B==null?void 0:B.el)||null;if(!G||G!==c){c.remove();continue}((B==null?void 0:B.score)??0)<=-100&&c.remove()}const l=new Map,m=n.querySelectorAll("article[data-md-id]");for(let u=0;u<m.length;u+=1){const c=m[u],p=fe(c),h=me(c),B=e.resolveOverlayNodeId(p),G=e.resolveOverlayNodeId(h),Y=a({idRaw:p,idNorm:B,anchorRaw:h,anchorNorm:G});if(!e.shouldKeepOverlayLinkedToGraph({nodeId:B||p,anchorNodeId:G||h})||!Y){c.remove();continue}const E=t(String(c.getAttribute("style")||"").trim()),X=l.get(Y);(!X||E>X.score)&&l.set(Y,{el:c,score:E}),Ft(c,{id:B||p,anchorNodeId:G||h})}for(let u=0;u<m.length;u+=1){const c=m[u];if(!c||!c.isConnected)continue;const p=fe(c),h=me(c),B=e.resolveOverlayNodeId(p),G=e.resolveOverlayNodeId(h),Y=a({idRaw:p,idNorm:B,anchorRaw:h,anchorNorm:G}),C=Y?l.get(Y):null,E=(C==null?void 0:C.el)||null;if(!E||E!==c){c.remove();continue}((C==null?void 0:C.score)??0)<=-100&&c.remove()}return n.innerHTML}catch{return i(r)}};async function Qt(e){try{rt()}catch{}const r=String(e.title||"").trim()||"Graph viewer",a=String(e.svgMarkup||"").trim(),t=e.initialView&&J(e.initialView.k)&&J(e.initialView.x)&&J(e.initialView.y)?e.initialView:null,i=J(e.viewportWidthPx)&&J(e.viewportHeightPx)?{w:Math.max(1,Math.floor(e.viewportWidthPx)),h:Math.max(1,Math.floor(e.viewportHeightPx))}:null,d=String(e.overlayHtml||""),n=Dt({graph:e.graphData,svgMarkup:a}),o=g=>n.resolveNodeId(g),s=g=>{if(n.nodeIdSet.size===0)return!0;const y=o(String(g.nodeId||"").trim()),b=o(String(g.anchorNodeId||"").trim());return y&&n.nodeIdSet.has(y)||b&&n.nodeIdSet.has(b)?n.edgeLinkedNodeIdSet.size<=0?!0:y&&n.edgeLinkedNodeIdSet.has(y)||b&&n.edgeLinkedNodeIdSet.has(b):!1},l=Ct({overlayHtml:d,hasGraphNodeIdentity:n.nodeIdSet.size>0,resolveOverlayNodeId:o,shouldKeepOverlayLinkedToGraph:s}),m=(()=>{var S;const g=[],y=[];if(!l.trim())return{mediaNodes:g,markdownBlocks:y};const b=()=>{const w=(k,F)=>{const M=String(k||""),v=`${F}="`,f=`${F}='`,x=M.indexOf(v);if(x>=0){const D=x+v.length,H=M.indexOf('"',D);return H>D?M.slice(D,H).trim():""}const _=M.indexOf(f);if(_>=0){const D=_+f.length,H=M.indexOf("'",D);return H>D?M.slice(D,H).trim():""}return""},A=/<article\b[^>]*data-kg-rich-media-panel=(?:"1"|'1')[^>]*>/gi;let I=null;for(;I=A.exec(l);){const k=I[0]||"",F=o(w(k,"data-node-id"));if(!F)continue;const M=w(k,"data-kg-url")||w(k,"data-kg-open-url");if(!M)continue;const v=w(k,"data-kg-kind").toLowerCase(),f=v==="image"||v==="svg"||v==="video"||v==="iframe"?v:he(M);g.push({id:F,title:F,url:M,openUrl:w(k,"data-kg-open-url")||M,interactive:!0,kind:f})}const L=/<article\b[^>]*data-md-id[^>]*>/gi;let N=null;for(;N=L.exec(l);){const k=N[0]||"",F=w(k,"data-md-id");if(!F)continue;const M=Number(w(k,"data-kg-world-x")),v=Number(w(k,"data-kg-world-y")),f=Number(w(k,"data-kg-world-w")),x=Number(w(k,"data-kg-world-h"));if(!Number.isFinite(M)||!Number.isFinite(v)||!Number.isFinite(f)||!Number.isFinite(x)||!(f>0)||!(x>0))continue;const _=o(w(k,"data-kg-anchor-node-id"));y.push({id:F,anchorNodeId:_||void 0,title:F,summary:"",preview:{kind:"other"},x:M,y:v,w:f,h:x})}};try{if(typeof DOMParser>"u")return b(),{mediaNodes:g,markdownBlocks:y};const A=new DOMParser().parseFromString(`<!doctype html><html><body><div id="kg-overlay-seed">${l}</div></body></html>`,"text/html").querySelector("#kg-overlay-seed");if(!A)return b(),{mediaNodes:g,markdownBlocks:y};const I=A.querySelectorAll('[data-kg-rich-media-panel="1"][data-kg-rich-media-render-surface="1"][data-node-id]');for(let N=0;N<I.length;N+=1){const k=I[N],F=String(k.getAttribute("data-node-id")||"").trim(),M=o(F);if(!M)continue;const v=String(k.getAttribute("data-kg-url")||"").trim(),f=String(k.getAttribute("data-kg-open-url")||"").trim(),x=k.querySelector("iframe[src],img[src],video[src],source[src]"),_=x?String(x.getAttribute("src")||"").trim():"",D=v||_||f;if(!D)continue;const H=String(k.getAttribute("data-kg-kind")||"").trim().toLowerCase(),re=H==="image"||H==="svg"||H==="video"||H==="iframe"?H:he(D),ae=ye(k)||M;g.push({id:M,title:ae,url:D,openUrl:f||D,interactive:!0,kind:re})}const L=A.querySelectorAll("[data-md-id]");for(let N=0;N<L.length;N+=1){const k=L[N],F=String(k.getAttribute("data-md-id")||"").trim();if(!F)continue;const M=oe(k,"data-kg-world-x"),v=oe(k,"data-kg-world-y"),f=oe(k,"data-kg-world-w"),x=oe(k,"data-kg-world-h");if(!J(M)||!J(v)||!J(f)||!J(x)||!(f>0)||!(x>0))continue;const _=String(k.getAttribute("data-kg-anchor-node-id")||"").trim(),D=o(_),H=String(((S=k.querySelector(".kg-mdTitle"))==null?void 0:S.textContent)||"").trim()||ye(k)||F;y.push({id:F,anchorNodeId:D||void 0,title:H,summary:"",preview:{kind:"other"},x:M,y:v,w:f,h:x})}g.length===0&&y.length===0&&b()}catch{b()}return{mediaNodes:g,markdownBlocks:y}})(),u={mediaNodes:m.mediaNodes.filter(g=>s({nodeId:g.id})),markdownBlocks:m.markdownBlocks.filter(g=>s({nodeId:g.id,anchorNodeId:g.anchorNodeId}))},c=(()=>{if(!t)return[];const g=e.graphData,y=Array.isArray(g==null?void 0:g.nodes)?g.nodes:[];if(y.length===0)return[];const b={};for(let M=0;M<y.length;M+=1){const v=y[M],f=String((v==null?void 0:v.id)||"").trim();if(!f)continue;const x=v.x,_=v.y;!J(x)||!J(_)||(b[f]={x,y:_})}const S=ce(a);for(const M of Object.keys(S))b[M]=S[M];const w=i?i.w:1920,A=i?i.h:1080,I=800,L=Math.max(1e-5,Number(t.k)),N=Number(t.x),k=Number(t.y),F=[];for(const M of Object.keys(b)){if(!s({nodeId:M}))continue;const v=b[M];if(!v)continue;const f=v.x*L+N,x=v.y*L+k;if(!(f<-I||f>w+I)&&!(x<-I||x>A+I)&&(F.push(M),F.length>=800))break}return F})();if(!!!a)return null;const h=$("--kg-canvas-bg")||"#ffffff",B=$("--kg-panel-bg")||"rgba(255,255,255,0.92)",G=$("--kg-border")||"rgba(0,0,0,0.12)",Y=$("--kg-text-primary")||"rgba(0,0,0,0.86)",C=$("--kg-text-secondary")||"rgba(0,0,0,0.7)",E=$("--kg-text-tertiary")||"rgba(0,0,0,0.55)",X=$("--kg-panel-action-bg")||"rgba(0,0,0,0.04)",T=$("--kg-panel-action-bg-hover")||"rgba(0,0,0,0.06)",z=$("--kg-canvas-edge-stroke")||"#9ca3af",P=$("--kg-canvas-node-stroke")||"#ffffff",R=$("--kg-canvas-accent")||"#3b82f6",U=$("--kg-canvas-label-fill")||Y,q=$("--kg-canvas-label-halo")||h,O=$("--kg-media-panel-header-bg")||"rgba(0,0,0,0.04)",K=e.mediaPanelDensity==="compact"?"compact":"default",Q=K==="compact"?22:28,ee=K==="compact"?9:10,te=K==="compact"?6:8,_e=K==="compact"?11:12,de=pe({density:"default",sizing:e.overlaySizing||null}),se=pe({density:"compact",sizing:e.overlaySizing||null}),De=de.widthRatio,Ce=se.widthRatio,Pe=de.widthMinPx,Re=se.widthMinPx,Oe=de.widthMaxPx,Le=se.widthMaxPx,ue=(()=>{if(e.includeRichMediaOverlays!==!0)return[];const g=e.graphData,y=Array.isArray(g==null?void 0:g.nodes)?g.nodes:[],b=J(e.mediaOverlayPoolMax)?Math.max(0,Math.floor(e.mediaOverlayPoolMax)):0,S=b>0?b:Math.min(2e3,Math.max(24,y.length));return et({enabled:!0,nodes:y,poolMax:S,preferredNodeIds:c})})(),Te=async g=>{if(!g||g.length===0)return[];const y=24e5,b=[];for(let S=0;S<g.length;S+=1){const w=g[S],A=String(w.url||"").trim(),I=Ie(A);if(!Ee(I)){b.push({...w,url:I});continue}const N=await Me(I,{maxBytes:y});if(!N){b.push({...w,url:I});continue}b.push({...w,url:N})}return b},ze=(()=>{const g=[],y=new Set,b=S=>{const w=o(String(S.id||"").trim());s({nodeId:w})&&(!w||y.has(w)||(y.add(w),g.push({...S,id:w})))};for(let S=0;S<ue.length;S+=1)b(ue[S]);for(let S=0;S<u.mediaNodes.length;S+=1)b(u.mediaNodes[S]);return g})(),We=await Te(ze),He=JSON.stringify(We),Ge=(()=>{const g={},y=e.graphData,b=Array.isArray(y==null?void 0:y.nodes)?y.nodes:[];for(let S=0;S<b.length;S+=1){const w=b[S],A=String((w==null?void 0:w.id)||"").trim();if(!A)continue;const I=String(w.label||"").trim();g[A]={label:I||A}}return JSON.stringify(g)})(),ge=g=>n.resolveNodeId(g),Ye=(()=>{const g={},y=e.graphData,b=Array.isArray(y==null?void 0:y.edges)?y.edges:[];for(let S=0;S<b.length;S+=1){const w=b[S],A=String((w==null?void 0:w.id)||"").trim();if(!A)continue;const I=String(w.label||"").trim(),L=String(w.source||"").trim(),N=String(w.target||"").trim(),k=ge(L),F=ge(N);g[A]={label:I||"",s:k,t:F}}return JSON.stringify(g)})(),Xe=(()=>{const g=e.graphData;if(!g)return JSON.stringify({nodeIds:[],edgeIds:[]});const y=at(g),b=Array.isArray(y.nodes)?y.nodes:[],S=Array.isArray(y.edges)?y.edges:[],w=b.map(I=>String((I==null?void 0:I.id)||"").trim()).filter(Boolean),A=S.map(I=>String((I==null?void 0:I.id)||"").trim()).filter(Boolean);return JSON.stringify({nodeIds:w,edgeIds:A})})(),Ve=(()=>{var f;const g=e.graphData;if(!g)return JSON.stringify({});const y=Array.isArray(g.nodes)?g.nodes:[],b=new Set,S={};for(let x=0;x<y.length;x+=1){const _=String(((f=y[x])==null?void 0:f.id)||"").trim();if(!_)continue;b.add(_);const D=_.split("::").pop()||"";D&&!S[D]&&(S[D]=_)}const w=x=>{const _=String(x||"").trim();if(!_)return"";if(b.has(_))return _;const D=_.split("::").pop()||"";return D?S[D]||_:""},A=g.metadata||{},I=A.kind==="keyword",L=nt({frontmatterModeEnabled:!1,multiDimTableModeEnabled:!1,documentSemanticMode:I?"keyword":"document",documentStructureBaselineLock:!1}).forceDocumentStructureGroups,N=A["kg:view"]&&typeof A["kg:view"]=="object"&&!Array.isArray(A["kg:view"])?A["kg:view"]:null,k=N&&Array.isArray(N.collapsedGroupIds)?N.collapsedGroupIds:[],F=new Set(k.map(x=>String(x||"").trim()).filter(Boolean)),M=yt({groups:it(g,{forceDocumentStructure:L}),collapsedGroupIdSet:F}),v={};for(let x=0;x<M.length;x+=1){const _=M[x],D=String(_.id||"").trim();if(!D)continue;const H=_.memberNodeIds,re=Array.isArray(H)?H.map(ne=>String(ne)).filter(Boolean):[];if(re.length===0)continue;const ae=re.map(w).filter(ne=>ne&&b.has(ne));ae.length!==0&&(v[D]=ae)}return JSON.stringify(v)})(),le=(()=>{const g=e.graphData,y=Array.isArray(g==null?void 0:g.nodes)?g.nodes:[],b={},S=new Set;for(let A=0;A<y.length;A+=1){const I=y[A],L=String((I==null?void 0:I.id)||"").trim();if(!L)continue;const N=I.x,k=I.y;!J(N)||!J(k)||(S.add(L),b[L]={x:N,y:k})}const w=ce(a);for(const A of Object.keys(w))b[A]=w[A];for(let A=0;A<u.markdownBlocks.length;A+=1){const I=u.markdownBlocks[A],L=Number(I.x),N=Number(I.y),k=Number(I.w),F=Number(I.h);if(!Number.isFinite(L)||!Number.isFinite(N)||!Number.isFinite(k)||!Number.isFinite(F)||!(k>0)||!(F>0))continue;const M=L+k*.5,v=N+F*.5,f=String(I.id||"").trim();f&&!S.has(f)&&(b[f]={x:M,y:v});const x=o(String(I.anchorNodeId||"").trim());x&&!S.has(x)&&(b[x]={x:M,y:v})}return b})(),qe=JSON.stringify(le),Ue=(()=>{var g;try{const y=e.graphData;if(!y&&u.markdownBlocks.length===0)return"[]";const b=pt({graphData:y,nodePosById:le}),S=b&&Array.isArray(b.blocks)?b.blocks:[],w=Array.isArray(y==null?void 0:y.nodes)?y.nodes:[],A=ft({layout:b,nodes:w}),L=[...S.map(v=>{const f=String((v==null?void 0:v.id)||"").trim(),x=f?o(String((A==null?void 0:A[f])||"").trim()):"";return x?{...v,anchorNodeId:x}:v})],N=new Map;for(let v=0;v<L.length;v+=1){const f=String(((g=L[v])==null?void 0:g.id)||"").trim();f&&N.set(f,L[v])}for(let v=0;v<u.markdownBlocks.length;v+=1){const f=u.markdownBlocks[v],x=N.get(f.id);if(!x){L.push(f),N.set(f.id,f);continue}const _=o(String(x.anchorNodeId||f.anchorNodeId||""));_&&(x.anchorNodeId=_);const D=Number(x.w),H=Number(x.h);Number.isFinite(D)&&D>0&&Number.isFinite(H)&&H>0||(x.x=f.x,x.y=f.y,x.w=f.w,x.h=f.h)}const k=L.filter(v=>s({nodeId:String((v==null?void 0:v.id)||""),anchorNodeId:String((v==null?void 0:v.anchorNodeId)||"")})),F=[],M=new Set;for(let v=0;v<k.length;v+=1){const f=k[v],x=o(String((f==null?void 0:f.anchorNodeId)||(f==null?void 0:f.anchorId)||"")),_=String((f==null?void 0:f.id)||"").trim(),D=(x?`md:${x}`:_?`md:${_}`:"").trim();D&&(M.has(D)||(M.add(D),F.push(f)))}return JSON.stringify(F)}catch{return"[]"}})(),je=Mt({svgMarkup:a,graphData:e.graphData||{nodes:[],edges:[]},nodePosById:le}),Ke=JSON.stringify({scaleExtent:vt({minK:e.zoomMinK,maxK:e.zoomMaxK}),wheelBehavior:e.wheelBehavior,viewportControlsPreset:e.viewportControlsPreset,panSpeed:e.panSpeed,zoomSpeed:e.zoomSpeed,flowWheelZoomSpeedMultiplier:e.flowWheelZoomSpeedMultiplier,flowWheelZoomIncrementMultiplier:e.flowWheelZoomIncrementMultiplier,flowWheelZoomSmoothMinDurationMs:e.flowWheelZoomSmoothMinDurationMs,flowWheelZoomSmoothMaxDurationMs:e.flowWheelZoomSmoothMaxDurationMs,wheelZoomCtrlMetaBoostMultiplier:e.wheelZoomCtrlMetaBoostMultiplier,canvasInteractionSpeedMultiplier:e.canvasInteractionSpeedMultiplier,canvasPanSpeedMultiplier:e.canvasPanSpeedMultiplier,snapGridEnabled:e.snapGridEnabled,snapGridSize:e.snapGridSize,dragConstraint:e.dragConstraint,allowNodeDrag:e.allowNodeDrag!==!1,allowEdgeDrag:e.allowEdgeDrag!==!1,allowGroupDrag:e.allowGroupDrag!==!1,preferWebgl3d:e.preferWebgl3d===!0,initialView:t||null,fixedViewport:i?{widthPx:i.w,heightPx:i.h,scaleToFit:e.viewportScaleToFit===!0}:null,enableDecorativeAnimation:e.enableDecorativeAnimation===!0,zoomLabelScaleMode2d:e.zoomLabelScaleMode2d,zoomLabelScaleExponent2d:e.zoomLabelScaleExponent2d,zoomLabelScaleClampMin2d:e.zoomLabelScaleClampMin2d,zoomLabelScaleClampMax2d:e.zoomLabelScaleClampMax2d,zoomStrokeScaleMode2d:e.zoomStrokeScaleMode2d,zoomStrokeScaleExponent2d:e.zoomStrokeScaleExponent2d,zoomStrokeScaleClampMin2d:e.zoomStrokeScaleClampMin2d,zoomStrokeScaleClampMax2d:e.zoomStrokeScaleClampMax2d,hideLabelsBelowScale:e.hideLabelsBelowScale}),Ze=(()=>{var g;try{if(typeof window>"u")return"";const y=String(((g=window.location)==null?void 0:g.origin)||"").trim();if(!y)return"";const b=new URL(y).hostname.toLowerCase();return b==="localhost"||b==="127.0.0.1"||b==="0.0.0.0"?y:""}catch{return""}})(),$e=je,Je=kt({interactionCfgJson:Ke,mediaNodesJson:He,markdownBlocksJson:Ue,nodeLabelByIdJson:Ge,edgeMetaByIdJson:Ye,frontmatterVisibilityJson:Xe,initialFrontmatterEnabled:e.initialFrontmatterEnabled===!0,nodePosByIdJson:qe,groupMembersByIdJson:Ve,density:K,widthRatioDefault:De,widthRatioCompact:Ce,widthMinDefault:Pe,widthMinCompact:Re,widthMaxDefault:Oe,widthMaxCompact:Le,proxyOrigin:Ze}),Qe=_t("--kg-font-family","ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial");return`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  ...
  <style>
    :root{
      --kg-canvas-bg:${V(h)};
      --kg-panel-bg:${V(B)};
      --kg-media-panel-bg:${V(B)};
      --kg-border:${V(G)};
      --kg-text:${V(Y)};
      --kg-text-primary:${V(Y)};
      --kg-text-secondary:${V(C)};
      --kg-text-tertiary:${V(E)};
      --kg-panel-action-bg:${V(X)};
      --kg-panel-action-bg-hover:${V(T)};
      --kg-canvas-edge-stroke:${V(z)};
      --kg-canvas-node-stroke:${V(P)};
      --kg-canvas-accent:${V(R)};
      --kg-canvas-label-fill:${V(U)};
      --kg-canvas-label-halo:${V(q)};
      --kg-md-panel-header-bg:${V(O)};
      --kg-md-panel-header-h:${Q}px;
      --kg-media-panel-border-w:1px;
      --kg-media-panel-radius:${ee}px;
      --kg-media-panel-padding:${te}px;
      --kg-media-panel-title-size:${_e}px;
      --kg-media-pointer-events:auto;
    }
    html,body{height:100%;width:100%;margin:0;background:var(--kg-canvas-bg);color:var(--kg-text);font-family:${V(Qe)};-webkit-user-select:none;user-select:none;-webkit-text-size-adjust:100%;text-size-adjust:100%;overscroll-behavior:none}
...
<body>
  <div id="kg-root">
    <div id="kg-stage">
      ...
      <div id="kg-svgWrap">${$e}</div>
    </div>
    <div id="kg-overlay">${l}</div>
...
</html>`}const Pt=e=>{const r=String(e||"").toLowerCase();return r==="href"||r==="xlink:href"||r==="src"||r==="poster"},Rt=e=>String(e||"").toLowerCase()==="srcset",Ot=e=>{const r=[],a=e.attributes;for(let t=0;t<a.length;t+=1){const i=a.item(t);if(!i||!Pt(i.name)&&!Rt(i.name)&&String(i.name||"").toLowerCase()!=="style")continue;const d=String(i.value||"").trim();d&&r.push({name:i.name,value:d})}return r},Lt=async(e,r)=>{const a=String(e||"").trim();if(!a)return"";const t=a.split(",").map(d=>d.trim()).filter(Boolean);if(t.length===0)return a;const i=[];for(let d=0;d<t.length;d+=1){const o=t[d].split(/\s+/g).filter(Boolean);if(o.length===0)continue;const s=o[0],l=await r(s),m=o.slice(1).join(" ");i.push(m?`${l} ${m}`:l)}return i.join(", ")},Tt=async(e,r)=>{const a=String(e||"");if(!a.trim())return a;const t=/url\(\s*(['"]?)([^'"\)]+)\1\s*\)/g;let i="",d=0;for(;;){const n=t.exec(a);if(!n)break;const o=n[0],s=String(n[2]||"").trim();i+=a.slice(d,n.index),d=n.index+o.length;const l=s&&await r(s),m=n[1]||"";i+=`url(${m}${l}${m})`}return i+=a.slice(d),i};async function er(e){const r=String(e.svgMarkup||"");if(!r.trim())return"";if(typeof DOMParser!="function"||typeof XMLSerializer!="function")return r;let a=null;try{a=new DOMParser().parseFromString(r,"image/svg+xml")}catch{a=null}if(!a)return r;const t=a.documentElement;if(!t||String(t.tagName||"").toLowerCase()!=="svg")return r;const i=Array.from(t.querySelectorAll("*"));if(i.length===0)return r;const d=typeof e.maxInlineRepoBytes=="number"&&Number.isFinite(e.maxInlineRepoBytes)?Math.max(0,Math.floor(e.maxInlineRepoBytes)):24e5,n=async o=>{const s=String(o||"").trim();if(!s)return"";const l=Ie(s);return await Me(l,{maxBytes:d})||l};for(let o=0;o<i.length;o+=1){const s=i[o],l=Ot(s);if(l.length!==0)for(let m=0;m<l.length;m+=1){const u=l[m],c=u.value,p=String(u.name||"").toLowerCase();if(p==="srcset"){const B=await Lt(c,n);if(!B||B===c)continue;try{s.setAttribute(u.name,B)}catch{}continue}if(p==="style"){const B=await Tt(c,n);if(!B||B===c)continue;try{s.setAttribute(u.name,B)}catch{}continue}const h=await n(c);if(!(!h||h===c))try{s.setAttribute(u.name,h)}catch{}}}try{return new XMLSerializer().serializeToString(t)||r}catch{return r}}function tr(e){const r=e.graphData,a=e.schema;if(!r||!a)return null;const t=mt({graphData:r,edges:null}),i=dt(e.collapsedGroupIds),d=st(a),n=lt(t),o=ct({graphData:t,graphDataRevision:typeof e.graphDataRevision=="number"&&Number.isFinite(e.graphDataRevision)?Math.floor(e.graphDataRevision):0}),s=ut(a),l=String(e.documentSemanticModeKey||"document"),m=gt({schemaLayoutEngineJson:d,frontmatterModeEnabled:e.frontmatterModeEnabled,documentSemanticMode:l,graphMetaKey:n,renderMediaAsNodes:e.renderMediaAsNodes,mediaPanelDensity:String(e.mediaPanelDensity),collapsedGroupIdsKey:i}),u=String(e.canvas2dRenderer||"d3"),h=ht({datasetKey:o,mode:s,frontmatterMode:e.frontmatterModeEnabled,semanticMode:l,renderMode:"2d",renderVariant:u,layoutVariant:"",viewKey:m,prevViewKey:null,prevDatasetKey:null,prevMode:null,prevFrontmatterMode:null,prevSemanticMode:null,prevRenderMode:null,prevRenderVariant:null,prevLayoutVariant:null,nodes:Array.isArray(t.nodes)?t.nodes:[],layoutPositionCacheByMode:e.layoutPositionCacheByMode??null}).layoutPositionsForMode;return!h||Object.keys(h).length===0?null:h}const Be=e=>{const r=e;if(!(!r||!r.style))try{r.style.removeProperty("transform"),r.style.removeProperty("transform-origin"),r.style.removeProperty("translate"),r.style.removeProperty("left"),r.style.removeProperty("top"),r.style.removeProperty("right"),r.style.removeProperty("bottom"),r.style.removeProperty("inset"),r.style.removeProperty("position"),r.style.removeProperty("width"),r.style.removeProperty("height"),r.style.removeProperty("z-index")}catch{}},Ae=e=>{const r=e;if(!(!r||!r.style))try{r.style.display="none"}catch{}},zt=(e,r)=>{const a=e.querySelectorAll("img,video,source,iframe"),t=r.querySelectorAll("img,video,source,iframe"),i=Math.min(a.length,t.length);for(let d=0;d<i;d+=1){const n=a[d],o=t[d],s=String(n.tagName||"").toLowerCase();if(s==="img"){const l=n,m=o,u=String(l.currentSrc||l.src||m.getAttribute("src")||"").trim();if(u)try{m.setAttribute("src",u)}catch{}continue}if(s==="video"||s==="source"){const l=String(n.currentSrc||n.src||o.getAttribute("src")||"").trim();if(l)try{o.setAttribute("src",l)}catch{}continue}if(s==="iframe"){const l=n,m=o,u=typeof l.srcdoc=="string"?l.srcdoc:"";if(u){try{m.removeAttribute("src")}catch{}try{m.setAttribute("srcdoc",u)}catch{}continue}const c=String(l.src||o.getAttribute("src")||"").trim();if(c)try{m.setAttribute("src",c)}catch{}}}},Wt=e=>{if(typeof document>"u")return[];const r=[],a=new Set;for(let t=0;t<e.length;t+=1){const i=String(e[t]||"").trim();if(!i)continue;const d=Array.from(document.querySelectorAll(i));for(let n=0;n<d.length;n+=1){const o=d[n];!o||a.has(o)||(a.add(o),r.push(o))}}return r},Ne=(e,r)=>e?[e]:Wt(r),be=e=>{try{const r=e,a=getComputedStyle(r);if(a.display==="none"||a.visibility==="hidden")return 0;const t=Number(a.opacity),i=r.getClientRects().length;return i>0&&Number.isFinite(t)&&t>.01?3:i>0?2:1}catch{return 1}},Fe=(e,r,a)=>{if(!r)return;const t=e.get(r);if(!t){e.set(r,a);return}be(a)>=be(t)&&e.set(r,a)},xe=e=>String(e.getAttribute("data-md-id")||"").trim(),Ht=e=>String(e.getAttribute("data-kg-anchor-node-id")||"").trim(),Gt=(e,r)=>{try{e.setAttribute("data-md-id",r.id)}catch{}try{r.anchorNodeId?e.setAttribute("data-kg-anchor-node-id",r.anchorNodeId):e.removeAttribute("data-kg-anchor-node-id")}catch{}},Yt=e=>{var r;try{const a=Ne(e.overlayRootEl,['section[aria-label="D3 rich media overlay"]','section[aria-label="Flow media overlay"]','section[aria-label="Design media overlay"]','section[aria-label="3D media overlay"]',"#kg-overlay",'[data-kg-rich-media-overlay="1"]','[data-kg-rich-media-panel="1"]']);if(a.length===0)return"";const t=new Map;for(let n=0;n<a.length;n+=1){const o=a[n],s=(r=o.matches)!=null&&r.call(o,'[data-kg-rich-media-panel="1"][data-node-id]')?[o]:Array.from(o.querySelectorAll('[data-kg-rich-media-panel="1"][data-node-id]'));for(let l=0;l<s.length;l+=1){const m=s[l],u=String(m.getAttribute("data-node-id")||"").trim();u&&Fe(t,u,m)}}const i=Array.from(t.values());if(i.length===0)return"";const d=document.createElement("div");for(let n=0;n<i.length;n+=1){const o=i[n],s=o.cloneNode(!0);ke(o,s,Se),Be(s),Ae(s);try{zt(o,s)}catch{}d.appendChild(s)}return d.innerHTML}catch{return""}},Xt=e=>{var r;try{const a=Ne(e.overlayRootEl,['section[aria-label="Design markdown overlay"]','section[aria-label="Flow media overlay"]',"#kg-overlay",'[data-kg-markdown-design-overlay="1"]',"[data-md-id]"]);if(a.length===0)return"";const t=new Map;for(let n=0;n<a.length;n+=1){const o=a[n],s=(r=o.matches)!=null&&r.call(o,"[data-md-id]")?[o]:Array.from(o.querySelectorAll("[data-md-id]"));for(let l=0;l<s.length;l+=1){const m=s[l],u=xe(m);u&&Fe(t,u,m)}}const i=Array.from(t.values());if(i.length===0)return"";const d=document.createElement("div");for(let n=0;n<i.length;n+=1){const o=i[n],s=o.cloneNode(!0);ke(o,s,Se),Be(s),Ae(s);try{const l=xe(o),m=Ht(o);l&&Gt(s,{id:l,anchorNodeId:m})}catch{}d.appendChild(s)}return d.innerHTML}catch{return""}},rr=()=>{try{const e=Yt({overlayRootEl:null}),r=Xt({overlayRootEl:null});return`${e}${r}`}catch{return""}},ar=()=>{var e;try{const r=String(((e=globalThis==null?void 0:globalThis.localStorage)==null?void 0:e.getItem(we.viewportControlsPreset))||"").trim();return r==="map"||r==="design"?r:null}catch{return null}};export{ce as a,Qt as b,rr as c,er as d,Mt as e,tr as p,ar as r};
