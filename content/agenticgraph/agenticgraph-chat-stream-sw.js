;(function installAgenticGraphDurableChatStreamWorker() {
  const START = 'AG_CHAT_STREAM_START'
  const ATTACH = 'AG_CHAT_STREAM_ATTACH'
  const ABORT = 'AG_CHAT_STREAM_ABORT'
  const FORGET = 'AG_CHAT_STREAM_FORGET'
  const RESPONSE = 'AG_CHAT_STREAM_RESPONSE'
  const CHUNK = 'AG_CHAT_STREAM_CHUNK'
  const DONE = 'AG_CHAT_STREAM_DONE'
  const ERROR = 'AG_CHAT_STREAM_ERROR'
  const RUNTIME_ATTEST_REQUEST = 'AG_CHAT_STREAM_RUNTIME_ATTEST_REQUEST'
  const RUNTIME_ATTEST_RESPONSE = 'AG_CHAT_STREAM_RUNTIME_ATTEST_RESPONSE'
  const RUNTIME_SCHEMA = 'agenticgraph-chat-stream-worker/v2'
