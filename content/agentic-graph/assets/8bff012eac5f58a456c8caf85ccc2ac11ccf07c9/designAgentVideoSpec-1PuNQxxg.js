...
`;function ne(e){const t=e.graphData||null,i=H(t,e.selectedNodeIds),o=j(i).map((k,x)=>({...k,trackIndex:x,startMs:Math.min(h-240,x*N),durationMs:Math.max(240,Math.min(C,h-x*N))})),l=Y(o),a=G(),p=K(o),m=P(o),d=W(l),b=X(),u=A({graphData:t,graphRevision:e.graphRevision,maxEntries:8}),s=R("design-agent-video",{graphData:t,graphRevision:e.graphRevision,graphSemanticKey:[u.semanticKey,Array.from(_(e.selectedNodeIds)).sort().join(","),o.map(k=>k.id).join(",")].filter(Boolean).join(":")}),r={html:B(o),css:U(),data:{schema:I,semanticKey:s,composition:{id:"agentic-graph-design-agent-video",durationMs:h,fps:w,width:f,height:v},workspaceFiles:a,compositions:p,assets:m,timelineTracks:l,timelineLanes:d,timelineTicks:b,layers:o,tokenSummary:u},durationMs:h,fps:w,width:f,height:v,engineHint:D.canvas2d},g={id:s,type:F,label:e.title||O,properties:{html:r.html,css:r.css,data_json:JSON.stringify(r.data),duration_ms:r.durationMs,fps:r.fps,width:r.width,height:r.height,engine_hint:r.engineHint}};return{schema:I,semanticKey:s,renderSpec:r,flowNode:g,manifest:{schema:I,semanticKey:s,layerCount:o.length,selectedLayerCount:_(e.selectedNodeIds).size,workspaceFiles:a,compositions:p,assets:m,timelineTracks:l,timelineLanes:d,timelineTicks:b,tokenSummary:u}}}export{ne as b};
