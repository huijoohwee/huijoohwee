const e=`---
title: "agentic-graph Native MMORPG World"
...
planned_run_ready_demo:
  ...
  canonical_source_file: "/docs/workspace-seeds/agentic-graph-game-mmorpg-demo.md"
  env_selector: "VITE_AGENTIC_OS_RUN_READY_DEMO=mmorpg"
  validation_seed_path: "/agentic-graph-game-mmorpg-demo.md"
...
planned_mmorpg_world:
  ...
  web_mcp_schema: "agentic-graph-mmorpg-mcp/v1"
  inspect_tool: "agentic-graph.inspect_local_mmorpg"
  control_tool: "agentic-graph.control_local_mmorpg"
...
From the repository root, run \`npm run dev\`. In agentic-graph, open **Explorer → Source Files → docs → workspace-seeds → agentic-graph-game-mmorpg-demo.md**. Applying this document shows its 2D Flow Canvas design while Explorer remains available. It must leave XR and the proposed MMORPG runtime inactive.
...
The planned control contract uses \`agentic-graph.control_local_mmorpg\` and \`/mmorpg @canvas #world\`, with browser-local schema \`agentic-graph-mmorpg-mcp/v1\`. Those commands and tools are not registered while this document remains draft; their definitions below are acceptance targets, not current runtime claims.
