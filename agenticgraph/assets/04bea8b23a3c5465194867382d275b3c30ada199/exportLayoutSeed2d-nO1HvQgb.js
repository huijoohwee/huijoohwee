import{l as dt}from"./mediaOverlayPool-BDy-11xw.js";import{L as Ee,b8 as st,bc as Z,kj as lt,cS as ct,eN as ut,hg as gt,cP as pt,cQ as vt,cR as yt,ck as mt,cm as ft,cl as ht}from"./settings-mcp-core-CIu-0qJV.js";import{ck as bt}from"./index-BB8XdVDb.js";import{f as xt}from"./groupVisibility-7_qcxsx3.js";import{e as fe,a as wt}from"./overlaySizing2d-MZUnDmEM.js";import{c as kt}from"./markdownPanelOverlayPool-BFqGGh-Z.js";import{n as St}from"./cynefin-OW5HDTMX-BVgVrTjE.js";import{a as Me,H as Ae}from"./svgSnapshot-DYP4693U.js";import{aQ as It}from"./settings-byteplusModelArkMcpApiDocs-0UAMJ8tX.js";import{g as Et}from"./displayFilter-DB-Eh6TW.js";import{d as Mt}from"./positioning-BRI_mhmZ.js";const At=[`(function(){
    ...
    var AG_TOGGLE_MEDIA_INTERACTION_LABEL = 'Toggle media interaction';

    var AG_PROXY_ORIGIN = __AG_PROXY_ORIGIN__;
    var AG_PROXY_ORIGIN_RUNTIME = (typeof AG_PROXY_ORIGIN === 'string' ? String(AG_PROXY_ORIGIN || '').trim() : '');
    var AG_ALLOW_RUNTIME_NETWORK = __AG_ALLOW_RUNTIME_NETWORK__;

    var kgGetProxyOrigin = function(){
      try { return String(AG_PROXY_ORIGIN_RUNTIME || '').trim(); } catch (e) { return ''; }
    };

    var kgShouldUseProxy = function(){
      try {
        if (!AG_ALLOW_RUNTIME_NETWORK) return false;
    ...
    var kgFetchWebpageMeta = function(absUrl, onDone){
      try {
        if (!AG_ALLOW_RUNTIME_NETWORK) { try { onDone && onDone(null); } catch (e00) {} return; }
    ...
    var kgResolveMediaSrc = function(url, kind){
      ...
      if (!AG_ALLOW_RUNTIME_NETWORK && (u.startsWith('/__') || u.startsWith('/@') || /^https?:\\\\/\\\\//i.test(u))) return '';
    ...
    var markdownBlocks = __AG_MD_BLOCKS__;
    var nodeMetaById = __AG_NODE_META__;
    var edgeMetaById = __AG_EDGE_META__;
    var nodePosById = __AG_NODE_POS__;
    var groupMembersById = __AG_GROUP_MEMBERS__;
        ...
        if (kind === 'image' || kind === 'svg') {
          ...
          imgEl.onerror = function(){
            try {
              ...
              if (AG_ALLOW_RUNTIME_NETWORK && raw && cur !== raw) imgEl.src = raw;
            } catch (e11) {
              void 0;
            }
          };
          ...
          vid.onerror = function(){
            try {
              ...
              if (AG_ALLOW_RUNTIME_NETWORK && raw && cur !== raw) vid.src = raw;
            } catch (e12) {
              void 0;
            }
          };
    ...
    function updateOverlays(){
      ...
      if (!mediaNodes || mediaNodes.length === 0) return;
      var vp = getViewport();
      var density = __AG_DENSITY__;
      var widthRatio = density === 'compact' ? __AG_WIDTH_RATIO_COMPACT__ : __AG_WIDTH_RATIO_DEFAULT__;
      var widthMin = density === 'compact' ? __AG_WIDTH_MIN_COMPACT__ : __AG_WIDTH_MIN_DEFAULT__;
      var widthMax = density === 'compact' ? __AG_WIDTH_MAX_COMPACT__ : __AG_WIDTH_MAX_DEFAULT__;
    ...
    var __kgFrontmatterVis = __AG_FRONTMATTER_VIS__;
    ...
    setRichVisible(true);
    setFrontmatterEnabled(__AG_INITIAL_FRONTMATTER_ENABLED__);
    try {
      var kgPanelMode = '';
      try { kgPanelMode = String((window && window.localStorage) ? (window.localStorage.getItem('__AG_RICH_MEDIA_PANEL_MODE_LS_KEY__') || '') : ''); } catch (e0) { kgPanelMode = ''; }
...
`],Bt=()=>At.join(""),H=(e,t,a)=>!t||!e.includes(t)?e:e.split(t).join(a),K=(e,t,a,r)=>{if(!t)return e;const i=a,d=e.indexOf(t);return d<0?e:e.slice(0,d)+i+e.slice(d+t.length)},Nt=e=>{const t=String(e||"");if(!t)return"";const a=t.replace(/`/g,"\\`").replace(/\$\{/g,"\\${");try{return Function("return `"+a+"`")()}catch{return t}};function Ft(e){const t=(()=>{const i=String(e.markdownBlocksJson||"").trim();if(!i)return"[]";try{const d=JSON.parse(i);return Array.isArray(d)?i:"[]"}catch{return"[]"}})(),a=Nt(Bt());if(!a)return"";let r=a;return r=H(r,`var UI_IGNORE_SELECTOR = '[data-kg-canvas-wheel-ignore="true"], [data-kg-canvas-pointer-ignore="true"]';`,"var UI_IGNORE_SELECTOR = '#kg-hud, #kg-hud *';"),r=H(r,"var pointerMode = 'select';","var pointerMode = 'pan';"),r=H(r,`var src = String(edgeEl.getAttribute('data-source-id') || edgeEl.getAttribute('data-source') || '').trim();
      ...
      var tgt = __kgResolveNodeId(String(edgeEl.getAttribute('data-target-id') || edgeEl.getAttribute('data-target') || '').trim());`),r=H(r,"var src = String(ee.getAttribute('data-source-id') || ee.getAttribute('data-source') || '').trim();","var src = __kgResolveNodeId(String(ee.getAttribute('data-source-id') || ee.getAttribute('data-source') || '').trim());"),r=H(r,"var tgt = String(ee.getAttribute('data-target-id') || ee.getAttribute('data-target') || '').trim();","var tgt = __kgResolveNodeId(String(ee.getAttribute('data-target-id') || ee.getAttribute('data-target') || '').trim());"),r=H(r,"__AG_CFG__",e.interactionCfgJson),r=H(r,"__AG_MEDIA_NODES__",e.mediaNodesJson),r=H(r,"__AG_MD_BLOCKS__",t),r=H(r,"__AG_NODE_META__",e.nodeLabelByIdJson),r=H(r,"__AG_EDGE_META__",e.edgeMetaByIdJson),r=H(r,"__AG_NODE_POS__",e.nodePosByIdJson),r=H(r,"__AG_GROUP_MEMBERS__",e.groupMembersByIdJson),r=H(r,"__AG_FRONTMATTER_VIS__",e.frontmatterVisibilityJson),r=H(r,"__AG_INITIAL_FRONTMATTER_ENABLED__",e.initialFrontmatterEnabled===!0?"true":"false"),r=H(r,"__AG_RICH_MEDIA_PANEL_MODE_LS_KEY__",JSON.stringify(Ee.renderRichMediaPanelMode)),r=H(r,"__AG_DENSITY__",JSON.stringify(e.density)),r=H(r,"__AG_WIDTH_RATIO_DEFAULT__",String(e.widthRatioDefault)),r=H(r,"__AG_WIDTH_RATIO_COMPACT__",String(e.widthRatioCompact)),r=H(r,"__AG_WIDTH_MIN_DEFAULT__",String(e.widthMinDefault)),r=H(r,"__AG_WIDTH_MIN_COMPACT__",String(e.widthMinCompact)),r=H(r,"__AG_WIDTH_MAX_DEFAULT__",String(e.widthMaxDefault)),r=H(r,"__AG_WIDTH_MAX_COMPACT__",String(e.widthMaxCompact)),r=K(r,"var tip = document.getElementById('kg-tooltip');",`var tip = document.getElementById('kg-tooltip');
    ...
    }`),r=K(r,`if (fitBtn) fitBtn.addEventListener('click', function(){ fitToCenter(); });
    if (resetBtn) resetBtn.addEventListener('click', function(){ resetView(); });
    ...
    if (resetBtn) resetBtn.addEventListener('click', function(){ resetView(); });
    if (mediaBtn) mediaBtn.addEventListener('click', function(){ setMediaInteractive(!mediaInteractive); });`),r=K(r,`function applyMediaPointerEvents(){
      var pe = (mediaInteractive && pointerMode !== 'pan' && !panHeld && !headerDrag) ? 'auto' : 'none';
      try {
        if (pe !== lastMediaPointerEvents) {
          lastMediaPointerEvents = pe;
    ...
    }`),r=K(r,"var mediaInteractive = false;","var mediaInteractive = true;"),r=K(r,`overlay.__kgMediaById[id] = panel;
      }
    }
    ...
    var overlayRaf = null;`),r=K(r,`ensureMediaDom();
      ...
      if ((!mediaNodes || mediaNodes.length === 0) && (!markdownBlocks || markdownBlocks.length === 0)) return;`),r=K(r,"if (!markdownBlocks || markdownBlocks.length === 0) return;",`if (!markdownBlocks || markdownBlocks.length === 0) return;
      try {
      ...
      } catch (eSkip) {}`),r=K(r,`lastBoxById[id] = { left: left, top: top, w: panelW, h: panelH, display: 'block' };
        }
      }
    }
    ...
    function onWheel(e){`),r=K(r,`} else {
        ...
        }`),r=K(r,"function fitToCenter(){",`function getContentCentroid(){
      try {
        if (nodePosById) {
          var sx = 0;
          var sy = 0;
    ...
    function fitToCenter(){`),r=K(r,`var cx = bb.x + bb.width / 2;
      var cy = bb.y + bb.height / 2;`,`var c = getContentCentroid();
      var cx = (c && isFinite(c.x)) ? c.x : (bb.x + bb.width / 2);
      var cy = (c && isFinite(c.y)) ? c.y : (bb.y + bb.height / 2);`),r=K(r,"if (t && t.closest && (t.closest('[data-node-id]') || t.closest('[data-edge-id]') || t.closest('[data-kg-group-id]') || t.closest('.kg-media'))) return;","if (t && t.closest && t.closest('[data-edge-id]')) return;"),r=K(r,`map[nodeId] = { x: ox + dx, y: oy + dy };
          return;`,`map[nodeId] = { x: ox + dx, y: oy + dy };
          try { scheduleEdgeGeometryUpdateForNode(nodeId); } catch (e0) {}
          return;`),r=K(r,"map[id] = { x: ox + dx, y: oy + dy };",`map[id] = { x: ox + dx, y: oy + dy };
              try { scheduleEdgeGeometryUpdateForNode(id); } catch (e0) {}`),r=K(r,"var allEdgeEls = svg ? svg.querySelectorAll('line[data-edge-id],path[data-edge-id],polyline[data-edge-id]') : null;",`try {
        if (svg && edgeMetaById && nodePosById) {
