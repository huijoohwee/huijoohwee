...
} from "../../canvas/src/features/agent-ready/agentic-graph-agent-ready-prompt-contract.mjs";
import {
...
} from "../../canvas/src/features/agent-ready/agentic-graph-agent-ready-resource-contract.mjs";
import {
  AGENTIC_OS_MCP_APP_RESOURCE_URI,
  AGENTIC_OS_MCP_REMOTE_TRANSPORT_TYPE,
...
import { buildAgenticGraphVdeoxplnMarkdownByName } from "../../canvas/src/features/agent-ready/agentic-graph-vdeoxpln-contract.mjs";
import {
...
} from "./agentic-graph-agent-ready-app-shell.mjs";
...
import {
  ...
  HEALTH_PATH,
  HEALTH_URL,
  AGENTIC_OS_AGENT_READY_ROUTE_OWNER,
  markdownResponse,
  ROOT_URL,
  SITE_ORIGIN,
...
const CONTROL_PLANE_MCP_URL = resolveAgenticOsControlPlaneMcpUrl(SITE_ORIGIN);
...
const readGitHubWriteConfig = (env) => {
  const repository = readEnvString(env, "AGENTIC_OS_GITHUB_WRITE_REPOSITORY");
  const token = readEnvString(env, "AGENTIC_OS_GITHUB_WRITE_TOKEN");
  const branch = readEnvString(env, "AGENTIC_OS_GITHUB_WRITE_BRANCH");
  const missing = [];
  if (!repository) missing.push("AGENTIC_OS_GITHUB_WRITE_REPOSITORY");
  if (!token) missing.push("AGENTIC_OS_GITHUB_WRITE_TOKEN");
  const parts = repository.split("/").map((part) => part.trim()).filter(Boolean);
  if (repository && parts.length !== 2) missing.push("AGENTIC_OS_GITHUB_WRITE_REPOSITORY:owner/repo");
...
const gitHubApiHeaders = (config) => ({
  ...
  "user-agent": "agentic-graph-cloudflare-pages",
...
const handleGitHubWorkspaceWrite = async (request, env) => {
  ...
  const message = messageText && messageText.length <= 160
    ? messageText
    : `agentic-graph chat artifact ${files[0].repositoryPath}`;
  if (body?.dryRun === true) {
...
const openApi = {
  openapi: "3.1.0",
  info: {
    title: "agentic-graph API",
    version: "0.1.0",
    description: "Agent discovery surface for the agentic-graph Cloudflare deployment.",
  },
  servers: [
    { url: SITE_ORIGIN, description: "agentic-graph Cloudflare deployment" },
  ],
...
const oauthProtectedResource = { resource: APP_URL, resource_name: "agentic-graph", authorization_servers: [SITE_ORIGIN], scopes_supported: ["agentic-graph:read", "agentic-graph:source-files:read"], bearer_methods_supported: ["header"], resource_documentation: `${APP_URL}llms.txt` };
...
const mcpServerCard = {
  ...
  transport: {
    type: AGENTIC_OS_MCP_REMOTE_TRANSPORT_TYPE,
    url: `${APP_URL}mcp`,
    stateless: true,
  },
  capabilities: {
  ...
  surfaceRoles: {
    ...
    controlPlaneMcpUrl: CONTROL_PLANE_MCP_URL,
    ...
    remoteGrammarInvokeToolName: "agentic-graph.agentic_canvas_os.docs.invoke",
  ...
  links: {
    ...
    controlPlaneMcp: CONTROL_PLANE_MCP_URL,
  },
};
...
const SEARCH_WEB_TOOL_NAME = findWebMcpToolName(AGENTIC_OS_AGENT_READY_TOOL_IDS.search);
const FETCH_WEB_TOOL_NAME = findWebMcpToolName(AGENTIC_OS_AGENT_READY_TOOL_IDS.fetch);
const LIST_SOURCE_FILES_WEB_TOOL_NAME = findWebMcpToolName(AGENTIC_OS_AGENT_READY_TOOL_IDS.listSourceFiles);
const READ_SOURCE_FILE_WEB_TOOL_NAME = findWebMcpToolName(AGENTIC_OS_AGENT_READY_TOOL_IDS.readSourceFile);
const READ_SHARED_DOCUMENT_WEB_TOOL_NAME = findWebMcpToolName(AGENTIC_OS_AGENT_READY_TOOL_IDS.readSharedDocument);
const INSPECT_SHARED_DOCUMENT_STRUCTURE_WEB_TOOL_NAME = findWebMcpToolName(AGENTIC_OS_AGENT_READY_TOOL_IDS.inspectSharedDocumentStructure);
const INSPECT_AGENT_SURFACE_WEB_TOOL_NAME = findWebMcpToolName(AGENTIC_OS_AGENT_READY_TOOL_IDS.inspectAgentSurface);
...
export const webMcpScript = `(() => {
  ...
  const createWebMcpLifecycleController = ${WEB_MCP_LIFECYCLE_CONTROLLER_BROWSER_SOURCE};
  const toolExecutors = createPublishedAgentReadyToolExecutors({
    ...
    buildAgentSurfaceInspection: createAgentSurfaceInspectionExecutor({
      ...
      toolName: ${JSON.stringify(AGENTIC_OS_AGENT_READY_TOOL_IDS.inspectAgentSurface)},
    }),
  });
...
const PUBLISHED_TOOL_NAME_CONFIG = {
  ...
  listSourceFiles: AGENTIC_OS_AGENT_READY_TOOL_IDS.listSourceFiles,
  readSourceFile: AGENTIC_OS_AGENT_READY_TOOL_IDS.readSourceFile,
  readSharedDocument: AGENTIC_OS_AGENT_READY_TOOL_IDS.readSharedDocument,
  inspectSharedDocumentStructure: AGENTIC_OS_AGENT_READY_TOOL_IDS.inspectSharedDocumentStructure,
  inspectAgentSurface: AGENTIC_OS_AGENT_READY_TOOL_IDS.inspectAgentSurface,
};
  ...
  keys: [
    {
      kty: "OKP",
      crv: "Ed25519",
      kid: "agentic-graph-agent-ready-2026-05-21",
      use: "sig",
      alg: "EdDSA",
...
const buildHealthStatusBody = () => ({
  status: "pass",
  service: "agentic-agent-ready-pages",
  homepage: APP_URL,
  health: HEALTH_URL,
  updatedAt: UPDATED_AT,
  checks: {
...
const withAgenticGraphRouteHeaders = (request, response) =>
  withAgentReadyRouteHeaders(response, {
    owner: AGENTIC_OS_AGENT_READY_ROUTE_OWNER,
...
const routeResponse = async (request, env = {}) => {
  ...
  switch (pathname) {
    ...
    case `${APP_BASE_PATH}/.well-known/mcp/apps/agentic-graph-agent-ready.html`:
...
export async function onRequest(context) {
  try {
    ...
    console.error(JSON.stringify({
      message: "agentic-graph Pages request failed",
      pathname,
