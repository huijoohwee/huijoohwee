import{aG as Ke,aH as Ze,a7 as $e,H as Je,J as Qe,aI as et,aJ as tt,aK as rt,aL as nt,aM as ue,aN as ve}from"./Canvas-BQQh-50a.js";import{L as pe,e as at,r as Q,bM as it,aK as ot,F as dt,H as st,E as lt,A as ct,aL as gt,G as ut}from"./index-virYFek3.js";const vt=[`(function(){
...
`],pt=()=>vt.join(""),M=(t,r,n)=>!r||!t.includes(r)?t:t.split(r).join(n),x=(t,r,n,e)=>{if(!r)return t;const o=typeof e=="string"?e:n,d=t.indexOf(r);return d<0?t:t.slice(0,d)+o+t.slice(d+r.length)},yt=t=>{const r=String(t||"");if(!r)return"";const n=r.replace(/`/g,"\\`").replace(/\$\{/g,"\\${");try{return Function("return `"+n+"`")()}catch{return r}};function ft(t){const r=(()=>{const d=String(t.markdownBlocksJson||"").trim();if(!d)return"[]";try{const a=JSON.parse(d);return Array.isArray(a)?d:"[]"}catch{return"[]"}})(),n=yt(pt());if(!n)return"";let e=n;if(e=x(e,`function ensureMediaDom(){
      if (!overlay) return;
      if (!mediaNodes || mediaNodes.length === 0) return;
      if (overlay.__kgMediaBuilt) return;
      overlay.__kgMediaBuilt = true;
...
`),e=x(e,"var kgResolveMediaSrc = function(url, kind){",`var kgIsWeChatHotlinkProtectedAssetUrl = function(absUrl){
    ...
    try {
      var kgPanelMode = '';
      try { kgPanelMode = String((window && window.localStorage) ? (window.localStorage.getItem('${pe.renderRichMediaPanelMode}') || '') : ''); } catch (e0) { kgPanelMode = ''; }
    ...
    function onWheel(e){`),e=M(e,"var xid2 = String(ex2.getAttribute('data-node-id') || '').trim();","var xid2 = __kgResolveNodeId(String(ex2.getAttribute('data-node-id') || '').trim());"),e=M(e,"var xanchor = String(ex.getAttribute('data-kg-anchor-node-id') || '').trim();","var xanchor = __kgResolveNodeId(String(ex.getAttribute('data-kg-anchor-node-id') || '').trim());"),e=M(e,`var src = String(edgeEl.getAttribute('data-source-id') || edgeEl.getAttribute('data-source') || '').trim();
      var tgt = String(edgeEl.getAttribute('data-target-id') || edgeEl.getAttribute('data-target') || '').trim();`,`var src = __kgResolveNodeId(String(edgeEl.getAttribute('data-source-id') || edgeEl.getAttribute('data-source') || '').trim());
      var tgt = __kgResolveNodeId(String(edgeEl.getAttribute('data-target-id') || edgeEl.getAttribute('data-target') || '').trim());`),e=M(e,"el.setAttribute('data-kg-canvas-wheel-ignore', 'true');",""),e=M(e,`var UI_IGNORE_SELECTOR = '[data-kg-canvas-wheel-ignore="true"], [data-kg-canvas-pointer-ignore="true"]';`,"var UI_IGNORE_SELECTOR = '#kg-hud, #kg-hud *';"),!e.includes("var markdownBlocks =")){const d=e.match(/var\s+mediaNodes\s*=\s*[\s\S]*?;/);if(d&&d.index!=null){const a=d.index+d[0].length;e=`${e.slice(0,a)}
    var markdownBlocks = ${r};${e.slice(a)}`}else{const i=e.indexOf("var nodeMetaById = ");i>=0?e=`${e.slice(0,i)}var markdownBlocks = ${r};
    ${e.slice(i)}`:e=`var markdownBlocks = ${r};
`+e}}return e=M(e,"__KG_PROXY_ORIGIN__",JSON.stringify(String(t.proxyOrigin||""))),e}function K(t){const r=Number(t);return Number.isFinite(r)?r:NaN}function mt(t){const r=String(t||"").trim();if(!r)return null;const n=r.match(/translate\(\s*([-0-9.]+)\s*(?:[, ]\s*([-0-9.]+)\s*)?\)/i);if(n){const o=Number(n[1]),d=Number(n[2]);if(Number.isFinite(o)&&Number.isFinite(d))return{x:o,y:d};if(Number.isFinite(o))return{x:o,y:0}}const e=r.match(/matrix\(\s*([-0-9.]+)\s*[ ,]\s*([-0-9.]+)\s*[ ,]\s*([-0-9.]+)\s*[ ,]\s*([-0-9.]+)\s*[ ,]\s*([-0-9.]+)\s*[ ,]\s*([-0-9.]+)\s*\)/i);if(e){const o=Number(e[5]),d=Number(e[6]);if(Number.isFinite(o)&&Number.isFinite(d))return{x:o,y:d}}return null}function ht(t){if(!t||!t.getAttribute)return null;const r=String(t.tagName||"").toLowerCase();if(r==="circle"){const e=K(t.getAttribute("cx")),o=K(t.getAttribute("cy"));return Number.isFinite(e)&&Number.isFinite(o)?{x:e,y:o}:null}if(r==="rect"){const e=K(t.getAttribute("x")),o=K(t.getAttribute("y")),d=K(t.getAttribute("width")),a=K(t.getAttribute("height"));return Number.isFinite(e)&&Number.isFinite(o)&&Number.isFinite(d)&&Number.isFinite(a)&&d>0&&a>0?{x:e+d/2,y:o+a/2}:null}const n=mt(String(t.getAttribute("transform")||""));if(r==="g"&&t.querySelector){let e=null;try{const o=t.querySelector('circle[data-role="node-circle"], circle[cx][cy]');if(o){const d=K(o.getAttribute("cx")),a=K(o.getAttribute("cy"));Number.isFinite(d)&&Number.isFinite(a)&&(e={x:d,y:a})}}catch{}if(!e)try{const o=t.querySelector("rect[x][y][width][height]");if(o){const d=K(o.getAttribute("x")),a=K(o.getAttribute("y")),i=K(o.getAttribute("width")),s=K(o.getAttribute("height"));Number.isFinite(d)&&Number.isFinite(a)&&Number.isFinite(i)&&Number.isFinite(s)&&i>0&&s>0&&(e={x:d+i/2,y:a+s/2})}}catch{}return e&&n?{x:e.x+n.x,y:e.y+n.y}:e||n||null}return n||null}function xt(t){try{return!!(t.hasAttribute("data-port-key")||t.hasAttribute("data-port-dir"))}catch{return!1}}function oe(t){const r=String(t||"").trim();if(!r)return{};const n=r.replace(/^<\?xml[^>]*>\s*/i,"");try{const o=new DOMParser().parseFromString(n,"image/svg+xml").querySelector("svg");if(!o)return{};const d={},a=o.querySelectorAll("[data-node-id]");for(let s=0;s<a.length;s+=1){const c=a[s];if(xt(c))continue;const b=String(c.getAttribute("data-node-id")||"").trim();if(!b||d[b])continue;const g=ht(c);if(g){d[b]=g;continue}}const i=o.querySelectorAll("line[data-source-id][data-target-id]");for(let s=0;s<i.length;s+=1){const c=i[s],b=String(c.getAttribute("data-source-id")||"").trim(),g=String(c.getAttribute("data-target-id")||"").trim();if(!b&&!g)continue;const l=K(c.getAttribute("x1")),p=K(c.getAttribute("y1")),w=K(c.getAttribute("x2")),N=K(c.getAttribute("y2"));b&&!d[b]&&Number.isFinite(l)&&Number.isFinite(p)&&(d[b]={x:l,y:p}),g&&!d[g]&&Number.isFinite(w)&&Number.isFinite(N)&&(d[g]={x:w,y:N})}return d}catch{return{}}}const ne=t=>typeof t=="number"&&Number.isFinite(t);function bt(t){var n,e,o;const r=String(t.svgMarkup||"").trim();if(!r||typeof DOMParser>"u"||typeof XMLSerializer>"u")return r;try{const d=new DOMParser().parseFromString(r.replace(/^<\?xml[^>]*>\s*/i,""),"image/svg+xml"),a=d.querySelector("svg");if(!a)return r;const i=a.querySelector('[data-kg-layer="links"]');if(!i||i.querySelector("line,path,polyline"))return r;const s=Array.isArray((n=t.graphData)==null?void 0:n.nodes)?t.graphData.nodes:[],c=Array.isArray((e=t.graphData)==null?void 0:e.edges)?t.graphData.edges:[];if(c.length===0)return r;const b=new Set,g={};for(let D=0;D<s.length;D+=1){const m=String(((o=s[D])==null?void 0:o.id)||"").trim();if(!m)continue;b.add(m);const F=m.split("::").pop()||"";F&&!g[F]&&(g[F]=m)}const l=D=>{const m=String(D||"").trim();if(!m)return"";if(b.has(m))return m;const F=m.split("::").pop()||"";return F&&g[F]?g[F]:m},w={...oe(r),...t.nodePosById||{}},N=a.namespaceURI||"http://www.w3.org/2000/svg";for(let D=0;D<c.length;D+=1){const m=c[D],F=String((m==null?void 0:m.id)||"").trim()||`e${D}`,L=String((m==null?void 0:m.sourceId)||(m==null?void 0:m.source)||"").trim(),Z=String((m==null?void 0:m.targetId)||(m==null?void 0:m.target)||"").trim(),$=l(L),q=l(Z);if(!F||!$||!q)continue;const X=w[$],z=w[q];if(!X||!z||!ne(X.x)||!ne(X.y)||!ne(z.x)||!ne(z.y))continue;const T=d.createElementNS(N,"line");T.setAttribute("data-edge-id",F),T.setAttribute("data-source-id",$),T.setAttribute("data-target-id",q),T.setAttribute("x1",String(X.x)),T.setAttribute("y1",String(X.y)),T.setAttribute("x2",String(z.x)),T.setAttribute("y2",String(z.y)),T.setAttribute("stroke","var(--kg-canvas-edge-stroke)"),T.setAttribute("stroke-opacity","1"),T.setAttribute("stroke-width","2"),T.setAttribute("stroke-linecap","round"),T.setAttribute("fill","none"),i.appendChild(T)}const H=new XMLSerializer().serializeToString(a);return String(H||"").trim()||r}catch{return r}}const le=t=>/^https?:\/\//i.test(String(t||"").trim()),ye=t=>{const r=String(t||"").trim();if(!r)return"";if(/^(data:|blob:|mailto:|tel:|javascript:)/i.test(r))return r;const n=a=>{try{return decodeURIComponent(a)}catch{return a}},o=(a=>{var i;try{if(/^https?:\/\//i.test(a))return new URL(a);if(typeof window>"u")return null;const s=(i=window.location)==null?void 0:i.origin;return!s||!/^https?:\/\//i.test(String(s))?null:new URL(a,s)}catch{return null}})(r);if(!o)return r;const d=o.pathname||"";if(d==="/__fetch_remote"||d==="/__webpage_proxy"||d==="/__webpage_asset_proxy"){const a=o.searchParams.get("url");if(!a)return r;const i=n(a);return le(i)?i:i||r}if(d.startsWith("/__webpage_asset_path/")){const i=d.slice(22).split("/").filter(Boolean);if(i.length>=1){const s=i[0]||"",c=n(s);if(le(c)){const b="/"+i.slice(1).join("/"),g=o.search||"";return c.replace(/\/+$/,"")+b+g}}}return r},fe=t=>{const r=String(t||"").trim();if(!r.startsWith("/__repo_file/"))return null;const n=r.slice(13);return n&&n.split("/").filter(Boolean).map(o=>{try{return decodeURIComponent(o)}catch{return o}}).join("/")||null},kt=t=>{const r=String(t||"").trim(),n=r.lastIndexOf("."),e=n>=0?r.slice(n+1).toLowerCase():"";return e==="svg"?"image/svg+xml":e==="png"?"image/png":e==="jpg"||e==="jpeg"?"image/jpeg":e==="gif"?"image/gif":e==="webp"?"image/webp":e==="mp4"?"video/mp4":e==="webm"?"video/webm":e==="json"?"application/json":e==="html"||e==="htm"?"text/html":"application/octet-stream"},wt=t=>{const r=globalThis.Buffer;if(r)return r.from(t).toString("base64");let n="";const e=32768;for(let d=0;d<t.length;d+=e){const a=t.subarray(d,Math.min(t.length,d+e));n+=String.fromCharCode(...a)}const o=globalThis.btoa;return o?o(n):""},St=async t=>{try{if(typeof fetch!="function")return null;const r=await fetch(t);if(!r||!r.ok)return null;const n=await r.arrayBuffer();return new Uint8Array(n)}catch{return null}},me=async(t,r)=>{const n=String(t||"").trim(),e=fe(n);if(!e)return null;const o=typeof(r==null?void 0:r.maxBytes)=="number"&&Number.isFinite(r.maxBytes)?Math.max(0,Math.floor(r.maxBytes)):9e5,d=(()=>{const c=String((r==null?void 0:r.origin)||"").trim();if(c&&/^https?:\/\//i.test(c))return c.replace(/\/+$/,"")+n;try{if(typeof window<"u"&&window.location&&/^https?:$/.test(String(window.location.protocol||"")))return String(window.location.origin||"").replace(/\/+$/,"")+n}catch{}return n})(),a=await St(d);if(!a||a.length===0||a.length>o)return null;const i=kt(e),s=wt(a);return s?`data:${i};base64,${s}`:null},W=t=>typeof t=="number"&&Number.isFinite(t),Y=t=>String(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),ae=(t,r)=>{const n=String(t.getAttribute(r)||"").trim();if(!n)return null;const e=Number(n);return Number.isFinite(e)?e:null},ce=t=>{const r=tt(t);if(r==="image"||r==="svg"||r==="video")return r;const n=String(t||"").trim().toLowerCase();return n&&(n.includes("wx_fmt=png")||n.includes("wx_fmt=jpg")||n.includes("wx_fmt=jpeg")||n.includes("wx_fmt=gif")||n.includes("wx_fmt=webp"))?"image":"iframe"},It=(t,r)=>{try{return typeof document>"u"?r:String(getComputedStyle(document.documentElement).getPropertyValue(t)||"").trim()||r}catch{return r}},Et=t=>{var g;const r=t.graph,n=String(t.svgMarkup||""),e=Array.isArray(r==null?void 0:r.nodes)?r.nodes:[],o=new Set;for(let l=0;l<e.length;l+=1){const p=String(((g=e[l])==null?void 0:g.id)||"").trim();p&&o.add(p)}if(n.trim()){const l=/\bdata-node-id\s*=\s*(?:"([^"]+)"|'([^']+)')/gi;let p=null;for(;p=l.exec(n);){const w=String(p[1]||p[2]||"").trim();w&&o.add(w)}}const d={};for(const l of o){const p=l.split("::").pop()||"";p&&!d[p]&&(d[p]=l)}const a=l=>{const p=String(l||"").trim();if(!p)return!0;const w=p.toLowerCase();return!!(w.includes("::blk:")||w.includes("::block:")||w.startsWith("blk:"))},i=l=>{const p=String(l||"").trim();if(!p)return"";if(o.has(p)||a(p))return p;const w=p.split("::").pop()||"";return w&&d[w]||p},s=Array.isArray(r==null?void 0:r.edges)?r.edges:[],c=new Set,b=l=>{if(typeof l=="string")return String(l||"").trim();if(l&&typeof l=="object"){const p=l.id;if(typeof p=="string")return String(p||"").trim()}return String(l||"").trim()};for(let l=0;l<s.length;l+=1){const p=s[l],w=p.source,N=p.target,H=p.sourceId??p.source_id,G=p.targetId??p.target_id,D=i(b(w??H)),m=i(b(N??G));D&&o.has(D)&&c.add(D),m&&o.has(m)&&c.add(m)}if(n.trim()){const l=/\bdata-(?:source-id|source|target-id|target)\s*=\s*(?:"([^"]+)"|'([^']+)')/gi;let p=null;for(;p=l.exec(n);){const w=String(p[1]||p[2]||"").trim();if(!w)continue;const N=i(w);N&&o.has(N)&&c.add(N)}}return{nodeIdSet:o,edgeLinkedNodeIdSet:c,resolveNodeId:i}},Mt=t=>{const r=String(t.overlayHtml||"");if(!r.trim())return"";if(!t.hasGraphNodeIdentity)return r;const n=d=>{const a=String(d.anchorNorm||d.anchorRaw||"").trim();if(a)return`md:${a}`;const i=String(d.idNorm||d.idRaw||"").trim();return i?`md:${i}`:""},e=d=>{const a=String(d||"").toLowerCase();if(!a)return 0;if(/(?:^|;)\s*display\s*:\s*none\b/i.test(a))return-200;if(/(?:^|;)\s*visibility\s*:\s*hidden\b/i.test(a))return-180;if(/(?:^|;)\s*opacity\s*:\s*0(?:\D|$)/i.test(a))return-160;const i=/(?:^|;)\s*position\s*:\s*(?:fixed|sticky)\b/i.test(a),s=/(?:^|;)\s*(?:left|top|right|bottom|inset)\s*:/i.test(a);return i&&s?-40:i?-20:0},o=d=>{var G,D;const a=String(d||"");if(!a.trim())return"";const i=(m,F,L)=>L?String(m||"").replace(new RegExp(`\\s${F}=(?:"[^"]*"|'[^']*')`,"i"),"").replace(/>$/,` ${F}="${L}">`):m,s=(m,F)=>{const L=String(m||""),Z=`${F}="`,$=`${F}='`,q=L.indexOf(Z);if(q>=0){const z=q+Z.length,T=L.indexOf('"',z);return T>z?L.slice(z,T).trim():""}const X=L.indexOf($);if(X>=0){const z=X+$.length,T=L.indexOf("'",z);return T>z?L.slice(z,T).trim():""}return""},c=[];let b=!1;const g=new Map,l=new Map,p=/<article\b[^>]*data-kg-rich-media-panel=(?:"1"|'1')[^>]*>[\s\S]*?<\/article>/gi;let w=null;for(;w=p.exec(a);){b=!0;const m=w[0]||"",F=((G=m.match(/<article\b[^>]*>/i))==null?void 0:G[0])||"",L=t.resolveOverlayNodeId(s(F,"data-node-id")),Z=L?`media:${L}`:"";if(!L||!Z||!t.shouldKeepOverlayLinkedToGraph({nodeId:L}))continue;const $=m.replace(F,i(F,"data-node-id",L)),q=e(s(F,"style")),X=g.get(Z);(!X||q>X.score)&&g.set(Z,{chunk:$,score:q})}const N=/<article\b[^>]*(?:data-kg-markdown-design-block|data-md-id)[^>]*>[\s\S]*?<\/article>/gi;let H=null;for(;H=N.exec(a);){b=!0;const m=H[0]||"",F=((D=m.match(/<article\b[^>]*>/i))==null?void 0:D[0])||"",L=s(F,"data-kg-markdown-design-block")||s(F,"data-md-id"),Z=s(F,"data-kg-anchor-node-id")||s(F,"data-node-id"),$=t.resolveOverlayNodeId(L),q=t.resolveOverlayNodeId(Z),X=n({idRaw:L,idNorm:$,anchorRaw:Z,anchorNorm:q});if(!X||!t.shouldKeepOverlayLinkedToGraph({nodeId:$||L,anchorNodeId:q||Z}))continue;let z=F;$&&(z=i(z,"data-md-id",$)),q&&(z=i(z,"data-kg-anchor-node-id",q));const T=m.replace(F,z),ee=e(s(F,"style")),re=l.get(X);(!re||ee>re.score)&&l.set(X,{chunk:T,score:ee})}for(const m of g.values())m.score<=-100||c.push(m.chunk);for(const m of l.values())m.score<=-100||c.push(m.chunk);return c.length<=0?b?"":a:c.join("")};try{if(typeof DOMParser>"u")return o(r);const a=new DOMParser().parseFromString(`<!doctype html><html><body><div id="kg-overlay-filter-root">${r}</div></body></html>`,"text/html").querySelector("#kg-overlay-filter-root");if(!a)return r;const i=new Map,s=a.querySelectorAll('[data-kg-rich-media-panel="1"][data-node-id], .kg-media[data-node-id]');for(let g=0;g<s.length;g+=1){const l=s[g],p=String(l.getAttribute("data-node-id")||"").trim(),w=t.resolveOverlayNodeId(p),N=w?`media:${w}`:"";if(!(!!w&&t.shouldKeepOverlayLinkedToGraph({nodeId:w}))||!N){l.remove();continue}const G=e(String(l.getAttribute("style")||"").trim()),D=i.get(N);(!D||G>D.score)&&i.set(N,{el:l,score:G});try{l.setAttribute("data-node-id",w)}catch{}}for(let g=0;g<s.length;g+=1){const l=s[g];if(!l||!l.isConnected)continue;const p=t.resolveOverlayNodeId(String(l.getAttribute("data-node-id")||"").trim()),w=p?`media:${p}`:"",N=w?i.get(w):null,H=(N==null?void 0:N.el)||null;if(!H||H!==l){l.remove();continue}((N==null?void 0:N.score)??0)<=-100&&l.remove()}const c=new Map,b=a.querySelectorAll("[data-kg-markdown-design-block],[data-md-id],[data-node-id][data-kg-md-panel]");for(let g=0;g<b.length;g+=1){const l=b[g],p=String(l.getAttribute("data-kg-markdown-design-block")||l.getAttribute("data-md-id")||"").trim(),w=String(l.getAttribute("data-kg-anchor-node-id")||l.getAttribute("data-node-id")||"").trim(),N=t.resolveOverlayNodeId(p),H=t.resolveOverlayNodeId(w),G=n({idRaw:p,idNorm:N,anchorRaw:w,anchorNorm:H});if(!t.shouldKeepOverlayLinkedToGraph({nodeId:N||p,anchorNodeId:H||w})||!G){l.remove();continue}const m=e(String(l.getAttribute("style")||"").trim()),F=c.get(G);if((!F||m>F.score)&&c.set(G,{el:l,score:m}),N)try{l.setAttribute("data-md-id",N)}catch{}if(H)try{l.setAttribute("data-kg-anchor-node-id",H)}catch{}}for(let g=0;g<b.length;g+=1){const l=b[g];if(!l||!l.isConnected)continue;const p=String(l.getAttribute("data-kg-markdown-design-block")||l.getAttribute("data-md-id")||"").trim(),w=String(l.getAttribute("data-kg-anchor-node-id")||l.getAttribute("data-node-id")||"").trim(),N=t.resolveOverlayNodeId(p),H=t.resolveOverlayNodeId(w),G=n({idRaw:p,idNorm:N,anchorRaw:w,anchorNorm:H}),D=G?c.get(G):null,m=(D==null?void 0:D.el)||null;if(!m||m!==l){l.remove();continue}((D==null?void 0:D.score)??0)<=-100&&l.remove()}return a.innerHTML}catch{return o(r)}};async function zt(t){try{at()}catch{}const r=String(t.title||"").trim()||"Graph viewer",n=String(t.svgMarkup||"").trim(),e=t.initialView&&W(t.initialView.k)&&W(t.initialView.x)&&W(t.initialView.y)?t.initialView:null,o=W(t.viewportWidthPx)&&W(t.viewportHeightPx)?{w:Math.max(1,Math.floor(t.viewportWidthPx)),h:Math.max(1,Math.floor(t.viewportHeightPx))}:null,d=String(t.overlayHtml||""),a=Et({graph:t.graphData,svgMarkup:n}),i=u=>a.resolveNodeId(u),s=u=>{if(a.nodeIdSet.size===0)return!0;const f=i(String(u.nodeId||"").trim()),k=i(String(u.anchorNodeId||"").trim());return f&&a.nodeIdSet.has(f)||k&&a.nodeIdSet.has(k)?a.edgeLinkedNodeIdSet.size<=0?!0:f&&a.edgeLinkedNodeIdSet.has(f)||k&&a.edgeLinkedNodeIdSet.has(k):!1},c=Mt({overlayHtml:d,hasGraphNodeIdentity:a.nodeIdSet.size>0,resolveOverlayNodeId:i,shouldKeepOverlayLinkedToGraph:s}),b=(()=>{var B,E,A;const u=[],f=[];if(!c.trim())return{mediaNodes:u,markdownBlocks:f};const k=()=>{const I=(h,y)=>{const v=String(h||""),S=`${y}="`,_=`${y}='`,C=v.indexOf(S);if(C>=0){const U=C+S.length,j=v.indexOf('"',U);return j>U?v.slice(U,j).trim():""}const J=v.indexOf(_);if(J>=0){const U=J+_.length,j=v.indexOf("'",U);return j>U?v.slice(U,j).trim():""}return""},R=/<article\b[^>]*data-kg-rich-media-panel=(?:"1"|'1')[^>]*>/gi;let P=null;for(;P=R.exec(c);){const h=P[0]||"",y=i(I(h,"data-node-id"));if(!y)continue;const v=I(h,"data-kg-url")||I(h,"data-kg-open-url");if(!v)continue;const S=I(h,"data-kg-kind").toLowerCase(),_=S==="image"||S==="svg"||S==="video"||S==="iframe"?S:ce(v);u.push({id:y,title:y,url:v,openUrl:I(h,"data-kg-open-url")||v,interactive:!0,kind:_})}const V=/<article\b[^>]*(?:data-kg-markdown-design-block|data-md-id)[^>]*>/gi;let O=null;for(;O=V.exec(c);){const h=O[0]||"",y=I(h,"data-kg-markdown-design-block")||I(h,"data-md-id");if(!y)continue;const v=Number(I(h,"data-kg-world-x")),S=Number(I(h,"data-kg-world-y")),_=Number(I(h,"data-kg-world-w")),C=Number(I(h,"data-kg-world-h"));if(!Number.isFinite(v)||!Number.isFinite(S)||!Number.isFinite(_)||!Number.isFinite(C)||!(_>0)||!(C>0))continue;const J=i(I(h,"data-kg-anchor-node-id")||I(h,"data-node-id"));f.push({id:y,anchorNodeId:J||void 0,title:y,summary:"",preview:{kind:"other"},x:v,y:S,w:_,h:C})}};try{if(typeof DOMParser>"u")return k(),{mediaNodes:u,markdownBlocks:f};const R=new DOMParser().parseFromString(`<!doctype html><html><body><div id="kg-overlay-seed">${c}</div></body></html>`,"text/html").querySelector("#kg-overlay-seed");if(!R)return k(),{mediaNodes:u,markdownBlocks:f};const P=R.querySelectorAll('[data-kg-rich-media-panel="1"][data-node-id], .kg-media[data-node-id]');for(let O=0;O<P.length;O+=1){const h=P[O],y=String(h.getAttribute("data-node-id")||"").trim(),v=i(y);if(!v)continue;const S=String(h.getAttribute("data-kg-url")||"").trim(),_=String(h.getAttribute("data-kg-open-url")||"").trim(),C=h.querySelector("iframe[src],img[src],video[src],source[src]"),J=C?String(C.getAttribute("src")||"").trim():"",U=S||J||_;if(!U)continue;const j=String(h.getAttribute("data-kg-kind")||"").trim().toLowerCase(),te=j==="image"||j==="svg"||j==="video"||j==="iframe"?j:ce(U),je=String(h.getAttribute("data-kg-title")||"").trim()||String(((B=h.querySelector(".kg-mediaTitle"))==null?void 0:B.textContent)||"").trim()||v;u.push({id:v,title:je,url:U,openUrl:_||U,interactive:!0,kind:te})}const V=R.querySelectorAll("[data-kg-markdown-design-block],[data-md-id]");for(let O=0;O<V.length;O+=1){const h=V[O],y=String(h.getAttribute("data-kg-markdown-design-block")||h.getAttribute("data-md-id")||"").trim();if(!y)continue;const v=ae(h,"data-kg-world-x"),S=ae(h,"data-kg-world-y"),_=ae(h,"data-kg-world-w"),C=ae(h,"data-kg-world-h");if(!W(v)||!W(S)||!W(_)||!W(C)||!(_>0)||!(C>0))continue;const J=String(h.getAttribute("data-kg-anchor-node-id")||h.getAttribute("data-node-id")||"").trim(),U=i(J),j=String(((E=h.querySelector(".kg-mdTitle"))==null?void 0:E.textContent)||"").trim()||String(((A=h.querySelector(".kg-mediaTitle"))==null?void 0:A.textContent)||"").trim()||y;f.push({id:y,anchorNodeId:U||void 0,title:j,summary:"",preview:{kind:"other"},x:v,y:S,w:_,h:C})}u.length===0&&f.length===0&&k()}catch{k()}return{mediaNodes:u,markdownBlocks:f}})(),g={mediaNodes:b.mediaNodes.filter(u=>s({nodeId:u.id})),markdownBlocks:b.markdownBlocks.filter(u=>s({nodeId:u.id,anchorNodeId:u.anchorNodeId}))},l=(()=>{if(!e)return[];const u=t.graphData,f=Array.isArray(u==null?void 0:u.nodes)?u.nodes:[];if(f.length===0)return[];const k={};for(let h=0;h<f.length;h+=1){const y=f[h],v=String((y==null?void 0:y.id)||"").trim();if(!v)continue;const S=y.x,_=y.y;!W(S)||!W(_)||(k[v]={x:S,y:_})}const B=oe(n);for(const h of Object.keys(B))k[h]=B[h];const E=o?o.w:1920,A=o?o.h:1080,I=800,R=Math.max(1e-5,Number(e.k)),P=Number(e.x),V=Number(e.y),O=[];for(const h of Object.keys(k)){if(!s({nodeId:h}))continue;const y=k[h];if(!y)continue;const v=y.x*R+P,S=y.y*R+V;if(!(v<-I||v>E+I)&&!(S<-I||S>A+I)&&(O.push(h),O.length>=800))break}return O})();if(!!!n)return null;const w=Q("--kg-canvas-bg")||"#ffffff",N=Q("--kg-panel-bg")||"rgba(255,255,255,0.92)",H=Q("--kg-border")||"rgba(0,0,0,0.12)",G=Q("--kg-text-primary")||"rgba(0,0,0,0.86)",D=Q("--kg-text-secondary")||"rgba(0,0,0,0.7)",m=Q("--kg-text-tertiary")||"rgba(0,0,0,0.55)",F=Q("--kg-panel-action-bg")||"rgba(0,0,0,0.04)",L=Q("--kg-panel-action-bg-hover")||"rgba(0,0,0,0.06)",Z=Q("--kg-canvas-edge-stroke")||"#9ca3af",$=Q("--kg-canvas-node-stroke")||"#ffffff",q=Q("--kg-canvas-accent")||"#3b82f6",X=Q("--kg-canvas-label-fill")||G,z=Q("--kg-canvas-label-halo")||w,T=Q("--kg-media-panel-header-bg")||"rgba(0,0,0,0.04)",ee=t.mediaPanelDensity==="compact"?"compact":"default",re=ee==="compact"?22:28,we=ee==="compact"?9:10,Se=ee==="compact"?6:8,Ie=ee==="compact"?11:12,Ee=8,Me=W(t.threeIframeOverlayBaseWidthRatioDefault)?Math.max(.001,t.threeIframeOverlayBaseWidthRatioDefault):.2,Be=W(t.threeIframeOverlayBaseWidthRatioCompact)?Math.max(.001,t.threeIframeOverlayBaseWidthRatioCompact):.2,Ne=W(t.threeIframeOverlayBaseWidthMinPxDefault)?Math.max(1,Math.floor(t.threeIframeOverlayBaseWidthMinPxDefault)):210,Ae=W(t.threeIframeOverlayBaseWidthMinPxCompact)?Math.max(1,Math.floor(t.threeIframeOverlayBaseWidthMinPxCompact)):210,_e=W(t.threeIframeOverlayBaseWidthMaxPxDefault)?Math.max(1,Math.floor(t.threeIframeOverlayBaseWidthMaxPxDefault)):360,Fe=W(t.threeIframeOverlayBaseWidthMaxPxCompact)?Math.max(1,Math.floor(t.threeIframeOverlayBaseWidthMaxPxCompact)):360,de=(()=>{if(t.includeRichMediaOverlays!==!0)return[];const u=t.graphData,f=Array.isArray(u==null?void 0:u.nodes)?u.nodes:[],k=W(t.mediaOverlayPoolMax)?Math.max(0,Math.floor(t.mediaOverlayPoolMax)):0,B=k>0?k:Math.min(2e3,Math.max(24,f.length));return Ke({enabled:!0,nodes:f,poolMax:B,preferredNodeIds:l})})(),De=async u=>{if(!u||u.length===0)return[];const f=24e5,k=[];for(let B=0;B<u.length;B+=1){const E=u[B],A=String(E.url||"").trim(),I=ye(A);if(!fe(I)){k.push({...E,url:I});continue}const P=await me(I,{maxBytes:f});if(!P){k.push({...E,url:I});continue}k.push({...E,url:P})}return k},Ce=(()=>{const u=[],f=new Set,k=B=>{const E=i(String(B.id||"").trim());s({nodeId:E})&&(!E||f.has(E)||(f.add(E),u.push({...B,id:E})))};for(let B=0;B<de.length;B+=1)k(de[B]);for(let B=0;B<g.mediaNodes.length;B+=1)k(g.mediaNodes[B]);return u})(),Pe=await De(Ce),Re=JSON.stringify(Pe),Oe=(()=>{const u={},f=t.graphData,k=Array.isArray(f==null?void 0:f.nodes)?f.nodes:[];for(let B=0;B<k.length;B+=1){const E=k[B],A=String((E==null?void 0:E.id)||"").trim();if(!A)continue;const I=String(E.label||"").trim();u[A]={label:I||A}}return JSON.stringify(u)})(),se=u=>a.resolveNodeId(u),Le=(()=>{const u={},f=t.graphData,k=Array.isArray(f==null?void 0:f.edges)?f.edges:[];for(let B=0;B<k.length;B+=1){const E=k[B],A=String((E==null?void 0:E.id)||"").trim();if(!A)continue;const I=String(E.label||"").trim(),R=String(E.source||"").trim(),P=String(E.target||"").trim(),V=se(R),O=se(P);u[A]={label:I||"",s:V,t:O}}return JSON.stringify(u)})(),Te=(()=>{const u=t.graphData;if(!u)return JSON.stringify({nodeIds:[],edgeIds:[]});const f=it(u),k=Array.isArray(f.nodes)?f.nodes:[],B=Array.isArray(f.edges)?f.edges:[],E=k.map(I=>String((I==null?void 0:I.id)||"").trim()).filter(Boolean),A=B.map(I=>String((I==null?void 0:I.id)||"").trim()).filter(Boolean);return JSON.stringify({nodeIds:E,edgeIds:A})})(),ze=(()=>{var v;const u=t.graphData;if(!u)return JSON.stringify({});const f=Array.isArray(u.nodes)?u.nodes:[],k=new Set,B={};for(let S=0;S<f.length;S+=1){const _=String(((v=f[S])==null?void 0:v.id)||"").trim();if(!_)continue;k.add(_);const C=_.split("::").pop()||"";C&&!B[C]&&(B[C]=_)}const E=S=>{const _=String(S||"").trim();if(!_)return"";if(k.has(_))return _;const C=_.split("::").pop()||"";return C?B[C]||_:""},A=u.metadata||{},I=A.kind==="keyword",R=ot({frontmatterModeEnabled:!1,multiDimTableModeEnabled:!1,documentSemanticMode:I?"keyword":"document",documentStructureBaselineLock:!1}),P=A["kg:view"]&&typeof A["kg:view"]=="object"&&!Array.isArray(A["kg:view"])?A["kg:view"]:null,V=P&&Array.isArray(P.collapsedGroupIds)?P.collapsedGroupIds:[],O=new Set(V.map(S=>String(S||"").trim()).filter(Boolean)),h=Ze({groups:$e(u,{forceDocumentStructure:R==="documentStructure"}),collapsedGroupIdSet:O}),y={};for(let S=0;S<h.length;S+=1){const _=h[S],C=String(_.id||"").trim();if(!C)continue;const J=_.memberNodeIds,U=Array.isArray(J)?J.map(te=>String(te)).filter(Boolean):[];if(U.length===0)continue;const j=U.map(E).filter(te=>te&&k.has(te));j.length!==0&&(y[C]=j)}return JSON.stringify(y)})(),ie=(()=>{const u=t.graphData,f=Array.isArray(u==null?void 0:u.nodes)?u.nodes:[],k={};for(let E=0;E<f.length;E+=1){const A=f[E],I=String((A==null?void 0:A.id)||"").trim();if(!I)continue;const R=A.x,P=A.y;!W(R)||!W(P)||(k[I]={x:R,y:P})}const B=oe(n);for(const E of Object.keys(B))k[E]=B[E];for(let E=0;E<g.markdownBlocks.length;E+=1){const A=g.markdownBlocks[E],I=Number(A.x),R=Number(A.y),P=Number(A.w),V=Number(A.h);if(!Number.isFinite(I)||!Number.isFinite(R)||!Number.isFinite(P)||!Number.isFinite(V)||!(P>0)||!(V>0))continue;const O=I+P*.5,h=R+V*.5,y=String(A.id||"").trim();y&&!k[y]&&(k[y]={x:O,y:h});const v=i(String(A.anchorNodeId||"").trim());v&&!k[v]&&(k[v]={x:O,y:h})}return k})(),We=JSON.stringify(ie),He=(()=>{var u;try{const f=t.graphData;if(!f&&g.markdownBlocks.length===0)return"[]";const k=Je({graphData:f,nodePosById:ie}),B=k&&Array.isArray(k.blocks)?k.blocks:[],E=Array.isArray(f==null?void 0:f.nodes)?f.nodes:[],A=Qe({layout:k,nodes:E}),R=[...B.map(y=>{const v=String((y==null?void 0:y.id)||"").trim(),S=v?i(String((A==null?void 0:A[v])||"").trim()):"";return S?{...y,anchorNodeId:S}:y})],P=new Map;for(let y=0;y<R.length;y+=1){const v=String(((u=R[y])==null?void 0:u.id)||"").trim();v&&P.set(v,R[y])}for(let y=0;y<g.markdownBlocks.length;y+=1){const v=g.markdownBlocks[y],S=P.get(v.id);if(!S){R.push(v),P.set(v.id,v);continue}const _=i(String(S.anchorNodeId||v.anchorNodeId||""));_&&(S.anchorNodeId=_);const C=Number(S.w),J=Number(S.h);Number.isFinite(C)&&C>0&&Number.isFinite(J)&&J>0||(S.x=v.x,S.y=v.y,S.w=v.w,S.h=v.h)}const V=R.filter(y=>s({nodeId:String((y==null?void 0:y.id)||""),anchorNodeId:String((y==null?void 0:y.anchorNodeId)||"")})),O=[],h=new Set;for(let y=0;y<V.length;y+=1){const v=V[y],S=i(String((v==null?void 0:v.anchorNodeId)||(v==null?void 0:v.anchorId)||"")),_=String((v==null?void 0:v.id)||"").trim(),C=(S?`md:${S}`:_?`md:${_}`:"").trim();C&&(h.has(C)||(h.add(C),O.push(v)))}return JSON.stringify(O)}catch{return"[]"}})(),Ge=bt({svgMarkup:n,graphData:t.graphData||{nodes:[],edges:[]},nodePosById:ie}),Ye=JSON.stringify({scaleExtent:et({minK:t.zoomMinK,maxK:t.zoomMaxK}),wheelBehavior:t.wheelBehavior,viewportControlsPreset:t.viewportControlsPreset,panSpeed:t.panSpeed,zoomSpeed:t.zoomSpeed,flowWheelZoomSpeedMultiplier:t.flowWheelZoomSpeedMultiplier,flowWheelZoomIncrementMultiplier:t.flowWheelZoomIncrementMultiplier,flowWheelZoomSmoothMinDurationMs:t.flowWheelZoomSmoothMinDurationMs,flowWheelZoomSmoothMaxDurationMs:t.flowWheelZoomSmoothMaxDurationMs,wheelZoomCtrlMetaBoostMultiplier:t.wheelZoomCtrlMetaBoostMultiplier,canvasInteractionSpeedMultiplier:t.canvasInteractionSpeedMultiplier,canvasPanSpeedMultiplier:t.canvasPanSpeedMultiplier,snapGridEnabled:t.snapGridEnabled,snapGridSize:t.snapGridSize,dragConstraint:t.dragConstraint,allowNodeDrag:t.allowNodeDrag!==!1,allowEdgeDrag:t.allowEdgeDrag!==!1,allowGroupDrag:t.allowGroupDrag!==!1,preferWebgl3d:t.preferWebgl3d===!0,initialView:e||null,fixedViewport:o?{widthPx:o.w,heightPx:o.h,scaleToFit:t.viewportScaleToFit===!0}:null,enableDecorativeAnimation:t.enableDecorativeAnimation===!0,zoomLabelScaleMode2d:t.zoomLabelScaleMode2d,zoomLabelScaleExponent2d:t.zoomLabelScaleExponent2d,zoomLabelScaleClampMin2d:t.zoomLabelScaleClampMin2d,zoomLabelScaleClampMax2d:t.zoomLabelScaleClampMax2d,zoomStrokeScaleMode2d:t.zoomStrokeScaleMode2d,zoomStrokeScaleExponent2d:t.zoomStrokeScaleExponent2d,zoomStrokeScaleClampMin2d:t.zoomStrokeScaleClampMin2d,zoomStrokeScaleClampMax2d:t.zoomStrokeScaleClampMax2d,hideLabelsBelowScale:t.hideLabelsBelowScale}),Xe=(()=>{var u;try{if(typeof window>"u")return"";const f=String(((u=window.location)==null?void 0:u.origin)||"").trim();if(!f)return"";const k=new URL(f).hostname.toLowerCase();return k==="localhost"||k==="127.0.0.1"||k==="0.0.0.0"?f:""}catch{return""}})(),Ve=Ge,Ue=ft({interactionCfgJson:Ye,mediaNodesJson:Re,markdownBlocksJson:He,nodeLabelByIdJson:Oe,edgeMetaByIdJson:Le,frontmatterVisibilityJson:Te,initialFrontmatterEnabled:t.initialFrontmatterEnabled===!0,nodePosByIdJson:We,groupMembersByIdJson:ze,density:ee,widthRatioDefault:Me,widthRatioCompact:Be,widthMinDefault:Ne,widthMinCompact:Ae,widthMaxDefault:_e,widthMaxCompact:Fe,proxyOrigin:Xe}),qe=It("--kg-font-family","ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial");return`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  ...
  <style>
    :root{
      --kg-canvas-bg:${Y(w)};
      --kg-panel-bg:${Y(N)};
      --kg-media-panel-bg:${Y(N)};
      --kg-border:${Y(H)};
      --kg-text:${Y(G)};
      --kg-text-primary:${Y(G)};
      --kg-text-secondary:${Y(D)};
      --kg-text-tertiary:${Y(m)};
      --kg-panel-action-bg:${Y(F)};
      --kg-panel-action-bg-hover:${Y(L)};
      --kg-canvas-edge-stroke:${Y(Z)};
      --kg-canvas-node-stroke:${Y($)};
      --kg-canvas-accent:${Y(q)};
      --kg-canvas-label-fill:${Y(X)};
      --kg-canvas-label-halo:${Y(z)};
      --kg-media-panel-header-bg:${Y(T)};
    ...
    html,body{height:100%;width:100%;margin:0;background:var(--kg-canvas-bg);color:var(--kg-text);font-family:${Y(qe)};-webkit-user-select:none;user-select:none;-webkit-text-size-adjust:100%;text-size-adjust:100%;overscroll-behavior:none}
...
</html>`}const Bt=t=>{const r=String(t||"").toLowerCase();return r==="href"||r==="xlink:href"||r==="src"||r==="poster"},Nt=t=>String(t||"").toLowerCase()==="srcset",At=t=>{const r=[],n=t.attributes;for(let e=0;e<n.length;e+=1){const o=n.item(e);if(!o||!Bt(o.name)&&!Nt(o.name)&&String(o.name||"").toLowerCase()!=="style")continue;const d=String(o.value||"").trim();d&&r.push({name:o.name,value:d})}return r},_t=async(t,r)=>{const n=String(t||"").trim();if(!n)return"";const e=n.split(",").map(d=>d.trim()).filter(Boolean);if(e.length===0)return n;const o=[];for(let d=0;d<e.length;d+=1){const i=e[d].split(/\s+/g).filter(Boolean);if(i.length===0)continue;const s=i[0],c=await r(s),b=i.slice(1).join(" ");o.push(b?`${c} ${b}`:c)}return o.join(", ")},Ft=async(t,r)=>{const n=String(t||"");if(!n.trim())return n;const e=/url\(\s*(['"]?)([^'"\)]+)\1\s*\)/g;let o="",d=0;for(;;){const a=e.exec(n);if(!a)break;const i=a[0],s=String(a[2]||"").trim();o+=n.slice(d,a.index),d=a.index+i.length;const c=s&&await r(s),b=a[1]||"";o+=`url(${b}${c}${b})`}return o+=n.slice(d),o};async function Wt(t){const r=String(t.svgMarkup||"");if(!r.trim())return"";if(typeof DOMParser!="function"||typeof XMLSerializer!="function")return r;let n=null;try{n=new DOMParser().parseFromString(r,"image/svg+xml")}catch{n=null}if(!n)return r;const e=n.documentElement;if(!e||String(e.tagName||"").toLowerCase()!=="svg")return r;const o=Array.from(e.querySelectorAll("*"));if(o.length===0)return r;const d=typeof t.maxInlineRepoBytes=="number"&&Number.isFinite(t.maxInlineRepoBytes)?Math.max(0,Math.floor(t.maxInlineRepoBytes)):24e5,a=async i=>{const s=String(i||"").trim();if(!s)return"";const c=ye(s);return await me(c,{maxBytes:d})||c};for(let i=0;i<o.length;i+=1){const s=o[i],c=At(s);if(c.length!==0)for(let b=0;b<c.length;b+=1){const g=c[b],l=g.value,p=String(g.name||"").toLowerCase();if(p==="srcset"){const N=await _t(l,a);if(!N||N===l)continue;try{s.setAttribute(g.name,N)}catch{}continue}if(p==="style"){const N=await Ft(l,a);if(!N||N===l)continue;try{s.setAttribute(g.name,N)}catch{}continue}const w=await a(l);if(!(!w||w===l))try{s.setAttribute(g.name,w)}catch{}}}try{return new XMLSerializer().serializeToString(e)||r}catch{return r}}function Ht(t){const r=t.graphData,n=t.schema;if(!r||!n)return null;const e=rt({graphData:r,edges:null}),o=dt(t.collapsedGroupIds),d=st(n),a=lt(e),i=ct({graphData:e,graphDataRevision:typeof t.graphDataRevision=="number"&&Number.isFinite(t.graphDataRevision)?Math.floor(t.graphDataRevision):0}),s=gt(n),c=String(t.documentSemanticModeKey||"document"),b=ut({schemaLayoutEngineJson:d,frontmatterModeEnabled:t.frontmatterModeEnabled,documentSemanticMode:c,graphMetaKey:a,renderMediaAsNodes:t.renderMediaAsNodes,mediaPanelDensity:String(t.mediaPanelDensity),collapsedGroupIdsKey:o}),g=String(t.canvas2dRenderer||"d3"),w=nt({datasetKey:i,mode:s,frontmatterMode:t.frontmatterModeEnabled,semanticMode:c,renderMode:"2d",renderVariant:g,layoutVariant:"",viewKey:b,prevViewKey:null,prevDatasetKey:null,prevMode:null,prevFrontmatterMode:null,prevSemanticMode:null,prevRenderMode:null,prevRenderVariant:null,prevLayoutVariant:null,nodes:Array.isArray(e.nodes)?e.nodes:[],layoutPositionCacheByMode:t.layoutPositionCacheByMode??null}).layoutPositionsForMode;return!w||Object.keys(w).length===0?null:w}const he=t=>{const r=t;if(!(!r||!r.style))try{r.style.removeProperty("transform"),r.style.removeProperty("transform-origin"),r.style.removeProperty("translate"),r.style.removeProperty("left"),r.style.removeProperty("top"),r.style.removeProperty("right"),r.style.removeProperty("bottom"),r.style.removeProperty("inset"),r.style.removeProperty("position"),r.style.removeProperty("width"),r.style.removeProperty("height"),r.style.removeProperty("z-index")}catch{}},xe=t=>{const r=t;if(!(!r||!r.style))try{r.style.display="none"}catch{}},Dt=(t,r)=>{const n=t.querySelectorAll("img,video,source,iframe"),e=r.querySelectorAll("img,video,source,iframe"),o=Math.min(n.length,e.length);for(let d=0;d<o;d+=1){const a=n[d],i=e[d],s=String(a.tagName||"").toLowerCase();if(s==="img"){const c=a,b=i,g=String(c.currentSrc||c.src||b.getAttribute("src")||"").trim();if(g)try{b.setAttribute("src",g)}catch{}continue}if(s==="video"||s==="source"){const c=String(a.currentSrc||a.src||i.getAttribute("src")||"").trim();if(c)try{i.setAttribute("src",c)}catch{}continue}if(s==="iframe"){const c=a,b=i,g=typeof c.srcdoc=="string"?c.srcdoc:"";if(g){try{b.removeAttribute("src")}catch{}try{b.setAttribute("srcdoc",g)}catch{}continue}const l=String(c.src||i.getAttribute("src")||"").trim();if(l)try{b.setAttribute("src",l)}catch{}}}},Ct=t=>{if(typeof document>"u")return[];const r=[],n=new Set;for(let e=0;e<t.length;e+=1){const o=String(t[e]||"").trim();if(!o)continue;const d=Array.from(document.querySelectorAll(o));for(let a=0;a<d.length;a+=1){const i=d[a];!i||n.has(i)||(n.add(i),r.push(i))}}return r},be=(t,r)=>t?[t]:Ct(r),ge=t=>{try{const r=t,n=getComputedStyle(r);if(n.display==="none"||n.visibility==="hidden")return 0;const e=Number(n.opacity),o=r.getClientRects().length;return o>0&&Number.isFinite(e)&&e>.01?3:o>0?2:1}catch{return 1}},ke=(t,r,n)=>{if(!r)return;const e=t.get(r);if(!e){t.set(r,n);return}ge(n)>=ge(e)&&t.set(r,n)},Pt=t=>{var r;try{const n=be(t.overlayRootEl,['section[aria-label="D3 rich media overlay"]','section[aria-label="Flow media overlay"]','section[aria-label="Design media overlay"]','section[aria-label="3D media overlay"]',"#kg-overlay",'[data-kg-rich-media-overlay="1"]','[data-kg-rich-media-panel="1"]']);if(n.length===0)return"";const e=new Map;for(let a=0;a<n.length;a+=1){const i=n[a],s=(r=i.matches)!=null&&r.call(i,'[data-kg-rich-media-panel="1"][data-node-id]')?[i]:Array.from(i.querySelectorAll('[data-kg-rich-media-panel="1"][data-node-id]'));for(let c=0;c<s.length;c+=1){const b=s[c],g=String(b.getAttribute("data-node-id")||"").trim();g&&ke(e,g,b)}}const o=Array.from(e.values());if(o.length===0)return"";const d=document.createElement("div");for(let a=0;a<o.length;a+=1){const i=o[a],s=i.cloneNode(!0);ue(i,s,ve),he(s),xe(s);try{Dt(i,s)}catch{}d.appendChild(s)}return d.innerHTML}catch{return""}},Rt=t=>{var r;try{const n=be(t.overlayRootEl,['section[aria-label="Design markdown overlay"]','section[aria-label="Flow media overlay"]',"#kg-overlay",'[data-kg-markdown-design-overlay="1"]',"[data-kg-markdown-design-block]"]);if(n.length===0)return"";const e=new Map;for(let a=0;a<n.length;a+=1){const i=n[a],s=(r=i.matches)!=null&&r.call(i,"[data-kg-markdown-design-block]")?[i]:Array.from(i.querySelectorAll("[data-kg-markdown-design-block]"));for(let c=0;c<s.length;c+=1){const b=s[c],g=String(b.getAttribute("data-kg-markdown-design-block")||"").trim();g&&ke(e,g,b)}}const o=Array.from(e.values());if(o.length===0)return"";const d=document.createElement("div");for(let a=0;a<o.length;a+=1){const i=o[a],s=i.cloneNode(!0);ue(i,s,ve),he(s),xe(s);try{const c=String(i.getAttribute("data-kg-markdown-design-block")||"").trim();c&&s.setAttribute("data-md-id",c)}catch{}d.appendChild(s)}return d.innerHTML}catch{return""}},Gt=()=>{try{const t=Pt({overlayRootEl:null}),r=Rt({overlayRootEl:null});return`${t}${r}`}catch{return""}},Yt=()=>{var t;try{const r=String(((t=globalThis==null?void 0:globalThis.localStorage)==null?void 0:t.getItem(pe.viewportControlsPreset))||"").trim();return r==="map"||r==="design"?r:null}catch{return null}};export{bt as a,zt as b,Yt as c,Gt as d,oe as e,Ht as p,Wt as r};
