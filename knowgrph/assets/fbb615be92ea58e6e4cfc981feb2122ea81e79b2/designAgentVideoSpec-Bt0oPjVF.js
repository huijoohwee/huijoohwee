...
`;function mt(t){const e=t.graphData||null,s=Q(e,t.selectedNodeIds),i=Z(s).map((k,x)=>({...k,trackIndex:x,startMs:Math.min(f-240,x*C),durationMs:Math.max(240,Math.min(U,f-x*C))})),o=tt(i),a=et(),r=st(i),p=it(i),d=nt(o),u=ot(),h=B({graphData:e,graphRevision:t.graphRevision,maxEntries:8}),n=z("design-agent-video",{graphData:e,graphRevision:t.graphRevision,graphSemanticKey:[h.semanticKey,Array.from(S(t.selectedNodeIds)).sort().join(","),i.map(k=>k.id).join(",")].filter(Boolean).join(":")}),c={html:at(i),css:rt(),data:{schema:A,semanticKey:n,composition:{id:"knowgrph-design-agent-video",durationMs:f,fps:D,width:b,height:M},workspaceFiles:a,compositions:r,assets:p,timelineTracks:o,timelineLanes:d,timelineTicks:u,layers:i,tokenSummary:h},durationMs:f,fps:D,width:b,height:M,engineHint:L.canvas2d},g={id:n,type:H,label:t.title||j,properties:{html:c.html,css:c.css,data_json:JSON.stringify(c.data),duration_ms:c.durationMs,fps:c.fps,width:c.width,height:c.height,engine_hint:c.engineHint}};return{schema:A,semanticKey:n,renderSpec:c,flowNode:g,manifest:{schema:A,semanticKey:n,layerCount:i.length,selectedLayerCount:S(t.selectedNodeIds).size,workspaceFiles:a,compositions:r,assets:p,timelineTracks:o,timelineLanes:d,timelineTicks:u,tokenSummary:h}}}export{mt as b,B as s};
