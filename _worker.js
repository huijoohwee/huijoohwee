...
`)},"buildAgentSurfaceInspectionPromptText"),Oo=n((e,t={})=>{let r=ds(e);if(!r)throw new Error(`Unknown Knowgrph MCP prompt: ${ar(e)}`);if(r.name===tt.researchSourceFiles)return{description:r.description,messages:[Co(hs(t))]};if(r.name===tt.inspectAgentSurface)return{description:r.description,messages:[Co(gs(t))]};throw new Error(`Unhandled Knowgrph MCP prompt: ${r.name}`)},"getKnowgrphAgentReadyPrompt");var fs=Object.freeze({sourceFileById:"knowgrph_source_file_by_id"}),Et="kgdoc://source-file/{id}",Io="kgdoc://source-file/",Mo="text/markdown",lt=n(e=>String(e||"").trim(),"normalizeString"),No=n(()=>[{uriTemplate:Et,name:fs.sourceFileById,title:"Knowgrph Source File By ID",description:"Read a complete published Knowgrph Source File markdown document using a stable kgdoc id returned by search.",mimeType:Mo,annotations:{audience:["user","assistant"],priority:.8},_meta:{readOnly:!0,source:"knowgrph-source-files",tool:"fetch"}}],"buildKnowgrphAgentReadyResourceTemplateContracts");var Uo=n(e=>{let t=lt(e);if(!t.startsWith(Io))return"";let r=t.slice(Io.length);if(!r)return"";try{return decodeURIComponent(r)}catch{return r}},"parseKnowgrphSourceFileResourceUri"),Lo=n(({uri:e,sourceFile:t}={})=>{let r=typeof t?.content=="string"?t.content:String(t?.text||"");return{contents:[{uri:lt(e),mimeType:Mo,text:r,_meta:{id:lt(t?.id),title:lt(t?.title),url:lt(t?.url),metadata:t?.metadata&&typeof t.metadata=="object"?{...t.metadata}:{}}}]}},"buildKnowgrphSourceFileResourceReadResult");var Ee=n(e=>String(e||"").trim(),"normalizeString"),ve=n(e=>Ee(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),"escapeHtml");var sr="Map intent. Orchestrate agents. Prove outcomes.",ir="A source-backed canvas where / routes work, # sets meaning, and @ binds context.",Oe="knowgrph.agentic_canvas_os.docs.invoke",Ct=Object.freeze(["/mcp.capabilities","#mcp","@mcp-gateway"]),cr="One canonical operator contract: install and discovery stay on the public endpoint, while live /, #, @ grammar stays on the approval-gated control plane or an app-owned forwarder until the host proves MCP session support.",lr="Canonicalize the contract first, not the transport. Keep the runtime split underneath until hosted proof supports a single runtime.",ys=Object.freeze(["Lovable","Vercel"]),pr="Hosted app builders such as Lovable and Vercel should keep /mcp for discovery and use an app-owned forwarder for live /, #, @ unless the host proves MCP session support.";var ur=n(({publicReadMcpUrl:e,controlPlaneMcpUrl:t}={})=>({promise:sr,grammarSummary:ir,canonicalOperatorContract:cr,canonicalTransportRule:lr,publicReadMcpUrl:Ee(e),controlPlaneMcpUrl:Ee(t),controlPlaneCondition:"Add the control plane only when the host can preserve MCP session state and needs live /, #, @ grammar lookup.",grammarToolName:Oe,grammarExamples:Ct.map(r=>Ee(r)),grammarExecutionBoundary:"Keep install on the public discovery endpoint and execute live grammar on the approval-gated control plane.",hostedBuilderExamples:ys.map(r=>Ee(r)),hostedGrammarDefaultPath:pr,hostedGrammarFallback:"app-owned-forwarder",cheapestProofPath:"Use the source-side README.md quick start or docs/documents/knowgrph-superagent-harness.md in the knowgrph repo before hosted setup.",steps:[{order:1,label:"Map intent",action:e?`Map intent: install ${Ee(e)} first for public discovery, retrieval, and inspection.`:"Map intent: install the public MCP endpoint first for discovery, retrieval, and inspection."},{order:2,label:"Orchestrate agents",action:t?`Orchestrate agents: add ${Ee(t)} only when the host can preserve MCP session state and needs live /, #, @ grammar lookup through ${Oe}.`:`Orchestrate agents: add the control plane only when the host can preserve MCP session state and needs live /, #, @ grammar lookup through ${Oe}.`},{order:3,label:"Prove outcomes",action:"Prove outcomes: for zero-spend evaluation, run the source-side README.md quick start or docs/documents/knowgrph-superagent-harness.md first."}]}),"buildMcpOnboarding"),$o=n(({publicReadMcpUrl:e,controlPlaneMcpUrl:t}={})=>`<section aria-label="Fastest path">
  ...
  appendText(onboardingEl, 'p', onboarding && onboarding.promise ? String(onboarding.promise) : '${sr}');
  appendText(onboardingEl, 'p', onboarding && onboarding.grammarSummary ? String(onboarding.grammarSummary) : '${ir}');
  appendText(
    onboardingEl,
    'p',
...
${v.canonicalPath}
${v.workspaceId}
${v.snippet}`.toLowerCase(),Be=$&&J.includes($)?H.length*4:0,Fe=H.reduce((_t,er)=>_t+(J.includes(er)?2:0),0);return{...v,score:Be+Fe}}),_=V.slice().sort((v,J)=>J.score-v.score||v.title.localeCompare(J.title)).slice(0,D).filter(v=>/\.md(?:$|[?#])/i.test(v.canonicalPath)),j=new Map;await te(_,async v=>{let J=Se(v.id);if(!J?.canonicalPath)return null;try{let Be=await s(o(J.canonicalPath,J.workspaceId));if(!Be.ok)return null;let Fe=(await Be.text()).slice(0,N),_t=Fe.toLowerCase(),er=$&&_t.includes($)?H.length*6:0,Ga=R(_t,H),eo=er+Ga;if(eo<=0)return null;j.set(v.id,{score:eo,snippet:T(Fe,H)})}catch{return null}return null});let ue=V.map(v=>{let J=j.get(v.id);return{...v,score:v.score+(J?.score||0),snippet:J?.snippet||v.snippet}}).filter(v=>v.score>0).sort((v,J)=>J.score-v.score||v.title.localeCompare(J.title)).slice(0,h).map(({score:v,...J})=>J);return{ids:ue.map(v=>v.id),results:ue,query:g,totalResults:ue.length}},"searchSourceFiles"),w=n(async(d={})=>{let g=Se(d.id);if(!g?.canonicalPath)throw new Error("id must be a stable Knowgrph Source File id returned by search");let h=await U(g),A=F(g);return{id:se(g),title:S(h.canonicalPath),content:h.markdown,text:h.markdown,url:A,metadata:{workspaceId:h.workspaceId,canonicalPath:h.canonicalPath,contentType:"text/markdown",source:"knowgrph-source-files"}}},"fetchSourceFileBySearchId"),y={};return t.search&&(y[t.search]=k),t.fetch&&(y[t.fetch]=w),t.listSourceFiles&&(y[t.listSourceFiles]=async()=>{let d=await a();if(!d.ok)throw new Error(`list_source_files failed with ${d.status}`);return{workspaceId:r,markdownIndex:await d.text()}}),t.readSourceFile&&(y[t.readSourceFile]=U),t.readSharedDocument&&(y[t.readSharedDocument]=L),t.inspectSharedDocumentStructure&&(y[t.inspectSharedDocumentStructure]=Y),t.inspectAgentSurface&&(y[t.inspectAgentSurface]=async()=>l()),y},"createPublishedAgentReadyToolExecutors"),Cs=n(e=>`((...args) => {
  const n = (value) => value
  const __name = (value) => value
  return (${Function.prototype.toString.call(e)})(...args)
})`,"createBrowserSafeFunctionSource"),Xo=Cs(br);var Qo=n((e={})=>{let t=n(k=>String(k||"").trim(),"normalizeString"),r=n(k=>String(k||"").replace(/\r\n/g,`
`).replace(/\r/g,`
...
`,"buildKnowgrphVdeoxplnMarkdown"),sn=n((e=nn())=>Object.fromEntries(e.map(t=>[t.id,Ms(t)])),"buildKnowgrphVdeoxplnMarkdownByName");var cn=n(({appUrl:e,rootUrl:t,storageSourceFilesUrl:r,storageLlmsUrl:o,storageManifestUrl:a,agentCardUrl:s,updatedAt:i})=>`<?xml version="1.0" encoding="UTF-8"?>
...
}`;var B="https://airvio.co",Ut="https://knowgrph-storage.huijoohwee.workers.dev",C="/knowgrph",x=`${B}${C}/`,Nt=`${B}/`,ut="kgws:canonical-docs",We="2026-06-05",ot=`${C}/health`,rt=`${B}${ot}`,An="/.well-known/agent-card.json",Cr=`${C}/.well-known/agent-card.json`,dt=`${B}${An}`,Tr="/api/storage/source-files",Gs=`${Tr}/`,Pn="/api/storage/doc-default/",kn="/api/storage/doc/",Lt=`${B}${Tr}`,Ws=`${B}${Pn}{canonicalPath}`,qs=`${B}${kn}{workspaceId}/{canonicalPath}`,Ys=`${B}/api/storage/blob/{workspaceId}/{canonicalPath}`,xn="knowgrph-agent-ready-pages";var Rn=n((e,t)=>`${kn}${encodeURIComponent(String(e||"").trim())}/${encodeURIComponent(String(t||"").trim())}`,"buildKnowgrphStorageDocPath"),En=n(e=>`${Pn}${encodeURIComponent(String(e||"").trim())}`,"buildKnowgrphStorageDefaultDocPath"),Cn=n(e=>{let t=String(e||"").trim();return t?`${Gs}${encodeURIComponent(t)}`:Tr},"buildKnowgrphStorageSourceFilesIndexPath"),Tn=['</.well-known/api-catalog>; rel="api-catalog"; type="application/linkset+json"',`<${C}/.well-known/openapi.json>; rel="service-desc"; type="application/vnd.oai.openapi+json;version=3.1"`,`<${C}/llms.txt>; rel="service-doc"; type="text/plain"`,'</auth.md>; rel="service-doc"; type="text/markdown"',`<${ot}>; rel="status"; type="application/health+json"`,`<${C}/.well-known/mcp/server-card.json>; rel="mcp-server-card"; type="application/json"`,`<${An}>; rel="describedby"; type="application/json"`].join(", "),vn=`# Knowgrph
...
- Auth.md registration instructions: ${Nt}auth.md
- Health: ${rt}
...
- Live Canvas Hero discovery markdown: ${Nt}knowgrph-live-canvas-hero.md
...
- Source Files index: ${Lt}
- Default Source File documents: ${Ws}
- Workspace Source File documents: ${qs}
- Workspace binary artifacts: ${Ys}
...
`,On=n(e=>new Response(e,{status:200,headers:{"content-type":"text/markdown; charset=utf-8","cache-control":"public, max-age=3600","access-control-allow-origin":"*",vary:"Accept","x-markdown-tokens":String(Math.ceil(String(e||"").length/4))}}),"markdownResponse"),mt=n(e=>(e.headers.get("accept")||"").toLowerCase().split(",").some(r=>r.trim().startsWith("text/markdown")),"wantsMarkdown"),In=n((e,t)=>{let r=new Response(e.body,e),o=String(t?.owner||"").trim(),a=String(t?.tag||"").trim();return o&&r.headers.set("x-knowgrph-route-owner",o),a&&r.headers.set("x-knowgrph-route-tag",a),r},"withAgentReadyRouteHeaders");var Dt=Sr({defaultWorkspaceId:ut}),Hn=vo(),Kn=No(),Vs=`${B}/api/storage/llms.txt`,Js=`${B}/api/storage/content-manifest.json`,jn=n((e,t="")=>{let r=String(e||"").trim(),o=String(t||"").trim();return o?Rn(o,r):En(r)},"buildStorageDocPath"),ft=n(e=>String(e||"").trim(),"normalizeToolString");var Pe=n((e,t="application/json; charset=utf-8")=>new Response(JSON.stringify(e,null,2),{status:200,headers:{"content-type":t,"cache-control":"public, max-age=3600","access-control-allow-origin":"*"}}),"jsonResponse"),he=n((e,t)=>new Response(JSON.stringify(t,null,2),{status:e,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store","access-control-allow-origin":"*"}}),"jsonStatusResponse"),Mn=n((e,t={})=>new Response(null,{status:e,headers:{"cache-control":"no-store","access-control-allow-origin":"*",...t}}),"emptyStatusResponse"),ht=n((e,t)=>new Response(e,{status:200,headers:{"content-type":t,"cache-control":"public, max-age=3600","access-control-allow-origin":"*"}}),"textResponse"),Xs=n(e=>ht(e,"text/html;profile=mcp-app; charset=utf-8"),"mcpAppsHtmlResponse"),Qs=n(e=>new Response(JSON.stringify(e,null,2),{status:200,headers:{"content-type":"application/health+json; charset=utf-8","cache-control":"no-store","access-control-allow-origin":"*"}}),"healthResponse"),Zs=`${C}/api/workspace/github/write`,ei="/api/workspace/github/write",Nn=12,Un=9e5,ti=new Set(["css","html","js","json","md","mdx","mjs","svg","ts","tsx","txt","yaml","yml"]),vr=n((e,t)=>String(e?.[t]||"").trim(),"readEnvString"),ri=n(e=>{let t=vr(e,"KNOWGRPH_GITHUB_WRITE_REPOSITORY"),r=vr(e,"KNOWGRPH_GITHUB_WRITE_TOKEN"),o=vr(e,"KNOWGRPH_GITHUB_WRITE_BRANCH"),a=[];t||a.push("KNOWGRPH_GITHUB_WRITE_REPOSITORY"),r||a.push("KNOWGRPH_GITHUB_WRITE_TOKEN");let s=t.split("/").map(i=>i.trim()).filter(Boolean);return t&&s.length!==2&&a.push("KNOWGRPH_GITHUB_WRITE_REPOSITORY:owner/repo"),a.length>0?{ok:!1,missing:a}:{ok:!0,owner:s[0],repo:s[1],branch:o,token:r}},"readGitHubWriteConfig"),oi=n(e=>{let t=String(e||"").trim().replace(/^workspace:/i,"").replace(/\\/g,"/").replace(/^\/+/,"");if(!t)return{ok:!1,error:"missing_workspace_path"};if(/[\u0000-\u001f\u007f]/.test(t))return{ok:!1,error:"invalid_workspace_path"};let r=t.split("/").filter(Boolean);if(r.some(s=>s==="."||s===".."))return{ok:!1,error:"path_traversal_forbidden"};if(r[0]!=="chat-log")return{ok:!1,error:"unsupported_workspace_root"};if(r.length<3)return{ok:!1,error:"chat_log_session_file_required"};let o=r[r.length-1]||"",a=o.includes(".")?o.split(".").pop().toLowerCase():"";return!a||!ti.has(a)?{ok:!1,error:"unsupported_text_extension"}:{ok:!0,path:r.join("/")}},"normalizeGitHubWriteWorkspacePath"),ni=n(e=>{let t=new TextEncoder().encode(String(e||"")),r=32768,o="";for(let a=0;a<t.length;a+=r)o+=String.fromCharCode(...t.slice(a,a+r));return btoa(o)},"encodeBase64Utf8"),nt=class extends Error{static{n(this,"GitHubWorkspaceWriteError")}constructor(t,r,o){super(t),this.name="GitHubWorkspaceWriteError",this.code=t,this.upstreamStatus=r,this.upstreamMessage=o}},Bn=n(e=>String(e||"unknown").replace(/[\u0000-\u001f\u007f]/g," ").replace(/\s+/g," ").trim().slice(0,240),"sanitizeGitHubApiMessage"),Fn=n((e,t)=>{let r=String(t||"").split("/").map(a=>encodeURIComponent(a)).join("/"),o=new URL(`https://api.github.com/repos/${encodeURIComponent(e.owner)}/${encodeURIComponent(e.repo)}/contents/${r}`);return e.branch&&o.searchParams.set("ref",e.branch),o},"buildGitHubContentsApiUrl"),zn=n(e=>({accept:"application/vnd.github+json",authorization:`Bearer ${e.token}`,"user-agent":"knowgrph-cloudflare-pages","x-github-api-version":"2022-11-28"}),"gitHubApiHeaders"),Gn=n(e=>{let t=String(e||"").replace(/\/+$/,"")||"/";return t===Zs||t===ei},"isGitHubWorkspaceWriteRoutePath"),ai=n(async(e,t)=>{let r=await fetch(Fn(e,t),{method:"GET",headers:zn(e)});if(r.status===404)return null;let o=await r.json().catch(()=>null);if(!r.ok)throw new nt("github_read_failed",r.status,Bn(o?.message||r.statusText));if(o?.type&&o.type!=="file")throw new nt("github_path_not_file",409,t);return String(o?.sha||"").trim()||null},"fetchGitHubExistingFileSha"),si=n(async(e,t,r)=>{let o=await ai(e,t.repositoryPath),a={message:r,content:ni(t.text),...e.branch?{branch:e.branch}:{},...o?{sha:o}:{}},s=await fetch(Fn(e,t.repositoryPath),{method:"PUT",headers:{...zn(e),"content-type":"application/json; charset=utf-8"},body:JSON.stringify(a)}),i=await s.json().catch(()=>null);if(!s.ok)throw new nt("github_write_failed",s.status,Bn(i?.message||s.statusText));return{workspacePath:t.workspacePath,repositoryPath:t.repositoryPath,action:o?"updated":"created",commitSha:String(i?.commit?.sha||""),contentSha:String(i?.content?.sha||""),htmlUrl:String(i?.content?.html_url||"")}},"putGitHubWorkspaceFile"),ii=n(async(e,t)=>{let r=ri(t);if(!r.ok)return he(503,{ok:!1,status:"skipped",error:"github_write_not_configured",missing:r.missing});let o=await e.json().catch(()=>null),a=Array.isArray(o?.files)?o.files:[];if(a.length<1)return he(400,{ok:!1,status:"failed",error:"files_required"});if(a.length>Nn)return he(413,{ok:!1,status:"failed",error:"too_many_files",maxFiles:Nn});let s=[],i=new Set;for(let p of a){let m=oi(p?.workspacePath||p?.path);if(!m.ok)return he(400,{ok:!1,status:"failed",error:m.error,workspacePath:String(p?.workspacePath||p?.path||"")});if(i.has(m.path))continue;i.add(m.path);let f=String(p?.text??"");if(new TextEncoder().encode(f).length>Un)return he(413,{ok:!1,status:"failed",error:"file_too_large",workspacePath:`/${m.path}`,maxTextBytes:Un});s.push({workspacePath:`/${m.path}`,repositoryPath:m.path,text:f})}if(s.length<1)return he(400,{ok:!1,status:"failed",error:"files_required"});let c=String(o?.message||"").trim(),l=c&&c.length<=160?c:`Knowgrph chat artifact ${s[0].repositoryPath}`;if(o?.dryRun===!0)return he(200,{ok:!0,status:"dry_run",repository:`${r.owner}/${r.repo}`,branch:r.branch||null,files:s.map(p=>({workspacePath:p.workspacePath,repositoryPath:p.repositoryPath,textBytes:new TextEncoder().encode(p.text).length}))});try{let p=[];for(let m of s)p.push(await si(r,m,l));return he(200,{ok:!0,status:"applied",repository:`${r.owner}/${r.repo}`,branch:r.branch||null,files:p})}catch(p){let m=p instanceof nt;return he(m?424:500,{ok:!1,status:"failed",error:m?p.code:p instanceof Error?p.message:String(p||"github_write_failed"),...m?{upstreamStatus:p.upstreamStatus,upstreamMessage:p.upstreamMessage}:{}})}},"handleGitHubWorkspaceWrite"),ci=n(e=>`User-agent: *
Allow: /knowgrph/
...
Sitemap: ${e}
`,"buildRobotsTxt"),li=ci(`${x}sitemap.xml`),Wn={appUrl:x,rootUrl:Nt,storageSourceFilesUrl:Lt,storageLlmsUrl:Vs,storageManifestUrl:Js,agentCardUrl:dt,updatedAt:We},pi=cn(Wn),_d=ln(Wn),qn={linkset:[{anchor:x,"service-desc":[{href:`${x}.well-known/openapi.json`,type:"application/vnd.oai.openapi+json;version=3.1"}],"service-doc":[{href:`${x}llms.txt`,type:"text/plain"}],status:[{href:rt,type:"application/health+json"}],"service-meta":[{href:`${x}.well-known/mcp/server-card.json`,type:"application/json"},{href:dt,type:"application/json"}]}]},Yn={openapi:"3.1.0",info:{title:"Knowgrph API",version:"0.1.0",description:"Agent discovery surface for the Knowgrph Cloudflare deployment."},servers:[{url:B,description:"Knowgrph Cloudflare deployment"}],paths:mn({appBasePath:C,appA2aAgentCardPath:Cr,healthPath:ot})},gt={resource:x,resource_name:"Knowgrph",authorization_servers:[B],scopes_supported:["knowgrph:read","knowgrph:source-files:read"],bearer_methods_supported:["header"],resource_documentation:`${x}llms.txt`},Or=`${B}/cdn-cgi/access`,Ne={skill:`${B}/auth.md`,register_uri:`${x}agent/auth`,claim_uri:`${x}agent/auth/claim`,revocation_uri:`${x}agent/auth/revoke`,identity_types_supported:["anonymous","identity_assertion"],anonymous:{credential_types_supported:["api_key"]},identity_assertion:{assertion_types_supported:["urn:ietf:params:oauth:token-type:id-jag","verified_email"],credential_types_supported:["access_token","api_key"]},events_supported:["https://schemas.workos.com/events/agent/auth/identity/assertion/revoked"],registration_status:"metadata_published_runtime_user_mediated"},Ln={issuer:B,resource:gt.resource,resource_name:gt.resource_name,authorization_servers:gt.authorization_servers,cloudflare_access_issuer:Or,authorization_endpoint:`${Or}/login`,token_endpoint:`${Or}/token`,jwks_uri:`${x}.well-known/http-message-signatures-directory`,response_types_supported:["code"],grant_types_supported:["authorization_code","client_credentials"],token_endpoint_auth_methods_supported:["client_secret_basic","private_key_jwt"],scopes_supported:gt.scopes_supported,agent_auth:Ne},ui=`# Knowgrph auth.md
...
- Pipeline rule: agents must not bypass MainPanel -> FloatingPanel Chat -> KGC -> Canvas for user-mediated graph work; published HTTP MCP tools remain read-only until mutation auth and conflict semantics are implemented.`,Vn={name:"Knowgrph Agent",description:"Agent-readable discovery, published-document retrieval, and WebMCP-ready metadata surface for Knowgrph.",version:"0.1.0",provider:{organization:"airvio / joohwee",url:x},url:`${x}mcp`,preferredTransport:"JSONRPC",supportedInterfaces:[{url:`${x}mcp`,protocolBinding:"JSONRPC",transportProtocol:"JSONRPC",description:"Primary machine interface for read-only discovery and source-file document access."},{url:Lt,protocolBinding:"HTTP+JSON/REST",transportProtocol:"HTTP+JSON/REST",description:"Published source-files index and storage-backed document read surface."}],capabilities:{streaming:!1,pushNotifications:!1,stateTransitionHistory:!1,extendedAgentCard:!1},defaultInputModes:["text/plain","text/markdown","application/json"],defaultOutputModes:["text/plain","text/markdown","application/json"],skills:un(Dt)},Te={serverInfo:{name:"knowgrph",version:"0.1.0"},transport:{type:Ie,url:`${x}mcp`,stateless:!0},capabilities:{tools:Dt.map(e=>({name:e.name,title:e.title,description:e.description,inputSchema:e.inputSchema,outputSchema:e.outputSchema,securitySchemes:e.securitySchemes,annotations:e.annotations,_meta:e._meta})),resources:{listChanged:!1},prompts:{listChanged:!1},...gr()},prompts:Hn,resourceTemplates:Kn,clientSetups:yr({baseUrl:x,mcpUrl:`${x}mcp`,serverName:"knowgrph"}),surfaceRoles:{publicReadMcpUrl:`${x}mcp`,publicReadMcpScope:"Canonical public install and discovery endpoint for read-only retrieval, prompt discovery, resource discovery, and inspection.",controlPlaneMcpUrl:`${x}control-plane/mcp`,controlPlaneMcpScope:"Approval-gated orchestration endpoint for control-plane tools, remote Agentic Canvas OS docs invocation, and spend-bearing workflows where deployed.",hostedGrammarDefaultPath:"app-owned-forwarder",hostedGrammarDefaultScope:"Hosted app builders should keep /mcp for discovery and route live /, #, @ through an app-owned forwarder unless the host proves MCP session support.",remoteGrammarInvokePublic:!0,remoteGrammarInvokeToolName:"knowgrph.agentic_canvas_os.docs.invoke",remoteGrammarInvokeStatus:"live-control-plane"},links:{apiCatalog:`${x}.well-known/api-catalog`,skills:`${x}.well-known/agent-skills/index.json`,status:rt,agentCard:dt,controlPlaneMcp:`${x}control-plane/mcp`}},di=vt({appUrl:x,updatedAt:We}),Mr=Dt.map(e=>({name:e.webName,title:e.title,description:e.description,inputSchema:e.inputSchema,outputSchema:e.outputSchema,securitySchemes:e.securitySchemes,annotations:e.annotations,_meta:e._meta})),qe=n(e=>ft(Dt.find(t=>t.name===e)?.webName),"findWebMcpToolName"),mi=qe(u.search),hi=qe(u.fetch),gi=qe(u.listSourceFiles),fi=qe(u.readSourceFile),yi=qe(u.readSharedDocument),wi=qe(u.inspectSharedDocumentStructure),Si=qe(u.inspectAgentSurface),bi=`(() => {
  ...
  const toolExecutors = createPublishedAgentReadyToolExecutors({
    toolNames: {
      search: ${JSON.stringify(mi)},
      fetch: ${JSON.stringify(hi)},
      listSourceFiles: ${JSON.stringify(gi)},
      readSourceFile: ${JSON.stringify(fi)},
      readSharedDocument: ${JSON.stringify(yi)},
      inspectSharedDocumentStructure: ${JSON.stringify(wi)},
      inspectAgentSurface: ${JSON.stringify(Si)},
    },
    defaultWorkspaceId,
...
})();`,_i=n(async e=>{if(!(e.headers.get("content-type")||"").toLowerCase().includes("text/html"))return e;let r=await e.text();if(Mr.every(i=>r.includes(i.name)))return new Response(r,e);let o=`<script>${bi}<\/script>`,a=r.includes("</head>")?r.replace("</head>",`${o}</head>`):`${r}${o}`,s=new Response(a,e);return s.headers.delete("content-length"),s},"injectWebMcpScript"),Ai={search:u.search,fetch:u.fetch,listSourceFiles:u.listSourceFiles,readSourceFile:u.readSourceFile,readSharedDocument:u.readSharedDocument,inspectSharedDocumentStructure:u.inspectSharedDocumentStructure,inspectAgentSurface:u.inspectAgentSurface},Pi=n(async e=>{let t=new TextEncoder().encode(e),r=await crypto.subtle.digest("SHA-256",t);return[...new Uint8Array(r)].map(o=>o.toString(16).padStart(2,"0")).join("")},"sha256Hex"),Jn=sn(),ki=Object.fromEntries(pt.map(e=>[e.name,Pi(Jn[e.name]||"")])),$n=new Map(pt.map(e=>[`${C}${e.path}`.replace(/\/+$/,""),Jn[e.name]||""]));var Xn=n(async()=>dn({appUrl:x,updatedAt:We,sha256ByName:ki}),"agentSkillsIndex"),xi={keys:[{kty:"OKP",crv:"Ed25519",kid:"knowgrph-agent-ready-2026-05-21",use:"sig",alg:"EdDSA",x:"11qYAYdkVKxA4G0wV47IxPtYfFVH_H7zmC2Di2PcvLU"}]},Ri={protocolVersion:"2025-06-18",capabilities:{tools:{},resources:{},prompts:{listChanged:!1},...gr()},serverInfo:Te.serverInfo},Nr=Te.capabilities.tools,Ei=[di],Ci=Hn,Ti=Kn,Qn=n(()=>({status:"pass",service:"knowgrph-agent-ready-pages",homepage:x,health:rt,updatedAt:We,checks:{linkHeaders:!0,markdownNegotiation:!0,httpMcp:!0,webMcp:!0,mcpApps:!0,commerce:{acp:!0,ucp:!0,mpp:!0,x402:!0},defaultWorkspaceId:ut}}),"buildHealthStatusBody"),vi=n(async()=>Jo({baseUrl:x,health:Qn(),apiCatalog:qn,openApi:Yn,mcpServerCard:Te,agentCard:Vn,agentSkills:await Xn(),commerce:nr({origin:B})}),"buildAgentSurfaceInspection"),Oi=br({toolNames:Ai,defaultWorkspaceId:ut,publicBaseUrl:B,buildStorageDocPath:jn,fetchSourceFilesIndexResponse:n(()=>fetch(`${Ut}${Cn()}`,{headers:{accept:"text/markdown"}}),"fetchSourceFilesIndexResponse"),fetchStorageMarkdownResponse:n(e=>fetch(`${Ut}${e}`,{headers:{accept:"text/markdown"}}),"fetchStorageMarkdownResponse"),resolveSharedDocumentInput:n((e={})=>Mt({shareToken:e?.shareToken,shareUrl:e?.shareUrl,appBasePath:C,baseUrl:B}),"resolveSharedDocumentInput"),inspectSharedDocumentStructure:Qo,buildAgentSurfaceInspection:vi}),Ur=n(e=>{try{let t=new URL(e,B);return Mt({shareUrl:`${t.pathname}${t.search}`,baseUrl:B,appBasePath:C})}catch{return null}},"resolvePublishedDocRequestIdentity"),Ii=n(e=>Mt({shareUrl:String(e||""),baseUrl:B,appBasePath:C}),"resolvePublishedDocPathIdentity"),Mi=n(async(e,t)=>{let r=new URL(jn(t.canonicalPath,t.workspaceId),Ut),o=await fetch(r,{method:"GET",headers:{accept:"text/markdown, text/plain;q=0.9, */*;q=0.1"}}),a=new Headers(o.headers),s=String(a.get("vary")||"");return a.set("vary",s?`${s}, Accept`:"Accept"),new Response(String(e.method||"").toUpperCase()==="HEAD"?null:o.body,{status:o.status,statusText:o.statusText,headers:a})},"proxyPublishedDocMarkdownResponse"),Ni=n(async e=>{try{let t=await e.json();return t&&typeof t=="object"?t:null}catch{return null}},"readJsonRpcRequest"),Me=n((e,t)=>he(200,{jsonrpc:"2.0",id:e??null,result:t}),"jsonRpcResult"),He=n((e,t,r)=>he(200,{jsonrpc:"2.0",id:e??null,error:{code:t,message:r}}),"jsonRpcError"),Ui=n(e=>String(e.headers.get("accept")||"").toLowerCase().split(",").some(t=>t.trim().startsWith("text/event-stream")),"requestAcceptsEventStream"),Ir=n((e,t)=>Object.prototype.hasOwnProperty.call(e,t),"hasOwnProperty"),Zn=n(e=>{if(Array.isArray(e))return e.length>0&&e.every(Zn);if(!e||typeof e!="object"||String(e.jsonrpc||"")!=="2.0")return!1;let t=typeof e.method=="string"&&e.method.length>0,r=Ir(e,"id"),o=Ir(e,"result")||Ir(e,"error");return t&&!r||!t&&o},"isJsonRpcNotificationOrResponse"),ea=n(async(e,t)=>{let r=Oi[e];if(typeof r!="function")throw new Error(`unknown tool: ${e}`);return r(t)},"executeMcpTool"),Li=n(async e=>{let t=ft(e);if(t===me)return wr({appUrl:x,updatedAt:We,toolNames:Nr.map(o=>o.name)});let r=Uo(t);if(r){let o=await ea(u.fetch,{id:r});return Lo({uri:t,sourceFile:o})}throw new Error(`unknown resource: ${e}`)},"readMcpResource"),$i=n(async e=>{let t=String(e.method||"GET").toUpperCase();if(t==="GET"||t==="HEAD")return Ui(e)?Mn(405,{allow:"POST"}):Pe({ok:!0,transport:Te.transport,serverInfo:Te.serverInfo,capabilities:Te.capabilities,links:Te.links,surfaceRoles:Te.surfaceRoles});if(t!=="POST")return he(405,{ok:!1,error:"unsupported_method"});let r=await Ni(e);if(!r)return He(null,-32700,"Parse error");if(Zn(r))return Mn(202);if(Array.isArray(r))return He(null,-32600,"Batch JSON-RPC requests are not supported");switch(r.method){case"initialize":return Me(r.id,Ri);case"tools/list":return Me(r.id,{tools:Nr});case"prompts/list":return Me(r.id,{prompts:Ci});case"resources/templates/list":return Me(r.id,{resourceTemplates:Ti});case"prompts/get":{let o=ft(r.params?.name),a=r.params?.arguments&&typeof r.params.arguments=="object"?r.params.arguments:{};if(!o)return He(r.id,-32602,"Prompt name is required");try{return Me(r.id,Oo(o,a))}catch(s){return He(r.id,-32602,s instanceof Error?s.message:String(s))}}case"resources/list":return Me(r.id,{resources:Ei});case"resources/read":{let o=ft(r.params?.uri);if(!o)return He(r.id,-32602,"Resource URI is required");try{return Me(r.id,await Li(o))}catch(a){return He(r.id,-32602,a instanceof Error?a.message:String(a))}}case"tools/call":{let o=ft(r.params?.name),a=r.params?.arguments&&typeof r.params.arguments=="object"?r.params.arguments:{};if(!o)return He(r.id,-32602,"Tool name is required");try{let s=await ea(o,a);return Me(r.id,{content:[{type:"text",text:typeof s?.markdown=="string"?s.markdown:JSON.stringify(s,null,2)}],structuredContent:s,isError:!1})}catch(s){return Me(r.id,{content:[{type:"text",text:s instanceof Error?s.message:String(s)}],isError:!0})}}default:return He(r.id,-32601,"Method not found")}},"handleMcpTransport"),Di=n(()=>wr({appUrl:x,updatedAt:We,toolNames:Nr.map(e=>e.name)}).contents[0].text,"buildKnowgrphMcpAppHtmlBody");var Ht=n(e=>e===C||e===`${C}/`,"handlesKnowgrphRoot"),Hi=n(e=>Ht(e)||!!Ii(e),"handlesKnowgrphHtmlSurface"),Ki=n(e=>e.startsWith(`${C}/assets/`),"handlesKnowgrphStaticAsset"),ji=n(async e=>{let t=new Headers(e.request.headers);t.delete("origin");let r=new Request(e.request.url,{method:e.request.method,headers:t});return typeof e.env?.ASSETS?.fetch=="function"?e.env.ASSETS.fetch(r):e.next(r)},"fetchKnowgrphStaticAsset"),Bi=n(e=>{let t=new URL(e.url),r=t.pathname.replace(/\/+$/,"")||"/",o=Ur(e.url);return r===ot?"health":r===`${C}/mcp`?"mcp":Gn(r)?"github-workspace-write":r===`${C}/robots.txt`?"robots":r===`${C}/sitemap.xml`?"sitemap":r===`${C}/auth.md`||r==="/auth.md"?"auth-md":r.startsWith(`${C}/.well-known/`)?"well-known":o?mt(e)?"shared-doc-markdown":"shared-doc-html":Ht(t.pathname)?mt(e)?"homepage-markdown":"homepage-html":"app-surface"},"resolveAgentReadyRouteTag"),$t=n((e,t)=>In(t,{owner:xn,tag:Bi(e)}),"withKnowgrphRouteHeaders"),Dn=n(async e=>{let t=new URL(e.url),r=t.pathname.replace(/\/+$/,"")||"/",o=Ur(e.url),a=pn(r);if(a)return Response.redirect(new URL(a,e.url),308);if(o&&mt(e))return Mi(e,o);if(Ht(t.pathname)&&mt(e))return On(vn);switch(r){case ot:return Qs(Qn());case`${C}/mcp`:return $i(e);case`${C}/robots.txt`:return ht(li,"text/plain; charset=utf-8");case`${C}/sitemap.xml`:return ht(pi,"application/xml; charset=utf-8");case`${C}/auth.md`:case"/auth.md":return ht(ui,"text/markdown; charset=utf-8");case`${C}/.well-known/api-catalog`:return Pe(qn,"application/linkset+json; charset=utf-8");case`${C}/.well-known/openapi.json`:return Pe(Yn,"application/vnd.oai.openapi+json; charset=utf-8");case Cr:return Pe(Vn);case`${C}/.well-known/oauth-protected-resource`:return Pe(gt);case`${C}/.well-known/oauth-authorization-server`:return Pe(Ln);case`${C}/.well-known/openid-configuration`:return Pe(Ln);case`${C}/.well-known/mcp/server-card.json`:return Pe(Te);case`${C}/.well-known/mcp/apps/knowgrph-agent-ready.html`:return Xs(Di());case`${C}/.well-known/mcp.json`:return Pe(Te);case`${C}/.well-known/agent-skills/index.json`:return Pe(await Xn());case`${C}/.well-known/http-message-signatures-directory`:return Pe(xi);default:return $n.has(r)?ht($n.get(r),"text/markdown; charset=utf-8"):null}},"routeResponse");async function Ke(e){let{env:t,request:r}=e,o=String(r.method||"GET").toUpperCase(),a=new URL(r.url);if(o==="OPTIONS")return new Response(null,{status:204,headers:{"access-control-allow-origin":"*","access-control-allow-methods":"GET, HEAD, POST, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(o==="POST"&&a.pathname.replace(/\/+$/,"")===`${C}/mcp`)return $t(r,await Dn(r));if(o==="POST"&&Gn(a.pathname))return $t(r,await ii(r,t));if(o!=="GET"&&o!=="HEAD")return he(405,{ok:!1,error:"unsupported_method"});if(Ki(a.pathname))return ji(e);let s=await Dn(r);if(s){let m=$t(r,s);return o==="HEAD"?new Response(null,m):m}let i=Ur(r.url),c=i?await hn(e,C):await e.next();if(!Hi(a.pathname))return c;let l=o==="HEAD"?c:await _i(c),p=new Response(o==="HEAD"?null:l.body,l);return p.headers.set("link",Tn),(Ht(a.pathname)||i)&&(p.headers.delete("x-frame-options"),p.headers.delete("content-security-policy-report-only"),p.headers.set("content-security-policy","frame-ancestors *")),$t(r,p)}n(Ke,"onRequest");async function ta(e){return Ke(e)}n(ta,"onRequest");async function ra(e){return Ke(e)}n(ra,"onRequest");async function oa(e){return Ke(e)}n(oa,"onRequest");var Fi=Object.freeze(new Set(["","80","443"])),zi=Object.freeze([".local",".localhost",".internal"]),Gi=Object.freeze(new Set(["localhost"]));function Ye(e){return String(e||"").trim().toLowerCase()}n(Ye,"normalizeHostname");function Wi(e){let t=Ye(e);if(!/^\d{1,3}(\.\d{1,3}){3}$/.test(t))return!1;let r=t.split(".").map(o=>Number(o));return r.length!==4?!1:r.every(o=>Number.isInteger(o)&&o>=0&&o<=255)}n(Wi,"isIpv4Literal");function na(e){let[t,r,o,a]=e.split(".").map(s=>Number(s));return(t<<24|r<<16|o<<8|a)>>>0}n(na,"ipv4ToInt");function qi(e,t,r){if(!Number.isInteger(r)||r<0||r>32)return!1;if(r===0)return!0;let o=4294967295<<32-r>>>0;return(e&o)===(t&o)}n(qi,"inIpv4Cidr");function Yi(e){let t=Ye(e);return!t||!t.includes(":")?!1:/^[0-9a-f:]+$/i.test(t)}n(Yi,"isIpv6Literal");function Vi(e){let t=Ye(e);return!!(!t||t==="::1"||t==="0:0:0:0:0:0:0:1"||t.startsWith("fc")||t.startsWith("fd")||/^fe[89ab]/i.test(t))}n(Vi,"isBlockedIpv6");function Ji(e,{blockedExact:t,blockedSuffixes:r}={}){let o=Ye(e);if(!o)return!0;let a=t||Gi;if(a instanceof Set&&a.has(o))return!0;let s=r||zi;if(Array.isArray(s))for(let i of s){let c=Ye(i);if(c&&(o===c||o.endsWith(c)))return!0}return!1}n(Ji,"isBlockedHostname");function Xi(e){let t=Ye(e);if(!t)return!0;if(Wi(t)){let r=na(t),o=[{base:"0.0.0.0",bits:8},{base:"10.0.0.0",bits:8},{base:"127.0.0.0",bits:8},{base:"169.254.0.0",bits:16},{base:"172.16.0.0",bits:12},{base:"192.168.0.0",bits:16},{base:"100.64.0.0",bits:10}];for(let a of o){let s=na(a.base);if(qi(r,s,a.bits))return!0}return!1}return Yi(t)?Vi(t):!1}n(Xi,"isBlockedIpLiteral");function Kt(e,{allowedPorts:t}={}){let r=String(e||"").trim();if(!r)throw new Error("invalid_url");let o;try{o=new URL(r)}catch{throw new Error("invalid_url")}if(o.protocol!=="http:"&&o.protocol!=="https:")throw new Error("invalid_url");if(o.username||o.password)throw new Error("invalid_url");let a=t||Fi,s=String(o.port||"");if(a instanceof Set&&!a.has(s))throw new Error("port_not_allowed");let i=Ye(o.hostname);if(!i)throw new Error("invalid_url");if(Ji(i))throw new Error("blocked_host");if(Xi(i))throw new Error("blocked_host");return o}n(Kt,"parseAndValidateExternalUrl");function jt(e){return String(e.headers.get("sec-fetch-site")||"").trim().toLowerCase()==="cross-site"}n(jt,"shouldRejectCrossSiteFetch");var Qi={"content-type":"application/json; charset=utf-8","cache-control":"public, max-age=600"};function Ve(e,t={}){return new Response(JSON.stringify(e),{...t,headers:{...Qi,...t.headers||{}}})}n(Ve,"json");function Bt(...e){for(let t of e){if(!t)continue;let r=String(t).trim();if(r)return r}return null}n(Bt,"pickFirst");function Zi(e){let t=e.slice(0,8e4),r=t.match(/<title[^>]*>([^<]*)<\/title>/i),o=t.match(/<meta[^>]+property=["']og:title["'][^>]*content=["']([^"']+)["'][^>]*>/i),a=t.match(/<meta[^>]+property=["']og:description["'][^>]*content=["']([^"']+)["'][^>]*>/i),s=t.match(/<meta[^>]+name=["']description["'][^>]*content=["']([^"']+)["'][^>]*>/i),i=t.match(/<meta[^>]+property=["']og:image["'][^>]*content=["']([^"']+)["'][^>]*>/i),c=t.match(/<meta[^>]+property=["']og:site_name["'][^>]*content=["']([^"']+)["'][^>]*>/i);return{title:Bt(o?.[1],r?.[1]),description:Bt(a?.[1],s?.[1]),image:Bt(i?.[1]),siteName:Bt(c?.[1])}}n(Zi,"extractMeta");async function aa(e){let t=e.request.url,r=new URL(t);if(r.searchParams.get("ping")==="1")return Ve({ok:!0,ping:!0});let o=r.searchParams.get("url")||"";if(jt(e.request))return Ve({ok:!1,error:"forbidden"},{status:403,headers:{"cache-control":"no-store"}});let a;try{a=Kt(o)}catch{return Ve({ok:!1,error:"invalid_url"},{status:400,headers:{"cache-control":"no-store"}})}try{let s=await fetch(a.toString(),{headers:{"user-agent":"Mozilla/5.0 (compatible; HackaMapLinkPreview/1.0)",accept:"text/html,application/xhtml+xml"},redirect:"follow",cf:{cacheTtl:600,cacheEverything:!0}}),i=s.headers.get("content-type")||"";if(!s.ok)return Ve({ok:!1,error:"fetch_failed",status:s.status,url:a.toString()},{status:200,headers:{"cache-control":"no-store"}});if(!i.includes("text/html"))return Ve({ok:!0,url:a.toString(),domain:a.host,contentType:i,title:null,description:null,image:null,siteName:null});let c=await s.text(),l=Zi(c);return Ve({ok:!0,url:a.toString(),domain:a.host,contentType:i,...l})}catch(s){return Ve({ok:!1,error:"exception",message:s?.message||String(s),url:a.toString()},{status:200,headers:{"cache-control":"no-store"}})}}n(aa,"onRequestGet");var sa=35e4;function ec(e){let t=e;return t=t.replace(/<script\b[\s\S]*?<\/script>/gi,""),t=t.replace(/<iframe\b[\s\S]*?<\/iframe>/gi,""),t=t.replace(/<object\b[\s\S]*?<\/object>/gi,""),t=t.replace(/<embed\b[\s\S]*?>/gi,""),t=t.replace(/<noscript\b[\s\S]*?<\/noscript>/gi,""),t=t.replace(/\son[a-z]+\s*=\s*"[^"]*"/gi,""),t=t.replace(/\son[a-z]+\s*=\s*'[^']*'/gi,""),t}n(ec,"stripActiveContent");function tc({url:e,title:t,innerHtml:r}){let o=t?String(t).slice(0,140):"Preview",a=String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;");return`<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
...
</html>`}n(tc,"buildWrapper");async function ia(e){let r=new URL(e.request.url).searchParams.get("url")||"";if(jt(e.request))return new Response("Forbidden",{status:403,headers:{"cache-control":"no-store"}});let o;try{o=Kt(r)}catch(a){let s=a instanceof Error?a.message:"invalid_url";return new Response(s,{status:400,headers:{"cache-control":"no-store"}})}try{let a=await fetch(o.toString(),{headers:{"user-agent":"Mozilla/5.0 (compatible; HackaMapLinkProxy/1.0)",accept:"text/html,application/xhtml+xml"},redirect:"follow",cf:{cacheTtl:600,cacheEverything:!0}}),s=a.headers.get("content-type")||"";if(!a.ok)return new Response(`Fetch failed (${a.status})`,{status:200,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}});if(!s.includes("text/html"))return new Response(`Unsupported content-type: ${s}`,{status:200,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"public, max-age=600"}});let i=await a.text();i.length>sa&&(i=i.slice(0,sa));let l=i.match(/<title[^>]*>([^<]*)<\/title>/i)?.[1]?.trim()||o.host;i=ec(i),/<base\s/i.test(i)||(i=i.replace(/<head([^>]*)>/i,`<head$1><base href="${o.origin}/">`));let m=tc({url:o.toString(),title:l,innerHtml:i});return new Response(m,{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"public, max-age=600","content-security-policy":"default-src 'none'; img-src https: data:; style-src 'unsafe-inline' https:; font-src https: data:; frame-ancestors 'self';"}})}catch(a){return new Response(`Exception: ${a?.message||String(a)}`,{status:200,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}})}}n(ia,"onRequestGet");var Lr="api.openai.com",$r="gateway.ai.cloudflare.com",Dr="api.cloudflare.com",Ft="api.miromind.ai",zt="apihub.agnes-ai.com",Gt="ark.ap-southeast.bytepluses.com",Hr="ark.eu-west.bytepluses.com",la=new Set(["localhost","127.0.0.1","0.0.0.0"]),ke=n(e=>String(e||"").trim().toLowerCase(),"normalizeHost"),pe=n((e,t)=>String(e.get(t)||"").trim(),"readHeader"),pa=n(e=>la.has(ke(e)),"isLocalHost"),ca=n(e=>{let t=String(e||"").trim();if(!t)return new Set;let r=new Set;return t.split(",").map(o=>ke(o)).filter(Boolean).forEach(o=>r.add(o)),r},"parseCsvSet"),ua=n((e,{includeOpenAi:t=!1,includeAiGateway:r=!1,includeMiroMind:o=!1,includeAgnes:a=!1,includeBytePlus:s=!1}={})=>{let i=ca(e.KNOWGRPH_INTEGRATION_ALLOWED_HOSTS),c=ca(e.KNOWGRPH_CHAT_PROXY_ALLOWED_HOSTS),l=i.size?i:c,p=l.size?l:new Set([...la]);return t&&p.add(Lr),r&&(p.add($r),p.add(Dr)),o&&p.add(Ft),a&&p.add(zt),s&&(p.add(Gt),p.add(Hr)),p},"parseAllowedHosts"),ce=n(e=>{let t=pe(e.headers,"origin");if(!t)return{};let r="";try{r=ke(new URL(t).host)}catch{return{}}let o=ke(new URL(e.url).host);return r===o||r.startsWith("localhost:")||r.startsWith("127.0.0.1:")?{"access-control-allow-origin":t,vary:"Origin"}:{}},"corsHeaders"),ge=n((e,t,r)=>new Response(JSON.stringify(t),{status:r,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store",...ce(e)}}),"jsonResponse");var Wt={"content-type":"application/json; charset=utf-8","cache-control":"no-store"};function yt(e,t,r=200){return new Response(JSON.stringify(t),{status:r,headers:{...Wt,...ce(e)}})}n(yt,"jsonResponse");async function rc(e){let t=new URL("/knowgrph/imports/hackamap/hackamap-graph.json",e.url),r=await fetch(t.toString(),{redirect:"follow"});return r.ok?await r.json():null}n(rc,"fetchHackamapGraphJson");async function Je(e,t){let r=new URL(t,e.url),o=await fetch(r.toString(),{redirect:"follow"});return o.ok?await o.json():null}n(Je,"fetchHackamapJson");async function oc(e){let t=await Je(e,"/knowgrph/imports/hackamap/hackamap_api_graph.json");return ga(t)?t:null}n(oc,"fetchHackamapApiGraphJson");async function nc(e){let t=await Je(e,"/knowgrph/imports/hackamap/hackamap_pipeline.json");return t&&typeof t=="object"&&!Array.isArray(t)?t:{}}n(nc,"fetchHackamapPipelineJson");async function ma(e){let t=await Je(e,"/knowgrph/imports/hackamap/hackamap_query_presets.json");return Array.isArray(t)?t.filter(Boolean):[]}n(ma,"fetchHackamapQueryPresetsJson");async function ha(e){let t=await Je(e,"/knowgrph/imports/hackamap/query-outputs/query-runs.manifest.json");return t&&typeof t=="object"&&!Array.isArray(t)?t:{}}n(ha,"fetchHackamapQueryRunsManifestJson");function ga(e){return!e||typeof e!="object"||Array.isArray(e)?!1:Array.isArray(e.nodes)&&Array.isArray(e.edges)}n(ga,"isApiGraphPayload");function fa(e,t){let r=String(e&&e.output&&e.output.per_table_prefix||e?.id||t?.preset||"").trim(),o=String(t?.output_suffix||"").trim();return o?`${r}-${o}`:r}n(fa,"buildHackamapTablePrefix");function Kr(e,t){if(!Array.isArray(e))return[];let r=[];for(let o of e){if(!o||typeof o!="object"||Array.isArray(o))continue;let a=String(o[t]||"").trim();a&&r.push(a)}return r}n(Kr,"collectRowIds");async function ac(e,t){let r=await Je(e,t);return Array.isArray(r)?r.length:0}n(ac,"countHackamapQueryRows");async function sc(e,t,r){let o=fa(t,r);if(!o)return{};let a=["events","demos","sources","organizer","team","techstack"],s=await Promise.all(a.map(async i=>[i,await ac(e,`/knowgrph/imports/hackamap/query-outputs/${i}.${o}.query.json`)]));return Object.fromEntries(s.filter(([,i])=>i>0))}n(sc,"readHackamapRunTableCounts");function jr(e){return Array.isArray(e)?e.map(jr):!e||typeof e!="object"?e:Object.fromEntries(Object.entries(e).sort(([t],[r])=>String(t).localeCompare(String(r))).map(([t,r])=>[t,jr(r)]))}n(jr,"sortObjectKeys");function ic(e){try{return JSON.stringify(jr(e))}catch{return""}}n(ic,"stableParamSignature");function cc(e){return typeof e=="string"?{value:e,label:e}:{value:e,label:JSON.stringify(e)}}n(cc,"toBuilderOption");function lc(e,t){return e.map(r=>{let o=String(r?.id||"").trim();if(!o)return null;let a=r?.params&&typeof r.params=="object"&&!Array.isArray(r.params)?r.params:{},s=t.filter(l=>String(l?.preset||"").trim()===o),i=Array.from(new Set([...Object.keys(a),...s.flatMap(l=>l?.params&&typeof l.params=="object"&&!Array.isArray(l.params)?Object.keys(l.params):[])])).sort((l,p)=>String(l).localeCompare(String(p))),c=Object.fromEntries(i.map(l=>{let p=new Set,m=[],f=[a[l],...s.map(P=>P?.params&&typeof P.params=="object"&&!Array.isArray(P.params)?P.params[l]:void 0)];for(let P of f){if(typeof P>"u")continue;let S=ic(P);!S||p.has(S)||(p.add(S),m.push(cc(P)))}return[l,m]}));return{id:o,title:String(r?.title||o).trim(),params:a,param_keys:i,published_param_options:c}}).filter(Boolean)}n(lc,"buildHackamapPresetRuntimeEntries");async function pc(e){let[t,r,o]=await Promise.all([nc(e),ma(e),ha(e)]),a=t&&typeof t=="object"?t.runtime||{}:{},s=String(a?.query_selection?.default_run_id||"").trim()||"enhanced",i=Array.isArray(o?.runs)?o.runs:[],c=(await Promise.all(i.map(async l=>{let p=String(l?.id||"").trim(),m=String(l?.preset||"").trim();if(!p)return null;let f=r.find(S=>String(S?.id||"").trim()===m),P=await sc(e,f,l);return{id:p,preset:m,title:String(l?.title||l?.id||"").trim(),params:l?.params&&typeof l.params=="object"&&!Array.isArray(l.params)?l.params:{},output_suffix:String(l?.output_suffix||"").trim(),is_default:p===s,table_counts:P}}))).filter(l=>l?.id);return{ok:!0,runtime:{...a&&typeof a=="object"?a:{},presets:lc(r,c),runs:c}}}n(pc,"buildHackamapRuntimeMeta");async function uc(e,t){let r=String(t||"").trim();if(!r)return null;let[o,a]=await Promise.all([ma(e),ha(e)]),i=(Array.isArray(a?.runs)?a.runs:[]).find(O=>String(O?.id||"").trim()===r);if(!i)return null;let c=o.find(O=>String(O?.id||"").trim()===String(i?.preset||"").trim()),l=fa(c,i);if(!l)return null;let[p,m]=await Promise.all([Je(e,`/knowgrph/imports/hackamap/query-outputs/events.${l}.query.json`),Je(e,`/knowgrph/imports/hackamap/query-outputs/demos.${l}.query.json`)]),f=new Set(Kr(p,"id")),P=new Set(Kr(m,"id")),S=Kr(m,"event_id");for(let O of S)f.add(O);return{eventIds:f,demoIds:P}}n(uc,"readHackamapQueryRunSelection");function da(e,t,r){if(!r||!ga(e))return e;if(r.eventIds.size===0&&r.demoIds.size===0)return{...e,meta:{...e?.meta&&typeof e.meta=="object"?e.meta:{},selected_run_id:t,selected_run_filter_skipped:"no-event-demo-rows"}};let o=new Set;r.eventIds.forEach(c=>o.add(`Event:${c}`)),r.demoIds.forEach(c=>o.add(`Demo:${c}`));let a=Array.isArray(e.nodes)?e.nodes.filter(c=>o.has(String(c?.id||"").trim())):[],s=new Set(a.map(c=>String(c?.id||"").trim()).filter(Boolean)),i=Array.isArray(e.edges)?e.edges.filter(c=>s.has(String(c?.source||"").trim())&&s.has(String(c?.target||"").trim())):[];return{...e,nodes:a,edges:i,meta:{...e?.meta&&typeof e.meta=="object"?e.meta:{},selected_run_id:t,selected_event_count:r.eventIds.size,selected_demo_count:r.demoIds.size,total_problems:a.filter(c=>String(c?.type||"").trim()==="problem").length,total_solutions:a.filter(c=>String(c?.type||"").trim()==="solution").length}}}n(da,"filterHackamapApiGraphPayloadByRun");function dc(e){let t=Array.isArray(e?.nodes)?e.nodes:[],r=Array.isArray(e?.links)?e.links:[],o=[],a=new Set;for(let i of t){let c=String(i?.id||"").trim(),l=String(i?.type||"").trim(),p=String(i?.label||"").trim();if(!(!c||!l||!p)){if(l==="Event"){o.push({id:c,type:"problem",label:p,cluster:"Event"}),a.add(c);continue}l==="Demo"&&(o.push({id:c,type:"solution",label:p,cluster:"Demo"}),a.add(c))}}let s=[];for(let i of r){let c=String(i?.source||"").trim(),l=String(i?.target||"").trim(),p=String(i?.type||"").trim();!c||!l||p==="has_demo"&&(!a.has(c)||!a.has(l)||s.push({source:c,target:l,type:"has_demo",strength:.35}))}return{nodes:o,edges:s,meta:{source:"hackamap-graph.json:fallback",total_problems:o.filter(i=>i.type==="problem").length,total_solutions:o.filter(i=>i.type==="solution").length,...e?.content_signature?{content_signature:String(e.content_signature)}:{}}}}n(dc,"toBipartiteApiPayload");async function ya(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase(),o=new URL(t.url);if(r==="OPTIONS")return new Response(null,{status:204,headers:{...ce(t),"access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(r!=="GET"&&r!=="HEAD")return yt(t,{ok:!1,error:"unsupported_method"},405);if(String(o.searchParams.get("view")||"").trim().toLowerCase()==="meta"){let m=await pc(t);return r==="HEAD"?new Response(null,{status:200,headers:{...Wt,...ce(t)}}):yt(t,m,200)}let a=String(o.searchParams.get("run")||"").trim(),s=await uc(t,a),i=await oc(t);if(i){let m=da(i,a,s);return r==="HEAD"?new Response(null,{status:200,headers:{...Wt,...ce(t)}}):yt(t,m,200)}let c=await rc(t);if(!c)return yt(t,{ok:!1,error:"missing_hackamap_graph",hint:"/knowgrph/imports/hackamap/{hackamap_api_graph.json,hackamap-graph.json} not found"},404);let l=dc(c),p=da(l,a,s);return r==="HEAD"?new Response(null,{status:200,headers:{...Wt,...ce(t)}}):yt(t,p,200)}n(ya,"onRequest");var mc=!0,wa=600,Sa={"content-type":"application/json; charset=utf-8","cache-control":`public, max-age=${wa}`};function Xe(e,t,r={}){return new Response(JSON.stringify(t),{...r,headers:{...Sa,...r.headers||{},...ce(e)}})}n(Xe,"jsonResponse");function hc(e){try{let t=new URL(String(e));return t.protocol==="http:"||t.protocol==="https:"}catch{return!1}}n(hc,"isHttpUrl");function wt(e){return String(e||"").trim().toLowerCase()}n(wt,"normalizeHost");function Br(e,{exact:t,suffixes:r}){let o=wt(e);return o?!!(Array.isArray(t)&&t.some(a=>o===wt(a))||Array.isArray(r)&&r.some(a=>o===wt(a)||o.endsWith(`.${wt(a)}`))):!1}n(Br,"isHostMatch");function gc(e){let t=wt(e.hostname),r=e.toString();return Br(t,{suffixes:["linkedin.com"]})?new URL(`https://www.linkedin.com/embeds/oembed.json?url=${encodeURIComponent(r)}`):Br(t,{exact:["twitter.com","x.com"],suffixes:["twitter.com","x.com"]})?new URL(`https://publish.twitter.com/oembed?omit_script=1&url=${encodeURIComponent(r)}`):Br(t,{exact:["reddit.com"],suffixes:["reddit.com"]})?new URL(`https://www.reddit.com/oembed?url=${encodeURIComponent(r)}`):null}n(gc,"buildOembedUpstreamUrl");async function fc({upstreamUrl:e}){return await fetch(e.toString(),{headers:{"user-agent":"Mozilla/5.0 (compatible; OEmbedProxy/1.0)",accept:"application/json,text/json;q=0.9,*/*;q=0.1"},redirect:"follow",cf:{cacheTtl:wa,cacheEverything:!0}})}n(fc,"fetchJsonUpstream");async function ba(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase(),o=new URL(t.url);if(r==="OPTIONS")return new Response(null,{status:204,headers:{...ce(t),"access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(!["GET","HEAD"].includes(r))return Xe(t,{ok:!1,error:"unsupported_method"},{status:405});if(o.searchParams.get("ping")==="1")return Xe(t,{ok:!0,ping:!0});if(mc)return Xe(t,{ok:!1,error:"disabled_by_policy"},{status:200,headers:{"cache-control":"no-store"}});let a=o.searchParams.get("url")||"";if(!hc(a))return Xe(t,{ok:!1,error:"invalid_url"},{status:400,headers:{"cache-control":"no-store"}});let s;try{s=new URL(a)}catch{return Xe(t,{ok:!1,error:"invalid_url"},{status:400,headers:{"cache-control":"no-store"}})}let i=gc(s);if(!i)return Xe(t,{ok:!1,error:"unsupported_provider"},{status:400,headers:{"cache-control":"no-store"}});let c=await fc({upstreamUrl:i}),l=new Headers(c.headers);l.delete("content-length"),l.set("cache-control",c.ok?Sa["cache-control"]:"no-store");for(let[m,f]of Object.entries(ce(t)))l.set(m,f);if(r==="HEAD")return new Response(null,{status:c.status,headers:l});let p=await c.text();try{JSON.parse(p)}catch{return Xe(t,{ok:!1,error:"invalid_upstream_json",status:c.status},{status:502,headers:{"cache-control":"no-store"}})}return l.set("content-type","application/json; charset=utf-8"),new Response(p,{status:c.status,headers:l})}n(ba,"onRequest");var _a="/__chat_proxy",qt="agnes-ai",Yt="byteplus-modelark",Vt="miromind",yc="x-kg-ai-gateway-route",wc="x-kg-ai-gateway-metadata",Sc="x-kg-ai-gateway-cache-ttl",bc=n(e=>{let t=ke(e);return t==="openai"?"openai":t===Yt||t==="byteplus"?Yt:t===Vt||t==="miromind-api"?Vt:t===qt||t==="agnes"||t==="agnes-ai-api"?qt:t},"normalizeProviderId"),_c=n(e=>ke(e)===zt,"isAgnesHost"),Ac=n(e=>{let t=ke(e);return t===Gt||t===Hr},"isBytePlusHost"),Pc=n(e=>ke(e)===Ft,"isMiroMindHost"),kc=n(e=>{let t=ke(e);return t===$r||t===Dr},"isAiGatewayHost"),xc=n(e=>{let t=String(e||"").trim();return/^dynamic\/[a-z0-9._/-]+$/i.test(t)?t.slice(0,128):""},"sanitizeAiGatewayRoute"),Rc=n(e=>{let t=String(e||"").trim();if(!t)return"";try{let r=JSON.parse(t),o=Object.entries(r||{}).filter(([,a])=>typeof a=="string"||typeof a=="number"||typeof a=="boolean").slice(0,5).map(([a,s])=>[String(a||"").trim().slice(0,64),typeof s=="string"?s.trim().slice(0,160):s]).filter(([a])=>a);return o.length?JSON.stringify(Object.fromEntries(o)):""}catch{return""}},"sanitizeAiGatewayMetadata"),Ec=n(e=>{let t=Number(String(e||"").trim());return Number.isFinite(t)?String(Math.max(1,Math.min(86400,Math.floor(t)))):""},"sanitizeAiGatewayCacheTtl"),Cc=n((e,t,r)=>{let a=`${e.pathname==="/"?"":String(e.pathname||"").replace(/\/+$/g,"")}${t.startsWith("/")?t:`/${t}`}`;return new URL(`${a}${r||""}`,`${e.protocol}//${e.host}`)},"buildUpstreamUrl"),Tc=n(({provider:e,requestedUpstream:t,env:r})=>e==="openai"?t||"https://api.openai.com":e===Vt?t||`https://${Ft}`:e===qt?t||`https://${zt}`:e===Yt?t||String(r.KNOWGRPH_CHAT_PROXY_UPSTREAM||"").trim()||`https://${Gt}`:t||String(r.KNOWGRPH_CHAT_PROXY_UPSTREAM||"").trim(),"pickUpstreamBase");async function Aa(e){let{request:t,env:r}=e,o=String(t.method||"GET").toUpperCase(),a=new URL(t.url);if(o==="OPTIONS")return new Response(null,{status:204,headers:{"access-control-allow-origin":pe(t.headers,"origin")||"*","access-control-allow-methods":"GET, HEAD, POST, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(!["GET","HEAD","POST"].includes(o))return ge(t,{ok:!1,error:"Unsupported method"},405);let s=bc(pe(t.headers,"x-kg-chat-provider")),i=xc(pe(t.headers,yc)),c=Rc(pe(t.headers,wc)),l=Ec(pe(t.headers,Sc)),p=String(r.KNOWGRPH_CHAT_PROXY_AI_GATEWAY_BASE_URL||"").trim(),m=String(r.KNOWGRPH_CHAT_PROXY_AI_GATEWAY_GATEWAY_ID||"").trim(),f=s==="openai"&&!!p&&!!i,P=Tc({provider:s,requestedUpstream:f?p:pe(t.headers,"x-kg-chat-upstream"),env:r});if(!P)return ge(t,{ok:!1,error:"Missing chat proxy upstream configuration"},500);let S;try{S=new URL(P)}catch{return ge(t,{ok:!1,error:"Invalid chat proxy upstream configuration"},500)}let O=ua(r,{includeOpenAi:!0,includeAiGateway:!0,includeMiroMind:!0,includeAgnes:!0,includeBytePlus:!0}),D=ke(S.hostname);if(!O.has(D))return ge(t,{ok:!1,error:"Chat proxy upstream host is not allowed"},403);if(!pa(D)&&S.protocol!=="https:")return ge(t,{ok:!1,error:"Chat proxy requires HTTPS for non-local upstream hosts"},403);let N=f||kc(D),G=!N&&(s==="openai"||D===Lr),Q=s===Vt||Pc(D),W=s===qt||_c(D),R=s===Yt||Ac(D),T=pe(t.headers,"x-kg-chat-api-key"),te=String(r.KNOWGRPH_CHAT_PROXY_AI_GATEWAY_TOKEN||r.AI_GATEWAY_TOKEN||r.CLOUDFLARE_API_TOKEN||"").trim(),F=String(r.KNOWGRPH_CHAT_PROXY_OPENAI_API_KEY||r.OPENAI_API_KEY||"").trim(),le=String(r.KNOWGRPH_CHAT_PROXY_MIROMIND_API_KEY||r.MIROMIND_API_KEY||"").trim(),U=String(r.KNOWGRPH_CHAT_PROXY_AGNES_API_KEY||r.AGNES_API_KEY||"").trim(),L=String(r.KNOWGRPH_CHAT_PROXY_BYTEPLUS_API_KEY||r.BYTEPLUS_API_KEY||"").trim(),Y=(T||te).slice(0,512),se=(T||F).slice(0,512),Se=(T||le).slice(0,512),xe=(T||U).slice(0,512),k=(T||L).slice(0,512),w=R?k:N?Y:W?xe:Q?Se:se;if(N&&!w)return ge(t,{ok:!1,error:"Missing Cloudflare AI Gateway token for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_AI_GATEWAY_TOKEN or AI_GATEWAY_TOKEN)"},401);if(G&&!se)return ge(t,{ok:!1,error:"Missing OpenAI API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_OPENAI_API_KEY or OPENAI_API_KEY)"},401);if(Q&&!w)return ge(t,{ok:!1,error:"Missing MiroMind API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_MIROMIND_API_KEY or MIROMIND_API_KEY)"},401);if(W&&!w)return ge(t,{ok:!1,error:"Missing Agnes API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_AGNES_API_KEY or AGNES_API_KEY)"},401);if(R&&!w)return ge(t,{ok:!1,error:"Missing BytePlus API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_BYTEPLUS_API_KEY or BYTEPLUS_API_KEY)"},401);if(o==="POST"&&!pe(t.headers,"content-type").toLowerCase().includes("application/json"))return ge(t,{ok:!1,error:"Chat proxy expects application/json payloads"},415);let y=a.pathname.startsWith(_a)&&a.pathname.slice(_a.length)||"/v1/chat/completions",d=y.startsWith("/")?y:`/${y}`,g=Cc(S,d,a.search||""),h=new Headers,A=pe(t.headers,"content-type"),E=pe(t.headers,"accept");A&&h.set("content-type",A),E&&h.set("accept",E),(G||N||Q||W||R)&&h.set("authorization",`Bearer ${w}`);let q=pe(t.headers,"x-client-request-id").slice(0,512);q&&h.set("x-client-request-id",q),N&&m&&h.set("cf-aig-gateway-id",m),c&&h.set("cf-aig-metadata",c),l&&h.set("cf-aig-cache-ttl",l);let H=new AbortController,$=Number(r.KNOWGRPH_CHAT_PROXY_TIMEOUT_MS),V=Number.isFinite($)?Math.max(5e3,Math.min(18e4,Math.floor($))):9e4,_=setTimeout(()=>H.abort(),V);try{let j=o==="GET"||o==="HEAD"?void 0:t.body;if(f&&o==="POST"){let Be=await t.clone().text();try{let Fe=JSON.parse(Be||"{}");Fe.model=i,j=JSON.stringify(Fe)}catch{j=Be}}let ue=await fetch(g.toString(),{method:o,headers:h,body:j,signal:H.signal,redirect:"follow"}),v=new Headers(ue.headers);v.delete("content-length"),v.delete("www-authenticate"),v.set("cache-control","no-store");let J=pe(t.headers,"origin");return J&&(v.set("access-control-allow-origin",J),v.set("vary","Origin")),o==="HEAD"?new Response(null,{status:ue.status,statusText:ue.statusText,headers:v}):new Response(ue.body,{status:ue.status,statusText:ue.statusText,headers:v})}catch(j){let ue=j&&typeof j=="object"&&"message"in j?String(j.message||""):"",v=H.signal.aborted||/aborted|timeout/i.test(ue);return ge(t,{ok:!1,error:ue||"Failed to reach chat upstream"},v?504:502)}finally{clearTimeout(_)}}n(Aa,"onRequest");function vc(e){let t=e.map(r=>r==null?"":typeof r=="boolean"?r?"1":"0":typeof r=="number"?Number.isFinite(r)?String(r):"":String(r)).join("|");return`rich-media-preview:${st(t)}`}n(vc,"buildRichMediaPreviewSemanticKey");var ka="png";function St(e){let t=typeof e=="number"?e:Number(String(e??"").trim());if(!Number.isFinite(t))return null;let r=Math.max(0,Number(t.toFixed(3)));return Number.isFinite(r)?r:null}n(St,"normalizeRemoteVideoFrameSeconds");function Oc(e){return String(St(e)??0).replace(/\./g,"_")}n(Oc,"formatRemoteVideoFrameSecondsForFileName");function Xt(e){let t=String(e||"").trim().toLowerCase();return t==="jpg"||t==="jpeg"?"jpg":"png"}n(Xt,"normalizeRemoteVideoFrameFormat");function Fr(e){let t=String(e.sourceUrl||"").trim(),r=St(e.timeSeconds)??0,o=Xt(e.format||ka);return vc(["remote-video-frame",t,r,o])}n(Fr,"buildRemoteVideoFrameSemanticKey");function xa(e){let t=St(e.timeSeconds)??0,r=Xt(e.format||ka),o=Fr({...e,timeSeconds:t,format:r});return`frame-${o.split(":").pop()||st(o)}-t${Oc(t)}.${r}`}n(xa,"buildRemoteVideoFrameFileName");var Jt=n(e=>{let t=String(e||"").trim();return t&&/^[A-Za-z0-9_-]{6,128}$/.test(t)?t:null},"normalizeYouTubeIdLikeValue"),Pa=n(e=>{try{let t=new URL(String(e||"").trim()),r=String(t.hostname||"").toLowerCase();if(r==="youtu.be"||r.endsWith(".youtu.be")){let o=t.pathname.replace(/^\/+/,"").split("/")[0]?.trim()||"";return Jt(o)}if(r==="youtube.com"||r.endsWith(".youtube.com")||r==="youtube-nocookie.com"||r.endsWith(".youtube-nocookie.com")){let o=String(t.searchParams.get("v")||"").trim();if(o)return Jt(o);let a=t.pathname.split("/").filter(Boolean),s=a[0]||"",i=a[1]||"";if((s==="embed"||s==="shorts"||s==="live")&&i)return Jt(i);if(s==="watch"){let c=String(t.searchParams.get("v")||"").trim();return Jt(c)}}}catch{return null}return null},"readYouTubeIdFromUrl");function Ic(e){let t=String(e||"").trim().replace(/^<|>$/g,"").trim();for(;/[),.;:!?]$/.test(t);){let r=t.slice(0,-1).trim();if(!r)break;let o=Pa(t),a=Pa(r);if(!a||o&&o!==a)break;t=r}return t}n(Ic,"stripYouTubeUrlTrailingPunctuation");function Ra(e){let t=n(r=>{let o=String(r||"").trim();if(!o)return null;if(/^\d+$/.test(o))return Number(o);let a=o.match(/^(?:(\d+)h)?(?:(\d+)m)?(?:(\d+)s)?$/i);if(!a)return null;let s=a[1]?Number(a[1]):0,i=a[2]?Number(a[2]):0,c=a[3]?Number(a[3]):0,l=s*3600+i*60+c;return l>0&&Number.isFinite(l)?l:null},"parseChunk");try{let r=new URL(Ic(e)),o=r.searchParams.get("t")||r.searchParams.get("start")||"",a=r.hash&&new URLSearchParams(r.hash.replace(/^#/,"")).get("t")||"";return t(o)??t(a)}catch{return null}}n(Ra,"parseYouTubeStartSeconds");var Mc="/image/knowgrph/video-frame",Nc=4096,Uc=720*60,Lc=/^frame-[a-f0-9]+-t\d+\.(?:png|jpg)$/i,qr={"access-control-allow-origin":"*","access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"accept, content-type"},Wr=n(e=>String(e||"").replace(/\s+/g," ").trim(),"cleanText"),zr=n((e,t=200,r="GET")=>new Response(r==="HEAD"?null:JSON.stringify(e),{status:t,headers:{...qr,"content-type":"application/json; charset=utf-8","cache-control":"no-store"}}),"jsonResponse"),Gr=n((e,t=200,r="GET")=>new Response(r==="HEAD"?null:e,{status:t,headers:{...qr,"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}}),"textResponse"),$c=n((e,t)=>e===t||e.endsWith(`.${t}`),"hostMatches"),Dc=n(e=>{let t=Wr(e?.KG_VIDEO_FRAME_ALLOWED_HOSTS);return t?t.split(",").map(r=>Wr(r).toLowerCase()).filter(Boolean):["youtube.com","youtu.be","youtube-nocookie.com","bilibili.com","b23.tv"]},"readAllowedHosts"),Hc=n(e=>Wr(e).replace(/^<|>$/g,"").trim(),"unwrapUrlInput"),Kc=n((e,t)=>{try{let r=new URL(e);if(r.protocol!=="https:"&&r.protocol!=="http:")return!1;let o=r.hostname.toLowerCase();return Dc(t).some(a=>$c(o,a))}catch{return!1}},"isAllowedSourceUrl"),jc=n((e,t)=>{let r=new URL(e.url),o=Hc(r.searchParams.get("url")||"");if(!o)return{error:"Missing url parameter"};if(o.length>Nc)return{error:"Video URL is too long"};if(!Kc(o,t))return{error:"Video frame extraction is limited to supported remote video hosts"};let a=St(r.searchParams.get("time"))??Ra(o);if(a==null)return{error:"Missing time parameter"};let s=Math.min(Uc,Math.max(0,a)),i=Xt(r.searchParams.get("format")||"png"),c=xa({sourceUrl:o,timeSeconds:s,format:i});if(!Lc.test(c))return{error:"Invalid frame cache key"};let l=`${Mc}/${c}`;return{sourceUrl:o,timeSeconds:s,format:i,fileName:c,publicUrl:l,semanticKey:Fr({sourceUrl:o,timeSeconds:s,format:i})}},"readFrameRequest"),Bc=n(async(e,t,r)=>{let o=new URL(t,e.request.url),a=new Request(o.toString(),{method:r});return typeof e.env?.ASSETS?.fetch=="function"?await e.env.ASSETS.fetch(a):await fetch(a)},"fetchStaticAsset"),Fc=n(e=>`Frame has not been generated yet. Run the local video-frame extractor and publish ${e.publicUrl}.`,"missingFrameMessage"),zc=n((e,t)=>{let r=new Headers;r.set("content-type",t.format==="jpg"?"image/jpeg":"image/png"),r.set("cache-control","public, max-age=31536000, immutable"),r.set("access-control-allow-origin","*");let o=e.headers.get("content-length");o&&r.set("content-length",o);let a=e.headers.get("etag");return a&&r.set("etag",a),r},"imageResponseHeaders");async function Ea(e){let t=e.request;if(t.method==="OPTIONS")return new Response(null,{status:204,headers:qr});if(t.method!=="GET"&&t.method!=="HEAD")return Gr("Method not allowed",405,t.method);let r=new URL(t.url).searchParams.get("emit")==="json",o=jc(t,e.env||{});if("error"in o)return r?zr({ok:!1,error:o.error},400,t.method):Gr(o.error,400,t.method);let a=r&&t.method!=="HEAD"?"GET":r||t.method==="HEAD"?"HEAD":"GET",s=await Bc(e,o.publicUrl,a);if(!s.ok){let i=Fc(o);return r?zr({ok:!1,error:i,publicUrl:o.publicUrl,semanticKey:o.semanticKey},404,t.method):Gr(i,404,t.method)}if(r){let i=Number(s.headers.get("content-length")||0);return(!Number.isFinite(i)||i<=0)&&t.method!=="HEAD"&&(i=(await s.arrayBuffer()).byteLength),zr({ok:!0,imageUrl:o.publicUrl,publicUrl:o.publicUrl,semanticKey:o.semanticKey,cached:!0,bytes:Number.isFinite(i)?Math.max(0,Math.floor(i)):0,timeSeconds:o.timeSeconds,format:o.format},200,t.method)}return new Response(t.method==="HEAD"?null:s.body,{status:200,headers:zc(s,o)})}n(Ea,"onRequest");var Ca={"content-type":"application/json; charset=utf-8","cache-control":"no-store","access-control-allow-origin":"*","access-control-allow-methods":"GET, HEAD, POST, OPTIONS","access-control-allow-headers":"content-type"},Qt=n((e,t=200,r="GET")=>new Response(r==="HEAD"?null:JSON.stringify(e),{status:t,headers:Ca}),"jsonResponse"),re=n(e=>String(e||"").replace(/\s+/g," ").trim(),"cleanText"),Gc=n(e=>{try{let t=new URL(String(e||"").trim());if(/youtu\.be$/i.test(t.hostname))return re(t.pathname.split("/").filter(Boolean)[0]);if(/youtube\.com$/i.test(t.hostname)||/youtube-nocookie\.com$/i.test(t.hostname)){let r=re(t.searchParams.get("v"));if(r)return r;let o=t.pathname.split("/").filter(Boolean),a=o.findIndex(s=>["embed","shorts","live"].includes(s));if(a>=0)return re(o[a+1])}}catch{}return""},"readVideoId"),Wc=n((e,t)=>{let r=e.indexOf(t);if(r<0)return null;let o=e.indexOf("{",r);if(o<0)return null;let a=0,s=!1,i=!1;for(let c=o;c<e.length;c+=1){let l=e[c];if(s){i?i=!1:l==="\\"?i=!0:l==='"'&&(s=!1);continue}if(l==='"')s=!0;else if(l==="{")a+=1;else if(l==="}"&&(a-=1,a===0))return e.slice(o,c+1)}return null},"extractJsonAfter"),qc=n(e=>{for(let t of["ytInitialPlayerResponse =","ytInitialPlayerResponse="]){let r=Wc(e,t);if(r)try{return JSON.parse(r)}catch{}}return null},"parsePlayerResponse"),Yc=n((e,t)=>{let r=re(t||"en").toLowerCase();return e.find(o=>re(o.languageCode).toLowerCase()===r)||e.find(o=>re(o.languageCode).toLowerCase().startsWith(r.split("-")[0]))||e.find(o=>re(o.kind)!=="asr")||e[0]||null},"pickCaptionTrack"),Vc=n(e=>{let t=new URL(e);return t.searchParams.set("fmt","json3"),t.toString()},"withJsonCaptionFormat"),Jc=n(e=>(Array.isArray(e?.events)?e.events:[]).map(r=>{let o=Array.isArray(r.segs)?re(r.segs.map(i=>i?.utf8||"").join("")):"",a=Number(r.tStartMs)/1e3,s=Number(r.dDurationMs||0)/1e3;return o&&Number.isFinite(a)?{text:o,start:a,duration:Number.isFinite(s)?s:0}:null}).filter(Boolean),"parseCaptionJson3"),Xc=n(e=>String(e||"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;|&apos;/g,"'"),"decodeXmlText"),Qc=n(e=>{let t=[],r=/<text\b([^>]*)>([\s\S]*?)<\/text>/gi,o=null;for(;o=r.exec(String(e||""));){let a=o[1]||"",s=Number(a.match(/\bstart="([^"]+)"/i)?.[1]),i=Number(a.match(/\bdur="([^"]+)"/i)?.[1]||0),c=re(Xc(o[2]||""));c&&Number.isFinite(s)&&t.push({text:c,start:s,duration:Number.isFinite(i)?i:0})}return t},"parseCaptionXml"),Zc=n((e,t)=>{let r=String(e||"").trim();if(!r)return[];if(String(t||"").toLowerCase().includes("json")||r.startsWith("{")||r.startsWith("["))try{return Jc(JSON.parse(r))}catch{return[]}return Qc(r)},"parseCaptionResponseText"),el=n(e=>{let t=Math.max(0,Math.floor(Number(e)||0)),r=Math.floor(t/60),o=String(t%60).padStart(2,"0");return`${r}:${o}`},"formatTimestamp"),tl=n((e,t)=>{let r=new URL(e);return r.searchParams.set("t",`${Math.max(0,Math.floor(Number(t)||0))}s`),r.toString()},"timestampUrl"),rl=n(({title:e,sourceUrl:t,videoId:r,authorName:o,thumbnailUrl:a,segments:s})=>[`# ${e||`YouTube ${r}`}`,"",`Video ID: ${r}`,o?`Author: ${o}`:"",`Source: [${t}](${t})`,a?`[![${e||r}](${a})](${t})`:"","",s.length>0?"## Transcript":"## Video Source","",...s.length>0?s.map(i=>`[${el(i.start)}](${tl(t,i.start)}) ${i.text}`):["Captions were not available from the source at import time.","The source URL, title, author, and thumbnail remain available for downstream storyboard reconstruction."],""].filter(i=>i!=="").join(`
`),"buildMarkdown"),Yr=n(({videoId:e,sourceUrl:t,title:r,authorName:o,thumbnailUrl:a,lang:s,languageCode:i,segments:c,captionStatus:l})=>{let p={type:"rag:YouTubeTranscript",video_id:e,source_url:t,title:r,author_name:o,thumbnail_url:a,language_code:re(i)||s,caption_status:l,segment_count:c.length,duration:c.reduce((m,f)=>Math.max(m,f.start+f.duration),0),segments:c};return{ok:!0,name:`youtube-${e.toLowerCase()}.md`,markdown:rl({title:r,sourceUrl:t,videoId:e,authorName:o,thumbnailUrl:a,segments:c}),transcript:p}},"buildPayload");async function ol({sourceUrl:e,lang:t="en",fetchImpl:r=fetch}){let o=Gc(e);if(!o)return{ok:!1,error:"unsupported_youtube_url"};let a=`https://www.youtube.com/watch?v=${encodeURIComponent(o)}`,[s,i]=await Promise.all([r(`https://www.youtube.com/oembed?url=${encodeURIComponent(a)}&format=json`,{headers:{accept:"application/json"}}).catch(()=>null),r(a,{headers:{accept:"text/html,application/xhtml+xml","accept-language":"en-US,en;q=0.9","user-agent":"Mozilla/5.0 Knowgrph YouTube transcript importer"}})]),c=s?.ok?await s.json().catch(()=>({})):{},l=i.ok?qc(await i.text()):null,p=re(c.title)||re(l?.videoDetails?.title)||`YouTube ${o}`,m=re(c.author_name)||re(l?.videoDetails?.author),f=re(c.thumbnail_url)||`https://i.ytimg.com/vi/${o}/hqdefault.jpg`;if(!i.ok)return Yr({videoId:o,sourceUrl:a,title:p,authorName:m,thumbnailUrl:f,lang:t,languageCode:t,segments:[],captionStatus:`watch-fetch-${i.status}`});let P=l?.captions?.playerCaptionsTracklistRenderer?.captionTracks||[],S=Yc(Array.isArray(P)?P:[],t);if(!S?.baseUrl)return Yr({videoId:o,sourceUrl:a,title:p,authorName:m,thumbnailUrl:f,lang:t,languageCode:t,segments:[],captionStatus:"captions-unavailable"});let O=await r(Vc(S.baseUrl),{headers:{accept:"application/json,text/xml,text/plain,*/*","user-agent":"Mozilla/5.0 Knowgrph YouTube transcript importer"}}).catch(()=>null),D=O?await O.text().catch(()=>""):"",N=O?.ok?Zc(D,O.headers.get("content-type")):[],G=N.length>0?"available":O?.ok?"captions-empty":`captions-fetch-${O?.status||"failed"}`;return Yr({videoId:o,sourceUrl:a,title:p,authorName:m,thumbnailUrl:f,lang:t,languageCode:S.languageCode,segments:N,captionStatus:G})}n(ol,"buildYouTubeTranscriptPayload");async function Ta(e){let t=e.request,r=String(t.method||"GET").toUpperCase();if(r==="OPTIONS")return new Response(null,{status:204,headers:Ca});if(r!=="GET"&&r!=="HEAD"&&r!=="POST")return Qt({ok:!1,error:"unsupported_method"},405,r);let o=new URL(t.url),a=re(o.searchParams.get("url")),s=re(o.searchParams.get("lang"))||"en";if(!a)return Qt({ok:!1,error:"missing_url"},400,r);try{let i=await ol({sourceUrl:a,lang:s});return Qt(i,i.ok?200:502,r)}catch(i){let c=i&&typeof i=="object"&&"message"in i?re(i.message):"";return Qt({ok:!1,error:c||"youtube_conversion_failed"},502,r)}}n(Ta,"onRequest");async function va(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase();if(r==="OPTIONS")return new Response(null,{status:204,headers:{...ce(t),"access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(r!=="GET"&&r!=="HEAD")return new Response(JSON.stringify({ok:!1,error:"unsupported_method"}),{status:405,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store",...ce(t)}});let o={ok:!0,service:"singabldr-pages",ts:new Date().toISOString()},a={"content-type":"application/json; charset=utf-8","cache-control":"no-store",...ce(t)};return r==="HEAD"?new Response(null,{status:200,headers:a}):new Response(JSON.stringify(o),{status:200,headers:a})}n(va,"onRequest");var Le="https://airvio.co";var ae="/knowgrph",Ue=`${Le}${ae}/`,Oa=`${Le}/`;var Ma=`${ae}/health`,Ia=`${Le}${Ma}`,Na="/.well-known/agent-card.json",km=`${ae}/.well-known/agent-card.json`,nl=`${Le}${Na}`,Ua="/api/storage/source-files",xm=`${Ua}/`,al="/api/storage/doc-default/",sl="/api/storage/doc/",il=`${Le}${Ua}`,cl=`${Le}${al}{canonicalPath}`,ll=`${Le}${sl}{workspaceId}/{canonicalPath}`,pl=`${Le}/api/storage/blob/{workspaceId}/{canonicalPath}`;var Vr="root-agent-ready-pages";var La=['</.well-known/api-catalog>; rel="api-catalog"; type="application/linkset+json"',`<${ae}/.well-known/openapi.json>; rel="service-desc"; type="application/vnd.oai.openapi+json;version=3.1"`,`<${ae}/llms.txt>; rel="service-doc"; type="text/plain"`,'</auth.md>; rel="service-doc"; type="text/markdown"',`<${Ma}>; rel="status"; type="application/health+json"`,`<${ae}/.well-known/mcp/server-card.json>; rel="mcp-server-card"; type="application/json"`,`<${Na}>; rel="describedby"; type="application/json"`].join(", "),$a=`# Knowgrph
...
- Auth.md registration instructions: ${Oa}auth.md
- Health: ${Ia}
...
- Live Canvas Hero discovery markdown: ${Oa}knowgrph-live-canvas-hero.md

## APIs

- Agent-ready status: ${Ia}
- HTTP MCP: ${Ue}mcp
- Storage API: ${Le}/api/storage/
- Source Files index: ${il}
- Default Source File documents: ${cl}
- Workspace Source File documents: ${ll}
- Workspace binary artifacts: ${pl}
...
- The View initiates the MCP Apps \`ui/initialize\` handshake, sends \`ui/notifications/initialized\` and \`ui/notifications/size-changed\`, handles host context/tool input/result/cancel notifications, and calls the originating server through \`tools/call\`.
- \`inspect_agent_surface.structuredContent.mcpAppsServerReadiness\` exposes the native server-readiness model used by the View: app tool/resource binding, prompt discovery, resource-template discovery, output-schema and structured-content readiness, sandbox/security metadata, widget accessibility, Streamable HTTP JSON-RPC transport, local stdio transport, and read-only search/fetch retrieval.
`,Da=n(e=>new Response(e,{status:200,headers:{"content-type":"text/markdown; charset=utf-8","cache-control":"public, max-age=3600","access-control-allow-origin":"*",vary:"Accept","x-markdown-tokens":String(Math.ceil(String(e||"").length/4))}}),"markdownResponse"),Ha=n(e=>(e.headers.get("accept")||"").toLowerCase().split(",").some(r=>r.trim().startsWith("text/markdown")),"wantsMarkdown"),Jr=n((e,t)=>{let r=new Response(e.body,e),o=String(t?.owner||"").trim(),a=String(t?.tag||"").trim();return o&&r.headers.set("x-knowgrph-route-owner",o),a&&r.headers.set("x-knowgrph-route-tag",a),r},"withAgentReadyRouteHeaders");var Qr="Agent-actionable chat-to-canvas knowledge graph workspace",Ka='<main id="root"></main>',ul=/<(?:main|div)\s+id=["']root["']\s*><\/(?:main|div)>/i,dl=n(e=>{let t=/<script>([\s\S]*?)<\/script>/g;for(let r of String(e||"").matchAll(t)){let o=r[1]||"";if(o.includes("createWebMcpLifecycleController")&&o.includes("toolDefinitions"))return o}return""},"extractWebMcpScript"),ja=n(()=>({"content-type":"text/html; charset=utf-8","cache-control":"no-store, no-cache, no-transform, must-revalidate, max-age=0","access-control-allow-origin":"*",link:La}),"rootHtmlHeaders"),ml=n(()=>`<noscript><a href="${ae}/">Enter Knowgrph</a></noscript>`,"rootNoscriptFallbackMarkup"),Xr=n((e,t)=>String(e||"").includes("</head>")?String(e||"").replace("</head>",`${t}</head>`):`${String(e||"")}${t}`,"injectIntoHead"),hl=n(e=>{let t=String(e||"").replace(ul,Ka);return/<meta\s+name=["']description["'][^>]*>/i.test(t)?t=t.replace(/<meta\s+name=["']description["'][^>]*>/i,`<meta name="description" content="${Qr}" />`):t=Xr(t,`    <meta name="description" content="${Qr}" />
...
`)),t},"rewriteRootAppHtml"),gl=n(async e=>{let t=new URL(`${ae}/?agentReadyRootWebMcp=1`,e.url),r=await fetch(t,{headers:{accept:"text/html"}});return r.ok?dl(await r.text()):""},"loadWebMcpScript"),fl=n(async e=>{let t=new URL(`${ae}/?agentReadyRootAlias=1`,e.url),r=await fetch(t,{headers:{accept:"text/html"}});if(!r.ok)return null;let o=hl(await r.text());return!o.includes(Ka)||!o.includes(`${ae}/assets/`)?null:new Response(o,{status:200,headers:ja()})},"loadKnowgrphAppShell"),yl=n((e="")=>new Response(`<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
</html>`,{status:200,headers:ja()}),"rootHtmlResponse");async function Ba(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase();if(r!=="GET"&&r!=="HEAD")return e.next();if(Ha(t)){let s=Jr(Da($a),{owner:Vr,tag:"root-homepage-markdown"});return r==="HEAD"?new Response(null,s):s}let o=r==="HEAD"?null:await fl(t),a=Jr(o||yl(r==="HEAD"?"":await gl(t)),{owner:Vr,tag:"root-homepage-html"});return r==="HEAD"?new Response(null,a):a}n(Ba,"onRequest");var b=[{routePath:"/api/llm/chat/completions",mountPath:"/api/llm/chat",method:"",middlewares:[],modules:[ro]},{routePath:"/api/payments/commerce/x402",mountPath:"/api/payments/commerce",method:"",middlewares:[],modules:[xo]},{routePath:"/api/llm/models",mountPath:"/api/llm",method:"",middlewares:[],modules:[Ro]},{routePath:"/api/llm/responses",mountPath:"/api/llm",method:"",middlewares:[],modules:[Eo]},{routePath:"/knowgrph/doc-default/:path*",mountPath:"/knowgrph/doc-default",method:"",middlewares:[],modules:[ta]},{routePath:"/knowgrph/doc/:path*",mountPath:"/knowgrph/doc",method:"",middlewares:[],modules:[ra]},{routePath:"/knowgrph/share/:path*",mountPath:"/knowgrph/share",method:"",middlewares:[],modules:[oa]},{routePath:"/api/link-preview",mountPath:"/api",method:"GET",middlewares:[],modules:[aa]},{routePath:"/api/link-proxy",mountPath:"/api",method:"GET",middlewares:[],modules:[ia]},{routePath:"/api/graph",mountPath:"/api",method:"",middlewares:[],modules:[ya]},{routePath:"/api/oembed",mountPath:"/api",method:"",middlewares:[],modules:[ba]},{routePath:"/__chat_proxy/:path*",mountPath:"/__chat_proxy",method:"",middlewares:[],modules:[Aa]},{routePath:"/knowgrph/:path*",mountPath:"/knowgrph",method:"",middlewares:[],modules:[Ke]},{routePath:"/__video_frame",mountPath:"/",method:"",middlewares:[],modules:[Ea]},{routePath:"/__youtube_transcript",mountPath:"/",method:"",middlewares:[],modules:[Ta]},{routePath:"/health",mountPath:"/",method:"",middlewares:[],modules:[va]},{routePath:"/",mountPath:"/",method:"",middlewares:[],modules:[Ba]}];function wl(e){for(var t=[],r=0;r<e.length;){var o=e[r];if(o==="*"||o==="+"||o==="?"){t.push({type:"MODIFIER",index:r,value:e[r++]});continue}if(o==="\\"){t.push({type:"ESCAPED_CHAR",index:r++,value:e[r++]});continue}if(o==="{"){t.push({type:"OPEN",index:r,value:e[r++]});continue}if(o==="}"){t.push({type:"CLOSE",index:r,value:e[r++]});continue}if(o===":"){for(var a="",s=r+1;s<e.length;){var i=e.charCodeAt(s);if(i>=48&&i<=57||i>=65&&i<=90||i>=97&&i<=122||i===95){a+=e[s++];continue}break}if(!a)throw new TypeError("Missing parameter name at ".concat(r));t.push({type:"NAME",index:r,value:a}),r=s;continue}if(o==="("){var c=1,l="",s=r+1;if(e[s]==="?")throw new TypeError('Pattern cannot start with "?" at '.concat(s));for(;s<e.length;){if(e[s]==="\\"){l+=e[s++]+e[s++];continue}if(e[s]===")"){if(c--,c===0){s++;break}}else if(e[s]==="("&&(c++,e[s+1]!=="?"))throw new TypeError("Capturing groups are not allowed at ".concat(s));l+=e[s++]}if(c)throw new TypeError("Unbalanced pattern at ".concat(r));if(!l)throw new TypeError("Missing pattern at ".concat(r));t.push({type:"PATTERN",index:r,value:l}),r=s;continue}t.push({type:"CHAR",index:r,value:e[r++]})}return t.push({type:"END",index:r,value:""}),t}n(wl,"lexer");function Sl(e,t){t===void 0&&(t={});for(var r=wl(e),o=t.prefixes,a=o===void 0?"./":o,s=t.delimiter,i=s===void 0?"/#?":s,c=[],l=0,p=0,m="",f=n(function(U){if(p<r.length&&r[p].type===U)return r[p++].value},"tryConsume"),P=n(function(U){var L=f(U);if(L!==void 0)return L;var Y=r[p],se=Y.type,Se=Y.index;throw new TypeError("Unexpected ".concat(se," at ").concat(Se,", expected ").concat(U))},"mustConsume"),S=n(function(){for(var U="",L;L=f("CHAR")||f("ESCAPED_CHAR");)U+=L;return U},"consumeText"),O=n(function(U){for(var L=0,Y=i;L<Y.length;L++){var se=Y[L];if(U.indexOf(se)>-1)return!0}return!1},"isSafe"),D=n(function(U){var L=c[c.length-1],Y=U||(L&&typeof L=="string"?L:"");if(L&&!Y)throw new TypeError('Must have text between two parameters, missing text after "'.concat(L.name,'"'));return!Y||O(Y)?"[^".concat(je(i),"]+?"):"(?:(?!".concat(je(Y),")[^").concat(je(i),"])+?")},"safePattern");p<r.length;){var N=f("CHAR"),G=f("NAME"),Q=f("PATTERN");if(G||Q){var W=N||"";a.indexOf(W)===-1&&(m+=W,W=""),m&&(c.push(m),m=""),c.push({name:G||l++,prefix:W,suffix:"",pattern:Q||D(W),modifier:f("MODIFIER")||""});continue}var R=N||f("ESCAPED_CHAR");if(R){m+=R;continue}m&&(c.push(m),m="");var T=f("OPEN");if(T){var W=S(),te=f("NAME")||"",F=f("PATTERN")||"",le=S();P("CLOSE"),c.push({name:te||(F?l++:""),pattern:te&&!F?D(W):F,prefix:W,suffix:le,modifier:f("MODIFIER")||""});continue}P("END")}return c}n(Sl,"parse");function bt(e,t){var r=[],o=za(e,r,t);return bl(o,r,t)}n(bt,"match");function bl(e,t,r){r===void 0&&(r={});var o=r.decode,a=o===void 0?function(s){return s}:o;return function(s){var i=e.exec(s);if(!i)return!1;for(var c=i[0],l=i.index,p=Object.create(null),m=n(function(P){if(i[P]===void 0)return"continue";var S=t[P-1];S.modifier==="*"||S.modifier==="+"?p[S.name]=i[P].split(S.prefix+S.suffix).map(function(O){return a(O,S)}):p[S.name]=a(i[P],S)},"_loop_1"),f=1;f<i.length;f++)m(f);return{path:c,index:l,params:p}}}n(bl,"regexpToFunction");function je(e){return e.replace(/([.+*?=^!:${}()[\]|/\\])/g,"\\$1")}n(je,"escapeString");function Fa(e){return e&&e.sensitive?"":"i"}n(Fa,"flags");function _l(e,t){if(!t)return e;for(var r=/\((?:\?<(.*?)>)?(?!\?)/g,o=0,a=r.exec(e.source);a;)t.push({name:a[1]||o++,prefix:"",suffix:"",modifier:"",pattern:""}),a=r.exec(e.source);return e}n(_l,"regexpToRegexp");function Al(e,t,r){var o=e.map(function(a){return za(a,t,r).source});return new RegExp("(?:".concat(o.join("|"),")"),Fa(r))}n(Al,"arrayToRegexp");function Pl(e,t,r){return kl(Sl(e,r),t,r)}n(Pl,"stringToRegexp");function kl(e,t,r){r===void 0&&(r={});for(var o=r.strict,a=o===void 0?!1:o,s=r.start,i=s===void 0?!0:s,c=r.end,l=c===void 0?!0:c,p=r.encode,m=p===void 0?function(L){return L}:p,f=r.delimiter,P=f===void 0?"/#?":f,S=r.endsWith,O=S===void 0?"":S,D="[".concat(je(O),"]|$"),N="[".concat(je(P),"]"),G=i?"^":"",Q=0,W=e;Q<W.length;Q++){var R=W[Q];if(typeof R=="string")G+=je(m(R));else{var T=je(m(R.prefix)),te=je(m(R.suffix));if(R.pattern)if(t&&t.push(R),T||te)if(R.modifier==="+"||R.modifier==="*"){var F=R.modifier==="*"?"?":"";G+="(?:".concat(T,"((?:").concat(R.pattern,")(?:").concat(te).concat(T,"(?:").concat(R.pattern,"))*)").concat(te,")").concat(F)}else G+="(?:".concat(T,"(").concat(R.pattern,")").concat(te,")").concat(R.modifier);else{if(R.modifier==="+"||R.modifier==="*")throw new TypeError('Can not repeat "'.concat(R.name,'" without a prefix and suffix'));G+="(".concat(R.pattern,")").concat(R.modifier)}else G+="(?:".concat(T).concat(te,")").concat(R.modifier)}}if(l)a||(G+="".concat(N,"?")),G+=r.endsWith?"(?=".concat(D,")"):"$";else{var le=e[e.length-1],U=typeof le=="string"?N.indexOf(le[le.length-1])>-1:le===void 0;a||(G+="(?:".concat(N,"(?=").concat(D,"))?")),U||(G+="(?=".concat(N,"|").concat(D,")"))}return new RegExp(G,Fa(r))}n(kl,"tokensToRegexp");function za(e,t,r){return e instanceof RegExp?_l(e,t):Array.isArray(e)?Al(e,t,r):Pl(e,t,r)}n(za,"pathToRegexp");var Zt=/[.+?^${}()|[\]\\]/g;function*xl(e){let t=new URL(e.url).pathname;for(let r of[...b].reverse()){if(r.method&&r.method!==e.method)continue;let o=bt(r.routePath.replace(Zt,"\\$&"),{end:!1}),a=bt(r.mountPath.replace(Zt,"\\$&"),{end:!1}),s=o(t),i=a(t);if(s&&i)for(let c of r.middlewares.flat())yield{handler:c,params:s.params,path:i.path}}for(let r of b){if(r.method&&r.method!==e.method)continue;let o=bt(r.routePath.replace(Zt,"\\$&"),{end:!0}),a=bt(r.mountPath.replace(Zt,"\\$&"),{end:!1}),s=o(t),i=a(t);if(s&&i&&r.modules.length){for(let c of r.modules.flat())yield{handler:c,params:s.params,path:s.path};break}}}n(xl,"executeRequest");var th={async fetch(e,t,r){let o=e,a=xl(o),s={},i=!1,c=n(async(l,p)=>{if(l!==void 0){let f=l;typeof l=="string"&&(f=new URL(l,o.url).toString()),o=new Request(f,p)}let m=a.next();if(m.done===!1){let{handler:f,params:P,path:S}=m.value,O={request:new Request(o.clone()),functionPath:S,next:c,params:P,get data(){return s},set data(N){if(typeof N!="object"||N===null)throw new Error("context.data must be an object");s=N},env:t,waitUntil:r.waitUntil.bind(r),passThroughOnException:n(()=>{i=!0},"passThroughOnException")},D=await f(O);if(!(D instanceof Response))throw new Error("Your Pages function should return a Response");return Zr(D)}else{let f=await t.ASSETS.fetch(o);return Zr(f)}},"next");try{return await c()}catch(l){if(i){let p=await t.ASSETS.fetch(o);return Zr(p)}throw l}}},Zr=n(e=>new Response([101,204,205,304].includes(e.status)?null:e.body,e),"cloneResponse");export{th as default};
