...
}`;var xt={push:"/api/storage/push",pull:"/api/storage/pull",collabSave:"/api/storage/collab/save",chatSession:"/api/storage/chat/session",chatRelay:"/api/storage/chat/relay",chatPoliciesPrefix:"/api/storage/chat/policies/",chatAuditPrefix:"/api/storage/chat/audit/",exportPrefix:"/api/storage/export/",docPrefix:"/api/storage/doc/",defaultDocPrefix:"/api/storage/doc-default/",blobPrefix:"/api/storage/blob/",mediaAssetPersist:"/api/storage/media/assets",mediaAssetPrefix:"/api/storage/media/assets/",mediaPrefix:"/api/storage/media/",sourceFilesIndex:"/api/storage/source-files",sourceFilesIndexPrefix:"/api/storage/source-files/",sourceFilesLlms:"/api/storage/llms.txt"};var Xo=n((e,t)=>`${xt.docPrefix}${encodeURIComponent(String(e||"").trim())}/${encodeURIComponent(String(t||"").trim())}`,"buildKnowgrphStorageDocPath"),Qo=n(e=>`${xt.defaultDocPrefix}${encodeURIComponent(String(e||"").trim())}`,"buildKnowgrphStorageDefaultDocPath");var Zo=n(e=>{let t=String(e||"").trim();return t?`${xt.sourceFilesIndexPrefix}${encodeURIComponent(t)}`:xt.sourceFilesIndex},"buildKnowgrphStorageSourceFilesIndexPath");var G="https://airvio.co",Et="https://knowgrph-storage.huijoohwee.workers.dev",T="/knowgrph",E=`${G}${T}/`,en=`${G}/`,rt="kgws:canonical-docs",ke="2026-06-05",Ve=`${T}/health`,qe=`${G}${Ve}`,tn="/.well-known/agent-card.json",hr=`${T}/.well-known/agent-card.json`,Ct=`${G}${tn}`,gr=`${G}/api/storage/source-files`,hs=`${G}/api/storage/doc-default/{canonicalPath}`,gs=`${G}/api/storage/doc/{workspaceId}/{canonicalPath}`,fs=`${G}/api/storage/blob/{workspaceId}/{canonicalPath}`,rn="knowgrph-agent-ready-pages";var on=['</.well-known/api-catalog>; rel="api-catalog"; type="application/linkset+json"',`<${T}/.well-known/openapi.json>; rel="service-desc"; type="application/vnd.oai.openapi+json;version=3.1"`,`<${T}/llms.txt>; rel="service-doc"; type="text/plain"`,'</auth.md>; rel="service-doc"; type="text/markdown"',`<${Ve}>; rel="status"; type="application/health+json"`,`<${T}/.well-known/mcp/server-card.json>; rel="mcp-server-card"; type="application/json"`,`<${tn}>; rel="describedby"; type="application/json"`].join(", "),nn=`# Knowgrph

Knowgrph is an Agent-actionable chat-to-canvas knowledge graph workspace served at ${E}.

## Discovery

- Crawl policy: ${E}robots.txt
- Sitemap: ${E}sitemap.xml
- API catalog: ${E}.well-known/api-catalog
- Auth.md registration instructions: ${en}auth.md
- Health: ${qe}
- MCP server card: ${E}.well-known/mcp/server-card.json
- A2A Agent Card: ${Ct}
- Agent skills: ${E}.well-known/agent-skills/index.json
- LLM reference: ${E}llms.txt
...
- Deployed HTML fallback injects the shared seven-tool WebMCP surface on \`${E}\` HTML routes.
...
`,"buildSitemapXml"),vs=Cs(`${E}sitemap.xml`),Os=Ts(E),bn={linkset:[{anchor:E,"service-desc":[{href:`${E}.well-known/openapi.json`,type:"application/vnd.oai.openapi+json;version=3.1"}],"service-doc":[{href:`${E}llms.txt`,type:"text/plain"}],status:[{href:qe,type:"application/health+json"}],"service-meta":[{href:`${E}.well-known/mcp/server-card.json`,type:"application/json"},{href:Ct,type:"application/json"}]}]},Rn={openapi:"3.1.0",info:{title:"Knowgrph API",version:"0.1.0",description:"Agent discovery surface for the Knowgrph Cloudflare deployment."},servers:[{url:G,description:"Knowgrph Cloudflare deployment"}],paths:Bo({appBasePath:T,appA2aAgentCardPath:hr,healthPath:Ve})},at={resource:E,resource_name:"Knowgrph",authorization_servers:[G],scopes_supported:["knowgrph:read","knowgrph:source-files:read"],bearer_methods_supported:["header"],resource_documentation:`${E}llms.txt`},yr=`${G}/cdn-cgi/access`,Ee={skill:`${G}/auth.md`,register_uri:`${E}agent/auth`,claim_uri:`${E}agent/auth/claim`,revocation_uri:`${E}agent/auth/revoke`,identity_types_supported:["anonymous","identity_assertion"],anonymous:{credential_types_supported:["api_key"]},identity_assertion:{assertion_types_supported:["urn:ietf:params:oauth:token-type:id-jag","verified_email"],credential_types_supported:["access_token","api_key"]},events_supported:["https://schemas.workos.com/events/agent/auth/identity/assertion/revoked"],registration_status:"metadata_published_runtime_user_mediated"},un={issuer:G,resource:at.resource,resource_name:at.resource_name,authorization_servers:at.authorization_servers,cloudflare_access_issuer:yr,authorization_endpoint:`${yr}/login`,token_endpoint:`${yr}/token`,jwks_uri:`${E}.well-known/http-message-signatures-directory`,response_types_supported:["code"],grant_types_supported:["authorization_code","client_credentials"],token_endpoint_auth_methods_supported:["client_secret_basic","private_key_jwt"],scopes_supported:at.scopes_supported,agent_auth:Ee},Is=`# Knowgrph auth.md

Knowgrph publishes agent registration metadata for the read-only agent surface at ${E}. Agents should first fetch ${G}/.well-known/oauth-protected-resource, follow its authorization_servers entry to ${G}/.well-known/oauth-authorization-server, and read the agent_auth block.
...
- Pipeline rule: agents must not bypass MainPanel -> FloatingPanel Chat -> KGC -> Canvas for user-mediated graph work; published HTTP MCP tools remain read-only until mutation auth and conflict semantics are implemented.`,Pn={name:"Knowgrph Agent",description:"Agent-readable discovery, published-document retrieval, and WebMCP-ready metadata surface for Knowgrph.",version:"0.1.0",provider:{organization:"airvio / joohwee",url:E},url:`${E}mcp`,preferredTransport:"JSONRPC",supportedInterfaces:[{url:`${E}mcp`,protocolBinding:"JSONRPC",transportProtocol:"JSONRPC",description:"Primary machine interface for read-only discovery and source-file document access."},{url:gr,protocolBinding:"HTTP+JSON/REST",transportProtocol:"HTTP+JSON/REST",description:"Published source-files index and storage-backed document read surface."}],capabilities:{streaming:!1,pushNotifications:!1,stateTransitionHistory:!1,extendedAgentCard:!1},defaultInputModes:["text/plain","text/markdown","application/json"],defaultOutputModes:["text/plain","text/markdown","application/json"],skills:Ho(vt)},Me={serverInfo:{name:"knowgrph",version:"0.1.0"},transport:{type:Ae,url:`${E}mcp`,stateless:!0},capabilities:{tools:vt.map(e=>({name:e.name,title:e.title,description:e.description,inputSchema:e.inputSchema,outputSchema:e.outputSchema,securitySchemes:e.securitySchemes,annotations:e.annotations,_meta:e._meta})),resources:{listChanged:!1},prompts:{listChanged:!1},...rr()},prompts:hn,resourceTemplates:gn,clientSetups:nr({baseUrl:E,mcpUrl:`${E}mcp`,serverName:"knowgrph"}),links:{apiCatalog:`${E}.well-known/api-catalog`,skills:`${E}.well-known/agent-skills/index.json`,status:qe,agentCard:Ct}},Ms=At({appUrl:E,updatedAt:ke}),Sr=vt.map(e=>({name:e.webName,title:e.title,description:e.description,inputSchema:e.inputSchema,outputSchema:e.outputSchema,securitySchemes:e.securitySchemes,annotations:e.annotations,_meta:e._meta})),$e=n(e=>st(vt.find(t=>t.name===e)?.webName),"findWebMcpToolName"),Ns=$e(u.search),Us=$e(u.fetch),Ls=$e(u.listSourceFiles),Ks=$e(u.readSourceFile),$s=$e(u.readSharedDocument),Ds=$e(u.inspectSharedDocumentStructure),Hs=$e(u.inspectAgentSurface),js=`(() => {
  ...
  const toolExecutors = createPublishedAgentReadyToolExecutors({
    toolNames: {
      ...
      readSourceFile: ${JSON.stringify(Ks)},
      readSharedDocument: ${JSON.stringify($s)},
...
})();`,Bs=n(async e=>{if(!(e.headers.get("content-type")||"").toLowerCase().includes("text/html"))return e;let r=await e.text();if(Sr.every(i=>r.includes(i.name)))return new Response(r,e);let o=`<script>${js}<\/script>`,a=r.includes("</head>")?r.replace("</head>",`${o}</head>`):`${r}${o}`,s=new Response(a,e);return s.headers.delete("content-length"),s},"injectWebMcpScript"),Gs={search:u.search,fetch:u.fetch,listSourceFiles:u.listSourceFiles,readSourceFile:u.readSourceFile,readSharedDocument:u.readSharedDocument,inspectSharedDocumentStructure:u.inspectSharedDocumentStructure,inspectAgentSurface:u.inspectAgentSurface},Ws=n(async e=>{let t=new TextEncoder().encode(e),r=await crypto.subtle.digest("SHA-256",t);return[...new Uint8Array(r)].map(o=>o.toString(16).padStart(2,"0")).join("")},"sha256Hex"),An=Do(),Fs=Object.fromEntries(tt.map(e=>[e.name,Ws(An[e.name]||"")])),dn=new Map(tt.map(e=>[`${T}${e.path}`.replace(/\/+$/,""),An[e.name]||""]));var kn=n(async()=>jo({appUrl:E,updatedAt:ke,sha256ByName:Fs}),"agentSkillsIndex"),zs={keys:[{kty:"OKP",crv:"Ed25519",kid:"knowgrph-agent-ready-2026-05-21",use:"sig",alg:"EdDSA",x:"11qYAYdkVKxA4G0wV47IxPtYfFVH_H7zmC2Di2PcvLU"}]},qs={protocolVersion:"2025-06-18",capabilities:{tools:{},resources:{},prompts:{listChanged:!1},...rr()},serverInfo:Me.serverInfo},_r=Me.capabilities.tools,Vs=[Ms],Ys=hn,Js=gn,xn=n(()=>({status:"pass",service:"knowgrph-agent-ready-pages",homepage:E,health:qe,updatedAt:ke,checks:{linkHeaders:!0,markdownNegotiation:!0,httpMcp:!0,webMcp:!0,mcpApps:!0,commerce:{acp:!0,ucp:!0,mpp:!0,x402:!0},defaultWorkspaceId:rt}}),"buildHealthStatusBody"),Xs=n(async()=>Io({baseUrl:E,health:xn(),apiCatalog:bn,openApi:Rn,mcpServerCard:Me,agentCard:Pn,agentSkills:await kn(),commerce:Xt({origin:G})}),"buildAgentSurfaceInspection"),Qs=ir({toolNames:Gs,defaultWorkspaceId:rt,publicBaseUrl:G,buildStorageDocPath:fn,fetchSourceFilesIndexResponse:n(()=>fetch(`${Et}${Zo()}`,{headers:{accept:"text/markdown"}}),"fetchSourceFilesIndexResponse"),fetchStorageMarkdownResponse:n(e=>fetch(`${Et}${e}`,{headers:{accept:"text/markdown"}}),"fetchStorageMarkdownResponse"),resolveSharedDocumentInput:n((e={})=>kt({shareToken:e?.shareToken,shareUrl:e?.shareUrl,appBasePath:T,baseUrl:G}),"resolveSharedDocumentInput"),inspectSharedDocumentStructure:No,buildAgentSurfaceInspection:Xs}),br=n(e=>{try{let t=new URL(e,G);return kt({shareUrl:`${t.pathname}${t.search}`,baseUrl:G,appBasePath:T})}catch{return null}},"resolvePublishedDocRequestIdentity"),Zs=n(e=>kt({shareUrl:String(e||""),baseUrl:G,appBasePath:T}),"resolvePublishedDocPathIdentity"),ei=n(async(e,t)=>{let r=new URL(fn(t.canonicalPath,t.workspaceId),Et),o=await fetch(r,{method:"GET",headers:{accept:"text/markdown, text/plain;q=0.9, */*;q=0.1"}}),a=new Headers(o.headers),s=String(a.get("vary")||"");return a.set("vary",s?`${s}, Accept`:"Accept"),new Response(String(e.method||"").toUpperCase()==="HEAD"?null:o.body,{status:o.status,statusText:o.statusText,headers:a})},"proxyPublishedDocMarkdownResponse"),ti=n(async e=>{try{let t=await e.json();return t&&typeof t=="object"?t:null}catch{return null}},"readJsonRpcRequest"),xe=n((e,t)=>ce(200,{jsonrpc:"2.0",id:e??null,result:t}),"jsonRpcResult"),Ie=n((e,t,r)=>ce(200,{jsonrpc:"2.0",id:e??null,error:{code:t,message:r}}),"jsonRpcError"),ri=n(e=>String(e.headers.get("accept")||"").toLowerCase().split(",").some(t=>t.trim().startsWith("text/event-stream")),"requestAcceptsEventStream"),wr=n((e,t)=>Object.prototype.hasOwnProperty.call(e,t),"hasOwnProperty"),En=n(e=>{if(Array.isArray(e))return e.length>0&&e.every(En);if(!e||typeof e!="object"||String(e.jsonrpc||"")!=="2.0")return!1;let t=typeof e.method=="string"&&e.method.length>0,r=wr(e,"id"),o=wr(e,"result")||wr(e,"error");return t&&!r||!t&&o},"isJsonRpcNotificationOrResponse"),Cn=n(async(e,t)=>{let r=Qs[e];if(typeof r!="function")throw new Error(`unknown tool: ${e}`);return r(t)},"executeMcpTool"),oi=n(async e=>{let t=st(e);if(t===ie)return ar({appUrl:E,updatedAt:ke,toolNames:_r.map(o=>o.name)});let r=_o(t);if(r){let o=await Cn(u.fetch,{id:r});return bo({uri:t,sourceFile:o})}throw new Error(`unknown resource: ${e}`)},"readMcpResource"),ni=n(async e=>{let t=String(e.method||"GET").toUpperCase();if(t==="GET"||t==="HEAD")return ri(e)?cn(405,{allow:"POST"}):ye({ok:!0,transport:Me.transport,serverInfo:Me.serverInfo,capabilities:Me.capabilities});if(t!=="POST")return ce(405,{ok:!1,error:"unsupported_method"});let r=await ti(e);if(!r)return Ie(null,-32700,"Parse error");if(En(r))return cn(202);if(Array.isArray(r))return Ie(null,-32600,"Batch JSON-RPC requests are not supported");switch(r.method){case"initialize":return xe(r.id,qs);case"tools/list":return xe(r.id,{tools:_r});case"prompts/list":return xe(r.id,{prompts:Ys});case"resources/templates/list":return xe(r.id,{resourceTemplates:Js});case"prompts/get":{let o=st(r.params?.name),a=r.params?.arguments&&typeof r.params.arguments=="object"?r.params.arguments:{};if(!o)return Ie(r.id,-32602,"Prompt name is required");try{return xe(r.id,fo(o,a))}catch(s){return Ie(r.id,-32602,s instanceof Error?s.message:String(s))}}case"resources/list":return xe(r.id,{resources:Vs});case"resources/read":{let o=st(r.params?.uri);if(!o)return Ie(r.id,-32602,"Resource URI is required");try{return xe(r.id,await oi(o))}catch(a){return Ie(r.id,-32602,a instanceof Error?a.message:String(a))}}case"tools/call":{let o=st(r.params?.name),a=r.params?.arguments&&typeof r.params.arguments=="object"?r.params.arguments:{};if(!o)return Ie(r.id,-32602,"Tool name is required");try{let s=await Cn(o,a);return xe(r.id,{content:[{type:"text",text:typeof s?.markdown=="string"?s.markdown:JSON.stringify(s,null,2)}],structuredContent:s,isError:!1})}catch(s){return xe(r.id,{content:[{type:"text",text:s instanceof Error?s.message:String(s)}],isError:!0})}}default:return Ie(r.id,-32601,"Method not found")}},"handleMcpTransport"),ai=n(()=>ar({appUrl:E,updatedAt:ke,toolNames:_r.map(e=>e.name)}).contents[0].text,"buildKnowgrphMcpAppHtmlBody");var Rr=n(e=>e===T||e===`${T}/`,"handlesKnowgrphRoot"),si=n(e=>Rr(e)||!!Zs(e),"handlesKnowgrphHtmlSurface"),ii=n(e=>{let t=new URL(e.url),r=t.pathname.replace(/\/+$/,"")||"/",o=br(e.url);return r===Ve?"health":r===`${T}/mcp`?"mcp":_n(r)?"github-workspace-write":r===`${T}/robots.txt`?"robots":r===`${T}/sitemap.xml`?"sitemap":r===`${T}/auth.md`||r==="/auth.md"?"auth-md":r.startsWith(`${T}/.well-known/`)?"well-known":o?ot(e)?"shared-doc-markdown":"shared-doc-html":Rr(t.pathname)?ot(e)?"homepage-markdown":"homepage-html":"app-surface"},"resolveAgentReadyRouteTag"),Tt=n((e,t)=>sn(t,{owner:rn,tag:ii(e)}),"withKnowgrphRouteHeaders"),mn=n(async e=>{let t=new URL(e.url),r=t.pathname.replace(/\/+$/,"")||"/",o=br(e.url);if(o&&ot(e))return ei(e,o);if(Rr(t.pathname)&&ot(e))return an(nn);switch(r){case Ve:return ws(xn());case`${T}/mcp`:return ni(e);case`${T}/robots.txt`:return nt(vs,"text/plain; charset=utf-8");case`${T}/sitemap.xml`:return nt(Os,"application/xml; charset=utf-8");case`${T}/auth.md`:case"/auth.md":return nt(Is,"text/markdown; charset=utf-8");case`${T}/.well-known/api-catalog`:return ye(bn,"application/linkset+json; charset=utf-8");case`${T}/.well-known/openapi.json`:return ye(Rn,"application/vnd.oai.openapi+json; charset=utf-8");case hr:return ye(Pn);case`${T}/.well-known/oauth-protected-resource`:return ye(at);case`${T}/.well-known/oauth-authorization-server`:return ye(un);case`${T}/.well-known/openid-configuration`:return ye(un);case`${T}/.well-known/mcp/server-card.json`:return ye(Me);case`${T}/.well-known/mcp/apps/knowgrph-agent-ready.html`:return ys(ai());case`${T}/.well-known/mcp.json`:return ye(Me);case`${T}/.well-known/agent-skills/index.json`:return ye(await kn());case`${T}/.well-known/http-message-signatures-directory`:return ye(zs);default:return dn.has(r)?nt(dn.get(r),"text/markdown; charset=utf-8"):null}},"routeResponse");async function Ne(e){let{env:t,request:r}=e,o=String(r.method||"GET").toUpperCase(),a=new URL(r.url);if(o==="OPTIONS")return new Response(null,{status:204,headers:{"access-control-allow-origin":"*","access-control-allow-methods":"GET, HEAD, POST, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(o==="POST"&&a.pathname.replace(/\/+$/,"")===`${T}/mcp`)return Tt(r,await mn(r));if(o==="POST"&&_n(a.pathname))return Tt(r,await Es(r,t));if(o!=="GET"&&o!=="HEAD")return ce(405,{ok:!1,error:"unsupported_method"});let s=await mn(r);if(s){let p=Tt(r,s);return o==="HEAD"?new Response(null,p):p}let i=br(r.url)?await Go(e,T):await e.next();if(!si(a.pathname))return i;let c=o==="HEAD"?i:await Bs(i),l=new Response(o==="HEAD"?null:c.body,c);return l.headers.set("link",on),Tt(r,l)}n(Ne,"onRequest");async function Tn(e){return Ne(e)}n(Tn,"onRequest");async function vn(e){return Ne(e)}n(vn,"onRequest");async function On(e){return Ne(e)}n(On,"onRequest");var ci=Object.freeze(new Set(["","80","443"])),li=Object.freeze([".local",".localhost",".internal"]),pi=Object.freeze(new Set(["localhost"]));function De(e){return String(e||"").trim().toLowerCase()}n(De,"normalizeHostname");function ui(e){let t=De(e);if(!/^\d{1,3}(\.\d{1,3}){3}$/.test(t))return!1;let r=t.split(".").map(o=>Number(o));return r.length!==4?!1:r.every(o=>Number.isInteger(o)&&o>=0&&o<=255)}n(ui,"isIpv4Literal");function In(e){let[t,r,o,a]=e.split(".").map(s=>Number(s));return(t<<24|r<<16|o<<8|a)>>>0}n(In,"ipv4ToInt");function di(e,t,r){if(!Number.isInteger(r)||r<0||r>32)return!1;if(r===0)return!0;let o=4294967295<<32-r>>>0;return(e&o)===(t&o)}n(di,"inIpv4Cidr");function mi(e){let t=De(e);return!t||!t.includes(":")?!1:/^[0-9a-f:]+$/i.test(t)}n(mi,"isIpv6Literal");function hi(e){let t=De(e);return!!(!t||t==="::1"||t==="0:0:0:0:0:0:0:1"||t.startsWith("fc")||t.startsWith("fd")||/^fe[89ab]/i.test(t))}n(hi,"isBlockedIpv6");function gi(e,{blockedExact:t,blockedSuffixes:r}={}){let o=De(e);if(!o)return!0;let a=t||pi;if(a instanceof Set&&a.has(o))return!0;let s=r||li;if(Array.isArray(s))for(let i of s){let c=De(i);if(c&&(o===c||o.endsWith(c)))return!0}return!1}n(gi,"isBlockedHostname");function fi(e){let t=De(e);if(!t)return!0;if(ui(t)){let r=In(t),o=[{base:"0.0.0.0",bits:8},{base:"10.0.0.0",bits:8},{base:"127.0.0.0",bits:8},{base:"169.254.0.0",bits:16},{base:"172.16.0.0",bits:12},{base:"192.168.0.0",bits:16},{base:"100.64.0.0",bits:10}];for(let a of o){let s=In(a.base);if(di(r,s,a.bits))return!0}return!1}return mi(t)?hi(t):!1}n(fi,"isBlockedIpLiteral");function Ot(e,{allowedPorts:t}={}){let r=String(e||"").trim();if(!r)throw new Error("invalid_url");let o;try{o=new URL(r)}catch{throw new Error("invalid_url")}if(o.protocol!=="http:"&&o.protocol!=="https:")throw new Error("invalid_url");if(o.username||o.password)throw new Error("invalid_url");let a=t||ci,s=String(o.port||"");if(a instanceof Set&&!a.has(s))throw new Error("port_not_allowed");let i=De(o.hostname);if(!i)throw new Error("invalid_url");if(gi(i))throw new Error("blocked_host");if(fi(i))throw new Error("blocked_host");return o}n(Ot,"parseAndValidateExternalUrl");function It(e){return String(e.headers.get("sec-fetch-site")||"").trim().toLowerCase()==="cross-site"}n(It,"shouldRejectCrossSiteFetch");var yi={"content-type":"application/json; charset=utf-8","cache-control":"public, max-age=600"};function He(e,t={}){return new Response(JSON.stringify(e),{...t,headers:{...yi,...t.headers||{}}})}n(He,"json");function Mt(...e){for(let t of e){if(!t)continue;let r=String(t).trim();if(r)return r}return null}n(Mt,"pickFirst");function wi(e){let t=e.slice(0,8e4),r=t.match(/<title[^>]*>([^<]*)<\/title>/i),o=t.match(/<meta[^>]+property=["']og:title["'][^>]*content=["']([^"']+)["'][^>]*>/i),a=t.match(/<meta[^>]+property=["']og:description["'][^>]*content=["']([^"']+)["'][^>]*>/i),s=t.match(/<meta[^>]+name=["']description["'][^>]*content=["']([^"']+)["'][^>]*>/i),i=t.match(/<meta[^>]+property=["']og:image["'][^>]*content=["']([^"']+)["'][^>]*>/i),c=t.match(/<meta[^>]+property=["']og:site_name["'][^>]*content=["']([^"']+)["'][^>]*>/i);return{title:Mt(o?.[1],r?.[1]),description:Mt(a?.[1],s?.[1]),image:Mt(i?.[1]),siteName:Mt(c?.[1])}}n(wi,"extractMeta");async function Mn(e){let t=e.request.url,r=new URL(t);if(r.searchParams.get("ping")==="1")return He({ok:!0,ping:!0});let o=r.searchParams.get("url")||"";if(It(e.request))return He({ok:!1,error:"forbidden"},{status:403,headers:{"cache-control":"no-store"}});let a;try{a=Ot(o)}catch{return He({ok:!1,error:"invalid_url"},{status:400,headers:{"cache-control":"no-store"}})}try{let s=await fetch(a.toString(),{headers:{"user-agent":"Mozilla/5.0 (compatible; HackaMapLinkPreview/1.0)",accept:"text/html,application/xhtml+xml"},redirect:"follow",cf:{cacheTtl:600,cacheEverything:!0}}),i=s.headers.get("content-type")||"";if(!s.ok)return He({ok:!1,error:"fetch_failed",status:s.status,url:a.toString()},{status:200,headers:{"cache-control":"no-store"}});if(!i.includes("text/html"))return He({ok:!0,url:a.toString(),domain:a.host,contentType:i,title:null,description:null,image:null,siteName:null});let c=await s.text(),l=wi(c);return He({ok:!0,url:a.toString(),domain:a.host,contentType:i,...l})}catch(s){return He({ok:!1,error:"exception",message:s?.message||String(s),url:a.toString()},{status:200,headers:{"cache-control":"no-store"}})}}n(Mn,"onRequestGet");var Nn=35e4;function Si(e){let t=e;return t=t.replace(/<script\b[\s\S]*?<\/script>/gi,""),t=t.replace(/<iframe\b[\s\S]*?<\/iframe>/gi,""),t=t.replace(/<object\b[\s\S]*?<\/object>/gi,""),t=t.replace(/<embed\b[\s\S]*?>/gi,""),t=t.replace(/<noscript\b[\s\S]*?<\/noscript>/gi,""),t=t.replace(/\son[a-z]+\s*=\s*"[^"]*"/gi,""),t=t.replace(/\son[a-z]+\s*=\s*'[^']*'/gi,""),t}n(Si,"stripActiveContent");function _i({url:e,title:t,innerHtml:r}){let o=t?String(t).slice(0,140):"Preview",a=String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;");return`<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
...
</html>`}n(_i,"buildWrapper");async function Un(e){let r=new URL(e.request.url).searchParams.get("url")||"";if(It(e.request))return new Response("Forbidden",{status:403,headers:{"cache-control":"no-store"}});let o;try{o=Ot(r)}catch(a){let s=a instanceof Error?a.message:"invalid_url";return new Response(s,{status:400,headers:{"cache-control":"no-store"}})}try{let a=await fetch(o.toString(),{headers:{"user-agent":"Mozilla/5.0 (compatible; HackaMapLinkProxy/1.0)",accept:"text/html,application/xhtml+xml"},redirect:"follow",cf:{cacheTtl:600,cacheEverything:!0}}),s=a.headers.get("content-type")||"";if(!a.ok)return new Response(`Fetch failed (${a.status})`,{status:200,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}});if(!s.includes("text/html"))return new Response(`Unsupported content-type: ${s}`,{status:200,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"public, max-age=600"}});let i=await a.text();i.length>Nn&&(i=i.slice(0,Nn));let l=i.match(/<title[^>]*>([^<]*)<\/title>/i)?.[1]?.trim()||o.host;i=Si(i),/<base\s/i.test(i)||(i=i.replace(/<head([^>]*)>/i,`<head$1><base href="${o.origin}/">`));let m=_i({url:o.toString(),title:l,innerHtml:i});return new Response(m,{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"public, max-age=600","content-security-policy":"default-src 'none'; img-src https: data:; style-src 'unsafe-inline' https:; font-src https: data:; frame-ancestors 'self';"}})}catch(a){return new Response(`Exception: ${a?.message||String(a)}`,{status:200,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}})}}n(Un,"onRequestGet");var Pr="api.openai.com",Nt="api.miromind.ai",Ut="apihub.agnes-ai.com",Lt="ark.ap-southeast.bytepluses.com",Ar="ark.eu-west.bytepluses.com",Kn=new Set(["localhost","127.0.0.1","0.0.0.0"]),Re=n(e=>String(e||"").trim().toLowerCase(),"normalizeHost"),we=n((e,t)=>String(e.get(t)||"").trim(),"readHeader"),$n=n(e=>Kn.has(Re(e)),"isLocalHost"),Ln=n(e=>{let t=String(e||"").trim();if(!t)return new Set;let r=new Set;return t.split(",").map(o=>Re(o)).filter(Boolean).forEach(o=>r.add(o)),r},"parseCsvSet"),Dn=n((e,{includeOpenAi:t=!1,includeMiroMind:r=!1,includeAgnes:o=!1,includeBytePlus:a=!1}={})=>{let s=Ln(e.KNOWGRPH_INTEGRATION_ALLOWED_HOSTS),i=Ln(e.KNOWGRPH_CHAT_PROXY_ALLOWED_HOSTS),c=s.size?s:i,l=c.size?c:new Set([...Kn]);return t&&l.add(Pr),r&&l.add(Nt),o&&l.add(Ut),a&&(l.add(Lt),l.add(Ar)),l},"parseAllowedHosts"),ae=n(e=>{let t=we(e.headers,"origin");if(!t)return{};let r="";try{r=Re(new URL(t).host)}catch{return{}}let o=Re(new URL(e.url).host);return r===o||r.startsWith("localhost:")||r.startsWith("127.0.0.1:")?{"access-control-allow-origin":t,vary:"Origin"}:{}},"corsHeaders"),he=n((e,t,r)=>new Response(JSON.stringify(t),{status:r,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store",...ae(e)}}),"jsonResponse");var Kt={"content-type":"application/json; charset=utf-8","cache-control":"no-store"};function it(e,t,r=200){return new Response(JSON.stringify(t),{status:r,headers:{...Kt,...ae(e)}})}n(it,"jsonResponse");async function bi(e){let t=new URL("/knowgrph/imports/hackamap/hackamap-graph.json",e.url),r=await fetch(t.toString(),{redirect:"follow"});return r.ok?await r.json():null}n(bi,"fetchHackamapGraphJson");async function je(e,t){let r=new URL(t,e.url),o=await fetch(r.toString(),{redirect:"follow"});return o.ok?await o.json():null}n(je,"fetchHackamapJson");async function Ri(e){let t=await je(e,"/knowgrph/imports/hackamap/hackamap_api_graph.json");return Gn(t)?t:null}n(Ri,"fetchHackamapApiGraphJson");async function Pi(e){let t=await je(e,"/knowgrph/imports/hackamap/hackamap_pipeline.json");return t&&typeof t=="object"&&!Array.isArray(t)?t:{}}n(Pi,"fetchHackamapPipelineJson");async function jn(e){let t=await je(e,"/knowgrph/imports/hackamap/hackamap_query_presets.json");return Array.isArray(t)?t.filter(Boolean):[]}n(jn,"fetchHackamapQueryPresetsJson");async function Bn(e){let t=await je(e,"/knowgrph/imports/hackamap/query-outputs/query-runs.manifest.json");return t&&typeof t=="object"&&!Array.isArray(t)?t:{}}n(Bn,"fetchHackamapQueryRunsManifestJson");function Gn(e){return!e||typeof e!="object"||Array.isArray(e)?!1:Array.isArray(e.nodes)&&Array.isArray(e.edges)}n(Gn,"isApiGraphPayload");function Wn(e,t){let r=String(e&&e.output&&e.output.per_table_prefix||e?.id||t?.preset||"").trim(),o=String(t?.output_suffix||"").trim();return o?`${r}-${o}`:r}n(Wn,"buildHackamapTablePrefix");function kr(e,t){if(!Array.isArray(e))return[];let r=[];for(let o of e){if(!o||typeof o!="object"||Array.isArray(o))continue;let a=String(o[t]||"").trim();a&&r.push(a)}return r}n(kr,"collectRowIds");async function Ai(e,t){let r=await je(e,t);return Array.isArray(r)?r.length:0}n(Ai,"countHackamapQueryRows");async function ki(e,t,r){let o=Wn(t,r);if(!o)return{};let a=["events","demos","sources","organizer","team","techstack"],s=await Promise.all(a.map(async i=>[i,await Ai(e,`/knowgrph/imports/hackamap/query-outputs/${i}.${o}.query.json`)]));return Object.fromEntries(s.filter(([,i])=>i>0))}n(ki,"readHackamapRunTableCounts");function xr(e){return Array.isArray(e)?e.map(xr):!e||typeof e!="object"?e:Object.fromEntries(Object.entries(e).sort(([t],[r])=>String(t).localeCompare(String(r))).map(([t,r])=>[t,xr(r)]))}n(xr,"sortObjectKeys");function xi(e){try{return JSON.stringify(xr(e))}catch{return""}}n(xi,"stableParamSignature");function Ei(e){return typeof e=="string"?{value:e,label:e}:{value:e,label:JSON.stringify(e)}}n(Ei,"toBuilderOption");function Ci(e,t){return e.map(r=>{let o=String(r?.id||"").trim();if(!o)return null;let a=r?.params&&typeof r.params=="object"&&!Array.isArray(r.params)?r.params:{},s=t.filter(l=>String(l?.preset||"").trim()===o),i=Array.from(new Set([...Object.keys(a),...s.flatMap(l=>l?.params&&typeof l.params=="object"&&!Array.isArray(l.params)?Object.keys(l.params):[])])).sort((l,p)=>String(l).localeCompare(String(p))),c=Object.fromEntries(i.map(l=>{let p=new Set,m=[],f=[a[l],...s.map(R=>R?.params&&typeof R.params=="object"&&!Array.isArray(R.params)?R.params[l]:void 0)];for(let R of f){if(typeof R>"u")continue;let _=xi(R);!_||p.has(_)||(p.add(_),m.push(Ei(R)))}return[l,m]}));return{id:o,title:String(r?.title||o).trim(),params:a,param_keys:i,published_param_options:c}}).filter(Boolean)}n(Ci,"buildHackamapPresetRuntimeEntries");async function Ti(e){let[t,r,o]=await Promise.all([Pi(e),jn(e),Bn(e)]),a=t&&typeof t=="object"?t.runtime||{}:{},s=String(a?.query_selection?.default_run_id||"").trim()||"enhanced",i=Array.isArray(o?.runs)?o.runs:[],c=(await Promise.all(i.map(async l=>{let p=String(l?.id||"").trim(),m=String(l?.preset||"").trim();if(!p)return null;let f=r.find(_=>String(_?.id||"").trim()===m),R=await ki(e,f,l);return{id:p,preset:m,title:String(l?.title||l?.id||"").trim(),params:l?.params&&typeof l.params=="object"&&!Array.isArray(l.params)?l.params:{},output_suffix:String(l?.output_suffix||"").trim(),is_default:p===s,table_counts:R}}))).filter(l=>l?.id);return{ok:!0,runtime:{...a&&typeof a=="object"?a:{},presets:Ci(r,c),runs:c}}}n(Ti,"buildHackamapRuntimeMeta");async function vi(e,t){let r=String(t||"").trim();if(!r)return null;let[o,a]=await Promise.all([jn(e),Bn(e)]),i=(Array.isArray(a?.runs)?a.runs:[]).find(x=>String(x?.id||"").trim()===r);if(!i)return null;let c=o.find(x=>String(x?.id||"").trim()===String(i?.preset||"").trim()),l=Wn(c,i);if(!l)return null;let[p,m]=await Promise.all([je(e,`/knowgrph/imports/hackamap/query-outputs/events.${l}.query.json`),je(e,`/knowgrph/imports/hackamap/query-outputs/demos.${l}.query.json`)]),f=new Set(kr(p,"id")),R=new Set(kr(m,"id")),_=kr(m,"event_id");for(let x of _)f.add(x);return{eventIds:f,demoIds:R}}n(vi,"readHackamapQueryRunSelection");function Hn(e,t,r){if(!r||!Gn(e))return e;if(r.eventIds.size===0&&r.demoIds.size===0)return{...e,meta:{...e?.meta&&typeof e.meta=="object"?e.meta:{},selected_run_id:t,selected_run_filter_skipped:"no-event-demo-rows"}};let o=new Set;r.eventIds.forEach(c=>o.add(`Event:${c}`)),r.demoIds.forEach(c=>o.add(`Demo:${c}`));let a=Array.isArray(e.nodes)?e.nodes.filter(c=>o.has(String(c?.id||"").trim())):[],s=new Set(a.map(c=>String(c?.id||"").trim()).filter(Boolean)),i=Array.isArray(e.edges)?e.edges.filter(c=>s.has(String(c?.source||"").trim())&&s.has(String(c?.target||"").trim())):[];return{...e,nodes:a,edges:i,meta:{...e?.meta&&typeof e.meta=="object"?e.meta:{},selected_run_id:t,selected_event_count:r.eventIds.size,selected_demo_count:r.demoIds.size,total_problems:a.filter(c=>String(c?.type||"").trim()==="problem").length,total_solutions:a.filter(c=>String(c?.type||"").trim()==="solution").length}}}n(Hn,"filterHackamapApiGraphPayloadByRun");function Oi(e){let t=Array.isArray(e?.nodes)?e.nodes:[],r=Array.isArray(e?.links)?e.links:[],o=[],a=new Set;for(let i of t){let c=String(i?.id||"").trim(),l=String(i?.type||"").trim(),p=String(i?.label||"").trim();if(!(!c||!l||!p)){if(l==="Event"){o.push({id:c,type:"problem",label:p,cluster:"Event"}),a.add(c);continue}l==="Demo"&&(o.push({id:c,type:"solution",label:p,cluster:"Demo"}),a.add(c))}}let s=[];for(let i of r){let c=String(i?.source||"").trim(),l=String(i?.target||"").trim(),p=String(i?.type||"").trim();!c||!l||p==="has_demo"&&(!a.has(c)||!a.has(l)||s.push({source:c,target:l,type:"has_demo",strength:.35}))}return{nodes:o,edges:s,meta:{source:"hackamap-graph.json:fallback",total_problems:o.filter(i=>i.type==="problem").length,total_solutions:o.filter(i=>i.type==="solution").length,...e?.content_signature?{content_signature:String(e.content_signature)}:{}}}}n(Oi,"toBipartiteApiPayload");async function Fn(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase(),o=new URL(t.url);if(r==="OPTIONS")return new Response(null,{status:204,headers:{...ae(t),"access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(r!=="GET"&&r!=="HEAD")return it(t,{ok:!1,error:"unsupported_method"},405);if(String(o.searchParams.get("view")||"").trim().toLowerCase()==="meta"){let m=await Ti(t);return r==="HEAD"?new Response(null,{status:200,headers:{...Kt,...ae(t)}}):it(t,m,200)}let a=String(o.searchParams.get("run")||"").trim(),s=await vi(t,a),i=await Ri(t);if(i){let m=Hn(i,a,s);return r==="HEAD"?new Response(null,{status:200,headers:{...Kt,...ae(t)}}):it(t,m,200)}let c=await bi(t);if(!c)return it(t,{ok:!1,error:"missing_hackamap_graph",hint:"/knowgrph/imports/hackamap/{hackamap_api_graph.json,hackamap-graph.json} not found"},404);let l=Oi(c),p=Hn(l,a,s);return r==="HEAD"?new Response(null,{status:200,headers:{...Kt,...ae(t)}}):it(t,p,200)}n(Fn,"onRequest");var Ii=!0,zn=600,qn={"content-type":"application/json; charset=utf-8","cache-control":`public, max-age=${zn}`};function Be(e,t,r={}){return new Response(JSON.stringify(t),{...r,headers:{...qn,...r.headers||{},...ae(e)}})}n(Be,"jsonResponse");function Mi(e){try{let t=new URL(String(e));return t.protocol==="http:"||t.protocol==="https:"}catch{return!1}}n(Mi,"isHttpUrl");function ct(e){return String(e||"").trim().toLowerCase()}n(ct,"normalizeHost");function Er(e,{exact:t,suffixes:r}){let o=ct(e);return o?!!(Array.isArray(t)&&t.some(a=>o===ct(a))||Array.isArray(r)&&r.some(a=>o===ct(a)||o.endsWith(`.${ct(a)}`))):!1}n(Er,"isHostMatch");function Ni(e){let t=ct(e.hostname),r=e.toString();return Er(t,{suffixes:["linkedin.com"]})?new URL(`https://www.linkedin.com/embeds/oembed.json?url=${encodeURIComponent(r)}`):Er(t,{exact:["twitter.com","x.com"],suffixes:["twitter.com","x.com"]})?new URL(`https://publish.twitter.com/oembed?omit_script=1&url=${encodeURIComponent(r)}`):Er(t,{exact:["reddit.com"],suffixes:["reddit.com"]})?new URL(`https://www.reddit.com/oembed?url=${encodeURIComponent(r)}`):null}n(Ni,"buildOembedUpstreamUrl");async function Ui({upstreamUrl:e}){return await fetch(e.toString(),{headers:{"user-agent":"Mozilla/5.0 (compatible; OEmbedProxy/1.0)",accept:"application/json,text/json;q=0.9,*/*;q=0.1"},redirect:"follow",cf:{cacheTtl:zn,cacheEverything:!0}})}n(Ui,"fetchJsonUpstream");async function Vn(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase(),o=new URL(t.url);if(r==="OPTIONS")return new Response(null,{status:204,headers:{...ae(t),"access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(!["GET","HEAD"].includes(r))return Be(t,{ok:!1,error:"unsupported_method"},{status:405});if(o.searchParams.get("ping")==="1")return Be(t,{ok:!0,ping:!0});if(Ii)return Be(t,{ok:!1,error:"disabled_by_policy"},{status:200,headers:{"cache-control":"no-store"}});let a=o.searchParams.get("url")||"";if(!Mi(a))return Be(t,{ok:!1,error:"invalid_url"},{status:400,headers:{"cache-control":"no-store"}});let s;try{s=new URL(a)}catch{return Be(t,{ok:!1,error:"invalid_url"},{status:400,headers:{"cache-control":"no-store"}})}let i=Ni(s);if(!i)return Be(t,{ok:!1,error:"unsupported_provider"},{status:400,headers:{"cache-control":"no-store"}});let c=await Ui({upstreamUrl:i}),l=new Headers(c.headers);l.delete("content-length"),l.set("cache-control",c.ok?qn["cache-control"]:"no-store");for(let[m,f]of Object.entries(ae(t)))l.set(m,f);if(r==="HEAD")return new Response(null,{status:c.status,headers:l});let p=await c.text();try{JSON.parse(p)}catch{return Be(t,{ok:!1,error:"invalid_upstream_json",status:c.status},{status:502,headers:{"cache-control":"no-store"}})}return l.set("content-type","application/json; charset=utf-8"),new Response(p,{status:c.status,headers:l})}n(Vn,"onRequest");var Yn="/__chat_proxy",$t="agnes-ai",Dt="byteplus-modelark",Ht="miromind",Li=n(e=>{let t=Re(e);return t==="openai"?"openai":t===Dt||t==="byteplus"?Dt:t===Ht||t==="miromind-api"?Ht:t===$t||t==="agnes"||t==="agnes-ai-api"?$t:t},"normalizeProviderId"),Ki=n(e=>Re(e)===Ut,"isAgnesHost"),$i=n(e=>{let t=Re(e);return t===Lt||t===Ar},"isBytePlusHost"),Di=n(e=>Re(e)===Nt,"isMiroMindHost"),Hi=n(({provider:e,requestedUpstream:t,env:r})=>e==="openai"?"https://api.openai.com":e===Ht?t||`https://${Nt}`:e===$t?t||`https://${Ut}`:e===Dt?t||String(r.KNOWGRPH_CHAT_PROXY_UPSTREAM||"").trim()||`https://${Lt}`:t||String(r.KNOWGRPH_CHAT_PROXY_UPSTREAM||"").trim(),"pickUpstreamBase");async function Jn(e){let{request:t,env:r}=e,o=String(t.method||"GET").toUpperCase(),a=new URL(t.url);if(o==="OPTIONS")return new Response(null,{status:204,headers:{"access-control-allow-origin":we(t.headers,"origin")||"*","access-control-allow-methods":"GET, HEAD, POST, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(!["GET","HEAD","POST"].includes(o))return he(t,{ok:!1,error:"Unsupported method"},405);let s=Li(we(t.headers,"x-kg-chat-provider")),i=Hi({provider:s,requestedUpstream:we(t.headers,"x-kg-chat-upstream"),env:r});if(!i)return he(t,{ok:!1,error:"Missing chat proxy upstream configuration"},500);let c;try{c=new URL(i)}catch{return he(t,{ok:!1,error:"Invalid chat proxy upstream configuration"},500)}let l=Dn(r,{includeOpenAi:!0,includeMiroMind:!0,includeAgnes:!0,includeBytePlus:!0}),p=Re(c.hostname);if(!l.has(p))return he(t,{ok:!1,error:"Chat proxy upstream host is not allowed"},403);if(!$n(p)&&c.protocol!=="https:")return he(t,{ok:!1,error:"Chat proxy requires HTTPS for non-local upstream hosts"},403);let m=s==="openai"||p===Pr,f=s===Ht||Di(p),R=s===$t||Ki(p),_=s===Dt||$i(p),x=we(t.headers,"x-kg-chat-api-key"),j=String(r.KNOWGRPH_CHAT_PROXY_OPENAI_API_KEY||r.OPENAI_API_KEY||"").trim(),L=String(r.KNOWGRPH_CHAT_PROXY_MIROMIND_API_KEY||r.MIROMIND_API_KEY||"").trim(),D=String(r.KNOWGRPH_CHAT_PROXY_AGNES_API_KEY||r.AGNES_API_KEY||"").trim(),re=String(r.KNOWGRPH_CHAT_PROXY_BYTEPLUS_API_KEY||r.BYTEPLUS_API_KEY||"").trim(),W=(x||j).slice(0,512),v=(x||L).slice(0,512),I=(x||D).slice(0,512),J=(x||re).slice(0,512),K=_?J:R?I:f?v:W;if(m&&!W)return he(t,{ok:!1,error:"Missing OpenAI API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_OPENAI_API_KEY or OPENAI_API_KEY)"},401);if(f&&!K)return he(t,{ok:!1,error:"Missing MiroMind API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_MIROMIND_API_KEY or MIROMIND_API_KEY)"},401);if(R&&!K)return he(t,{ok:!1,error:"Missing Agnes API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_AGNES_API_KEY or AGNES_API_KEY)"},401);if(_&&!K)return he(t,{ok:!1,error:"Missing BytePlus API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_BYTEPLUS_API_KEY or BYTEPLUS_API_KEY)"},401);if(o==="POST"&&!we(t.headers,"content-type").toLowerCase().includes("application/json"))return he(t,{ok:!1,error:"Chat proxy expects application/json payloads"},415);let X=a.pathname.startsWith(Yn)&&a.pathname.slice(Yn.length)||"/v1/chat/completions",M=X.startsWith("/")?X:`/${X}`,N=new URL(`${M}${a.search||""}`,c),B=new Headers,oe=we(t.headers,"content-type"),pe=we(t.headers,"accept");oe&&B.set("content-type",oe),pe&&B.set("accept",pe),(m||f||R||_)&&B.set("authorization",`Bearer ${K}`);let Se=we(t.headers,"x-client-request-id").slice(0,512);Se&&B.set("x-client-request-id",Se);let P=new AbortController,y=Number(r.KNOWGRPH_CHAT_PROXY_TIMEOUT_MS),S=Number.isFinite(y)?Math.max(5e3,Math.min(18e4,Math.floor(y))):9e4,d=setTimeout(()=>P.abort(),S);try{let h=await fetch(N.toString(),{method:o,headers:B,body:o==="GET"||o==="HEAD"?void 0:t.body,signal:P.signal,redirect:"follow"}),g=new Headers(h.headers);g.delete("content-length"),g.delete("www-authenticate"),g.set("cache-control","no-store");let b=we(t.headers,"origin");return b&&(g.set("access-control-allow-origin",b),g.set("vary","Origin")),o==="HEAD"?new Response(null,{status:h.status,statusText:h.statusText,headers:g}):new Response(h.body,{status:h.status,statusText:h.statusText,headers:g})}catch(h){let g=h&&typeof h=="object"&&"message"in h?String(h.message||""):"",b=P.signal.aborted||/aborted|timeout/i.test(g);return he(t,{ok:!1,error:g||"Failed to reach chat upstream"},b?504:502)}finally{clearTimeout(d)}}n(Jn,"onRequest");function ji(e){let t=e.map(r=>r==null?"":typeof r=="boolean"?r?"1":"0":typeof r=="number"?Number.isFinite(r)?String(r):"":String(r)).join("|");return`rich-media-preview:${Xe(t)}`}n(ji,"buildRichMediaPreviewSemanticKey");var Qn="png";function lt(e){let t=typeof e=="number"?e:Number(String(e??"").trim());if(!Number.isFinite(t))return null;let r=Math.max(0,Number(t.toFixed(3)));return Number.isFinite(r)?r:null}n(lt,"normalizeRemoteVideoFrameSeconds");function Bi(e){return String(lt(e)??0).replace(/\./g,"_")}n(Bi,"formatRemoteVideoFrameSecondsForFileName");function Bt(e){let t=String(e||"").trim().toLowerCase();return t==="jpg"||t==="jpeg"?"jpg":"png"}n(Bt,"normalizeRemoteVideoFrameFormat");function Cr(e){let t=String(e.sourceUrl||"").trim(),r=lt(e.timeSeconds)??0,o=Bt(e.format||Qn);return ji(["remote-video-frame",t,r,o])}n(Cr,"buildRemoteVideoFrameSemanticKey");function Zn(e){let t=lt(e.timeSeconds)??0,r=Bt(e.format||Qn),o=Cr({...e,timeSeconds:t,format:r});return`frame-${o.split(":").pop()||Xe(o)}-t${Bi(t)}.${r}`}n(Zn,"buildRemoteVideoFrameFileName");var jt=n(e=>{let t=String(e||"").trim();return t&&/^[A-Za-z0-9_-]{6,128}$/.test(t)?t:null},"normalizeYouTubeIdLikeValue"),Xn=n(e=>{try{let t=new URL(String(e||"").trim()),r=String(t.hostname||"").toLowerCase();if(r==="youtu.be"||r.endsWith(".youtu.be")){let o=t.pathname.replace(/^\/+/,"").split("/")[0]?.trim()||"";return jt(o)}if(r==="youtube.com"||r.endsWith(".youtube.com")||r==="youtube-nocookie.com"||r.endsWith(".youtube-nocookie.com")){let o=String(t.searchParams.get("v")||"").trim();if(o)return jt(o);let a=t.pathname.split("/").filter(Boolean),s=a[0]||"",i=a[1]||"";if((s==="embed"||s==="shorts"||s==="live")&&i)return jt(i);if(s==="watch"){let c=String(t.searchParams.get("v")||"").trim();return jt(c)}}}catch{return null}return null},"readYouTubeIdFromUrl");function Gi(e){let t=String(e||"").trim().replace(/^<|>$/g,"").trim();for(;/[),.;:!?]$/.test(t);){let r=t.slice(0,-1).trim();if(!r)break;let o=Xn(t),a=Xn(r);if(!a||o&&o!==a)break;t=r}return t}n(Gi,"stripYouTubeUrlTrailingPunctuation");function ea(e){let t=n(r=>{let o=String(r||"").trim();if(!o)return null;if(/^\d+$/.test(o))return Number(o);let a=o.match(/^(?:(\d+)h)?(?:(\d+)m)?(?:(\d+)s)?$/i);if(!a)return null;let s=a[1]?Number(a[1]):0,i=a[2]?Number(a[2]):0,c=a[3]?Number(a[3]):0,l=s*3600+i*60+c;return l>0&&Number.isFinite(l)?l:null},"parseChunk");try{let r=new URL(Gi(e)),o=r.searchParams.get("t")||r.searchParams.get("start")||"",a=r.hash&&new URLSearchParams(r.hash.replace(/^#/,"")).get("t")||"";return t(o)??t(a)}catch{return null}}n(ea,"parseYouTubeStartSeconds");var Wi="/image/knowgrph/video-frame",Fi=4096,zi=720*60,qi=/^frame-[a-f0-9]+-t\d+\.(?:png|jpg)$/i,Ir={"access-control-allow-origin":"*","access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"accept, content-type"},Or=n(e=>String(e||"").replace(/\s+/g," ").trim(),"cleanText"),Tr=n((e,t=200,r="GET")=>new Response(r==="HEAD"?null:JSON.stringify(e),{status:t,headers:{...Ir,"content-type":"application/json; charset=utf-8","cache-control":"no-store"}}),"jsonResponse"),vr=n((e,t=200,r="GET")=>new Response(r==="HEAD"?null:e,{status:t,headers:{...Ir,"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}}),"textResponse"),Vi=n((e,t)=>e===t||e.endsWith(`.${t}`),"hostMatches"),Yi=n(e=>{let t=Or(e?.KG_VIDEO_FRAME_ALLOWED_HOSTS);return t?t.split(",").map(r=>Or(r).toLowerCase()).filter(Boolean):["youtube.com","youtu.be","youtube-nocookie.com","bilibili.com","b23.tv"]},"readAllowedHosts"),Ji=n(e=>Or(e).replace(/^<|>$/g,"").trim(),"unwrapUrlInput"),Xi=n((e,t)=>{try{let r=new URL(e);if(r.protocol!=="https:"&&r.protocol!=="http:")return!1;let o=r.hostname.toLowerCase();return Yi(t).some(a=>Vi(o,a))}catch{return!1}},"isAllowedSourceUrl"),Qi=n((e,t)=>{let r=new URL(e.url),o=Ji(r.searchParams.get("url")||"");if(!o)return{error:"Missing url parameter"};if(o.length>Fi)return{error:"Video URL is too long"};if(!Xi(o,t))return{error:"Video frame extraction is limited to supported remote video hosts"};let a=lt(r.searchParams.get("time"))??ea(o);if(a==null)return{error:"Missing time parameter"};let s=Math.min(zi,Math.max(0,a)),i=Bt(r.searchParams.get("format")||"png"),c=Zn({sourceUrl:o,timeSeconds:s,format:i});if(!qi.test(c))return{error:"Invalid frame cache key"};let l=`${Wi}/${c}`;return{sourceUrl:o,timeSeconds:s,format:i,fileName:c,publicUrl:l,semanticKey:Cr({sourceUrl:o,timeSeconds:s,format:i})}},"readFrameRequest"),Zi=n(async(e,t,r)=>{let o=new URL(t,e.request.url),a=new Request(o.toString(),{method:r});return typeof e.env?.ASSETS?.fetch=="function"?await e.env.ASSETS.fetch(a):await fetch(a)},"fetchStaticAsset"),ec=n(e=>`Frame has not been generated yet. Run the local video-frame extractor and publish ${e.publicUrl}.`,"missingFrameMessage"),tc=n((e,t)=>{let r=new Headers;r.set("content-type",t.format==="jpg"?"image/jpeg":"image/png"),r.set("cache-control","public, max-age=31536000, immutable"),r.set("access-control-allow-origin","*");let o=e.headers.get("content-length");o&&r.set("content-length",o);let a=e.headers.get("etag");return a&&r.set("etag",a),r},"imageResponseHeaders");async function ta(e){let t=e.request;if(t.method==="OPTIONS")return new Response(null,{status:204,headers:Ir});if(t.method!=="GET"&&t.method!=="HEAD")return vr("Method not allowed",405,t.method);let r=new URL(t.url).searchParams.get("emit")==="json",o=Qi(t,e.env||{});if("error"in o)return r?Tr({ok:!1,error:o.error},400,t.method):vr(o.error,400,t.method);let a=r&&t.method!=="HEAD"?"GET":r||t.method==="HEAD"?"HEAD":"GET",s=await Zi(e,o.publicUrl,a);if(!s.ok){let i=ec(o);return r?Tr({ok:!1,error:i,publicUrl:o.publicUrl,semanticKey:o.semanticKey},404,t.method):vr(i,404,t.method)}if(r){let i=Number(s.headers.get("content-length")||0);return(!Number.isFinite(i)||i<=0)&&t.method!=="HEAD"&&(i=(await s.arrayBuffer()).byteLength),Tr({ok:!0,imageUrl:o.publicUrl,publicUrl:o.publicUrl,semanticKey:o.semanticKey,cached:!0,bytes:Number.isFinite(i)?Math.max(0,Math.floor(i)):0,timeSeconds:o.timeSeconds,format:o.format},200,t.method)}return new Response(t.method==="HEAD"?null:s.body,{status:200,headers:tc(s,o)})}n(ta,"onRequest");var ra={"content-type":"application/json; charset=utf-8","cache-control":"no-store","access-control-allow-origin":"*","access-control-allow-methods":"GET, HEAD, POST, OPTIONS","access-control-allow-headers":"content-type"},Gt=n((e,t=200,r="GET")=>new Response(r==="HEAD"?null:JSON.stringify(e),{status:t,headers:ra}),"jsonResponse"),Y=n(e=>String(e||"").replace(/\s+/g," ").trim(),"cleanText"),rc=n(e=>{try{let t=new URL(String(e||"").trim());if(/youtu\.be$/i.test(t.hostname))return Y(t.pathname.split("/").filter(Boolean)[0]);if(/youtube\.com$/i.test(t.hostname)||/youtube-nocookie\.com$/i.test(t.hostname)){let r=Y(t.searchParams.get("v"));if(r)return r;let o=t.pathname.split("/").filter(Boolean),a=o.findIndex(s=>["embed","shorts","live"].includes(s));if(a>=0)return Y(o[a+1])}}catch{}return""},"readVideoId"),oc=n((e,t)=>{let r=e.indexOf(t);if(r<0)return null;let o=e.indexOf("{",r);if(o<0)return null;let a=0,s=!1,i=!1;for(let c=o;c<e.length;c+=1){let l=e[c];if(s){i?i=!1:l==="\\"?i=!0:l==='"'&&(s=!1);continue}if(l==='"')s=!0;else if(l==="{")a+=1;else if(l==="}"&&(a-=1,a===0))return e.slice(o,c+1)}return null},"extractJsonAfter"),nc=n(e=>{for(let t of["ytInitialPlayerResponse =","ytInitialPlayerResponse="]){let r=oc(e,t);if(r)try{return JSON.parse(r)}catch{}}return null},"parsePlayerResponse"),ac=n((e,t)=>{let r=Y(t||"en").toLowerCase();return e.find(o=>Y(o.languageCode).toLowerCase()===r)||e.find(o=>Y(o.languageCode).toLowerCase().startsWith(r.split("-")[0]))||e.find(o=>Y(o.kind)!=="asr")||e[0]||null},"pickCaptionTrack"),sc=n(e=>{let t=new URL(e);return t.searchParams.set("fmt","json3"),t.toString()},"withJsonCaptionFormat"),ic=n(e=>(Array.isArray(e?.events)?e.events:[]).map(r=>{let o=Array.isArray(r.segs)?Y(r.segs.map(i=>i?.utf8||"").join("")):"",a=Number(r.tStartMs)/1e3,s=Number(r.dDurationMs||0)/1e3;return o&&Number.isFinite(a)?{text:o,start:a,duration:Number.isFinite(s)?s:0}:null}).filter(Boolean),"parseCaptionJson3"),cc=n(e=>String(e||"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;|&apos;/g,"'"),"decodeXmlText"),lc=n(e=>{let t=[],r=/<text\b([^>]*)>([\s\S]*?)<\/text>/gi,o=null;for(;o=r.exec(String(e||""));){let a=o[1]||"",s=Number(a.match(/\bstart="([^"]+)"/i)?.[1]),i=Number(a.match(/\bdur="([^"]+)"/i)?.[1]||0),c=Y(cc(o[2]||""));c&&Number.isFinite(s)&&t.push({text:c,start:s,duration:Number.isFinite(i)?i:0})}return t},"parseCaptionXml"),pc=n((e,t)=>{let r=String(e||"").trim();if(!r)return[];if(String(t||"").toLowerCase().includes("json")||r.startsWith("{")||r.startsWith("["))try{return ic(JSON.parse(r))}catch{return[]}return lc(r)},"parseCaptionResponseText"),uc=n(e=>{let t=Math.max(0,Math.floor(Number(e)||0)),r=Math.floor(t/60),o=String(t%60).padStart(2,"0");return`${r}:${o}`},"formatTimestamp"),dc=n((e,t)=>{let r=new URL(e);return r.searchParams.set("t",`${Math.max(0,Math.floor(Number(t)||0))}s`),r.toString()},"timestampUrl"),mc=n(({title:e,sourceUrl:t,videoId:r,authorName:o,thumbnailUrl:a,segments:s})=>[`# ${e||`YouTube ${r}`}`,"",`Video ID: ${r}`,o?`Author: ${o}`:"",`Source: [${t}](${t})`,a?`[![${e||r}](${a})](${t})`:"","",s.length>0?"## Transcript":"## Video Source","",...s.length>0?s.map(i=>`[${uc(i.start)}](${dc(t,i.start)}) ${i.text}`):["Captions were not available from the source at import time.","The source URL, title, author, and thumbnail remain available for downstream storyboard reconstruction."],""].filter(i=>i!=="").join(`
`),"buildMarkdown"),Mr=n(({videoId:e,sourceUrl:t,title:r,authorName:o,thumbnailUrl:a,lang:s,languageCode:i,segments:c,captionStatus:l})=>{let p={type:"rag:YouTubeTranscript",video_id:e,source_url:t,title:r,author_name:o,thumbnail_url:a,language_code:Y(i)||s,caption_status:l,segment_count:c.length,duration:c.reduce((m,f)=>Math.max(m,f.start+f.duration),0),segments:c};return{ok:!0,name:`youtube-${e.toLowerCase()}.md`,markdown:mc({title:r,sourceUrl:t,videoId:e,authorName:o,thumbnailUrl:a,segments:c}),transcript:p}},"buildPayload");async function hc({sourceUrl:e,lang:t="en",fetchImpl:r=fetch}){let o=rc(e);if(!o)return{ok:!1,error:"unsupported_youtube_url"};let a=`https://www.youtube.com/watch?v=${encodeURIComponent(o)}`,[s,i]=await Promise.all([r(`https://www.youtube.com/oembed?url=${encodeURIComponent(a)}&format=json`,{headers:{accept:"application/json"}}).catch(()=>null),r(a,{headers:{accept:"text/html,application/xhtml+xml","accept-language":"en-US,en;q=0.9","user-agent":"Mozilla/5.0 Knowgrph YouTube transcript importer"}})]),c=s?.ok?await s.json().catch(()=>({})):{},l=i.ok?nc(await i.text()):null,p=Y(c.title)||Y(l?.videoDetails?.title)||`YouTube ${o}`,m=Y(c.author_name)||Y(l?.videoDetails?.author),f=Y(c.thumbnail_url)||`https://i.ytimg.com/vi/${o}/hqdefault.jpg`;if(!i.ok)return Mr({videoId:o,sourceUrl:a,title:p,authorName:m,thumbnailUrl:f,lang:t,languageCode:t,segments:[],captionStatus:`watch-fetch-${i.status}`});let R=l?.captions?.playerCaptionsTracklistRenderer?.captionTracks||[],_=ac(Array.isArray(R)?R:[],t);if(!_?.baseUrl)return Mr({videoId:o,sourceUrl:a,title:p,authorName:m,thumbnailUrl:f,lang:t,languageCode:t,segments:[],captionStatus:"captions-unavailable"});let x=await r(sc(_.baseUrl),{headers:{accept:"application/json,text/xml,text/plain,*/*","user-agent":"Mozilla/5.0 Knowgrph YouTube transcript importer"}}).catch(()=>null),j=x?await x.text().catch(()=>""):"",L=x?.ok?pc(j,x.headers.get("content-type")):[],D=L.length>0?"available":x?.ok?"captions-empty":`captions-fetch-${x?.status||"failed"}`;return Mr({videoId:o,sourceUrl:a,title:p,authorName:m,thumbnailUrl:f,lang:t,languageCode:_.languageCode,segments:L,captionStatus:D})}n(hc,"buildYouTubeTranscriptPayload");async function oa(e){let t=e.request,r=String(t.method||"GET").toUpperCase();if(r==="OPTIONS")return new Response(null,{status:204,headers:ra});if(r!=="GET"&&r!=="HEAD"&&r!=="POST")return Gt({ok:!1,error:"unsupported_method"},405,r);let o=new URL(t.url),a=Y(o.searchParams.get("url")),s=Y(o.searchParams.get("lang"))||"en";if(!a)return Gt({ok:!1,error:"missing_url"},400,r);try{let i=await hc({sourceUrl:a,lang:s});return Gt(i,i.ok?200:502,r)}catch(i){let c=i&&typeof i=="object"&&"message"in i?Y(i.message):"";return Gt({ok:!1,error:c||"youtube_conversion_failed"},502,r)}}n(oa,"onRequest");async function na(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase();if(r==="OPTIONS")return new Response(null,{status:204,headers:{...ae(t),"access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(r!=="GET"&&r!=="HEAD")return new Response(JSON.stringify({ok:!1,error:"unsupported_method"}),{status:405,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store",...ae(t)}});let o={ok:!0,service:"singabldr-pages",ts:new Date().toISOString()},a={"content-type":"application/json; charset=utf-8","cache-control":"no-store",...ae(t)};return r==="HEAD"?new Response(null,{status:200,headers:a}):new Response(JSON.stringify(o),{status:200,headers:a})}n(na,"onRequest");var Te="https://airvio.co";var le="/knowgrph",Ce=`${Te}${le}/`,gc=`${Te}/`;var sa=`${le}/health`,aa=`${Te}${sa}`,ia="/.well-known/agent-card.json",ud=`${le}/.well-known/agent-card.json`,fc=`${Te}${ia}`,yc=`${Te}/api/storage/source-files`,wc=`${Te}/api/storage/doc-default/{canonicalPath}`,Sc=`${Te}/api/storage/doc/{workspaceId}/{canonicalPath}`,_c=`${Te}/api/storage/blob/{workspaceId}/{canonicalPath}`;var Nr="root-agent-ready-pages",ca=['</.well-known/api-catalog>; rel="api-catalog"; type="application/linkset+json"',`<${le}/.well-known/openapi.json>; rel="service-desc"; type="application/vnd.oai.openapi+json;version=3.1"`,`<${le}/llms.txt>; rel="service-doc"; type="text/plain"`,'</auth.md>; rel="service-doc"; type="text/markdown"',`<${sa}>; rel="status"; type="application/health+json"`,`<${le}/.well-known/mcp/server-card.json>; rel="mcp-server-card"; type="application/json"`,`<${ia}>; rel="describedby"; type="application/json"`].join(", "),la=`# Knowgrph
...
- Auth.md registration instructions: ${gc}auth.md
- Health: ${aa}
...
- Source Files index: ${yc}
- Default Source File documents: ${wc}
- Workspace Source File documents: ${Sc}
- Workspace binary artifacts: ${_c}
...
`,pa=n(e=>new Response(e,{status:200,headers:{"content-type":"text/markdown; charset=utf-8","cache-control":"public, max-age=3600","access-control-allow-origin":"*",vary:"Accept","x-markdown-tokens":String(Math.ceil(String(e||"").length/4))}}),"markdownResponse"),ua=n(e=>(e.headers.get("accept")||"").toLowerCase().split(",").some(r=>r.trim().startsWith("text/markdown")),"wantsMarkdown"),Ur=n((e,t)=>{let r=new Response(e.body,e),o=String(t?.owner||"").trim(),a=String(t?.tag||"").trim();return o&&r.headers.set("x-knowgrph-route-owner",o),a&&r.headers.set("x-knowgrph-route-tag",a),r},"withAgentReadyRouteHeaders");var Kr="Agent-actionable chat-to-canvas knowledge graph workspace",pt='<main id="root"></main>',bc=/<(?:main|div)\s+id=["']root["']\s*><\/(?:main|div)>/i,Rc=n(e=>{let t=/<script>([\s\S]*?)<\/script>/g;for(let r of String(e||"").matchAll(t)){let o=r[1]||"";if(o.includes("createWebMcpLifecycleController")&&o.includes("toolDefinitions"))return o}return""},"extractWebMcpScript"),da=n(()=>({"content-type":"text/html; charset=utf-8","cache-control":"no-store, no-cache, no-transform, must-revalidate, max-age=0","access-control-allow-origin":"*",link:ca}),"rootHtmlHeaders"),Lr=n((e,t)=>String(e||"").includes("</head>")?String(e||"").replace("</head>",`${t}</head>`):`${String(e||"")}${t}`,"injectIntoHead"),ma=n(e=>String(e||"").replace(bc,pt),"canonicalizeRootMount"),Wt=n(()=>`<main id="knowgrph-root-fallback" data-knowgrph-root-fallback="visible" aria-label="Knowgrph root alias" style="position:fixed;inset:0;z-index:2147483000;display:grid;place-content:center;gap:1rem;padding:2rem;box-sizing:border-box;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#101820;color:#f4f7fb;text-align:center">
    ...
    <\/script>`,"rootVisibleFallbackMarkup"),Pc=n(e=>{let t=ma(e);return/<main\s+id=["']knowgrph-root-fallback["']/i.test(t)?t:t.includes(pt)?t.replace(pt,`${pt}
    ${Wt()}`):t.includes("</body>")?t.replace("</body>",`    ${Wt()}
  </body>`):`${t}
${Wt()}`},"injectRootVisibleFallback"),Ac=n(e=>{let t=ma(e);return/<meta\s+name=["']description["'][^>]*>/i.test(t)?t=t.replace(/<meta\s+name=["']description["'][^>]*>/i,`<meta name="description" content="${Kr}" />`):t=Lr(t,`    <meta name="description" content="${Kr}" />
...
`)),Pc(t)},"rewriteRootAppHtml"),kc=n(async e=>{let t=new URL(`${le}/?agentReadyRootWebMcp=1`,e.url),r=await fetch(t,{headers:{accept:"text/html"}});return r.ok?Rc(await r.text()):""},"loadWebMcpScript"),xc=n(async e=>{let t=new URL(`${le}/?agentReadyRootAlias=1`,e.url),r=await fetch(t,{headers:{accept:"text/html"}});if(!r.ok)return null;let o=Ac(await r.text());return!o.includes(pt)||!o.includes(`${le}/assets/`)?null:new Response(o,{status:200,headers:da()})},"loadKnowgrphAppShell"),Ec=n((e="")=>new Response(`<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
</html>`,{status:200,headers:da()}),"rootHtmlResponse");async function ha(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase();if(r!=="GET"&&r!=="HEAD")return e.next();if(ua(t)){let s=Ur(pa(la),{owner:Nr,tag:"root-homepage-markdown"});return r==="HEAD"?new Response(null,s):s}let o=r==="HEAD"?null:await xc(t),a=Ur(o||Ec(r==="HEAD"?"":await kc(t)),{owner:Nr,tag:"root-homepage-html"});return r==="HEAD"?new Response(null,a):a}n(ha,"onRequest");var A=[{routePath:"/api/llm/chat/completions",mountPath:"/api/llm/chat",method:"",middlewares:[],modules:[jr]},{routePath:"/api/payments/commerce/x402",mountPath:"/api/payments/commerce",method:"",middlewares:[],modules:[lo]},{routePath:"/api/llm/models",mountPath:"/api/llm",method:"",middlewares:[],modules:[po]},{routePath:"/api/llm/responses",mountPath:"/api/llm",method:"",middlewares:[],modules:[uo]},{routePath:"/knowgrph/doc-default/:path*",mountPath:"/knowgrph/doc-default",method:"",middlewares:[],modules:[Tn]},{routePath:"/knowgrph/doc/:path*",mountPath:"/knowgrph/doc",method:"",middlewares:[],modules:[vn]},{routePath:"/knowgrph/share/:path*",mountPath:"/knowgrph/share",method:"",middlewares:[],modules:[On]},{routePath:"/api/link-preview",mountPath:"/api",method:"GET",middlewares:[],modules:[Mn]},{routePath:"/api/link-proxy",mountPath:"/api",method:"GET",middlewares:[],modules:[Un]},{routePath:"/api/graph",mountPath:"/api",method:"",middlewares:[],modules:[Fn]},{routePath:"/api/oembed",mountPath:"/api",method:"",middlewares:[],modules:[Vn]},{routePath:"/__chat_proxy/:path*",mountPath:"/__chat_proxy",method:"",middlewares:[],modules:[Jn]},{routePath:"/knowgrph/:path*",mountPath:"/knowgrph",method:"",middlewares:[],modules:[Ne]},{routePath:"/__video_frame",mountPath:"/",method:"",middlewares:[],modules:[ta]},{routePath:"/__youtube_transcript",mountPath:"/",method:"",middlewares:[],modules:[oa]},{routePath:"/health",mountPath:"/",method:"",middlewares:[],modules:[na]},{routePath:"/",mountPath:"/",method:"",middlewares:[],modules:[ha]}];function Cc(e){for(var t=[],r=0;r<e.length;){var o=e[r];if(o==="*"||o==="+"||o==="?"){t.push({type:"MODIFIER",index:r,value:e[r++]});continue}if(o==="\\"){t.push({type:"ESCAPED_CHAR",index:r++,value:e[r++]});continue}if(o==="{"){t.push({type:"OPEN",index:r,value:e[r++]});continue}if(o==="}"){t.push({type:"CLOSE",index:r,value:e[r++]});continue}if(o===":"){for(var a="",s=r+1;s<e.length;){var i=e.charCodeAt(s);if(i>=48&&i<=57||i>=65&&i<=90||i>=97&&i<=122||i===95){a+=e[s++];continue}break}if(!a)throw new TypeError("Missing parameter name at ".concat(r));t.push({type:"NAME",index:r,value:a}),r=s;continue}if(o==="("){var c=1,l="",s=r+1;if(e[s]==="?")throw new TypeError('Pattern cannot start with "?" at '.concat(s));for(;s<e.length;){if(e[s]==="\\"){l+=e[s++]+e[s++];continue}if(e[s]===")"){if(c--,c===0){s++;break}}else if(e[s]==="("&&(c++,e[s+1]!=="?"))throw new TypeError("Capturing groups are not allowed at ".concat(s));l+=e[s++]}if(c)throw new TypeError("Unbalanced pattern at ".concat(r));if(!l)throw new TypeError("Missing pattern at ".concat(r));t.push({type:"PATTERN",index:r,value:l}),r=s;continue}t.push({type:"CHAR",index:r,value:e[r++]})}return t.push({type:"END",index:r,value:""}),t}n(Cc,"lexer");function Tc(e,t){t===void 0&&(t={});for(var r=Cc(e),o=t.prefixes,a=o===void 0?"./":o,s=t.delimiter,i=s===void 0?"/#?":s,c=[],l=0,p=0,m="",f=n(function(M){if(p<r.length&&r[p].type===M)return r[p++].value},"tryConsume"),R=n(function(M){var N=f(M);if(N!==void 0)return N;var B=r[p],oe=B.type,pe=B.index;throw new TypeError("Unexpected ".concat(oe," at ").concat(pe,", expected ").concat(M))},"mustConsume"),_=n(function(){for(var M="",N;N=f("CHAR")||f("ESCAPED_CHAR");)M+=N;return M},"consumeText"),x=n(function(M){for(var N=0,B=i;N<B.length;N++){var oe=B[N];if(M.indexOf(oe)>-1)return!0}return!1},"isSafe"),j=n(function(M){var N=c[c.length-1],B=M||(N&&typeof N=="string"?N:"");if(N&&!B)throw new TypeError('Must have text between two parameters, missing text after "'.concat(N.name,'"'));return!B||x(B)?"[^".concat(Ue(i),"]+?"):"(?:(?!".concat(Ue(B),")[^").concat(Ue(i),"])+?")},"safePattern");p<r.length;){var L=f("CHAR"),D=f("NAME"),re=f("PATTERN");if(D||re){var W=L||"";a.indexOf(W)===-1&&(m+=W,W=""),m&&(c.push(m),m=""),c.push({name:D||l++,prefix:W,suffix:"",pattern:re||j(W),modifier:f("MODIFIER")||""});continue}var v=L||f("ESCAPED_CHAR");if(v){m+=v;continue}m&&(c.push(m),m="");var I=f("OPEN");if(I){var W=_(),J=f("NAME")||"",K=f("PATTERN")||"",X=_();R("CLOSE"),c.push({name:J||(K?l++:""),pattern:J&&!K?j(W):K,prefix:W,suffix:X,modifier:f("MODIFIER")||""});continue}R("END")}return c}n(Tc,"parse");function ut(e,t){var r=[],o=fa(e,r,t);return vc(o,r,t)}n(ut,"match");function vc(e,t,r){r===void 0&&(r={});var o=r.decode,a=o===void 0?function(s){return s}:o;return function(s){var i=e.exec(s);if(!i)return!1;for(var c=i[0],l=i.index,p=Object.create(null),m=n(function(R){if(i[R]===void 0)return"continue";var _=t[R-1];_.modifier==="*"||_.modifier==="+"?p[_.name]=i[R].split(_.prefix+_.suffix).map(function(x){return a(x,_)}):p[_.name]=a(i[R],_)},"_loop_1"),f=1;f<i.length;f++)m(f);return{path:c,index:l,params:p}}}n(vc,"regexpToFunction");function Ue(e){return e.replace(/([.+*?=^!:${}()[\]|/\\])/g,"\\$1")}n(Ue,"escapeString");function ga(e){return e&&e.sensitive?"":"i"}n(ga,"flags");function Oc(e,t){if(!t)return e;for(var r=/\((?:\?<(.*?)>)?(?!\?)/g,o=0,a=r.exec(e.source);a;)t.push({name:a[1]||o++,prefix:"",suffix:"",modifier:"",pattern:""}),a=r.exec(e.source);return e}n(Oc,"regexpToRegexp");function Ic(e,t,r){var o=e.map(function(a){return fa(a,t,r).source});return new RegExp("(?:".concat(o.join("|"),")"),ga(r))}n(Ic,"arrayToRegexp");function Mc(e,t,r){return Nc(Tc(e,r),t,r)}n(Mc,"stringToRegexp");function Nc(e,t,r){r===void 0&&(r={});for(var o=r.strict,a=o===void 0?!1:o,s=r.start,i=s===void 0?!0:s,c=r.end,l=c===void 0?!0:c,p=r.encode,m=p===void 0?function(N){return N}:p,f=r.delimiter,R=f===void 0?"/#?":f,_=r.endsWith,x=_===void 0?"":_,j="[".concat(Ue(x),"]|$"),L="[".concat(Ue(R),"]"),D=i?"^":"",re=0,W=e;re<W.length;re++){var v=W[re];if(typeof v=="string")D+=Ue(m(v));else{var I=Ue(m(v.prefix)),J=Ue(m(v.suffix));if(v.pattern)if(t&&t.push(v),I||J)if(v.modifier==="+"||v.modifier==="*"){var K=v.modifier==="*"?"?":"";D+="(?:".concat(I,"((?:").concat(v.pattern,")(?:").concat(J).concat(I,"(?:").concat(v.pattern,"))*)").concat(J,")").concat(K)}else D+="(?:".concat(I,"(").concat(v.pattern,")").concat(J,")").concat(v.modifier);else{if(v.modifier==="+"||v.modifier==="*")throw new TypeError('Can not repeat "'.concat(v.name,'" without a prefix and suffix'));D+="(".concat(v.pattern,")").concat(v.modifier)}else D+="(?:".concat(I).concat(J,")").concat(v.modifier)}}if(l)a||(D+="".concat(L,"?")),D+=r.endsWith?"(?=".concat(j,")"):"$";else{var X=e[e.length-1],M=typeof X=="string"?L.indexOf(X[X.length-1])>-1:X===void 0;a||(D+="(?:".concat(L,"(?=").concat(j,"))?")),M||(D+="(?=".concat(L,"|").concat(j,")"))}return new RegExp(D,ga(r))}n(Nc,"tokensToRegexp");function fa(e,t,r){return e instanceof RegExp?Oc(e,t):Array.isArray(e)?Ic(e,t,r):Mc(e,t,r)}n(fa,"pathToRegexp");var Ft=/[.+?^${}()|[\]\\]/g;function*Uc(e){let t=new URL(e.url).pathname;for(let r of[...A].reverse()){if(r.method&&r.method!==e.method)continue;let o=ut(r.routePath.replace(Ft,"\\$&"),{end:!1}),a=ut(r.mountPath.replace(Ft,"\\$&"),{end:!1}),s=o(t),i=a(t);if(s&&i)for(let c of r.middlewares.flat())yield{handler:c,params:s.params,path:i.path}}for(let r of A){if(r.method&&r.method!==e.method)continue;let o=ut(r.routePath.replace(Ft,"\\$&"),{end:!0}),a=ut(r.mountPath.replace(Ft,"\\$&"),{end:!1}),s=o(t),i=a(t);if(s&&i&&r.modules.length){for(let c of r.modules.flat())yield{handler:c,params:s.params,path:s.path};break}}}n(Uc,"executeRequest");var jd={async fetch(e,t,r){let o=e,a=Uc(o),s={},i=!1,c=n(async(l,p)=>{if(l!==void 0){let f=l;typeof l=="string"&&(f=new URL(l,o.url).toString()),o=new Request(f,p)}let m=a.next();if(m.done===!1){let{handler:f,params:R,path:_}=m.value,x={request:new Request(o.clone()),functionPath:_,next:c,params:R,get data(){return s},set data(L){if(typeof L!="object"||L===null)throw new Error("context.data must be an object");s=L},env:t,waitUntil:r.waitUntil.bind(r),passThroughOnException:n(()=>{i=!0},"passThroughOnException")},j=await f(x);if(!(j instanceof Response))throw new Error("Your Pages function should return a Response");return $r(j)}else{let f=await t.ASSETS.fetch(o);return $r(f)}},"next");try{return await c()}catch(l){if(i){let p=await t.ASSETS.fetch(o);return $r(p)}throw l}}},$r=n(e=>new Response([101,204,205,304].includes(e.status)?null:e.body,e),"cloneResponse");export{jd as default};
