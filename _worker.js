...
`),g=new Map,b=n(({workspaceId:N="",canonicalPath:F="",line:w=""}={})=>{let E=p(F);if(!E)return;let Q=p(N),Pe=oe({workspaceId:Q,canonicalPath:E});g.has(Pe)||g.set(Pe,{id:Pe,title:_(E),url:L({workspaceId:Q,canonicalPath:E}),snippet:k(w||E),workspaceId:Q||r,canonicalPath:E})},"addEntry");for(let N of h){let F=/\/(?:api\/storage\/doc|knowgrph\/doc)\/([^/\s)\]]+)\/([^\s)\]]+)/g,w=/\/(?:api\/storage\/doc-default|knowgrph\/doc-default)\/([^\s)\]]+)/g;for(let E of N.matchAll(F))b({workspaceId:P(E[1]||""),canonicalPath:P(E[2]||""),line:N});for(let E of N.matchAll(w))b({workspaceId:"",canonicalPath:P(E[1]||""),line:N})}return Array.from(g.values())},"extractSearchEntriesFromSourceFilesIndex"),R=n(async(d={})=>{let h=p(d.query);if(!h)throw new Error("query is required");let g=Math.max(1,Math.min(25,Number.isFinite(Number(d.limit))?Math.floor(Number(d.limit)):10)),b=await a();if(!b.ok)throw new Error(`search failed with ${b.status}`);let N=await b.text(),F=we(N),w=W(h),E=w.join(" "),Q=F.map(K=>{let Z=`${K.title}
${K.canonicalPath}
${K.workspaceId}
${K.snippet}`.toLowerCase(),dt=E&&Z.includes(E)?w.length*4:0,mt=w.reduce((ht,qt)=>ht+(Z.includes(qt)?2:0),0);return{...K,score:dt+mt}}),Pe=Q.slice().sort((K,Z)=>Z.score-K.score||K.title.localeCompare(Z.title)).slice(0,H).filter(K=>/\.md(?:$|[?#])/i.test(K.canonicalPath)),ut=new Map;await J(Pe,async K=>{let Z=pe(K.id);if(!Z?.canonicalPath)return null;try{let dt=await s(o(Z.canonicalPath,Z.workspaceId));if(!dt.ok)return null;let mt=(await dt.text()).slice(0,U),ht=mt.toLowerCase(),qt=E&&ht.includes(E)?w.length*6:0,ya=T(ht,w),Dr=qt+ya;if(Dr<=0)return null;ut.set(K.id,{score:Dr,snippet:v(mt,w)})}catch{return null}return null});let zt=Q.map(K=>{let Z=ut.get(K.id);return{...K,score:K.score+(Z?.score||0),snippet:Z?.snippet||K.snippet}}).filter(K=>K.score>0).sort((K,Z)=>Z.score-K.score||K.title.localeCompare(Z.title)).slice(0,g).map(({score:K,...Z})=>Z);return{ids:zt.map(K=>K.id),results:zt,query:h,totalResults:zt.length}},"searchSourceFiles"),y=n(async(d={})=>{let h=pe(d.id);if(!h?.canonicalPath)throw new Error("id must be a stable Knowgrph Source File id returned by search");let g=await I(h),b=L(h);return{id:oe(h),title:_(g.canonicalPath),content:g.markdown,text:g.markdown,url:b,metadata:{workspaceId:g.workspaceId,canonicalPath:g.canonicalPath,contentType:"text/markdown",source:"knowgrph-source-files"}}},"fetchSourceFileBySearchId"),S={};return t.search&&(S[t.search]=R),t.fetch&&(S[t.fetch]=y),t.listSourceFiles&&(S[t.listSourceFiles]=async()=>{let d=await a();if(!d.ok)throw new Error(`list_source_files failed with ${d.status}`);return{workspaceId:r,markdownIndex:await d.text()}}),t.readSourceFile&&(S[t.readSourceFile]=I),t.readSharedDocument&&(S[t.readSharedDocument]=M),t.inspectSharedDocumentStructure&&(S[t.inspectSharedDocumentStructure]=j),t.inspectAgentSurface&&(S[t.inspectAgentSurface]=async()=>l()),S},"createPublishedAgentReadyToolExecutors"),Qa=n(e=>`((...args) => {
  const n = (value) => value
...
}`;var kt={push:"/api/storage/push",pull:"/api/storage/pull",collabSave:"/api/storage/collab/save",exportPrefix:"/api/storage/export/",docPrefix:"/api/storage/doc/",defaultDocPrefix:"/api/storage/doc-default/",blobPrefix:"/api/storage/blob/",sourceFilesIndex:"/api/storage/source-files",sourceFilesIndexPrefix:"/api/storage/source-files/",sourceFilesLlms:"/api/storage/llms.txt"};var Xo=n((e,t)=>`${kt.docPrefix}${encodeURIComponent(String(e||"").trim())}/${encodeURIComponent(String(t||"").trim())}`,"buildKnowgrphStorageDocPath"),Qo=n(e=>`${kt.defaultDocPrefix}${encodeURIComponent(String(e||"").trim())}`,"buildKnowgrphStorageDefaultDocPath");var Zo=n(e=>{let t=String(e||"").trim();return t?`${kt.sourceFilesIndexPrefix}${encodeURIComponent(t)}`:kt.sourceFilesIndex},"buildKnowgrphStorageSourceFilesIndexPath");var B="https://airvio.co",xt="https://knowgrph-storage.huijoohwee.workers.dev",C="/knowgrph",x=`${B}${C}/`,en=`${B}/`,rt="kgws:canonical-docs",Ae="2026-06-05",Ye=`${C}/health`,qe=`${B}${Ye}`,tn="/.well-known/agent-card.json",hr=`${C}/.well-known/agent-card.json`,Et=`${B}${tn}`,gr=`${B}/api/storage/source-files`,hs=`${B}/api/storage/doc-default/{canonicalPath}`,gs=`${B}/api/storage/doc/{workspaceId}/{canonicalPath}`,fs=`${B}/api/storage/blob/{workspaceId}/{canonicalPath}`,rn="knowgrph-agent-ready-pages";var on=['</.well-known/api-catalog>; rel="api-catalog"; type="application/linkset+json"',`<${C}/.well-known/openapi.json>; rel="service-desc"; type="application/vnd.oai.openapi+json;version=3.1"`,`<${C}/llms.txt>; rel="service-doc"; type="text/plain"`,'</auth.md>; rel="service-doc"; type="text/markdown"',`<${Ye}>; rel="status"; type="application/health+json"`,`<${C}/.well-known/mcp/server-card.json>; rel="mcp-server-card"; type="application/json"`,`<${tn}>; rel="describedby"; type="application/json"`].join(", "),nn=`# Knowgrph
...
${Wt()}`},"injectRootVisibleFallback"),Rc=n(e=>{let t=ma(e);return/<meta\s+name=["']description["'][^>]*>/i.test(t)?t=t.replace(/<meta\s+name=["']description["'][^>]*>/i,`<meta name="description" content="${Kr}" />`):t=Lr(t,`    <meta name="description" content="${Kr}" />
