...
`,"buildSitemapXml"),vs=Cs(`${x}sitemap.xml`),Os=Ts(x),bn={linkset:[{anchor:x,"service-desc":[{href:`${x}.well-known/openapi.json`,type:"application/vnd.oai.openapi+json;version=3.1"}],"service-doc":[{href:`${x}llms.txt`,type:"text/plain"}],status:[{href:qe,type:"application/health+json"}],"service-meta":[{href:`${x}.well-known/mcp/server-card.json`,type:"application/json"},{href:Ct,type:"application/json"}]}]},Rn={openapi:"3.1.0",info:{title:"Knowgrph API",version:"0.1.0",description:"Agent discovery surface for the Knowgrph Cloudflare deployment."},servers:[{url:G,description:"Knowgrph Cloudflare deployment"}],paths:Bo({appBasePath:T,appA2aAgentCardPath:hr,healthPath:Ve})},at={resource:x,resource_name:"Knowgrph",authorization_servers:[G],scopes_supported:["knowgrph:read","knowgrph:source-files:read"],bearer_methods_supported:["header"],resource_documentation:`${x}llms.txt`},yr=`${G}/cdn-cgi/access`,Ee={skill:`${G}/auth.md`,register_uri:`${x}agent/auth`,claim_uri:`${x}agent/auth/claim`,revocation_uri:`${x}agent/auth/revoke`,identity_types_supported:["anonymous","identity_assertion"],anonymous:{credential_types_supported:["api_key"]},identity_assertion:{assertion_types_supported:["urn:ietf:params:oauth:token-type:id-jag","verified_email"],credential_types_supported:["access_token","api_key"]},events_supported:["https://schemas.workos.com/events/agent/auth/identity/assertion/revoked"],registration_status:"metadata_published_runtime_user_mediated"},un={issuer:G,resource:at.resource,resource_name:at.resource_name,authorization_servers:at.authorization_servers,cloudflare_access_issuer:yr,authorization_endpoint:`${yr}/login`,token_endpoint:`${yr}/token`,jwks_uri:`${x}.well-known/http-message-signatures-directory`,response_types_supported:["code"],grant_types_supported:["authorization_code","client_credentials"],token_endpoint_auth_methods_supported:["client_secret_basic","private_key_jwt"],scopes_supported:at.scopes_supported,agent_auth:Ee},Is=`# Knowgrph auth.md
...
- Pipeline rule: agents must not bypass MainPanel -> FloatingPanel Chat -> KGC -> Canvas for user-mediated graph work; published HTTP MCP tools remain read-only until mutation auth and conflict semantics are implemented.`,Pn={name:"Knowgrph Agent",description:"Agent-readable discovery, published-document retrieval, and WebMCP-ready metadata surface for Knowgrph.",version:"0.1.0",provider:{organization:"airvio / joohwee",url:x},url:`${x}mcp`,preferredTransport:"JSONRPC",supportedInterfaces:[{url:`${x}mcp`,protocolBinding:"JSONRPC",transportProtocol:"JSONRPC",description:"Primary machine interface for read-only discovery and source-file document access."},{url:gr,protocolBinding:"HTTP+JSON/REST",transportProtocol:"HTTP+JSON/REST",description:"Published source-files index and storage-backed document read surface."}],capabilities:{streaming:!1,pushNotifications:!1,stateTransitionHistory:!1,extendedAgentCard:!1},defaultInputModes:["text/plain","text/markdown","application/json"],defaultOutputModes:["text/plain","text/markdown","application/json"],skills:Ho(vt)},Me={serverInfo:{name:"knowgrph",version:"0.1.0"},transport:{type:Ae,url:`${x}mcp`,stateless:!0},capabilities:{tools:vt.map(e=>({name:e.name,title:e.title,description:e.description,inputSchema:e.inputSchema,outputSchema:e.outputSchema,securitySchemes:e.securitySchemes,annotations:e.annotations,_meta:e._meta})),resources:{listChanged:!1},prompts:{listChanged:!1},...rr()},prompts:hn,resourceTemplates:gn,clientSetups:nr({baseUrl:x,mcpUrl:`${x}mcp`,serverName:"knowgrph"}),links:{apiCatalog:`${x}.well-known/api-catalog`,skills:`${x}.well-known/agent-skills/index.json`,status:qe,agentCard:Ct}},Ms=At({appUrl:x,updatedAt:ke}),Sr=vt.map(e=>({name:e.webName,title:e.title,description:e.description,inputSchema:e.inputSchema,outputSchema:e.outputSchema,securitySchemes:e.securitySchemes,annotations:e.annotations,_meta:e._meta})),De=n(e=>st(vt.find(t=>t.name===e)?.webName),"findWebMcpToolName"),Ns=De(u.search),Us=De(u.fetch),Ks=De(u.listSourceFiles),Ls=De(u.readSourceFile),Ds=De(u.readSharedDocument),$s=De(u.inspectSharedDocumentStructure),Hs=De(u.inspectAgentSurface),js=`(() => {
  ...
  const toolExecutors = createPublishedAgentReadyToolExecutors({
    toolNames: {
      ...
      listSourceFiles: ${JSON.stringify(Ks)},
      readSourceFile: ${JSON.stringify(Ls)},
      readSharedDocument: ${JSON.stringify(Ds)},
      inspectSharedDocumentStructure: ${JSON.stringify($s)},
...
${Wt()}`},"injectRootVisibleFallback"),Ac=n(e=>{let t=ma(e);return/<meta\s+name=["']description["'][^>]*>/i.test(t)?t=t.replace(/<meta\s+name=["']description["'][^>]*>/i,`<meta name="description" content="${Lr}" />`):t=Kr(t,`    <meta name="description" content="${Lr}" />
