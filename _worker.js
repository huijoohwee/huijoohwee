...
${$.canonicalPath}
${$.workspaceId}
${$.snippet}`.toLowerCase(),dt=E&&Z.includes(E)?w.length*4:0,mt=w.reduce((ht,qt)=>ht+(Z.includes(qt)?2:0),0);return{...$,score:dt+mt}}),Pe=Q.slice().sort(($,Z)=>Z.score-$.score||$.title.localeCompare(Z.title)).slice(0,j).filter($=>/\.md(?:$|[?#])/i.test($.canonicalPath)),ut=new Map;await J(Pe,async $=>{let Z=pe($.id);if(!Z?.canonicalPath)return null;try{let dt=await s(o(Z.canonicalPath,Z.workspaceId));if(!dt.ok)return null;let mt=(await dt.text()).slice(0,K),ht=mt.toLowerCase(),qt=E&&ht.includes(E)?w.length*6:0,ya=T(ht,w),Dr=qt+ya;if(Dr<=0)return null;ut.set($.id,{score:Dr,snippet:I(mt,w)})}catch{return null}return null});let zt=Q.map($=>{let Z=ut.get($.id);return{...$,score:$.score+(Z?.score||0),snippet:Z?.snippet||$.snippet}}).filter($=>$.score>0).sort(($,Z)=>Z.score-$.score||$.title.localeCompare(Z.title)).slice(0,g).map(({score:$,...Z})=>Z);return{ids:zt.map($=>$.id),results:zt,query:h,totalResults:zt.length}},"searchSourceFiles"),y=n(async(d={})=>{let h=pe(d.id);if(!h?.canonicalPath)throw new Error("id must be a stable Knowgrph Source File id returned by search");let g=await M(h),b=L(h);return{id:oe(h),title:_(g.canonicalPath),content:g.markdown,text:g.markdown,url:b,metadata:{workspaceId:g.workspaceId,canonicalPath:g.canonicalPath,contentType:"text/markdown",source:"knowgrph-source-files"}}},"fetchSourceFileBySearchId"),S={};return t.search&&(S[t.search]=P),t.fetch&&(S[t.fetch]=y),t.listSourceFiles&&(S[t.listSourceFiles]=async()=>{let d=await a();if(!d.ok)throw new Error(`list_source_files failed with ${d.status}`);return{workspaceId:r,markdownIndex:await d.text()}}),t.readSourceFile&&(S[t.readSourceFile]=M),t.readSharedDocument&&(S[t.readSharedDocument]=N),t.inspectSharedDocumentStructure&&(S[t.inspectSharedDocumentStructure]=B),t.inspectAgentSurface&&(S[t.inspectAgentSurface]=async()=>l()),S},"createPublishedAgentReadyToolExecutors"),Qa=n(e=>`((...args) => {
  const n = (value) => value
