...
`,"buildSitemapXml"),Ls=Ns(`${x}sitemap.xml`),Ks=Us(x),En={linkset:[{anchor:x,"service-desc":[{href:`${x}.well-known/openapi.json`,type:"application/vnd.oai.openapi+json;version=3.1"}],"service-doc":[{href:`${x}llms.txt`,type:"text/plain"}],status:[{href:qe,type:"application/health+json"}],"service-meta":[{href:`${x}.well-known/mcp/server-card.json`,type:"application/json"},{href:vt,type:"application/json"}]}]},Cn={openapi:"3.1.0",info:{title:"Knowgrph API",version:"0.1.0",description:"Agent discovery surface for the Knowgrph Cloudflare deployment."},servers:[{url:G,description:"Knowgrph Cloudflare deployment"}],paths:Vo({appBasePath:C,appA2aAgentCardPath:yr,healthPath:Ve})},at={resource:x,resource_name:"Knowgrph",authorization_servers:[G],scopes_supported:["knowgrph:read","knowgrph:source-files:read"],bearer_methods_supported:["header"],resource_documentation:`${x}llms.txt`},_r=`${G}/cdn-cgi/access`,Ee={skill:`${G}/auth.md`,register_uri:`${x}agent/auth`,claim_uri:`${x}agent/auth/claim`,revocation_uri:`${x}agent/auth/revoke`,identity_types_supported:["anonymous","identity_assertion"],anonymous:{credential_types_supported:["api_key"]},identity_assertion:{assertion_types_supported:["urn:ietf:params:oauth:token-type:id-jag","verified_email"],credential_types_supported:["access_token","api_key"]},events_supported:["https://schemas.workos.com/events/agent/auth/identity/assertion/revoked"],registration_status:"metadata_published_runtime_user_mediated"},yn={issuer:G,resource:at.resource,resource_name:at.resource_name,authorization_servers:at.authorization_servers,cloudflare_access_issuer:_r,authorization_endpoint:`${_r}/login`,token_endpoint:`${_r}/token`,jwks_uri:`${x}.well-known/http-message-signatures-directory`,response_types_supported:["code"],grant_types_supported:["authorization_code","client_credentials"],token_endpoint_auth_methods_supported:["client_secret_basic","private_key_jwt"],scopes_supported:at.scopes_supported,agent_auth:Ee},Ds=`# Knowgrph auth.md
...
${zt()}`},"injectRootVisibleFallback"),vc=n(e=>{let t=Sa(e);return/<meta\s+name=["']description["'][^>]*>/i.test(t)?t=t.replace(/<meta\s+name=["']description["'][^>]*>/i,`<meta name="description" content="${Hr}" />`):t=$r(t,`    <meta name="description" content="${Hr}" />
`),/<link\s+rel=["']canonical["'][^>]*>/i.test(t)||(t=$r(t,`    <link rel="canonical" href="${le}/" />
`)),/<meta\s+name=["']x-knowgrph-root-alias["'][^>]*>/i.test(t)||(t=$r(t,`    <meta name="x-knowgrph-root-alias" content="${le}/" />
