var Ba=Object.defineProperty;var n=(e,t)=>Ba(e,"name",{value:t,configurable:!0});var Wa="https://api.openai.com/v1";var rr=Object.freeze(["gpt-5.4-nano","gpt-4o-mini"]);function or(e){return String(e||"").trim()}n(or,"normalizeOrigin");function Fa(e){let t=or(e);return t?t.startsWith("http://localhost:")||t.startsWith("http://127.0.0.1:")||t.startsWith("http://0.0.0.0:"):!1}n(Fa,"isAllowedOrigin");function to(e){let t=or(e);return Fa(t)?{"access-control-allow-origin":t,vary:"Origin","access-control-allow-methods":"GET, POST, OPTIONS","access-control-allow-headers":"content-type, x-flowinfish-session","access-control-max-age":"86400"}:{}}n(to,"corsHeaders");function _e(e,{status:t=200,origin:r=""}={}){return new Response(JSON.stringify(e),{status:t,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store",...to(r)}})}n(_e,"json");async function Rt(e,{maxBytes:t=1e6}={}){let r=await e.arrayBuffer();if(r.byteLength>t)throw new Error("Request too large");let o=new TextDecoder().decode(r);try{return o?JSON.parse(o):{}}catch{throw new Error("Invalid JSON body")}}n(Rt,"readJsonBody");function za(e){let t=String(e?.model||"").trim();if(!t)throw new Error("Missing model");if(!rr.includes(t))throw new Error(`Model not allowed: ${t}`);return t}n(za,"enforceAllowedModel");function qa(e){let t=String(e.OPENAI_API_KEY||"").trim();if(!t)throw new Error("Missing server OPENAI_API_KEY");return t}n(qa,"requireOpenAiKey");async function At({request:e,env:t,pathname:r,payload:o}){let a=qa(t);za(o);let i=`${or(t.OPENAI_API_BASE)||Wa}${r}`,c=await fetch(i,{method:"POST",headers:{authorization:`Bearer ${a}`,"content-type":"application/json"},body:JSON.stringify(o)}),l=new Headers(c.headers);return l.delete("content-length"),l.set("cache-control","no-store"),new Response(c.body,{status:c.status,headers:l})}n(At,"proxyToOpenAi");function Qe(e){let t=e.headers.get("origin")||"";return new Response(null,{status:204,headers:{...to(t)}})}n(Qe,"handleOptions");async function ro(e){let{request:t,env:r}=e,o=String(t.method||"GET").toUpperCase(),a=t.headers.get("origin")||"";if(o==="OPTIONS")return Qe(t);if(o!=="POST")return _e({ok:!1,error:"Method not allowed"},{status:405,origin:a});try{if(!String(t.headers.get("content-type")||"").toLowerCase().includes("application/json"))return _e({ok:!1,error:"Expected application/json"},{status:415,origin:a});let i=await Rt(t);return await At({request:t,env:r,pathname:"/chat/completions",payload:i})}catch(s){let i=s instanceof Error?s.message:String(s||"Unknown error");return _e({ok:!1,error:i},{status:400,origin:a})}}n(ro,"onRequest");var Pt={checkoutSession:"/api/payments/stripe/checkout/session",webhook:"/api/payments/stripe/webhook"};var fe={restrictedKey:"STRIPE_RESTRICTED_KEY",secretKey:"STRIPE_SECRET_KEY",webhookSecret:"STRIPE_WEBHOOK_SECRET",checkoutPriceId:"STRIPE_CHECKOUT_PRICE_ID",checkoutCurrency:"STRIPE_CHECKOUT_CURRENCY",checkoutUnitAmount:"STRIPE_CHECKOUT_UNIT_AMOUNT",checkoutProductName:"STRIPE_CHECKOUT_PRODUCT_NAME",checkoutMode:"STRIPE_CHECKOUT_MODE",checkoutReturnOrigin:"STRIPE_CHECKOUT_RETURN_ORIGIN"},Cl=[fe.restrictedKey,fe.secretKey,fe.webhookSecret];var at={configure:"npm run payment:stripe:configure",d1MigrateRemote:"npm run payment:d1:migrate:remote",readiness:"npm run payment:stripe:readiness",x402Configure:"npm run payment:x402:configure",x402Readiness:"npm run payment:x402:readiness",paymentReadiness:"npm run payment:readiness",applyConfirmation:"apply-stripe-payment-worker-config",writeVisibleVarsFlag:"--write-visible-vars",deployVisibleVarsFlag:"--deploy-visible-vars"};var Tl=[`Configure Stripe secrets on the server runtime that owns ${Pt.checkoutSession}.`,"Cloudflare Pages project variables are available to Pages builds/functions, but they are not read by separate Worker routes.","Stripe Projects can provision and sync credentials locally; copy only required server secret names into the payment server runtime."].join(" "),vl=[`Payment server runtime for ${Pt.checkoutSession}`,"not Cloudflare Pages project variables"].join("; "),Va=[fe.restrictedKey,fe.secretKey].join(" or "),Ol=[fe.checkoutPriceId,`${fe.checkoutCurrency} + ${fe.checkoutUnitAmount} + ${fe.checkoutProductName}`].join(" or "),Il=[`${fe.checkoutMode}=payment`,`${fe.checkoutMode}=subscription with ${fe.checkoutPriceId}`].join(" or "),Ml=["Worker secret names","visible Worker [vars]","remote D1 payment tables","required webhook-processing columns/constraints","bounded optional hosted Checkout create-and-expire smoke"].join(" + "),Nl=[at.configure,`write visible Worker [vars] with ${at.writeVisibleVarsFlag}`,`deploy visible Worker [vars] with ${at.deployVisibleVarsFlag}`,`apply with -- --apply --yes --confirm=${at.applyConfirmation}`,at.readiness].join(" -> "),Ul=[`Missing server-managed Stripe key. Set ${Va} on the payment server runtime.`,"Pages project variables alone do not satisfy separate Worker routes."].join(" ");var Ya=n(e=>{let t=2166136261;for(let r=0;r<e.length;r+=1)t^=e.charCodeAt(r),t=Math.imul(t,16777619);return t>>>0},"fnv1a32");function oo(e){return Ya(String(e??""))}n(oo,"hashString32");function st(e){return oo(e).toString(16).padStart(8,"0")}n(st,"hashStringToHex");var Ja=n(e=>e==null?"":typeof e=="boolean"?e?"1":"0":typeof e=="number"?Number.isFinite(e)?String(e):"":String(e),"normalizePrimitive"),Xa=n(e=>e.map(Ja).join("|"),"buildSignatureText"),kt=n(e=>st(Xa(e)),"hashSignatureParts");var xe=n((e,t)=>kt(["agentic-commerce",e,...t]),"buildAgenticCommerceSemanticKey");var no="solana_pay",ao="/api/payments/commerce/solana-pay/settle";var Ke={recipient:"SOLANA_PAY_RECIPIENT",splToken:"SOLANA_PAY_SPL_TOKEN",label:"SOLANA_PAY_LABEL",rpcUrl:"SOLANA_PAY_RPC_URL",amountScale:"SOLANA_PAY_AMOUNT_SCALE",network:"SOLANA_PAY_NETWORK",commitment:"SOLANA_PAY_COMMITMENT"};var it="2026-01-30",io="1000",co="USDC",nr="https://x402.org/facilitator",lo="eip155:84532",Qa="$0.001",po="x402-payment-required",Za="0x0000000000000000000000000000000000000000",Ze="2026-04-08",so="https://ucp.dev/2026-04-08/specification/overview/",es=["checkout"],ts=["rest"];var I={acpDiscovery:"/.well-known/acp.json",acpConfig:"/.well-known/acp-config",ucpProfile:"/.well-known/ucp",mppOpenApi:"/openapi.json",x402ApiRoot:"/api",x402ApiV1:"/api/v1",checkoutSessions:"/checkout/sessions",x402PaymentRequired:"/api/payments/commerce/x402",commerceWebhook:"/api/payments/commerce/webhook",commerceProofArtifact:"/api/payments/commerce/harness-proof.json",commerceTraceArtifact:"/api/payments/commerce/trace.jsonl",openboxIngest:"/api/payments/commerce/openbox/ingest",web3Settle:"/api/payments/commerce/web3/settle",solanaPaySettle:ao},op=[I.x402ApiRoot,I.x402ApiV1,I.x402PaymentRequired],W={sellerId:"SELLER_ID",checkoutBaseUrl:"CHECKOUT_BASE_URL",web3Enabled:"WEB3_ENABLED",web3DepositAddress:"WEB3_DEPOSIT_ADDRESS",baseRpcUrl:"BASE_RPC_URL",baseConfirmationBlocks:"BASE_CONFIRMATION_BLOCKS",easAttestUrl:"EAS_ATTEST_URL",openboxApiUrl:"OPENBOX_API_URL",openboxIngestUrl:"OPENBOX_INGEST_URL",openboxApiKey:"OPENBOX_API_KEY",stripeDelegatePaymentUrl:"STRIPE_DELEGATE_PAYMENT_URL",acpBearerToken:"ACP_BEARER_TOKEN",x402PayToAddress:"X402_PAY_TO_ADDRESS",x402Network:"X402_NETWORK",x402Asset:"X402_ASSET",x402Amount:"X402_AMOUNT",x402FacilitatorUrl:"X402_FACILITATOR_URL",x402Price:"X402_PRICE"},np=[W.sellerId,W.checkoutBaseUrl,W.web3Enabled,W.web3DepositAddress,W.baseRpcUrl,W.baseConfirmationBlocks,W.easAttestUrl,W.openboxApiUrl,W.openboxIngestUrl,W.stripeDelegatePaymentUrl,W.x402Network,W.x402Asset,W.x402Amount,W.x402FacilitatorUrl,W.x402Price,Ke.recipient,Ke.rpcUrl],ap=[Ke.splToken,Ke.label,Ke.amountScale,Ke.network,Ke.commitment],sp=[W.acpBearerToken,W.openboxApiKey],ip=["Visible Worker [vars] for ACP, Web3, x402, OpenBOX, and Solana Pay.","Worker secrets for ACP bearer auth and OpenBOX API access."].join(" "),$e=n((e,t)=>String(e[t]||"").trim(),"readEnvString"),uo=n((e,t)=>{let r=$e(e,W.sellerId);if(r)return r;try{return new URL(t).host}catch{return"knowgrph-seller"}},"readAgenticCommerceSellerId");var mo=n(e=>{let t=$e(e,W.web3Enabled).toLowerCase();return t?t==="0"||t==="false"||t==="no"?!1:t==="1"||t==="true"||t==="yes":!0},"isAgenticCommerceWeb3Enabled");var ct=n(e=>String(e||"").trim().replace(/\/+$/g,""),"normalizeAgenticCommerceBaseUrl"),de=n((e,t)=>`${ct(e)}${t}`,"buildAgenticCommerceUrl"),oe=n((e,t,r,o,a=o.startsWith("/")?o:null)=>({id:t,label:r,value:o,path:a,semanticKey:xe("mainpanel-commerce-readiness-row",[e,t,r,o,a||""])}),"buildAgenticCommerceMainPanelReadinessRow"),et=n((e,t,r)=>({id:e,title:t,rows:r}),"buildAgenticCommerceMainPanelReadinessSection"),rs=n(()=>{let e=[et("overview","Overview",[oe("overview","acp-discovery","ACP discovery",`GET ${I.acpDiscovery}`,I.acpDiscovery),oe("overview","acp-config","ACP config",`GET ${I.acpConfig}`,I.acpConfig),oe("overview","api-version","API version",it,null)]),et("discovery","Discovery",[oe("discovery","ucp-profile","UCP profile",I.ucpProfile),oe("discovery","mpp-openapi","MPP OpenAPI",I.mppOpenApi),oe("discovery","x402-payment-required","x402 payment required",I.x402PaymentRequired),oe("discovery","x402-api-root","x402 API root",I.x402ApiRoot)]),et("sessions","Sessions",[oe("sessions","checkout-sessions","Checkout sessions",I.checkoutSessions),oe("sessions","stripe-webhook","Stripe webhook",Pt.webhook)]),et("web3","Web3",[oe("web3","settle","Settle",I.web3Settle),oe("web3","solana-pay-settle","Solana Pay settle",I.solanaPaySettle),oe("web3","signals","Signals","Base RPC + Solana RPC confirmation",null)]),et("governance","Governance",[oe("governance","openbox-ingest","OpenBOX ingest",I.openboxIngest),oe("governance","risk-source","Risk source","OpenBOX risk signal",null)]),et("proofs","Proofs",[oe("proofs","harness-proof","Harness proof",I.commerceProofArtifact),oe("proofs","trace-artifact","Trace artifact",I.commerceTraceArtifact)])],t=e.flatMap(a=>a.rows),r=t.map(a=>a.path||"").filter(a=>a.length>0),o=t.filter(a=>!a.path).map(a=>`${a.label}: ${a.value}`);return{surface:"mainpanel-commerce",semanticKey:xe("mainpanel-commerce-readiness",[it,...t.map(a=>a.semanticKey)]),sections:e,routePaths:r,routeCount:r.length,signals:o}},"buildAgenticCommerceMainPanelReadiness"),cp=rs(),ho=n((e,t)=>{let r=$e(e,W.web3DepositAddress);if(/^0x[0-9a-fA-F]{40}$/.test(r))return r;let o=xe("deposit-address",[t,"0"]),a=xe("deposit-address",[t,"1"]),s=xe("deposit-address",[t,"2"]),i=xe("deposit-address",[t,"3"]),c=xe("deposit-address",[t,"4"]);return`0x${o}${a}${s}${i}${c}`.slice(0,42)},"buildAgenticCommerceDepositAddress");var go=n((e,t=po)=>{let r=$e(e,W.x402PayToAddress);return/^0x[0-9a-fA-F]{40}$/.test(r)&&r.toLowerCase()!==Za?r:ho(e,t)},"readAgenticCommerceX402PayToAddress"),lp=ho({},po),os=/^[a-z0-9]{3,8}:[-_a-zA-Z0-9]{1,64}$/,fo=n(e=>{let t=$e(e,W.x402Network);return os.test(t)?t:lo},"readAgenticCommerceX402Network"),yo=n(e=>$e(e,W.x402Asset)||co,"readAgenticCommerceX402Asset"),wo=n(e=>{let t=$e(e,W.x402Amount);return/^[1-9][0-9]*$/.test(t)?t:io},"readAgenticCommerceX402Amount");var So=n(e=>{let t=$e(e,W.x402FacilitatorUrl);try{let r=new URL(t||nr);return r.protocol==="https:"||r.protocol==="http:"?r.toString().replace(/\/+$/g,""):nr}catch{return nr}},"readAgenticCommerceX402FacilitatorUrl"),_o=n(e=>{let t=ct(e.baseUrl);return{protocol:{name:"acp",version:it,supported_versions:[it],documentation_url:"https://agenticcommerce.dev"},api_base_url:t,transports:[...ts],capabilities:{services:[...es],...e.web3Enabled?{extensions:[{name:"x-web3"}]}:{}},links:{config:de(t,I.acpConfig),ucp:de(t,I.ucpProfile),mpp:de(t,I.mppOpenApi),x402:de(t,I.x402PaymentRequired)}}},"buildAgenticCommerceAcpDiscovery"),bo=n(e=>{let t=ct(e.baseUrl),r={acp:de(t,I.acpDiscovery),api:de(t,I.x402ApiRoot),checkout_sessions:de(t,I.checkoutSessions),mpp_openapi:de(t,I.mppOpenApi),proof:de(t,I.commerceProofArtifact),trace:de(t,I.commerceTraceArtifact),x402_payment_required:de(t,I.x402PaymentRequired),solana_pay_settle:de(t,I.solanaPaySettle)},o={checkout_sessions:!0,content_payments:!0,proof_artifacts:!0,risk_signals:!0,web3_settlement:e.web3Enabled,solana_pay:e.web3Enabled},a={"dev.ucp.shopping":[{version:Ze,spec:so,transport:"rest",endpoint:r.api,schema:"https://ucp.dev/2026-04-08/services/shopping/rest.openapi.json"}]};return{ucp:{version:Ze,protocol_version:Ze,services:a,capabilities:{"dev.ucp.shopping.checkout":[{version:Ze,spec:"https://ucp.dev/2026-04-08/specification/checkout/",schema:"https://ucp.dev/2026-04-08/schemas/shopping/checkout.json"}]},payment_handlers:{},endpoints:r},protocol_version:Ze,protocol:{name:"ucp",version:Ze},seller:{id:e.sellerId},services:[{id:"knowgrph-content-payments",type:"content-payments",endpoints:{x402:r.x402_payment_required,checkout_sessions:r.checkout_sessions,solana_pay_settle:r.solana_pay_settle,proof:r.proof,trace:r.trace}}],capabilities:o,endpoints:r,spec_urls:[so],schema_urls:["https://ucp.dev/2026-04-08/services/shopping/rest.openapi.json","https://ucp.dev/2026-04-08/schemas/shopping/checkout.json"]}},"buildAgenticCommerceUcpProfile"),Ro=n(e=>{let t=ct(e.baseUrl);return{openapi:"3.1.0",info:{title:"Knowgrph Machine Payment Protocol",version:it,description:"Machine-readable payable-operation discovery for Knowgrph commerce routes."},servers:[{url:t}],paths:{[I.x402PaymentRequired]:{get:{operationId:"getKnowgrphX402PaymentRequirement",summary:"Return x402 payment requirements for an agent-readable paid resource.","x-payment-info":{intent:"charge",method:"x402",amount:Qa,currency:"usdc"},responses:{402:{description:"Payment Required"}}}},[I.checkoutSessions]:{post:{operationId:"createKnowgrphCommerceCheckoutSession",summary:"Create an agentic commerce checkout session.","x-payment-info":{intent:"session",method:"stripe",amount:"dynamic",currency:"request.currency"},responses:{201:{description:"Checkout session created"}}}},[I.solanaPaySettle]:{post:{operationId:"settleKnowgrphSolanaPayCheckoutSession",summary:"Settle an agentic commerce checkout session from a verified Solana Pay transaction signature.","x-payment-info":{intent:"settlement",method:no,amount:"dynamic",currency:"request.currency"},responses:{200:{description:"Solana Pay session settled"},409:{description:"Solana Pay transaction is not confirmed yet"},422:{description:"Solana Pay transaction does not match the session"}}}}}}},"buildAgenticCommerceMppOpenApi"),Ao=n(e=>{let t=ct(e.baseUrl),r=de(t,I.x402PaymentRequired),o=String(e.amount||io);return{x402Version:2,error:"Payment required",resource:{url:r,description:"Knowgrph agentic commerce paid-resource readiness probe",mimeType:"application/json"},accepts:[{scheme:"exact",network:String(e.network||lo),amount:o,maxAmountRequired:o,asset:String(e.asset||co),resource:r,mimeType:"application/json",payTo:e.payTo,maxTimeoutSeconds:300,extra:{name:"USDC",version:"2",resourceUrl:r,...e.facilitatorUrl?{facilitatorUrl:e.facilitatorUrl}:{}}}]}},"buildAgenticCommerceX402PaymentRequired");var ns=n(e=>JSON.stringify(e,null,2),"jsonBody"),as=n(e=>String(e||"").trim().replace(/\/+$/g,""),"trimOrigin"),ss=n(e=>typeof btoa=="function"?btoa(e):typeof Buffer<"u"?Buffer.from(e).toString("base64"):"","encodeBase64"),is=n((e,t)=>{try{return new URL(e).origin}catch{return as(t)}},"rootOriginFromRequest"),ar=n((e={})=>{let t=is(e.requestUrl,e.origin),r=e.env||{},o=uo(r,`${t}/`),a=mo(r),s=Ao({baseUrl:t,payTo:go(r),network:fo(r),asset:yo(r),amount:wo(r),facilitatorUrl:So(r)});return{acpDiscovery:_o({sellerId:o,baseUrl:t,web3Enabled:a}),ucpProfile:bo({sellerId:o,baseUrl:t,web3Enabled:a}),mppOpenApi:Ro({baseUrl:t}),x402PaymentRequired:s}},"buildKnowgrphCommerceDiscovery");var Po=n((e,t={})=>{let r=ar({requestUrl:e?.url,env:t}).x402PaymentRequired,o=ss(JSON.stringify(r));return new Response(ns(r),{status:402,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store","access-control-allow-origin":"*",...o?{"payment-required":o}:{}}})},"buildKnowgrphX402PaymentRequiredResponse");async function ko(e){return Po(e.request,e.env||{})}n(ko,"onRequest");async function xo(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase(),o=t.headers.get("origin")||"";return r==="OPTIONS"?Qe(t):r!=="GET"&&r!=="HEAD"?_e({ok:!1,error:"Method not allowed"},{status:405,origin:o}):_e({ok:!0,models:rr.map(a=>({model:a,display_name:a}))},{status:200,origin:o})}n(xo,"onRequest");async function Eo(e){let{request:t,env:r}=e,o=String(t.method||"GET").toUpperCase(),a=t.headers.get("origin")||"";if(o==="OPTIONS")return Qe(t);if(o!=="POST")return _e({ok:!1,error:"Method not allowed"},{status:405,origin:a});try{if(!String(t.headers.get("content-type")||"").toLowerCase().includes("application/json"))return _e({ok:!1,error:"Expected application/json"},{status:415,origin:a});let i=await Rt(t);return await At({request:t,env:r,pathname:"/responses",payload:i})}catch(s){let i=s instanceof Error?s.message:String(s||"Unknown error");return _e({ok:!1,error:i},{status:400,origin:a})}}n(Eo,"onRequest");var tt=Object.freeze({researchSourceFiles:"knowgrph_research_source_files",inspectAgentSurface:"knowgrph_inspect_agent_surface"}),sr=n(e=>String(e||"").trim(),"normalizeString"),cs=n(e=>({...e,arguments:Array.isArray(e.arguments)?e.arguments.map(t=>({...t})):void 0,_meta:e._meta&&typeof e._meta=="object"?{...e._meta,tools:Array.isArray(e._meta.tools)?[...e._meta.tools]:void 0}:void 0}),"clonePrompt"),To=Object.freeze([Object.freeze({name:tt.researchSourceFiles,title:"Research Knowgrph Source Files",description:"Guide an MCP host through read-only Knowgrph Source Files research using search and fetch with citation-ready URLs.",arguments:Object.freeze([Object.freeze({name:"query",description:"Research question or topic to pass to the read-only search tool.",required:!0}),Object.freeze({name:"limit",description:"Optional decimal string for the maximum search results to inspect.",required:!1}),Object.freeze({name:"focus",description:"Optional aspect to prioritize when reading fetched Source Files.",required:!1})]),_meta:Object.freeze({readOnly:!0,tools:Object.freeze(["search","fetch"])})}),Object.freeze({name:tt.inspectAgentSurface,title:"Inspect Knowgrph Agent Surface",description:"Guide an MCP host through read-only inspection of Knowgrph agent, MCP, and MCP Apps readiness metadata.",arguments:Object.freeze([Object.freeze({name:"focus",description:"Optional readiness area to emphasize, such as transport, tools, resources, prompts, retrieval, or app metadata.",required:!1})]),_meta:Object.freeze({readOnly:!0,tools:Object.freeze(["inspect_agent_surface"])})})]),vo=n(()=>To.map(cs),"buildKnowgrphAgentReadyPromptContracts"),ls=n(e=>To.find(t=>t.name===sr(e))||null,"findPromptContract"),xt=n((e,t)=>!e||typeof e!="object"?"":sr(e[t]),"readPromptArg"),ps=n((e,t)=>{let r=xt(e,t);if(!r)throw new Error(`Missing required prompt argument: ${t}`);return r},"readRequiredPromptArg"),Co=n(e=>({role:"user",content:{type:"text",text:e}}),"buildPromptMessage"),us=n((e={})=>{let t=ps(e,"query"),r=xt(e,"limit"),o=xt(e,"focus");return[`Research Knowgrph Source Files for: ${t}`,"","Use the MCP server read-only retrieval path:",`1. Call search with query=${JSON.stringify(t)}${r?` and limit=${JSON.stringify(r)}`:""}.`,"2. Select the most relevant returned ids and call fetch for each id before answering.","3. Ground the answer in fetched markdown content and cite the returned result URLs when summarizing.",o?`4. Prioritize this focus: ${o}.`:"","","Do not mutate graph, canvas, workspace, storage, or browser-local state for this research prompt."].filter(Boolean).join(`
`)},"buildSourceFilesResearchPromptText"),ds=n((e={})=>{let t=xt(e,"focus");return["Inspect the Knowgrph agent-ready surface through the read-only inspect_agent_surface tool.","","Review health, API catalog, MCP server card, A2A card, agent skills, commerce discovery, and mcpAppsServerReadiness.","For MCP Apps readiness, verify tool/resource linkage, output schema, text fallback, structured content, sandbox/security metadata, no-auth security-scheme mirroring, widget accessibility, prompts, search/fetch retrieval, Streamable HTTP, and local stdio support.",t?`Emphasize this readiness area: ${t}.`:"","","Report checklist ids and evidence from structuredContent. Do not infer readiness from prose alone."].filter(Boolean).join(`
`)},"buildAgentSurfaceInspectionPromptText"),Oo=n((e,t={})=>{let r=ls(e);if(!r)throw new Error(`Unknown Knowgrph MCP prompt: ${sr(e)}`);if(r.name===tt.researchSourceFiles)return{description:r.description,messages:[Co(us(t))]};if(r.name===tt.inspectAgentSurface)return{description:r.description,messages:[Co(ds(t))]};throw new Error(`Unhandled Knowgrph MCP prompt: ${r.name}`)},"getKnowgrphAgentReadyPrompt");var ms=Object.freeze({sourceFileById:"knowgrph_source_file_by_id"}),Et="kgdoc://source-file/{id}",Io="kgdoc://source-file/",Mo="text/markdown",lt=n(e=>String(e||"").trim(),"normalizeString"),No=n(()=>[{uriTemplate:Et,name:ms.sourceFileById,title:"Knowgrph Source File By ID",description:"Read a complete published Knowgrph Source File markdown document using a stable kgdoc id returned by search.",mimeType:Mo,annotations:{audience:["user","assistant"],priority:.8},_meta:{readOnly:!0,source:"knowgrph-source-files",tool:"fetch"}}],"buildKnowgrphAgentReadyResourceTemplateContracts");var Uo=n(e=>{let t=lt(e);if(!t.startsWith(Io))return"";let r=t.slice(Io.length);if(!r)return"";try{return decodeURIComponent(r)}catch{return r}},"parseKnowgrphSourceFileResourceUri"),Lo=n(({uri:e,sourceFile:t}={})=>{let r=typeof t?.content=="string"?t.content:String(t?.text||"");return{contents:[{uri:lt(e),mimeType:Mo,text:r,_meta:{id:lt(t?.id),title:lt(t?.title),url:lt(t?.url),metadata:t?.metadata&&typeof t.metadata=="object"?{...t.metadata}:{}}}]}},"buildKnowgrphSourceFileResourceReadResult");var Ee=n(e=>String(e||"").trim(),"normalizeString"),ve=n(e=>Ee(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),"escapeHtml");var ir="Map intent. Orchestrate agents. Prove outcomes.",cr="A source-backed canvas where / routes work, # sets meaning, and @ binds context.",Oe="knowgrph.agentic_canvas_os.docs.invoke",Ct=Object.freeze(["/mcp.capabilities","#mcp","@mcp-gateway"]),lr="One canonical operator contract: install and discovery stay on the public endpoint, while live /, #, @ grammar stays on the approval-gated control plane or an app-owned forwarder until the host proves MCP session support.",pr="Canonicalize the contract first, not the transport. Keep the runtime split underneath until hosted proof supports a single runtime.",hs=Object.freeze(["Lovable","Vercel"]),ur="Hosted app builders such as Lovable and Vercel should keep /mcp for discovery and use an app-owned forwarder for live /, #, @ unless the host proves MCP session support.";var dr=n(({publicReadMcpUrl:e,controlPlaneMcpUrl:t}={})=>({promise:ir,grammarSummary:cr,canonicalOperatorContract:lr,canonicalTransportRule:pr,publicReadMcpUrl:Ee(e),controlPlaneMcpUrl:Ee(t),controlPlaneCondition:"Add the control plane only when the host can preserve MCP session state and needs live /, #, @ grammar lookup.",grammarToolName:Oe,grammarExamples:Ct.map(r=>Ee(r)),grammarExecutionBoundary:"Keep install on the public discovery endpoint and execute live grammar on the approval-gated control plane.",hostedBuilderExamples:hs.map(r=>Ee(r)),hostedGrammarDefaultPath:ur,hostedGrammarFallback:"app-owned-forwarder",cheapestProofPath:"Use the source-side README.md quick start or docs/documents/knowgrph-superagent-harness.md in the knowgrph repo before hosted setup.",steps:[{order:1,label:"Map intent",action:e?`Map intent: install ${Ee(e)} first for public discovery, retrieval, and inspection.`:"Map intent: install the public MCP endpoint first for discovery, retrieval, and inspection."},{order:2,label:"Orchestrate agents",action:t?`Orchestrate agents: add ${Ee(t)} only when the host can preserve MCP session state and needs live /, #, @ grammar lookup through ${Oe}.`:`Orchestrate agents: add the control plane only when the host can preserve MCP session state and needs live /, #, @ grammar lookup through ${Oe}.`},{order:3,label:"Prove outcomes",action:"Prove outcomes: for zero-spend evaluation, run the source-side README.md quick start or docs/documents/knowgrph-superagent-harness.md first."}]}),"buildMcpOnboarding"),Ko=n(({publicReadMcpUrl:e,controlPlaneMcpUrl:t}={})=>`<section aria-label="Fastest path">
  <section id="onboarding" class="readiness">
    ...
    <p>${ve(lr)}</p>
    <p>${ve(pr)}</p>
    <p>${ve(`Live grammar executes through ${Oe} on the control plane. Try ${Ct.join(", ")}.`)}</p>
    <p>${ve(ur)}</p>
    <ol>
...
</section>`,"buildMcpOnboardingHtml"),$o=`const renderOnboarding = (payload) => {
  ...
  appendText(
    ...
    onboarding && onboarding.grammarToolName
      ...
      : 'Live grammar executes through ${Oe} on the control plane. Try ${Ct.join(", ")}.',
  );
  appendText(
    onboardingEl,
    'p',
...
</section>`,"buildPromotionRecoveryHtml"),fr=n(()=>({extensions:{[Tt]:{mimeTypes:[We]}}}),"buildKnowgrphMcpAppsCapabilities"),ne=n(e=>Array.isArray(e)?e:[],"arrayFrom"),yr=n(()=>fs.map(e=>({...e})),"buildKnowgrphMcpNoauthSecuritySchemes"),Fo=n(e=>(Array.isArray(e)&&e.length?e:yr()).filter(r=>r&&typeof r=="object").map(r=>({...r})),"normalizeSecuritySchemes"),Ho=n(e=>ne(e).some(t=>t?.type==="noauth"),"hasNoauthSecurityScheme"),jo=n(e=>Array.isArray(e)?Fo(e):[],"readSecuritySchemes"),_s=n(e=>{let t=H(e);return t.includes("window.openai")&&t.includes("openai:set_globals")&&t.includes("toolInput")&&t.includes("toolOutput")&&t.includes("callTool")&&t.includes("request('ui/initialize'")},"hasOpenAiWidgetBridgeHtml"),Go=n((e,t=[])=>e?.outputSchema?.type==="object"&&t.every(r=>ne(e.outputSchema?.required).includes(r)),"hasToolOutputSchemaFields"),hr=n(e=>e?.annotations?.readOnlyHint===!0&&e?.annotations?.destructiveHint===!1&&e?.annotations?.openWorldHint===!1&&e?.annotations?.idempotentHint===!0,"hasReadOnlyToolAnnotations"),X=n((e,t,r,o=[])=>({id:e,label:t,ok:r===!0,evidence:ne(o).map(H).filter(Boolean)}),"booleanCheck"),wr=n((e={})=>{let t=H(e.baseUrl).replace(/\/+$/,""),r=H(e.serverName)||"knowgrph",o=H(e.mcpUrl)||(t?`${t}/mcp`:"");return{[ie.openAiApps]:{id:ie.openAiApps,label:"OpenAI Apps / ChatGPT",transport:Ie,url:o,appResourceUri:me,appToolName:Fe,requiredMetadata:["openai/outputTemplate","openai/widgetAccessible","openai/widgetCSP","openai/widgetDomain"],requiredTools:[Fe,...Re]},[ie.claude]:{id:ie.claude,label:"Claude MCP connector",transport:Ie,url:o,beta:"mcp-client-2025-11-20",mcp_servers:[{type:"url",url:o,name:r}],tools:[{type:"mcp_toolset",mcp_server_name:r}],requiredTools:Re},[ie.qwenCode]:{id:ie.qwenCode,label:"Qwen Code",transport:"http",url:o,command:`qwen mcp add --transport http ${r} ${o}`,settingsJson:{mcpServers:{[r]:{httpUrl:o,timeout:3e4,trust:!1,includeTools:["search","fetch",Fe]}}},requiredTools:Re,primaryFlow:"Call search with a natural-language query, then call fetch with the returned kgdoc id."},[ie.kimiCli]:{id:ie.kimiCli,label:"Kimi CLI",transport:"http",url:o,command:`kimi mcp add --transport http ${r} ${o}`,configFile:"~/.kimi/mcp.json",mcpJson:{mcpServers:{[r]:{url:o,transport:"http"}}},requiredTools:Re,primaryFlow:"Call search with a natural-language query, then call fetch with the returned kgdoc id."},[ie.bytePlusModelArk]:{id:ie.bytePlusModelArk,label:"BytePlus ModelArk Responses API",transport:Ie,url:o,apiBaseUrl:"https://ark.ap-southeast.bytepluses.com/api/v3",endpoint:"/responses",requiredHeaders:{"ark-beta-mcp":"true"},tools:[{type:"mcp",server_label:r,server_url:o,require_approval:"never"}],openAiCompatible:{base_url:"https://ark.ap-southeast.bytepluses.com/api/v3",default_headers:{"ark-beta-mcp":"true"},responsesCreate:{model:"<MODELARK_MODEL_OR_ENDPOINT_ID>",tools:[{type:"mcp",server_label:r,server_url:o,require_approval:"never"}]}},invocationScope:"ModelArk Responses API with MCP service and model permissions enabled.",requiredTools:Re,primaryFlow:"Use ModelArk Responses API with the Knowgrph MCP tool entry, then ask the model to call search and fetch."},[ie.generic]:{id:ie.generic,label:"Generic MCP clients",transport:Ie,url:o,initialize:{method:"initialize",accept:["application/json","text/event-stream"]},requiredMethods:["initialize","notifications/initialized","tools/list","tools/call"],optionalMethods:["prompts/list","prompts/get","resources/list","resources/templates/list","resources/read"],requiredTools:Re}}},"buildKnowgrphMcpClientSetups"),zo=n((e={})=>{let t=H(e.baseUrl).replace(/\/+$/,""),r=H(e.updatedAt),o=e.mcpServerCard&&typeof e.mcpServerCard=="object"?e.mcpServerCard:{},a=o.capabilities&&typeof o.capabilities=="object"?o.capabilities:{},s=ne(e.tools).length?ne(e.tools):ne(a.tools),i=ne(e.resources).length?ne(e.resources):[vt({appUrl:t,updatedAt:r})],c=ne(e.prompts).length?ne(e.prompts):ne(o.prompts),l=ne(e.resourceTemplates).length?ne(e.resourceTemplates):ne(o.resourceTemplates),p=s.filter(b=>b?._meta?.ui?.resourceUri===me),m=p.find(b=>b?.name===Fe)||p[0]||null,f=i.find(b=>b?.uri===me)||null,A=a.extensions?.[Tt],S=H(o.transport?.url)||(t?`${t}/mcp`:""),O=H(o.surfaceRoles?.publicReadMcpUrl)||S,$=H(o.surfaceRoles?.controlPlaneMcpUrl)||(t?`${t}/control-plane/mcp`:""),N=H(o.transport?.type)||Ie,F=H(e.appResourceHtml)||Yo({appUrl:t,updatedAt:r,toolName:m?.name||Fe}),Q=e.clientSetups&&typeof e.clientSetups=="object"?e.clientSetups:wr({baseUrl:t,mcpUrl:S,serverName:o.serverInfo?.name}),z=m?.outputSchema&&typeof m.outputSchema=="object",x=!!m?.name,T=z,te=m?._meta?.["openai/outputTemplate"]===me,B=_s(F),le=Ho(m?.securitySchemes)&&Ho(m?._meta?.securitySchemes),U=hr(m),L=m?._meta?.["openai/widgetAccessible"]===!0,V=c.map(b=>H(b?.name)).filter(Boolean),se=o.capabilities?.prompts&&typeof o.capabilities.prompts=="object",Se=Do.every(b=>V.includes(b)),ke=l.map(b=>H(b?.uriTemplate)).filter(Boolean),P=ke.includes(Et),w=Object.fromEntries(Re.map(b=>[b,s.find(j=>j?.name===b)||null])),y=Re.every(b=>{let j=w[b];return hr(j)&&Go(j,mr[b])}),d=Q[ie.qwenCode],g=d?.transport==="http"&&d?.url===S&&d?.settingsJson?.mcpServers?.[o.serverInfo?.name||"knowgrph"]?.httpUrl===S&&String(d?.command||"").includes("--transport http")&&String(d?.command||"").includes(S),h=Q[ie.kimiCli],R=h?.transport==="http"&&h?.url===S&&h?.mcpJson?.mcpServers?.[o.serverInfo?.name||"knowgrph"]?.url===S&&h?.mcpJson?.mcpServers?.[o.serverInfo?.name||"knowgrph"]?.transport==="http"&&String(h?.command||"").includes("kimi mcp add --transport http")&&String(h?.command||"").includes(S),E=Q[ie.bytePlusModelArk],q=E?.transport===Ie&&E?.url===S&&E?.endpoint==="/responses"&&E?.requiredHeaders?.["ark-beta-mcp"]==="true"&&ne(E?.tools).some(b=>b?.type==="mcp"&&b?.server_label===(o.serverInfo?.name||"knowgrph")&&b?.server_url===S&&b?.require_approval==="never")&&E?.openAiCompatible?.responsesCreate?.tools?.some(b=>b?.type==="mcp"&&b?.server_label===(o.serverInfo?.name||"knowgrph")&&b?.server_url===S&&b?.require_approval==="never"),D=[X("app-tool-resource-link","App tool is linked to the UI resource",p.length>0,p.map(b=>b.name)),X("output-schema","App tool exposes an output schema",z,[m?.name]),X("text-fallback","Tool result keeps a text fallback for non-UI hosts",x,[m?.name]),X("structured-content","Tool result returns structured content for the View",T,[m?.name]),X("resource-descriptor","MCP resource descriptor uses the MCP Apps MIME type",f?.mimeType===We,[f?.uri]),X("resource-security-meta","Resource declares UI sandbox metadata",f?._meta?.ui?.prefersBorder===!0&&!!f?._meta?.ui?.csp,[f?.uri]),X("openai-output-template","App tool exposes the OpenAI output template metadata key",te,[m?.name]),X("openai-widget-bridge","App resource supports the OpenAI Apps widget bridge",B,["window.openai","openai:set_globals"]),X("tool-security-schemes","App tool exposes no-auth securitySchemes and mirrors them in _meta",le,[m?.name]),X("tool-impact-annotations","App tool exposes complete read-only impact annotations",U,[m?.name]),X("widget-accessible","App tool allows the widget bridge to call tools",L,[m?.name]),X("prompt-discovery","Server exposes MCP prompt templates for multi-host guidance",se&&Se,V),X("source-file-resource-template","Server exposes a dynamic Source Files resource template",P,ke),X("deep-research-search-fetch","Server exposes read-only search and fetch tools",y,Re),X("qwen-code-http-client-setup","Server advertises Qwen Code HTTP MCP setup",g,[d?.command]),X("kimi-cli-http-client-setup","Server advertises Kimi CLI HTTP MCP setup",R,[h?.command]),X("byteplus-modelark-responses-mcp-setup","Server advertises BytePlus ModelArk Responses API MCP setup",q,[E?.apiBaseUrl,E?.endpoint]),X("extension-capability","Server advertises the MCP Apps extension capability",A?.mimeTypes?.includes(We),[Tt]),X("streamable-http-transport","Server exposes a stateless Streamable HTTP JSON-RPC transport",!!S&&N===Ie,[S,N]),X("stdio-transport","Repo-local MCP server supports stdio host configuration",e.localStdio!==!1,["node mcp/server.js"])],K=D.every(b=>b.ok),Y=Wo();return{schemaVersion:gs,ready:K,updatedAt:r,app:{name:gr,protocolVersion:Bo,resourceUri:me,resourceMimeType:We,extensionId:Tt},tool:{name:m?.name||Fe,title:m?.title||"Inspect Agent Surface",resourceUri:m?._meta?.ui?.resourceUri||me,visibility:ne(m?._meta?.ui?.visibility).length?m._meta.ui.visibility:["model","app"],readOnly:m?.annotations?.readOnlyHint===!0,destructive:m?.annotations?.destructiveHint===!0,openWorld:m?.annotations?.openWorldHint===!0,idempotent:m?.annotations?.idempotentHint===!0,annotationsReady:U,hasOutputSchema:!!z,textFallback:x,structuredContent:T,openAiOutputTemplate:te,openAiWidgetBridge:B,securitySchemes:jo(m?.securitySchemes),mirroredSecuritySchemes:jo(m?._meta?.securitySchemes),widgetAccessible:L},resource:{uri:f?.uri||me,name:f?.name||gr,mimeType:f?.mimeType||We,prefersBorder:f?._meta?.ui?.prefersBorder===!0,domain:H(f?._meta?.ui?.domain),csp:f?._meta?.ui?.csp||{},openAiWidgetBridge:B},retrieval:{mode:"deep-research-search-fetch",requiredTools:Re,tools:Re.map(b=>{let j=w[b];return{name:b,readOnly:j?.annotations?.readOnlyHint===!0,destructive:j?.annotations?.destructiveHint===!0,openWorld:j?.annotations?.openWorldHint===!0,idempotent:j?.annotations?.idempotentHint===!0,annotationsReady:hr(j),requiredOutputFields:mr[b],outputSchemaReady:Go(j,mr[b])}})},prompts:{requiredPrompts:Do,names:V,capability:!!se,ready:se&&Se},resourceTemplates:{requiredTemplates:[Et],uriTemplates:ke,ready:P},clients:Q,transports:[{id:"pages-http-jsonrpc",type:N,url:S,stateless:!0,serverFactory:!0},{id:"local-stdio-jsonrpc",type:"stdio",command:"node mcp/server.js",stateless:!1,serverFactory:!0}],dataModel:{source:"inspect_agent_surface.structuredContent",categories:[{id:"discovery",label:"Discovery metadata",count:["health","apiCatalog","openApi","mcpServerCard","agentCard","agentSkills"].length},{id:"commerce",label:"Commerce discovery",count:["acpDiscovery","ucpProfile","mppOpenApi"].length},{id:"mcp-apps",label:"MCP Apps server bindings",count:D.length}],renderMode:"structured-summary-with-json-fallback"},onboarding:dr({publicReadMcpUrl:O,controlPlaneMcpUrl:$}),operatorContracts:{promotionRecovery:Y},checklist:D}},"buildKnowgrphMcpAppsServerReadiness"),qo=n((e={})=>{let t=H(e.resourceUri)||me;return{securitySchemes:Fo(e.securitySchemes),ui:{resourceUri:t,visibility:Array.isArray(e.visibility)&&e.visibility.length?e.visibility:["model","app"]},"openai/outputTemplate":t,"openai/widgetAccessible":e.widgetAccessible!==!1,"openai/toolInvocation/invoking":H(e.invoking)||"Inspecting Knowgrph.","openai/toolInvocation/invoked":H(e.invoked)||"Knowgrph is ready."}},"buildKnowgrphMcpAppsToolMeta"),Vo=Object.freeze({type:"object",additionalProperties:!0,required:["baseUrl","healthUrl","mcpUrl"],properties:{baseUrl:{type:"string"},healthUrl:{type:"string"},mcpUrl:{type:"string"},apiCatalogUrl:{type:"string"},openApiUrl:{type:"string"},mcpServerCardUrl:{type:"string"},agentCardUrl:{type:"string"},agentSkillsUrl:{type:"string"},commerceUrls:{type:"object",additionalProperties:{type:"string"}},health:{type:"object",additionalProperties:!0},apiCatalog:{type:"object",additionalProperties:!0},openApi:{type:"object",additionalProperties:!0},mcpServerCard:{type:"object",additionalProperties:!0},agentCard:{type:"object",additionalProperties:!0},agentSkills:{type:"object",additionalProperties:!0},commerce:{type:"object",additionalProperties:!0},mcpAppsServerReadiness:{type:"object",additionalProperties:!0}}}),vt=n((e={})=>{let t=H(e.appUrl),r=H(e.updatedAt),o=H(e.domain)||ws(t),a={connectDomains:[],resourceDomains:[],frameDomains:[],baseUriDomains:[]};return{uri:me,name:gr,description:["Interactive MCP Apps view for the existing Knowgrph agent-ready surface.",t?`App URL: ${t}`:"",r?`Updated: ${r}`:""].filter(Boolean).join(" "),mimeType:We,_meta:{ui:{csp:a,...o?{domain:o}:{},prefersBorder:!0},"openai/widgetDescription":"Interactive Knowgrph agent-ready server-readiness view.","openai/widgetPrefersBorder":!0,...o?{"openai/widgetDomain":o}:{},"openai/widgetCSP":{connect_domains:a.connectDomains,resource_domains:a.resourceDomains,frame_domains:a.frameDomains}}}},"buildKnowgrphMcpAppsResourceDescriptor"),Yo=n((e={})=>{let t=H(e.appUrl),r=H(e.updatedAt),o=H(e.toolName)||Fe,a=t?`${t.replace(/\/+$/,"")}/mcp`:"",s=t?`${t.replace(/\/+$/,"")}/control-plane/mcp`:"",i=Array.isArray(e.toolNames)?e.toolNames.map(H).filter(Boolean):[o],c={appUrl:t,updatedAt:r,resourceUri:me,toolName:o,toolNames:i,protocolVersion:Bo,onboarding:dr({publicReadMcpUrl:a,controlPlaneMcpUrl:s}),promotionRecovery:Wo()};return`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  <main>
    ...
    ${Ko({publicReadMcpUrl:a,controlPlaneMcpUrl:s})}
    ${Ss(c.promotionRecovery)}
...
${v.canonicalPath}
${v.workspaceId}
${v.snippet}`.toLowerCase(),Ge=K&&J.includes(K)?D.length*4:0,Be=D.reduce((bt,tr)=>bt+(J.includes(tr)?2:0),0);return{...v,score:Ge+Be}}),b=Y.slice().sort((v,J)=>J.score-v.score||v.title.localeCompare(J.title)).slice(0,$).filter(v=>/\.md(?:$|[?#])/i.test(v.canonicalPath)),j=new Map;await te(b,async v=>{let J=Se(v.id);if(!J?.canonicalPath)return null;try{let Ge=await s(o(J.canonicalPath,J.workspaceId));if(!Ge.ok)return null;let Be=(await Ge.text()).slice(0,N),bt=Be.toLowerCase(),tr=K&&bt.includes(K)?D.length*6:0,Ga=x(bt,D),eo=tr+Ga;if(eo<=0)return null;j.set(v.id,{score:eo,snippet:T(Be,D)})}catch{return null}return null});let ue=Y.map(v=>{let J=j.get(v.id);return{...v,score:v.score+(J?.score||0),snippet:J?.snippet||v.snippet}}).filter(v=>v.score>0).sort((v,J)=>J.score-v.score||v.title.localeCompare(J.title)).slice(0,h).map(({score:v,...J})=>J);return{ids:ue.map(v=>v.id),results:ue,query:g,totalResults:ue.length}},"searchSourceFiles"),w=n(async(d={})=>{let g=Se(d.id);if(!g?.canonicalPath)throw new Error("id must be a stable Knowgrph Source File id returned by search");let h=await U(g),R=B(g);return{id:se(g),title:S(h.canonicalPath),content:h.markdown,text:h.markdown,url:R,metadata:{workspaceId:h.workspaceId,canonicalPath:h.canonicalPath,contentType:"text/markdown",source:"knowgrph-source-files"}}},"fetchSourceFileBySearchId"),y={};return t.search&&(y[t.search]=P),t.fetch&&(y[t.fetch]=w),t.listSourceFiles&&(y[t.listSourceFiles]=async()=>{let d=await a();if(!d.ok)throw new Error(`list_source_files failed with ${d.status}`);return{workspaceId:r,markdownIndex:await d.text()}}),t.readSourceFile&&(y[t.readSourceFile]=U),t.readSharedDocument&&(y[t.readSharedDocument]=L),t.inspectSharedDocumentStructure&&(y[t.inspectSharedDocumentStructure]=V),t.inspectAgentSurface&&(y[t.inspectAgentSurface]=async()=>l()),y},"createPublishedAgentReadyToolExecutors"),ks=n(e=>`((...args) => {
  const n = (value) => value
  const __name = (value) => value
  return (${Function.prototype.toString.call(e)})(...args)
})`,"createBrowserSafeFunctionSource"),Xo=ks(br);var Qo=n((e={})=>{let t=n(P=>String(P||"").trim(),"normalizeString"),r=n(P=>String(P||"").replace(/\r\n/g,`
`).replace(/\r/g,`
...
`,"buildKnowgrphVdeoxplnMarkdown"),sn=n((e=nn())=>Object.fromEntries(e.map(t=>[t.id,vs(t)])),"buildKnowgrphVdeoxplnMarkdownByName");var cn=n(({appUrl:e,rootUrl:t,storageSourceFilesUrl:r,storageLlmsUrl:o,storageManifestUrl:a,agentCardUrl:s,updatedAt:i})=>`<?xml version="1.0" encoding="UTF-8"?>
...
`,"buildMarkdownDiscoverySitemapXml"),ln=n(({appUrl:e,rootUrl:t,storageLlmsUrl:r,storageManifestUrl:o,agentCardUrl:a})=>`# Airvio
...
}`;var Nt={push:"/api/storage/push",pull:"/api/storage/pull",collabSave:"/api/storage/collab/save",canvasRoomPrefix:"/api/storage/canvas-room/",chatSession:"/api/storage/chat/session",chatRelay:"/api/storage/chat/relay",chatPoliciesPrefix:"/api/storage/chat/policies/",chatAuditPrefix:"/api/storage/chat/audit/",exportPrefix:"/api/storage/export/",docPrefix:"/api/storage/doc/",defaultDocPrefix:"/api/storage/doc-default/",blobPrefix:"/api/storage/blob/",mediaAssetPersist:"/api/storage/media/assets",mediaAssetPrefix:"/api/storage/media/assets/",mediaPrefix:"/api/storage/media/",sourceFilesIndex:"/api/storage/source-files",sourceFilesIndexPrefix:"/api/storage/source-files/",sourceFilesLlms:"/api/storage/llms.txt"};var Rn=n((e,t)=>`${Nt.docPrefix}${encodeURIComponent(String(e||"").trim())}/${encodeURIComponent(String(t||"").trim())}`,"buildKnowgrphStorageDocPath"),An=n(e=>`${Nt.defaultDocPrefix}${encodeURIComponent(String(e||"").trim())}`,"buildKnowgrphStorageDefaultDocPath");var Pn=n(e=>{let t=String(e||"").trim();return t?`${Nt.sourceFilesIndexPrefix}${encodeURIComponent(t)}`:Nt.sourceFilesIndex},"buildKnowgrphStorageSourceFilesIndexPath");var G="https://airvio.co",Lt="https://knowgrph-storage.huijoohwee.workers.dev",C="/knowgrph",k=`${G}${C}/`,Ut=`${G}/`,ut="kgws:canonical-docs",ze="2026-06-05",ot=`${C}/health`,rt=`${G}${ot}`,kn="/.well-known/agent-card.json",Tr=`${C}/.well-known/agent-card.json`,dt=`${G}${kn}`,Kt=`${G}/api/storage/source-files`,Gs=`${G}/api/storage/doc-default/{canonicalPath}`,Bs=`${G}/api/storage/doc/{workspaceId}/{canonicalPath}`,Ws=`${G}/api/storage/blob/{workspaceId}/{canonicalPath}`,xn="knowgrph-agent-ready-pages";var En=['</.well-known/api-catalog>; rel="api-catalog"; type="application/linkset+json"',`<${C}/.well-known/openapi.json>; rel="service-desc"; type="application/vnd.oai.openapi+json;version=3.1"`,`<${C}/llms.txt>; rel="service-doc"; type="text/plain"`,'</auth.md>; rel="service-doc"; type="text/markdown"',`<${ot}>; rel="status"; type="application/health+json"`,`<${C}/.well-known/mcp/server-card.json>; rel="mcp-server-card"; type="application/json"`,`<${kn}>; rel="describedby"; type="application/json"`].join(", "),Cn=`# Knowgrph
...
- Default Source File documents: ${Gs}
- Workspace Source File documents: ${Bs}
- Workspace binary artifacts: ${Ws}
...
`,Tn=n(e=>new Response(e,{status:200,headers:{"content-type":"text/markdown; charset=utf-8","cache-control":"public, max-age=3600","access-control-allow-origin":"*",vary:"Accept","x-markdown-tokens":String(Math.ceil(String(e||"").length/4))}}),"markdownResponse"),mt=n(e=>(e.headers.get("accept")||"").toLowerCase().split(",").some(r=>r.trim().startsWith("text/markdown")),"wantsMarkdown"),vn=n((e,t)=>{let r=new Response(e.body,e),o=String(t?.owner||"").trim(),a=String(t?.tag||"").trim();return o&&r.headers.set("x-knowgrph-route-owner",o),a&&r.headers.set("x-knowgrph-route-tag",a),r},"withAgentReadyRouteHeaders");var Dt=_r({defaultWorkspaceId:ut}),Kn=vo(),$n=No(),Fs=`${G}/api/storage/llms.txt`,zs=`${G}/api/storage/content-manifest.json`,Dn=n((e,t="")=>{let r=String(e||"").trim(),o=String(t||"").trim();return o?Rn(o,r):An(r)},"buildStorageDocPath"),ft=n(e=>String(e||"").trim(),"normalizeToolString");var Ae=n((e,t="application/json; charset=utf-8")=>new Response(JSON.stringify(e,null,2),{status:200,headers:{"content-type":t,"cache-control":"public, max-age=3600","access-control-allow-origin":"*"}}),"jsonResponse"),he=n((e,t)=>new Response(JSON.stringify(t,null,2),{status:e,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store","access-control-allow-origin":"*"}}),"jsonStatusResponse"),On=n((e,t={})=>new Response(null,{status:e,headers:{"cache-control":"no-store","access-control-allow-origin":"*",...t}}),"emptyStatusResponse"),ht=n((e,t)=>new Response(e,{status:200,headers:{"content-type":t,"cache-control":"public, max-age=3600","access-control-allow-origin":"*"}}),"textResponse"),qs=n(e=>ht(e,"text/html;profile=mcp-app; charset=utf-8"),"mcpAppsHtmlResponse"),Vs=n(e=>new Response(JSON.stringify(e,null,2),{status:200,headers:{"content-type":"application/health+json; charset=utf-8","cache-control":"no-store","access-control-allow-origin":"*"}}),"healthResponse"),Ys=`${C}/api/workspace/github/write`,Js="/api/workspace/github/write",In=12,Mn=9e5,Xs=new Set(["css","html","js","json","md","mdx","mjs","svg","ts","tsx","txt","yaml","yml"]),vr=n((e,t)=>String(e?.[t]||"").trim(),"readEnvString"),Qs=n(e=>{let t=vr(e,"KNOWGRPH_GITHUB_WRITE_REPOSITORY"),r=vr(e,"KNOWGRPH_GITHUB_WRITE_TOKEN"),o=vr(e,"KNOWGRPH_GITHUB_WRITE_BRANCH"),a=[];t||a.push("KNOWGRPH_GITHUB_WRITE_REPOSITORY"),r||a.push("KNOWGRPH_GITHUB_WRITE_TOKEN");let s=t.split("/").map(i=>i.trim()).filter(Boolean);return t&&s.length!==2&&a.push("KNOWGRPH_GITHUB_WRITE_REPOSITORY:owner/repo"),a.length>0?{ok:!1,missing:a}:{ok:!0,owner:s[0],repo:s[1],branch:o,token:r}},"readGitHubWriteConfig"),Zs=n(e=>{let t=String(e||"").trim().replace(/^workspace:/i,"").replace(/\\/g,"/").replace(/^\/+/,"");if(!t)return{ok:!1,error:"missing_workspace_path"};if(/[\u0000-\u001f\u007f]/.test(t))return{ok:!1,error:"invalid_workspace_path"};let r=t.split("/").filter(Boolean);if(r.some(s=>s==="."||s===".."))return{ok:!1,error:"path_traversal_forbidden"};if(r[0]!=="chat-log")return{ok:!1,error:"unsupported_workspace_root"};if(r.length<3)return{ok:!1,error:"chat_log_session_file_required"};let o=r[r.length-1]||"",a=o.includes(".")?o.split(".").pop().toLowerCase():"";return!a||!Xs.has(a)?{ok:!1,error:"unsupported_text_extension"}:{ok:!0,path:r.join("/")}},"normalizeGitHubWriteWorkspacePath"),ei=n(e=>{let t=new TextEncoder().encode(String(e||"")),r=32768,o="";for(let a=0;a<t.length;a+=r)o+=String.fromCharCode(...t.slice(a,a+r));return btoa(o)},"encodeBase64Utf8"),nt=class extends Error{static{n(this,"GitHubWorkspaceWriteError")}constructor(t,r,o){super(t),this.name="GitHubWorkspaceWriteError",this.code=t,this.upstreamStatus=r,this.upstreamMessage=o}},Hn=n(e=>String(e||"unknown").replace(/[\u0000-\u001f\u007f]/g," ").replace(/\s+/g," ").trim().slice(0,240),"sanitizeGitHubApiMessage"),jn=n((e,t)=>{let r=String(t||"").split("/").map(a=>encodeURIComponent(a)).join("/"),o=new URL(`https://api.github.com/repos/${encodeURIComponent(e.owner)}/${encodeURIComponent(e.repo)}/contents/${r}`);return e.branch&&o.searchParams.set("ref",e.branch),o},"buildGitHubContentsApiUrl"),Gn=n(e=>({accept:"application/vnd.github+json",authorization:`Bearer ${e.token}`,"user-agent":"knowgrph-cloudflare-pages","x-github-api-version":"2022-11-28"}),"gitHubApiHeaders"),Bn=n(e=>{let t=String(e||"").replace(/\/+$/,"")||"/";return t===Ys||t===Js},"isGitHubWorkspaceWriteRoutePath"),ti=n(async(e,t)=>{let r=await fetch(jn(e,t),{method:"GET",headers:Gn(e)});if(r.status===404)return null;let o=await r.json().catch(()=>null);if(!r.ok)throw new nt("github_read_failed",r.status,Hn(o?.message||r.statusText));if(o?.type&&o.type!=="file")throw new nt("github_path_not_file",409,t);return String(o?.sha||"").trim()||null},"fetchGitHubExistingFileSha"),ri=n(async(e,t,r)=>{let o=await ti(e,t.repositoryPath),a={message:r,content:ei(t.text),...e.branch?{branch:e.branch}:{},...o?{sha:o}:{}},s=await fetch(jn(e,t.repositoryPath),{method:"PUT",headers:{...Gn(e),"content-type":"application/json; charset=utf-8"},body:JSON.stringify(a)}),i=await s.json().catch(()=>null);if(!s.ok)throw new nt("github_write_failed",s.status,Hn(i?.message||s.statusText));return{workspacePath:t.workspacePath,repositoryPath:t.repositoryPath,action:o?"updated":"created",commitSha:String(i?.commit?.sha||""),contentSha:String(i?.content?.sha||""),htmlUrl:String(i?.content?.html_url||"")}},"putGitHubWorkspaceFile"),oi=n(async(e,t)=>{let r=Qs(t);if(!r.ok)return he(503,{ok:!1,status:"skipped",error:"github_write_not_configured",missing:r.missing});let o=await e.json().catch(()=>null),a=Array.isArray(o?.files)?o.files:[];if(a.length<1)return he(400,{ok:!1,status:"failed",error:"files_required"});if(a.length>In)return he(413,{ok:!1,status:"failed",error:"too_many_files",maxFiles:In});let s=[],i=new Set;for(let p of a){let m=Zs(p?.workspacePath||p?.path);if(!m.ok)return he(400,{ok:!1,status:"failed",error:m.error,workspacePath:String(p?.workspacePath||p?.path||"")});if(i.has(m.path))continue;i.add(m.path);let f=String(p?.text??"");if(new TextEncoder().encode(f).length>Mn)return he(413,{ok:!1,status:"failed",error:"file_too_large",workspacePath:`/${m.path}`,maxTextBytes:Mn});s.push({workspacePath:`/${m.path}`,repositoryPath:m.path,text:f})}if(s.length<1)return he(400,{ok:!1,status:"failed",error:"files_required"});let c=String(o?.message||"").trim(),l=c&&c.length<=160?c:`Knowgrph chat artifact ${s[0].repositoryPath}`;if(o?.dryRun===!0)return he(200,{ok:!0,status:"dry_run",repository:`${r.owner}/${r.repo}`,branch:r.branch||null,files:s.map(p=>({workspacePath:p.workspacePath,repositoryPath:p.repositoryPath,textBytes:new TextEncoder().encode(p.text).length}))});try{let p=[];for(let m of s)p.push(await ri(r,m,l));return he(200,{ok:!0,status:"applied",repository:`${r.owner}/${r.repo}`,branch:r.branch||null,files:p})}catch(p){let m=p instanceof nt;return he(m?424:500,{ok:!1,status:"failed",error:m?p.code:p instanceof Error?p.message:String(p||"github_write_failed"),...m?{upstreamStatus:p.upstreamStatus,upstreamMessage:p.upstreamMessage}:{}})}},"handleGitHubWorkspaceWrite"),ni=n(e=>`User-agent: *
Allow: /knowgrph/
...
Sitemap: ${e}
`,"buildRobotsTxt"),ai=ni(`${k}sitemap.xml`),Wn={appUrl:k,rootUrl:Ut,storageSourceFilesUrl:Kt,storageLlmsUrl:Fs,storageManifestUrl:zs,agentCardUrl:dt,updatedAt:ze},si=cn(Wn),Sd=ln(Wn),Fn={linkset:[{anchor:k,"service-desc":[{href:`${k}.well-known/openapi.json`,type:"application/vnd.oai.openapi+json;version=3.1"}],"service-doc":[{href:`${k}llms.txt`,type:"text/plain"}],status:[{href:rt,type:"application/health+json"}],"service-meta":[{href:`${k}.well-known/mcp/server-card.json`,type:"application/json"},{href:dt,type:"application/json"}]}]},zn={openapi:"3.1.0",info:{title:"Knowgrph API",version:"0.1.0",description:"Agent discovery surface for the Knowgrph Cloudflare deployment."},servers:[{url:G,description:"Knowgrph Cloudflare deployment"}],paths:mn({appBasePath:C,appA2aAgentCardPath:Tr,healthPath:ot})},gt={resource:k,resource_name:"Knowgrph",authorization_servers:[G],scopes_supported:["knowgrph:read","knowgrph:source-files:read"],bearer_methods_supported:["header"],resource_documentation:`${k}llms.txt`},Or=`${G}/cdn-cgi/access`,Ne={skill:`${G}/auth.md`,register_uri:`${k}agent/auth`,claim_uri:`${k}agent/auth/claim`,revocation_uri:`${k}agent/auth/revoke`,identity_types_supported:["anonymous","identity_assertion"],anonymous:{credential_types_supported:["api_key"]},identity_assertion:{assertion_types_supported:["urn:ietf:params:oauth:token-type:id-jag","verified_email"],credential_types_supported:["access_token","api_key"]},events_supported:["https://schemas.workos.com/events/agent/auth/identity/assertion/revoked"],registration_status:"metadata_published_runtime_user_mediated"},Nn={issuer:G,resource:gt.resource,resource_name:gt.resource_name,authorization_servers:gt.authorization_servers,cloudflare_access_issuer:Or,authorization_endpoint:`${Or}/login`,token_endpoint:`${Or}/token`,jwks_uri:`${k}.well-known/http-message-signatures-directory`,response_types_supported:["code"],grant_types_supported:["authorization_code","client_credentials"],token_endpoint_auth_methods_supported:["client_secret_basic","private_key_jwt"],scopes_supported:gt.scopes_supported,agent_auth:Ne},ii=`# Knowgrph auth.md
...
- Pipeline rule: agents must not bypass MainPanel -> FloatingPanel Chat -> KGC -> Canvas for user-mediated graph work; published HTTP MCP tools remain read-only until mutation auth and conflict semantics are implemented.`,qn={name:"Knowgrph Agent",description:"Agent-readable discovery, published-document retrieval, and WebMCP-ready metadata surface for Knowgrph.",version:"0.1.0",provider:{organization:"airvio / joohwee",url:k},url:`${k}mcp`,preferredTransport:"JSONRPC",supportedInterfaces:[{url:`${k}mcp`,protocolBinding:"JSONRPC",transportProtocol:"JSONRPC",description:"Primary machine interface for read-only discovery and source-file document access."},{url:Kt,protocolBinding:"HTTP+JSON/REST",transportProtocol:"HTTP+JSON/REST",description:"Published source-files index and storage-backed document read surface."}],capabilities:{streaming:!1,pushNotifications:!1,stateTransitionHistory:!1,extendedAgentCard:!1},defaultInputModes:["text/plain","text/markdown","application/json"],defaultOutputModes:["text/plain","text/markdown","application/json"],skills:un(Dt)},Te={serverInfo:{name:"knowgrph",version:"0.1.0"},transport:{type:Ie,url:`${k}mcp`,stateless:!0},capabilities:{tools:Dt.map(e=>({name:e.name,title:e.title,description:e.description,inputSchema:e.inputSchema,outputSchema:e.outputSchema,securitySchemes:e.securitySchemes,annotations:e.annotations,_meta:e._meta})),resources:{listChanged:!1},prompts:{listChanged:!1},...fr()},prompts:Kn,resourceTemplates:$n,clientSetups:wr({baseUrl:k,mcpUrl:`${k}mcp`,serverName:"knowgrph"}),surfaceRoles:{publicReadMcpUrl:`${k}mcp`,publicReadMcpScope:"Canonical public install and discovery endpoint for read-only retrieval, prompt discovery, resource discovery, and inspection.",controlPlaneMcpUrl:`${k}control-plane/mcp`,controlPlaneMcpScope:"Approval-gated orchestration endpoint for control-plane tools, remote Agentic Canvas OS docs invocation, and spend-bearing workflows where deployed.",hostedGrammarDefaultPath:"app-owned-forwarder",hostedGrammarDefaultScope:"Hosted app builders should keep /mcp for discovery and route live /, #, @ through an app-owned forwarder unless the host proves MCP session support.",remoteGrammarInvokePublic:!0,remoteGrammarInvokeToolName:"knowgrph.agentic_canvas_os.docs.invoke",remoteGrammarInvokeStatus:"live-control-plane"},links:{apiCatalog:`${k}.well-known/api-catalog`,skills:`${k}.well-known/agent-skills/index.json`,status:rt,agentCard:dt,controlPlaneMcp:`${k}control-plane/mcp`}},ci=vt({appUrl:k,updatedAt:ze}),Mr=Dt.map(e=>({name:e.webName,title:e.title,description:e.description,inputSchema:e.inputSchema,outputSchema:e.outputSchema,securitySchemes:e.securitySchemes,annotations:e.annotations,_meta:e._meta})),qe=n(e=>ft(Dt.find(t=>t.name===e)?.webName),"findWebMcpToolName"),li=qe(u.search),pi=qe(u.fetch),ui=qe(u.listSourceFiles),di=qe(u.readSourceFile),mi=qe(u.readSharedDocument),hi=qe(u.inspectSharedDocumentStructure),gi=qe(u.inspectAgentSurface),fi=`(() => {
  ...
  const createPublishedDocIdentityResolver = ${bn};
  ...
  const toolExecutors = createPublishedAgentReadyToolExecutors({
    toolNames: {
      search: ${JSON.stringify(li)},
      fetch: ${JSON.stringify(pi)},
      listSourceFiles: ${JSON.stringify(ui)},
      readSourceFile: ${JSON.stringify(di)},
      readSharedDocument: ${JSON.stringify(mi)},
      inspectSharedDocumentStructure: ${JSON.stringify(hi)},
      inspectAgentSurface: ${JSON.stringify(gi)},
    },
    defaultWorkspaceId,
...
})();`,yi=n(async e=>{if(!(e.headers.get("content-type")||"").toLowerCase().includes("text/html"))return e;let r=await e.text();if(Mr.every(i=>r.includes(i.name)))return new Response(r,e);let o=`<script>${fi}<\/script>`,a=r.includes("</head>")?r.replace("</head>",`${o}</head>`):`${r}${o}`,s=new Response(a,e);return s.headers.delete("content-length"),s},"injectWebMcpScript"),wi={search:u.search,fetch:u.fetch,listSourceFiles:u.listSourceFiles,readSourceFile:u.readSourceFile,readSharedDocument:u.readSharedDocument,inspectSharedDocumentStructure:u.inspectSharedDocumentStructure,inspectAgentSurface:u.inspectAgentSurface},Si=n(async e=>{let t=new TextEncoder().encode(e),r=await crypto.subtle.digest("SHA-256",t);return[...new Uint8Array(r)].map(o=>o.toString(16).padStart(2,"0")).join("")},"sha256Hex"),Vn=sn(),_i=Object.fromEntries(pt.map(e=>[e.name,Si(Vn[e.name]||"")])),Un=new Map(pt.map(e=>[`${C}${e.path}`.replace(/\/+$/,""),Vn[e.name]||""]));var Yn=n(async()=>dn({appUrl:k,updatedAt:ze,sha256ByName:_i}),"agentSkillsIndex"),bi={keys:[{kty:"OKP",crv:"Ed25519",kid:"knowgrph-agent-ready-2026-05-21",use:"sig",alg:"EdDSA",x:"11qYAYdkVKxA4G0wV47IxPtYfFVH_H7zmC2Di2PcvLU"}]},Ri={protocolVersion:"2025-06-18",capabilities:{tools:{},resources:{},prompts:{listChanged:!1},...fr()},serverInfo:Te.serverInfo},Nr=Te.capabilities.tools,Ai=[ci],Pi=Kn,ki=$n,Jn=n(()=>({status:"pass",service:"knowgrph-agent-ready-pages",homepage:k,health:rt,updatedAt:ze,checks:{linkHeaders:!0,markdownNegotiation:!0,httpMcp:!0,webMcp:!0,mcpApps:!0,commerce:{acp:!0,ucp:!0,mpp:!0,x402:!0},defaultWorkspaceId:ut}}),"buildHealthStatusBody"),xi=n(async()=>Jo({baseUrl:k,health:Jn(),apiCatalog:Fn,openApi:zn,mcpServerCard:Te,agentCard:qn,agentSkills:await Yn(),commerce:ar({origin:G})}),"buildAgentSurfaceInspection"),Ei=br({toolNames:wi,defaultWorkspaceId:ut,publicBaseUrl:G,buildStorageDocPath:Dn,fetchSourceFilesIndexResponse:n(()=>fetch(`${Lt}${Pn()}`,{headers:{accept:"text/markdown"}}),"fetchSourceFilesIndexResponse"),fetchStorageMarkdownResponse:n(e=>fetch(`${Lt}${e}`,{headers:{accept:"text/markdown"}}),"fetchStorageMarkdownResponse"),resolveSharedDocumentInput:n((e={})=>Mt({shareToken:e?.shareToken,shareUrl:e?.shareUrl,appBasePath:C,baseUrl:G}),"resolveSharedDocumentInput"),inspectSharedDocumentStructure:Qo,buildAgentSurfaceInspection:xi}),Ur=n(e=>{try{let t=new URL(e,G);return Mt({shareUrl:`${t.pathname}${t.search}`,baseUrl:G,appBasePath:C})}catch{return null}},"resolvePublishedDocRequestIdentity"),Ci=n(e=>Mt({shareUrl:String(e||""),baseUrl:G,appBasePath:C}),"resolvePublishedDocPathIdentity"),Ti=n(async(e,t)=>{let r=new URL(Dn(t.canonicalPath,t.workspaceId),Lt),o=await fetch(r,{method:"GET",headers:{accept:"text/markdown, text/plain;q=0.9, */*;q=0.1"}}),a=new Headers(o.headers),s=String(a.get("vary")||"");return a.set("vary",s?`${s}, Accept`:"Accept"),new Response(String(e.method||"").toUpperCase()==="HEAD"?null:o.body,{status:o.status,statusText:o.statusText,headers:a})},"proxyPublishedDocMarkdownResponse"),vi=n(async e=>{try{let t=await e.json();return t&&typeof t=="object"?t:null}catch{return null}},"readJsonRpcRequest"),Me=n((e,t)=>he(200,{jsonrpc:"2.0",id:e??null,result:t}),"jsonRpcResult"),De=n((e,t,r)=>he(200,{jsonrpc:"2.0",id:e??null,error:{code:t,message:r}}),"jsonRpcError"),Oi=n(e=>String(e.headers.get("accept")||"").toLowerCase().split(",").some(t=>t.trim().startsWith("text/event-stream")),"requestAcceptsEventStream"),Ir=n((e,t)=>Object.prototype.hasOwnProperty.call(e,t),"hasOwnProperty"),Xn=n(e=>{if(Array.isArray(e))return e.length>0&&e.every(Xn);if(!e||typeof e!="object"||String(e.jsonrpc||"")!=="2.0")return!1;let t=typeof e.method=="string"&&e.method.length>0,r=Ir(e,"id"),o=Ir(e,"result")||Ir(e,"error");return t&&!r||!t&&o},"isJsonRpcNotificationOrResponse"),Qn=n(async(e,t)=>{let r=Ei[e];if(typeof r!="function")throw new Error(`unknown tool: ${e}`);return r(t)},"executeMcpTool"),Ii=n(async e=>{let t=ft(e);if(t===me)return Sr({appUrl:k,updatedAt:ze,toolNames:Nr.map(o=>o.name)});let r=Uo(t);if(r){let o=await Qn(u.fetch,{id:r});return Lo({uri:t,sourceFile:o})}throw new Error(`unknown resource: ${e}`)},"readMcpResource"),Mi=n(async e=>{let t=String(e.method||"GET").toUpperCase();if(t==="GET"||t==="HEAD")return Oi(e)?On(405,{allow:"POST"}):Ae({ok:!0,transport:Te.transport,serverInfo:Te.serverInfo,capabilities:Te.capabilities,links:Te.links,surfaceRoles:Te.surfaceRoles});if(t!=="POST")return he(405,{ok:!1,error:"unsupported_method"});let r=await vi(e);if(!r)return De(null,-32700,"Parse error");if(Xn(r))return On(202);if(Array.isArray(r))return De(null,-32600,"Batch JSON-RPC requests are not supported");switch(r.method){case"initialize":return Me(r.id,Ri);case"tools/list":return Me(r.id,{tools:Nr});case"prompts/list":return Me(r.id,{prompts:Pi});case"resources/templates/list":return Me(r.id,{resourceTemplates:ki});case"prompts/get":{let o=ft(r.params?.name),a=r.params?.arguments&&typeof r.params.arguments=="object"?r.params.arguments:{};if(!o)return De(r.id,-32602,"Prompt name is required");try{return Me(r.id,Oo(o,a))}catch(s){return De(r.id,-32602,s instanceof Error?s.message:String(s))}}case"resources/list":return Me(r.id,{resources:Ai});case"resources/read":{let o=ft(r.params?.uri);if(!o)return De(r.id,-32602,"Resource URI is required");try{return Me(r.id,await Ii(o))}catch(a){return De(r.id,-32602,a instanceof Error?a.message:String(a))}}case"tools/call":{let o=ft(r.params?.name),a=r.params?.arguments&&typeof r.params.arguments=="object"?r.params.arguments:{};if(!o)return De(r.id,-32602,"Tool name is required");try{let s=await Qn(o,a);return Me(r.id,{content:[{type:"text",text:typeof s?.markdown=="string"?s.markdown:JSON.stringify(s,null,2)}],structuredContent:s,isError:!1})}catch(s){return Me(r.id,{content:[{type:"text",text:s instanceof Error?s.message:String(s)}],isError:!0})}}default:return De(r.id,-32601,"Method not found")}},"handleMcpTransport"),Ni=n(()=>Sr({appUrl:k,updatedAt:ze,toolNames:Nr.map(e=>e.name)}).contents[0].text,"buildKnowgrphMcpAppHtmlBody");var Ht=n(e=>e===C||e===`${C}/`,"handlesKnowgrphRoot"),Ui=n(e=>Ht(e)||!!Ci(e),"handlesKnowgrphHtmlSurface"),Li=n(e=>e.startsWith(`${C}/assets/`),"handlesKnowgrphStaticAsset"),Ki=n(async e=>{let t=new Headers(e.request.headers);t.delete("origin");let r=new Request(e.request.url,{method:e.request.method,headers:t});return typeof e.env?.ASSETS?.fetch=="function"?e.env.ASSETS.fetch(r):e.next(r)},"fetchKnowgrphStaticAsset"),$i=n(e=>{let t=new URL(e.url),r=t.pathname.replace(/\/+$/,"")||"/",o=Ur(e.url);return r===ot?"health":r===`${C}/mcp`?"mcp":Bn(r)?"github-workspace-write":r===`${C}/robots.txt`?"robots":r===`${C}/sitemap.xml`?"sitemap":r===`${C}/auth.md`||r==="/auth.md"?"auth-md":r.startsWith(`${C}/.well-known/`)?"well-known":o?mt(e)?"shared-doc-markdown":"shared-doc-html":Ht(t.pathname)?mt(e)?"homepage-markdown":"homepage-html":"app-surface"},"resolveAgentReadyRouteTag"),$t=n((e,t)=>vn(t,{owner:xn,tag:$i(e)}),"withKnowgrphRouteHeaders"),Ln=n(async e=>{let t=new URL(e.url),r=t.pathname.replace(/\/+$/,"")||"/",o=Ur(e.url),a=pn(r);if(a)return Response.redirect(new URL(a,e.url),308);if(o&&mt(e))return Ti(e,o);if(Ht(t.pathname)&&mt(e))return Tn(Cn);switch(r){case ot:return Vs(Jn());case`${C}/mcp`:return Mi(e);case`${C}/robots.txt`:return ht(ai,"text/plain; charset=utf-8");case`${C}/sitemap.xml`:return ht(si,"application/xml; charset=utf-8");case`${C}/auth.md`:case"/auth.md":return ht(ii,"text/markdown; charset=utf-8");case`${C}/.well-known/api-catalog`:return Ae(Fn,"application/linkset+json; charset=utf-8");case`${C}/.well-known/openapi.json`:return Ae(zn,"application/vnd.oai.openapi+json; charset=utf-8");case Tr:return Ae(qn);case`${C}/.well-known/oauth-protected-resource`:return Ae(gt);case`${C}/.well-known/oauth-authorization-server`:return Ae(Nn);case`${C}/.well-known/openid-configuration`:return Ae(Nn);case`${C}/.well-known/mcp/server-card.json`:return Ae(Te);case`${C}/.well-known/mcp/apps/knowgrph-agent-ready.html`:return qs(Ni());case`${C}/.well-known/mcp.json`:return Ae(Te);case`${C}/.well-known/agent-skills/index.json`:return Ae(await Yn());case`${C}/.well-known/http-message-signatures-directory`:return Ae(bi);default:return Un.has(r)?ht(Un.get(r),"text/markdown; charset=utf-8"):null}},"routeResponse");async function He(e){let{env:t,request:r}=e,o=String(r.method||"GET").toUpperCase(),a=new URL(r.url);if(o==="OPTIONS")return new Response(null,{status:204,headers:{"access-control-allow-origin":"*","access-control-allow-methods":"GET, HEAD, POST, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(o==="POST"&&a.pathname.replace(/\/+$/,"")===`${C}/mcp`)return $t(r,await Ln(r));if(o==="POST"&&Bn(a.pathname))return $t(r,await oi(r,t));if(o!=="GET"&&o!=="HEAD")return he(405,{ok:!1,error:"unsupported_method"});if(Li(a.pathname))return Ki(e);let s=await Ln(r);if(s){let m=$t(r,s);return o==="HEAD"?new Response(null,m):m}let i=Ur(r.url),c=i?await hn(e,C):await e.next();if(!Ui(a.pathname))return c;let l=o==="HEAD"?c:await yi(c),p=new Response(o==="HEAD"?null:l.body,l);return p.headers.set("link",En),(Ht(a.pathname)||i)&&(p.headers.delete("x-frame-options"),p.headers.delete("content-security-policy-report-only"),p.headers.set("content-security-policy","frame-ancestors *")),$t(r,p)}n(He,"onRequest");async function Zn(e){return He(e)}n(Zn,"onRequest");async function ea(e){return He(e)}n(ea,"onRequest");async function ta(e){return He(e)}n(ta,"onRequest");var Di=Object.freeze(new Set(["","80","443"])),Hi=Object.freeze([".local",".localhost",".internal"]),ji=Object.freeze(new Set(["localhost"]));function Ve(e){return String(e||"").trim().toLowerCase()}n(Ve,"normalizeHostname");function Gi(e){let t=Ve(e);if(!/^\d{1,3}(\.\d{1,3}){3}$/.test(t))return!1;let r=t.split(".").map(o=>Number(o));return r.length!==4?!1:r.every(o=>Number.isInteger(o)&&o>=0&&o<=255)}n(Gi,"isIpv4Literal");function ra(e){let[t,r,o,a]=e.split(".").map(s=>Number(s));return(t<<24|r<<16|o<<8|a)>>>0}n(ra,"ipv4ToInt");function Bi(e,t,r){if(!Number.isInteger(r)||r<0||r>32)return!1;if(r===0)return!0;let o=4294967295<<32-r>>>0;return(e&o)===(t&o)}n(Bi,"inIpv4Cidr");function Wi(e){let t=Ve(e);return!t||!t.includes(":")?!1:/^[0-9a-f:]+$/i.test(t)}n(Wi,"isIpv6Literal");function Fi(e){let t=Ve(e);return!!(!t||t==="::1"||t==="0:0:0:0:0:0:0:1"||t.startsWith("fc")||t.startsWith("fd")||/^fe[89ab]/i.test(t))}n(Fi,"isBlockedIpv6");function zi(e,{blockedExact:t,blockedSuffixes:r}={}){let o=Ve(e);if(!o)return!0;let a=t||ji;if(a instanceof Set&&a.has(o))return!0;let s=r||Hi;if(Array.isArray(s))for(let i of s){let c=Ve(i);if(c&&(o===c||o.endsWith(c)))return!0}return!1}n(zi,"isBlockedHostname");function qi(e){let t=Ve(e);if(!t)return!0;if(Gi(t)){let r=ra(t),o=[{base:"0.0.0.0",bits:8},{base:"10.0.0.0",bits:8},{base:"127.0.0.0",bits:8},{base:"169.254.0.0",bits:16},{base:"172.16.0.0",bits:12},{base:"192.168.0.0",bits:16},{base:"100.64.0.0",bits:10}];for(let a of o){let s=ra(a.base);if(Bi(r,s,a.bits))return!0}return!1}return Wi(t)?Fi(t):!1}n(qi,"isBlockedIpLiteral");function jt(e,{allowedPorts:t}={}){let r=String(e||"").trim();if(!r)throw new Error("invalid_url");let o;try{o=new URL(r)}catch{throw new Error("invalid_url")}if(o.protocol!=="http:"&&o.protocol!=="https:")throw new Error("invalid_url");if(o.username||o.password)throw new Error("invalid_url");let a=t||Di,s=String(o.port||"");if(a instanceof Set&&!a.has(s))throw new Error("port_not_allowed");let i=Ve(o.hostname);if(!i)throw new Error("invalid_url");if(zi(i))throw new Error("blocked_host");if(qi(i))throw new Error("blocked_host");return o}n(jt,"parseAndValidateExternalUrl");function Gt(e){return String(e.headers.get("sec-fetch-site")||"").trim().toLowerCase()==="cross-site"}n(Gt,"shouldRejectCrossSiteFetch");var Vi={"content-type":"application/json; charset=utf-8","cache-control":"public, max-age=600"};function Ye(e,t={}){return new Response(JSON.stringify(e),{...t,headers:{...Vi,...t.headers||{}}})}n(Ye,"json");function Bt(...e){for(let t of e){if(!t)continue;let r=String(t).trim();if(r)return r}return null}n(Bt,"pickFirst");function Yi(e){let t=e.slice(0,8e4),r=t.match(/<title[^>]*>([^<]*)<\/title>/i),o=t.match(/<meta[^>]+property=["']og:title["'][^>]*content=["']([^"']+)["'][^>]*>/i),a=t.match(/<meta[^>]+property=["']og:description["'][^>]*content=["']([^"']+)["'][^>]*>/i),s=t.match(/<meta[^>]+name=["']description["'][^>]*content=["']([^"']+)["'][^>]*>/i),i=t.match(/<meta[^>]+property=["']og:image["'][^>]*content=["']([^"']+)["'][^>]*>/i),c=t.match(/<meta[^>]+property=["']og:site_name["'][^>]*content=["']([^"']+)["'][^>]*>/i);return{title:Bt(o?.[1],r?.[1]),description:Bt(a?.[1],s?.[1]),image:Bt(i?.[1]),siteName:Bt(c?.[1])}}n(Yi,"extractMeta");async function oa(e){let t=e.request.url,r=new URL(t);if(r.searchParams.get("ping")==="1")return Ye({ok:!0,ping:!0});let o=r.searchParams.get("url")||"";if(Gt(e.request))return Ye({ok:!1,error:"forbidden"},{status:403,headers:{"cache-control":"no-store"}});let a;try{a=jt(o)}catch{return Ye({ok:!1,error:"invalid_url"},{status:400,headers:{"cache-control":"no-store"}})}try{let s=await fetch(a.toString(),{headers:{"user-agent":"Mozilla/5.0 (compatible; HackaMapLinkPreview/1.0)",accept:"text/html,application/xhtml+xml"},redirect:"follow",cf:{cacheTtl:600,cacheEverything:!0}}),i=s.headers.get("content-type")||"";if(!s.ok)return Ye({ok:!1,error:"fetch_failed",status:s.status,url:a.toString()},{status:200,headers:{"cache-control":"no-store"}});if(!i.includes("text/html"))return Ye({ok:!0,url:a.toString(),domain:a.host,contentType:i,title:null,description:null,image:null,siteName:null});let c=await s.text(),l=Yi(c);return Ye({ok:!0,url:a.toString(),domain:a.host,contentType:i,...l})}catch(s){return Ye({ok:!1,error:"exception",message:s?.message||String(s),url:a.toString()},{status:200,headers:{"cache-control":"no-store"}})}}n(oa,"onRequestGet");var na=35e4;function Ji(e){let t=e;return t=t.replace(/<script\b[\s\S]*?<\/script>/gi,""),t=t.replace(/<iframe\b[\s\S]*?<\/iframe>/gi,""),t=t.replace(/<object\b[\s\S]*?<\/object>/gi,""),t=t.replace(/<embed\b[\s\S]*?>/gi,""),t=t.replace(/<noscript\b[\s\S]*?<\/noscript>/gi,""),t=t.replace(/\son[a-z]+\s*=\s*"[^"]*"/gi,""),t=t.replace(/\son[a-z]+\s*=\s*'[^']*'/gi,""),t}n(Ji,"stripActiveContent");function Xi({url:e,title:t,innerHtml:r}){let o=t?String(t).slice(0,140):"Preview",a=String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;");return`<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
...
</html>`}n(Xi,"buildWrapper");async function aa(e){let r=new URL(e.request.url).searchParams.get("url")||"";if(Gt(e.request))return new Response("Forbidden",{status:403,headers:{"cache-control":"no-store"}});let o;try{o=jt(r)}catch(a){let s=a instanceof Error?a.message:"invalid_url";return new Response(s,{status:400,headers:{"cache-control":"no-store"}})}try{let a=await fetch(o.toString(),{headers:{"user-agent":"Mozilla/5.0 (compatible; HackaMapLinkProxy/1.0)",accept:"text/html,application/xhtml+xml"},redirect:"follow",cf:{cacheTtl:600,cacheEverything:!0}}),s=a.headers.get("content-type")||"";if(!a.ok)return new Response(`Fetch failed (${a.status})`,{status:200,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}});if(!s.includes("text/html"))return new Response(`Unsupported content-type: ${s}`,{status:200,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"public, max-age=600"}});let i=await a.text();i.length>na&&(i=i.slice(0,na));let l=i.match(/<title[^>]*>([^<]*)<\/title>/i)?.[1]?.trim()||o.host;i=Ji(i),/<base\s/i.test(i)||(i=i.replace(/<head([^>]*)>/i,`<head$1><base href="${o.origin}/">`));let m=Xi({url:o.toString(),title:l,innerHtml:i});return new Response(m,{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"public, max-age=600","content-security-policy":"default-src 'none'; img-src https: data:; style-src 'unsafe-inline' https:; font-src https: data:; frame-ancestors 'self';"}})}catch(a){return new Response(`Exception: ${a?.message||String(a)}`,{status:200,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}})}}n(aa,"onRequestGet");var Lr="api.openai.com",Kr="gateway.ai.cloudflare.com",$r="api.cloudflare.com",Wt="api.miromind.ai",Ft="apihub.agnes-ai.com",zt="ark.ap-southeast.bytepluses.com",Dr="ark.eu-west.bytepluses.com",ia=new Set(["localhost","127.0.0.1","0.0.0.0"]),Pe=n(e=>String(e||"").trim().toLowerCase(),"normalizeHost"),pe=n((e,t)=>String(e.get(t)||"").trim(),"readHeader"),ca=n(e=>ia.has(Pe(e)),"isLocalHost"),sa=n(e=>{let t=String(e||"").trim();if(!t)return new Set;let r=new Set;return t.split(",").map(o=>Pe(o)).filter(Boolean).forEach(o=>r.add(o)),r},"parseCsvSet"),la=n((e,{includeOpenAi:t=!1,includeAiGateway:r=!1,includeMiroMind:o=!1,includeAgnes:a=!1,includeBytePlus:s=!1}={})=>{let i=sa(e.KNOWGRPH_INTEGRATION_ALLOWED_HOSTS),c=sa(e.KNOWGRPH_CHAT_PROXY_ALLOWED_HOSTS),l=i.size?i:c,p=l.size?l:new Set([...ia]);return t&&p.add(Lr),r&&(p.add(Kr),p.add($r)),o&&p.add(Wt),a&&p.add(Ft),s&&(p.add(zt),p.add(Dr)),p},"parseAllowedHosts"),ce=n(e=>{let t=pe(e.headers,"origin");if(!t)return{};let r="";try{r=Pe(new URL(t).host)}catch{return{}}let o=Pe(new URL(e.url).host);return r===o||r.startsWith("localhost:")||r.startsWith("127.0.0.1:")?{"access-control-allow-origin":t,vary:"Origin"}:{}},"corsHeaders"),ge=n((e,t,r)=>new Response(JSON.stringify(t),{status:r,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store",...ce(e)}}),"jsonResponse");var qt={"content-type":"application/json; charset=utf-8","cache-control":"no-store"};function yt(e,t,r=200){return new Response(JSON.stringify(t),{status:r,headers:{...qt,...ce(e)}})}n(yt,"jsonResponse");async function Qi(e){let t=new URL("/knowgrph/imports/hackamap/hackamap-graph.json",e.url),r=await fetch(t.toString(),{redirect:"follow"});return r.ok?await r.json():null}n(Qi,"fetchHackamapGraphJson");async function Je(e,t){let r=new URL(t,e.url),o=await fetch(r.toString(),{redirect:"follow"});return o.ok?await o.json():null}n(Je,"fetchHackamapJson");async function Zi(e){let t=await Je(e,"/knowgrph/imports/hackamap/hackamap_api_graph.json");return ma(t)?t:null}n(Zi,"fetchHackamapApiGraphJson");async function ec(e){let t=await Je(e,"/knowgrph/imports/hackamap/hackamap_pipeline.json");return t&&typeof t=="object"&&!Array.isArray(t)?t:{}}n(ec,"fetchHackamapPipelineJson");async function ua(e){let t=await Je(e,"/knowgrph/imports/hackamap/hackamap_query_presets.json");return Array.isArray(t)?t.filter(Boolean):[]}n(ua,"fetchHackamapQueryPresetsJson");async function da(e){let t=await Je(e,"/knowgrph/imports/hackamap/query-outputs/query-runs.manifest.json");return t&&typeof t=="object"&&!Array.isArray(t)?t:{}}n(da,"fetchHackamapQueryRunsManifestJson");function ma(e){return!e||typeof e!="object"||Array.isArray(e)?!1:Array.isArray(e.nodes)&&Array.isArray(e.edges)}n(ma,"isApiGraphPayload");function ha(e,t){let r=String(e&&e.output&&e.output.per_table_prefix||e?.id||t?.preset||"").trim(),o=String(t?.output_suffix||"").trim();return o?`${r}-${o}`:r}n(ha,"buildHackamapTablePrefix");function Hr(e,t){if(!Array.isArray(e))return[];let r=[];for(let o of e){if(!o||typeof o!="object"||Array.isArray(o))continue;let a=String(o[t]||"").trim();a&&r.push(a)}return r}n(Hr,"collectRowIds");async function tc(e,t){let r=await Je(e,t);return Array.isArray(r)?r.length:0}n(tc,"countHackamapQueryRows");async function rc(e,t,r){let o=ha(t,r);if(!o)return{};let a=["events","demos","sources","organizer","team","techstack"],s=await Promise.all(a.map(async i=>[i,await tc(e,`/knowgrph/imports/hackamap/query-outputs/${i}.${o}.query.json`)]));return Object.fromEntries(s.filter(([,i])=>i>0))}n(rc,"readHackamapRunTableCounts");function jr(e){return Array.isArray(e)?e.map(jr):!e||typeof e!="object"?e:Object.fromEntries(Object.entries(e).sort(([t],[r])=>String(t).localeCompare(String(r))).map(([t,r])=>[t,jr(r)]))}n(jr,"sortObjectKeys");function oc(e){try{return JSON.stringify(jr(e))}catch{return""}}n(oc,"stableParamSignature");function nc(e){return typeof e=="string"?{value:e,label:e}:{value:e,label:JSON.stringify(e)}}n(nc,"toBuilderOption");function ac(e,t){return e.map(r=>{let o=String(r?.id||"").trim();if(!o)return null;let a=r?.params&&typeof r.params=="object"&&!Array.isArray(r.params)?r.params:{},s=t.filter(l=>String(l?.preset||"").trim()===o),i=Array.from(new Set([...Object.keys(a),...s.flatMap(l=>l?.params&&typeof l.params=="object"&&!Array.isArray(l.params)?Object.keys(l.params):[])])).sort((l,p)=>String(l).localeCompare(String(p))),c=Object.fromEntries(i.map(l=>{let p=new Set,m=[],f=[a[l],...s.map(A=>A?.params&&typeof A.params=="object"&&!Array.isArray(A.params)?A.params[l]:void 0)];for(let A of f){if(typeof A>"u")continue;let S=oc(A);!S||p.has(S)||(p.add(S),m.push(nc(A)))}return[l,m]}));return{id:o,title:String(r?.title||o).trim(),params:a,param_keys:i,published_param_options:c}}).filter(Boolean)}n(ac,"buildHackamapPresetRuntimeEntries");async function sc(e){let[t,r,o]=await Promise.all([ec(e),ua(e),da(e)]),a=t&&typeof t=="object"?t.runtime||{}:{},s=String(a?.query_selection?.default_run_id||"").trim()||"enhanced",i=Array.isArray(o?.runs)?o.runs:[],c=(await Promise.all(i.map(async l=>{let p=String(l?.id||"").trim(),m=String(l?.preset||"").trim();if(!p)return null;let f=r.find(S=>String(S?.id||"").trim()===m),A=await rc(e,f,l);return{id:p,preset:m,title:String(l?.title||l?.id||"").trim(),params:l?.params&&typeof l.params=="object"&&!Array.isArray(l.params)?l.params:{},output_suffix:String(l?.output_suffix||"").trim(),is_default:p===s,table_counts:A}}))).filter(l=>l?.id);return{ok:!0,runtime:{...a&&typeof a=="object"?a:{},presets:ac(r,c),runs:c}}}n(sc,"buildHackamapRuntimeMeta");async function ic(e,t){let r=String(t||"").trim();if(!r)return null;let[o,a]=await Promise.all([ua(e),da(e)]),i=(Array.isArray(a?.runs)?a.runs:[]).find(O=>String(O?.id||"").trim()===r);if(!i)return null;let c=o.find(O=>String(O?.id||"").trim()===String(i?.preset||"").trim()),l=ha(c,i);if(!l)return null;let[p,m]=await Promise.all([Je(e,`/knowgrph/imports/hackamap/query-outputs/events.${l}.query.json`),Je(e,`/knowgrph/imports/hackamap/query-outputs/demos.${l}.query.json`)]),f=new Set(Hr(p,"id")),A=new Set(Hr(m,"id")),S=Hr(m,"event_id");for(let O of S)f.add(O);return{eventIds:f,demoIds:A}}n(ic,"readHackamapQueryRunSelection");function pa(e,t,r){if(!r||!ma(e))return e;if(r.eventIds.size===0&&r.demoIds.size===0)return{...e,meta:{...e?.meta&&typeof e.meta=="object"?e.meta:{},selected_run_id:t,selected_run_filter_skipped:"no-event-demo-rows"}};let o=new Set;r.eventIds.forEach(c=>o.add(`Event:${c}`)),r.demoIds.forEach(c=>o.add(`Demo:${c}`));let a=Array.isArray(e.nodes)?e.nodes.filter(c=>o.has(String(c?.id||"").trim())):[],s=new Set(a.map(c=>String(c?.id||"").trim()).filter(Boolean)),i=Array.isArray(e.edges)?e.edges.filter(c=>s.has(String(c?.source||"").trim())&&s.has(String(c?.target||"").trim())):[];return{...e,nodes:a,edges:i,meta:{...e?.meta&&typeof e.meta=="object"?e.meta:{},selected_run_id:t,selected_event_count:r.eventIds.size,selected_demo_count:r.demoIds.size,total_problems:a.filter(c=>String(c?.type||"").trim()==="problem").length,total_solutions:a.filter(c=>String(c?.type||"").trim()==="solution").length}}}n(pa,"filterHackamapApiGraphPayloadByRun");function cc(e){let t=Array.isArray(e?.nodes)?e.nodes:[],r=Array.isArray(e?.links)?e.links:[],o=[],a=new Set;for(let i of t){let c=String(i?.id||"").trim(),l=String(i?.type||"").trim(),p=String(i?.label||"").trim();if(!(!c||!l||!p)){if(l==="Event"){o.push({id:c,type:"problem",label:p,cluster:"Event"}),a.add(c);continue}l==="Demo"&&(o.push({id:c,type:"solution",label:p,cluster:"Demo"}),a.add(c))}}let s=[];for(let i of r){let c=String(i?.source||"").trim(),l=String(i?.target||"").trim(),p=String(i?.type||"").trim();!c||!l||p==="has_demo"&&(!a.has(c)||!a.has(l)||s.push({source:c,target:l,type:"has_demo",strength:.35}))}return{nodes:o,edges:s,meta:{source:"hackamap-graph.json:fallback",total_problems:o.filter(i=>i.type==="problem").length,total_solutions:o.filter(i=>i.type==="solution").length,...e?.content_signature?{content_signature:String(e.content_signature)}:{}}}}n(cc,"toBipartiteApiPayload");async function ga(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase(),o=new URL(t.url);if(r==="OPTIONS")return new Response(null,{status:204,headers:{...ce(t),"access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(r!=="GET"&&r!=="HEAD")return yt(t,{ok:!1,error:"unsupported_method"},405);if(String(o.searchParams.get("view")||"").trim().toLowerCase()==="meta"){let m=await sc(t);return r==="HEAD"?new Response(null,{status:200,headers:{...qt,...ce(t)}}):yt(t,m,200)}let a=String(o.searchParams.get("run")||"").trim(),s=await ic(t,a),i=await Zi(t);if(i){let m=pa(i,a,s);return r==="HEAD"?new Response(null,{status:200,headers:{...qt,...ce(t)}}):yt(t,m,200)}let c=await Qi(t);if(!c)return yt(t,{ok:!1,error:"missing_hackamap_graph",hint:"/knowgrph/imports/hackamap/{hackamap_api_graph.json,hackamap-graph.json} not found"},404);let l=cc(c),p=pa(l,a,s);return r==="HEAD"?new Response(null,{status:200,headers:{...qt,...ce(t)}}):yt(t,p,200)}n(ga,"onRequest");var lc=!0,fa=600,ya={"content-type":"application/json; charset=utf-8","cache-control":`public, max-age=${fa}`};function Xe(e,t,r={}){return new Response(JSON.stringify(t),{...r,headers:{...ya,...r.headers||{},...ce(e)}})}n(Xe,"jsonResponse");function pc(e){try{let t=new URL(String(e));return t.protocol==="http:"||t.protocol==="https:"}catch{return!1}}n(pc,"isHttpUrl");function wt(e){return String(e||"").trim().toLowerCase()}n(wt,"normalizeHost");function Gr(e,{exact:t,suffixes:r}){let o=wt(e);return o?!!(Array.isArray(t)&&t.some(a=>o===wt(a))||Array.isArray(r)&&r.some(a=>o===wt(a)||o.endsWith(`.${wt(a)}`))):!1}n(Gr,"isHostMatch");function uc(e){let t=wt(e.hostname),r=e.toString();return Gr(t,{suffixes:["linkedin.com"]})?new URL(`https://www.linkedin.com/embeds/oembed.json?url=${encodeURIComponent(r)}`):Gr(t,{exact:["twitter.com","x.com"],suffixes:["twitter.com","x.com"]})?new URL(`https://publish.twitter.com/oembed?omit_script=1&url=${encodeURIComponent(r)}`):Gr(t,{exact:["reddit.com"],suffixes:["reddit.com"]})?new URL(`https://www.reddit.com/oembed?url=${encodeURIComponent(r)}`):null}n(uc,"buildOembedUpstreamUrl");async function dc({upstreamUrl:e}){return await fetch(e.toString(),{headers:{"user-agent":"Mozilla/5.0 (compatible; OEmbedProxy/1.0)",accept:"application/json,text/json;q=0.9,*/*;q=0.1"},redirect:"follow",cf:{cacheTtl:fa,cacheEverything:!0}})}n(dc,"fetchJsonUpstream");async function wa(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase(),o=new URL(t.url);if(r==="OPTIONS")return new Response(null,{status:204,headers:{...ce(t),"access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(!["GET","HEAD"].includes(r))return Xe(t,{ok:!1,error:"unsupported_method"},{status:405});if(o.searchParams.get("ping")==="1")return Xe(t,{ok:!0,ping:!0});if(lc)return Xe(t,{ok:!1,error:"disabled_by_policy"},{status:200,headers:{"cache-control":"no-store"}});let a=o.searchParams.get("url")||"";if(!pc(a))return Xe(t,{ok:!1,error:"invalid_url"},{status:400,headers:{"cache-control":"no-store"}});let s;try{s=new URL(a)}catch{return Xe(t,{ok:!1,error:"invalid_url"},{status:400,headers:{"cache-control":"no-store"}})}let i=uc(s);if(!i)return Xe(t,{ok:!1,error:"unsupported_provider"},{status:400,headers:{"cache-control":"no-store"}});let c=await dc({upstreamUrl:i}),l=new Headers(c.headers);l.delete("content-length"),l.set("cache-control",c.ok?ya["cache-control"]:"no-store");for(let[m,f]of Object.entries(ce(t)))l.set(m,f);if(r==="HEAD")return new Response(null,{status:c.status,headers:l});let p=await c.text();try{JSON.parse(p)}catch{return Xe(t,{ok:!1,error:"invalid_upstream_json",status:c.status},{status:502,headers:{"cache-control":"no-store"}})}return l.set("content-type","application/json; charset=utf-8"),new Response(p,{status:c.status,headers:l})}n(wa,"onRequest");var Sa="/__chat_proxy",Vt="agnes-ai",Yt="byteplus-modelark",Jt="miromind",mc="x-kg-ai-gateway-route",hc="x-kg-ai-gateway-metadata",gc="x-kg-ai-gateway-cache-ttl",fc=n(e=>{let t=Pe(e);return t==="openai"?"openai":t===Yt||t==="byteplus"?Yt:t===Jt||t==="miromind-api"?Jt:t===Vt||t==="agnes"||t==="agnes-ai-api"?Vt:t},"normalizeProviderId"),yc=n(e=>Pe(e)===Ft,"isAgnesHost"),wc=n(e=>{let t=Pe(e);return t===zt||t===Dr},"isBytePlusHost"),Sc=n(e=>Pe(e)===Wt,"isMiroMindHost"),_c=n(e=>{let t=Pe(e);return t===Kr||t===$r},"isAiGatewayHost"),bc=n(e=>{let t=String(e||"").trim();return/^dynamic\/[a-z0-9._/-]+$/i.test(t)?t.slice(0,128):""},"sanitizeAiGatewayRoute"),Rc=n(e=>{let t=String(e||"").trim();if(!t)return"";try{let r=JSON.parse(t),o=Object.entries(r||{}).filter(([,a])=>typeof a=="string"||typeof a=="number"||typeof a=="boolean").slice(0,5).map(([a,s])=>[String(a||"").trim().slice(0,64),typeof s=="string"?s.trim().slice(0,160):s]).filter(([a])=>a);return o.length?JSON.stringify(Object.fromEntries(o)):""}catch{return""}},"sanitizeAiGatewayMetadata"),Ac=n(e=>{let t=Number(String(e||"").trim());return Number.isFinite(t)?String(Math.max(1,Math.min(86400,Math.floor(t)))):""},"sanitizeAiGatewayCacheTtl"),Pc=n((e,t,r)=>{let a=`${e.pathname==="/"?"":String(e.pathname||"").replace(/\/+$/g,"")}${t.startsWith("/")?t:`/${t}`}`;return new URL(`${a}${r||""}`,`${e.protocol}//${e.host}`)},"buildUpstreamUrl"),kc=n(({provider:e,requestedUpstream:t,env:r})=>e==="openai"?t||"https://api.openai.com":e===Jt?t||`https://${Wt}`:e===Vt?t||`https://${Ft}`:e===Yt?t||String(r.KNOWGRPH_CHAT_PROXY_UPSTREAM||"").trim()||`https://${zt}`:t||String(r.KNOWGRPH_CHAT_PROXY_UPSTREAM||"").trim(),"pickUpstreamBase");async function _a(e){let{request:t,env:r}=e,o=String(t.method||"GET").toUpperCase(),a=new URL(t.url);if(o==="OPTIONS")return new Response(null,{status:204,headers:{"access-control-allow-origin":pe(t.headers,"origin")||"*","access-control-allow-methods":"GET, HEAD, POST, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(!["GET","HEAD","POST"].includes(o))return ge(t,{ok:!1,error:"Unsupported method"},405);let s=fc(pe(t.headers,"x-kg-chat-provider")),i=bc(pe(t.headers,mc)),c=Rc(pe(t.headers,hc)),l=Ac(pe(t.headers,gc)),p=String(r.KNOWGRPH_CHAT_PROXY_AI_GATEWAY_BASE_URL||"").trim(),m=String(r.KNOWGRPH_CHAT_PROXY_AI_GATEWAY_GATEWAY_ID||"").trim(),f=s==="openai"&&!!p&&!!i,A=kc({provider:s,requestedUpstream:f?p:pe(t.headers,"x-kg-chat-upstream"),env:r});if(!A)return ge(t,{ok:!1,error:"Missing chat proxy upstream configuration"},500);let S;try{S=new URL(A)}catch{return ge(t,{ok:!1,error:"Invalid chat proxy upstream configuration"},500)}let O=la(r,{includeOpenAi:!0,includeAiGateway:!0,includeMiroMind:!0,includeAgnes:!0,includeBytePlus:!0}),$=Pe(S.hostname);if(!O.has($))return ge(t,{ok:!1,error:"Chat proxy upstream host is not allowed"},403);if(!ca($)&&S.protocol!=="https:")return ge(t,{ok:!1,error:"Chat proxy requires HTTPS for non-local upstream hosts"},403);let N=f||_c($),F=!N&&(s==="openai"||$===Lr),Q=s===Jt||Sc($),z=s===Vt||yc($),x=s===Yt||wc($),T=pe(t.headers,"x-kg-chat-api-key"),te=String(r.KNOWGRPH_CHAT_PROXY_AI_GATEWAY_TOKEN||r.AI_GATEWAY_TOKEN||r.CLOUDFLARE_API_TOKEN||"").trim(),B=String(r.KNOWGRPH_CHAT_PROXY_OPENAI_API_KEY||r.OPENAI_API_KEY||"").trim(),le=String(r.KNOWGRPH_CHAT_PROXY_MIROMIND_API_KEY||r.MIROMIND_API_KEY||"").trim(),U=String(r.KNOWGRPH_CHAT_PROXY_AGNES_API_KEY||r.AGNES_API_KEY||"").trim(),L=String(r.KNOWGRPH_CHAT_PROXY_BYTEPLUS_API_KEY||r.BYTEPLUS_API_KEY||"").trim(),V=(T||te).slice(0,512),se=(T||B).slice(0,512),Se=(T||le).slice(0,512),ke=(T||U).slice(0,512),P=(T||L).slice(0,512),w=x?P:N?V:z?ke:Q?Se:se;if(N&&!w)return ge(t,{ok:!1,error:"Missing Cloudflare AI Gateway token for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_AI_GATEWAY_TOKEN or AI_GATEWAY_TOKEN)"},401);if(F&&!se)return ge(t,{ok:!1,error:"Missing OpenAI API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_OPENAI_API_KEY or OPENAI_API_KEY)"},401);if(Q&&!w)return ge(t,{ok:!1,error:"Missing MiroMind API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_MIROMIND_API_KEY or MIROMIND_API_KEY)"},401);if(z&&!w)return ge(t,{ok:!1,error:"Missing Agnes API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_AGNES_API_KEY or AGNES_API_KEY)"},401);if(x&&!w)return ge(t,{ok:!1,error:"Missing BytePlus API key for chat proxy upstream (set KNOWGRPH_CHAT_PROXY_BYTEPLUS_API_KEY or BYTEPLUS_API_KEY)"},401);if(o==="POST"&&!pe(t.headers,"content-type").toLowerCase().includes("application/json"))return ge(t,{ok:!1,error:"Chat proxy expects application/json payloads"},415);let y=a.pathname.startsWith(Sa)&&a.pathname.slice(Sa.length)||"/v1/chat/completions",d=y.startsWith("/")?y:`/${y}`,g=Pc(S,d,a.search||""),h=new Headers,R=pe(t.headers,"content-type"),E=pe(t.headers,"accept");R&&h.set("content-type",R),E&&h.set("accept",E),(F||N||Q||z||x)&&h.set("authorization",`Bearer ${w}`);let q=pe(t.headers,"x-client-request-id").slice(0,512);q&&h.set("x-client-request-id",q),N&&m&&h.set("cf-aig-gateway-id",m),c&&h.set("cf-aig-metadata",c),l&&h.set("cf-aig-cache-ttl",l);let D=new AbortController,K=Number(r.KNOWGRPH_CHAT_PROXY_TIMEOUT_MS),Y=Number.isFinite(K)?Math.max(5e3,Math.min(18e4,Math.floor(K))):9e4,b=setTimeout(()=>D.abort(),Y);try{let j=o==="GET"||o==="HEAD"?void 0:t.body;if(f&&o==="POST"){let Ge=await t.clone().text();try{let Be=JSON.parse(Ge||"{}");Be.model=i,j=JSON.stringify(Be)}catch{j=Ge}}let ue=await fetch(g.toString(),{method:o,headers:h,body:j,signal:D.signal,redirect:"follow"}),v=new Headers(ue.headers);v.delete("content-length"),v.delete("www-authenticate"),v.set("cache-control","no-store");let J=pe(t.headers,"origin");return J&&(v.set("access-control-allow-origin",J),v.set("vary","Origin")),o==="HEAD"?new Response(null,{status:ue.status,statusText:ue.statusText,headers:v}):new Response(ue.body,{status:ue.status,statusText:ue.statusText,headers:v})}catch(j){let ue=j&&typeof j=="object"&&"message"in j?String(j.message||""):"",v=D.signal.aborted||/aborted|timeout/i.test(ue);return ge(t,{ok:!1,error:ue||"Failed to reach chat upstream"},v?504:502)}finally{clearTimeout(b)}}n(_a,"onRequest");function xc(e){let t=e.map(r=>r==null?"":typeof r=="boolean"?r?"1":"0":typeof r=="number"?Number.isFinite(r)?String(r):"":String(r)).join("|");return`rich-media-preview:${st(t)}`}n(xc,"buildRichMediaPreviewSemanticKey");var Ra="png";function St(e){let t=typeof e=="number"?e:Number(String(e??"").trim());if(!Number.isFinite(t))return null;let r=Math.max(0,Number(t.toFixed(3)));return Number.isFinite(r)?r:null}n(St,"normalizeRemoteVideoFrameSeconds");function Ec(e){return String(St(e)??0).replace(/\./g,"_")}n(Ec,"formatRemoteVideoFrameSecondsForFileName");function Qt(e){let t=String(e||"").trim().toLowerCase();return t==="jpg"||t==="jpeg"?"jpg":"png"}n(Qt,"normalizeRemoteVideoFrameFormat");function Br(e){let t=String(e.sourceUrl||"").trim(),r=St(e.timeSeconds)??0,o=Qt(e.format||Ra);return xc(["remote-video-frame",t,r,o])}n(Br,"buildRemoteVideoFrameSemanticKey");function Aa(e){let t=St(e.timeSeconds)??0,r=Qt(e.format||Ra),o=Br({...e,timeSeconds:t,format:r});return`frame-${o.split(":").pop()||st(o)}-t${Ec(t)}.${r}`}n(Aa,"buildRemoteVideoFrameFileName");var Xt=n(e=>{let t=String(e||"").trim();return t&&/^[A-Za-z0-9_-]{6,128}$/.test(t)?t:null},"normalizeYouTubeIdLikeValue"),ba=n(e=>{try{let t=new URL(String(e||"").trim()),r=String(t.hostname||"").toLowerCase();if(r==="youtu.be"||r.endsWith(".youtu.be")){let o=t.pathname.replace(/^\/+/,"").split("/")[0]?.trim()||"";return Xt(o)}if(r==="youtube.com"||r.endsWith(".youtube.com")||r==="youtube-nocookie.com"||r.endsWith(".youtube-nocookie.com")){let o=String(t.searchParams.get("v")||"").trim();if(o)return Xt(o);let a=t.pathname.split("/").filter(Boolean),s=a[0]||"",i=a[1]||"";if((s==="embed"||s==="shorts"||s==="live")&&i)return Xt(i);if(s==="watch"){let c=String(t.searchParams.get("v")||"").trim();return Xt(c)}}}catch{return null}return null},"readYouTubeIdFromUrl");function Cc(e){let t=String(e||"").trim().replace(/^<|>$/g,"").trim();for(;/[),.;:!?]$/.test(t);){let r=t.slice(0,-1).trim();if(!r)break;let o=ba(t),a=ba(r);if(!a||o&&o!==a)break;t=r}return t}n(Cc,"stripYouTubeUrlTrailingPunctuation");function Pa(e){let t=n(r=>{let o=String(r||"").trim();if(!o)return null;if(/^\d+$/.test(o))return Number(o);let a=o.match(/^(?:(\d+)h)?(?:(\d+)m)?(?:(\d+)s)?$/i);if(!a)return null;let s=a[1]?Number(a[1]):0,i=a[2]?Number(a[2]):0,c=a[3]?Number(a[3]):0,l=s*3600+i*60+c;return l>0&&Number.isFinite(l)?l:null},"parseChunk");try{let r=new URL(Cc(e)),o=r.searchParams.get("t")||r.searchParams.get("start")||"",a=r.hash&&new URLSearchParams(r.hash.replace(/^#/,"")).get("t")||"";return t(o)??t(a)}catch{return null}}n(Pa,"parseYouTubeStartSeconds");var Tc="/image/knowgrph/video-frame",vc=4096,Oc=720*60,Ic=/^frame-[a-f0-9]+-t\d+\.(?:png|jpg)$/i,qr={"access-control-allow-origin":"*","access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"accept, content-type"},zr=n(e=>String(e||"").replace(/\s+/g," ").trim(),"cleanText"),Wr=n((e,t=200,r="GET")=>new Response(r==="HEAD"?null:JSON.stringify(e),{status:t,headers:{...qr,"content-type":"application/json; charset=utf-8","cache-control":"no-store"}}),"jsonResponse"),Fr=n((e,t=200,r="GET")=>new Response(r==="HEAD"?null:e,{status:t,headers:{...qr,"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}}),"textResponse"),Mc=n((e,t)=>e===t||e.endsWith(`.${t}`),"hostMatches"),Nc=n(e=>{let t=zr(e?.KG_VIDEO_FRAME_ALLOWED_HOSTS);return t?t.split(",").map(r=>zr(r).toLowerCase()).filter(Boolean):["youtube.com","youtu.be","youtube-nocookie.com","bilibili.com","b23.tv"]},"readAllowedHosts"),Uc=n(e=>zr(e).replace(/^<|>$/g,"").trim(),"unwrapUrlInput"),Lc=n((e,t)=>{try{let r=new URL(e);if(r.protocol!=="https:"&&r.protocol!=="http:")return!1;let o=r.hostname.toLowerCase();return Nc(t).some(a=>Mc(o,a))}catch{return!1}},"isAllowedSourceUrl"),Kc=n((e,t)=>{let r=new URL(e.url),o=Uc(r.searchParams.get("url")||"");if(!o)return{error:"Missing url parameter"};if(o.length>vc)return{error:"Video URL is too long"};if(!Lc(o,t))return{error:"Video frame extraction is limited to supported remote video hosts"};let a=St(r.searchParams.get("time"))??Pa(o);if(a==null)return{error:"Missing time parameter"};let s=Math.min(Oc,Math.max(0,a)),i=Qt(r.searchParams.get("format")||"png"),c=Aa({sourceUrl:o,timeSeconds:s,format:i});if(!Ic.test(c))return{error:"Invalid frame cache key"};let l=`${Tc}/${c}`;return{sourceUrl:o,timeSeconds:s,format:i,fileName:c,publicUrl:l,semanticKey:Br({sourceUrl:o,timeSeconds:s,format:i})}},"readFrameRequest"),$c=n(async(e,t,r)=>{let o=new URL(t,e.request.url),a=new Request(o.toString(),{method:r});return typeof e.env?.ASSETS?.fetch=="function"?await e.env.ASSETS.fetch(a):await fetch(a)},"fetchStaticAsset"),Dc=n(e=>`Frame has not been generated yet. Run the local video-frame extractor and publish ${e.publicUrl}.`,"missingFrameMessage"),Hc=n((e,t)=>{let r=new Headers;r.set("content-type",t.format==="jpg"?"image/jpeg":"image/png"),r.set("cache-control","public, max-age=31536000, immutable"),r.set("access-control-allow-origin","*");let o=e.headers.get("content-length");o&&r.set("content-length",o);let a=e.headers.get("etag");return a&&r.set("etag",a),r},"imageResponseHeaders");async function ka(e){let t=e.request;if(t.method==="OPTIONS")return new Response(null,{status:204,headers:qr});if(t.method!=="GET"&&t.method!=="HEAD")return Fr("Method not allowed",405,t.method);let r=new URL(t.url).searchParams.get("emit")==="json",o=Kc(t,e.env||{});if("error"in o)return r?Wr({ok:!1,error:o.error},400,t.method):Fr(o.error,400,t.method);let a=r&&t.method!=="HEAD"?"GET":r||t.method==="HEAD"?"HEAD":"GET",s=await $c(e,o.publicUrl,a);if(!s.ok){let i=Dc(o);return r?Wr({ok:!1,error:i,publicUrl:o.publicUrl,semanticKey:o.semanticKey},404,t.method):Fr(i,404,t.method)}if(r){let i=Number(s.headers.get("content-length")||0);return(!Number.isFinite(i)||i<=0)&&t.method!=="HEAD"&&(i=(await s.arrayBuffer()).byteLength),Wr({ok:!0,imageUrl:o.publicUrl,publicUrl:o.publicUrl,semanticKey:o.semanticKey,cached:!0,bytes:Number.isFinite(i)?Math.max(0,Math.floor(i)):0,timeSeconds:o.timeSeconds,format:o.format},200,t.method)}return new Response(t.method==="HEAD"?null:s.body,{status:200,headers:Hc(s,o)})}n(ka,"onRequest");var xa={"content-type":"application/json; charset=utf-8","cache-control":"no-store","access-control-allow-origin":"*","access-control-allow-methods":"GET, HEAD, POST, OPTIONS","access-control-allow-headers":"content-type"},Zt=n((e,t=200,r="GET")=>new Response(r==="HEAD"?null:JSON.stringify(e),{status:t,headers:xa}),"jsonResponse"),re=n(e=>String(e||"").replace(/\s+/g," ").trim(),"cleanText"),jc=n(e=>{try{let t=new URL(String(e||"").trim());if(/youtu\.be$/i.test(t.hostname))return re(t.pathname.split("/").filter(Boolean)[0]);if(/youtube\.com$/i.test(t.hostname)||/youtube-nocookie\.com$/i.test(t.hostname)){let r=re(t.searchParams.get("v"));if(r)return r;let o=t.pathname.split("/").filter(Boolean),a=o.findIndex(s=>["embed","shorts","live"].includes(s));if(a>=0)return re(o[a+1])}}catch{}return""},"readVideoId"),Gc=n((e,t)=>{let r=e.indexOf(t);if(r<0)return null;let o=e.indexOf("{",r);if(o<0)return null;let a=0,s=!1,i=!1;for(let c=o;c<e.length;c+=1){let l=e[c];if(s){i?i=!1:l==="\\"?i=!0:l==='"'&&(s=!1);continue}if(l==='"')s=!0;else if(l==="{")a+=1;else if(l==="}"&&(a-=1,a===0))return e.slice(o,c+1)}return null},"extractJsonAfter"),Bc=n(e=>{for(let t of["ytInitialPlayerResponse =","ytInitialPlayerResponse="]){let r=Gc(e,t);if(r)try{return JSON.parse(r)}catch{}}return null},"parsePlayerResponse"),Wc=n((e,t)=>{let r=re(t||"en").toLowerCase();return e.find(o=>re(o.languageCode).toLowerCase()===r)||e.find(o=>re(o.languageCode).toLowerCase().startsWith(r.split("-")[0]))||e.find(o=>re(o.kind)!=="asr")||e[0]||null},"pickCaptionTrack"),Fc=n(e=>{let t=new URL(e);return t.searchParams.set("fmt","json3"),t.toString()},"withJsonCaptionFormat"),zc=n(e=>(Array.isArray(e?.events)?e.events:[]).map(r=>{let o=Array.isArray(r.segs)?re(r.segs.map(i=>i?.utf8||"").join("")):"",a=Number(r.tStartMs)/1e3,s=Number(r.dDurationMs||0)/1e3;return o&&Number.isFinite(a)?{text:o,start:a,duration:Number.isFinite(s)?s:0}:null}).filter(Boolean),"parseCaptionJson3"),qc=n(e=>String(e||"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;|&apos;/g,"'"),"decodeXmlText"),Vc=n(e=>{let t=[],r=/<text\b([^>]*)>([\s\S]*?)<\/text>/gi,o=null;for(;o=r.exec(String(e||""));){let a=o[1]||"",s=Number(a.match(/\bstart="([^"]+)"/i)?.[1]),i=Number(a.match(/\bdur="([^"]+)"/i)?.[1]||0),c=re(qc(o[2]||""));c&&Number.isFinite(s)&&t.push({text:c,start:s,duration:Number.isFinite(i)?i:0})}return t},"parseCaptionXml"),Yc=n((e,t)=>{let r=String(e||"").trim();if(!r)return[];if(String(t||"").toLowerCase().includes("json")||r.startsWith("{")||r.startsWith("["))try{return zc(JSON.parse(r))}catch{return[]}return Vc(r)},"parseCaptionResponseText"),Jc=n(e=>{let t=Math.max(0,Math.floor(Number(e)||0)),r=Math.floor(t/60),o=String(t%60).padStart(2,"0");return`${r}:${o}`},"formatTimestamp"),Xc=n((e,t)=>{let r=new URL(e);return r.searchParams.set("t",`${Math.max(0,Math.floor(Number(t)||0))}s`),r.toString()},"timestampUrl"),Qc=n(({title:e,sourceUrl:t,videoId:r,authorName:o,thumbnailUrl:a,segments:s})=>[`# ${e||`YouTube ${r}`}`,"",`Video ID: ${r}`,o?`Author: ${o}`:"",`Source: [${t}](${t})`,a?`[![${e||r}](${a})](${t})`:"","",s.length>0?"## Transcript":"## Video Source","",...s.length>0?s.map(i=>`[${Jc(i.start)}](${Xc(t,i.start)}) ${i.text}`):["Captions were not available from the source at import time.","The source URL, title, author, and thumbnail remain available for downstream storyboard reconstruction."],""].filter(i=>i!=="").join(`
`),"buildMarkdown"),Vr=n(({videoId:e,sourceUrl:t,title:r,authorName:o,thumbnailUrl:a,lang:s,languageCode:i,segments:c,captionStatus:l})=>{let p={type:"rag:YouTubeTranscript",video_id:e,source_url:t,title:r,author_name:o,thumbnail_url:a,language_code:re(i)||s,caption_status:l,segment_count:c.length,duration:c.reduce((m,f)=>Math.max(m,f.start+f.duration),0),segments:c};return{ok:!0,name:`youtube-${e.toLowerCase()}.md`,markdown:Qc({title:r,sourceUrl:t,videoId:e,authorName:o,thumbnailUrl:a,segments:c}),transcript:p}},"buildPayload");async function Zc({sourceUrl:e,lang:t="en",fetchImpl:r=fetch}){let o=jc(e);if(!o)return{ok:!1,error:"unsupported_youtube_url"};let a=`https://www.youtube.com/watch?v=${encodeURIComponent(o)}`,[s,i]=await Promise.all([r(`https://www.youtube.com/oembed?url=${encodeURIComponent(a)}&format=json`,{headers:{accept:"application/json"}}).catch(()=>null),r(a,{headers:{accept:"text/html,application/xhtml+xml","accept-language":"en-US,en;q=0.9","user-agent":"Mozilla/5.0 Knowgrph YouTube transcript importer"}})]),c=s?.ok?await s.json().catch(()=>({})):{},l=i.ok?Bc(await i.text()):null,p=re(c.title)||re(l?.videoDetails?.title)||`YouTube ${o}`,m=re(c.author_name)||re(l?.videoDetails?.author),f=re(c.thumbnail_url)||`https://i.ytimg.com/vi/${o}/hqdefault.jpg`;if(!i.ok)return Vr({videoId:o,sourceUrl:a,title:p,authorName:m,thumbnailUrl:f,lang:t,languageCode:t,segments:[],captionStatus:`watch-fetch-${i.status}`});let A=l?.captions?.playerCaptionsTracklistRenderer?.captionTracks||[],S=Wc(Array.isArray(A)?A:[],t);if(!S?.baseUrl)return Vr({videoId:o,sourceUrl:a,title:p,authorName:m,thumbnailUrl:f,lang:t,languageCode:t,segments:[],captionStatus:"captions-unavailable"});let O=await r(Fc(S.baseUrl),{headers:{accept:"application/json,text/xml,text/plain,*/*","user-agent":"Mozilla/5.0 Knowgrph YouTube transcript importer"}}).catch(()=>null),$=O?await O.text().catch(()=>""):"",N=O?.ok?Yc($,O.headers.get("content-type")):[],F=N.length>0?"available":O?.ok?"captions-empty":`captions-fetch-${O?.status||"failed"}`;return Vr({videoId:o,sourceUrl:a,title:p,authorName:m,thumbnailUrl:f,lang:t,languageCode:S.languageCode,segments:N,captionStatus:F})}n(Zc,"buildYouTubeTranscriptPayload");async function Ea(e){let t=e.request,r=String(t.method||"GET").toUpperCase();if(r==="OPTIONS")return new Response(null,{status:204,headers:xa});if(r!=="GET"&&r!=="HEAD"&&r!=="POST")return Zt({ok:!1,error:"unsupported_method"},405,r);let o=new URL(t.url),a=re(o.searchParams.get("url")),s=re(o.searchParams.get("lang"))||"en";if(!a)return Zt({ok:!1,error:"missing_url"},400,r);try{let i=await Zc({sourceUrl:a,lang:s});return Zt(i,i.ok?200:502,r)}catch(i){let c=i&&typeof i=="object"&&"message"in i?re(i.message):"";return Zt({ok:!1,error:c||"youtube_conversion_failed"},502,r)}}n(Ea,"onRequest");async function Ca(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase();if(r==="OPTIONS")return new Response(null,{status:204,headers:{...ce(t),"access-control-allow-methods":"GET, HEAD, OPTIONS","access-control-allow-headers":"*","access-control-max-age":"86400"}});if(r!=="GET"&&r!=="HEAD")return new Response(JSON.stringify({ok:!1,error:"unsupported_method"}),{status:405,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store",...ce(t)}});let o={ok:!0,service:"singabldr-pages",ts:new Date().toISOString()},a={"content-type":"application/json; charset=utf-8","cache-control":"no-store",...ce(t)};return r==="HEAD"?new Response(null,{status:200,headers:a}):new Response(JSON.stringify(o),{status:200,headers:a})}n(Ca,"onRequest");var Le="https://airvio.co";var ae="/knowgrph",Ue=`${Le}${ae}/`,Ta=`${Le}/`;var Oa=`${ae}/health`,va=`${Le}${Oa}`,Ia="/.well-known/agent-card.json",Rm=`${ae}/.well-known/agent-card.json`,el=`${Le}${Ia}`,tl=`${Le}/api/storage/source-files`,rl=`${Le}/api/storage/doc-default/{canonicalPath}`,ol=`${Le}/api/storage/doc/{workspaceId}/{canonicalPath}`,nl=`${Le}/api/storage/blob/{workspaceId}/{canonicalPath}`;var Yr="root-agent-ready-pages",Ma=['</.well-known/api-catalog>; rel="api-catalog"; type="application/linkset+json"',`<${ae}/.well-known/openapi.json>; rel="service-desc"; type="application/vnd.oai.openapi+json;version=3.1"`,`<${ae}/llms.txt>; rel="service-doc"; type="text/plain"`,'</auth.md>; rel="service-doc"; type="text/markdown"',`<${Oa}>; rel="status"; type="application/health+json"`,`<${ae}/.well-known/mcp/server-card.json>; rel="mcp-server-card"; type="application/json"`,`<${Ia}>; rel="describedby"; type="application/json"`].join(", "),Na=`# Knowgrph
...
- Auth.md registration instructions: ${Ta}auth.md
- Health: ${va}
...
- Live Canvas Hero discovery markdown: ${Ta}knowgrph-live-canvas-hero.md

## APIs

- Agent-ready status: ${va}
- HTTP MCP: ${Ue}mcp
- Storage API: ${Le}/api/storage/
- Source Files index: ${tl}
- Default Source File documents: ${rl}
- Workspace Source File documents: ${ol}
- Workspace binary artifacts: ${nl}
...
- The View initiates the MCP Apps \`ui/initialize\` handshake, sends \`ui/notifications/initialized\` and \`ui/notifications/size-changed\`, handles host context/tool input/result/cancel notifications, and calls the originating server through \`tools/call\`.
- \`inspect_agent_surface.structuredContent.mcpAppsServerReadiness\` exposes the native server-readiness model used by the View: app tool/resource binding, prompt discovery, resource-template discovery, output-schema and structured-content readiness, sandbox/security metadata, widget accessibility, Streamable HTTP JSON-RPC transport, local stdio transport, and read-only search/fetch retrieval.
`,Ua=n(e=>new Response(e,{status:200,headers:{"content-type":"text/markdown; charset=utf-8","cache-control":"public, max-age=3600","access-control-allow-origin":"*",vary:"Accept","x-markdown-tokens":String(Math.ceil(String(e||"").length/4))}}),"markdownResponse"),La=n(e=>(e.headers.get("accept")||"").toLowerCase().split(",").some(r=>r.trim().startsWith("text/markdown")),"wantsMarkdown"),Jr=n((e,t)=>{let r=new Response(e.body,e),o=String(t?.owner||"").trim(),a=String(t?.tag||"").trim();return o&&r.headers.set("x-knowgrph-route-owner",o),a&&r.headers.set("x-knowgrph-route-tag",a),r},"withAgentReadyRouteHeaders");var Qr="Agent-actionable chat-to-canvas knowledge graph workspace",Ka='<main id="root"></main>',al=/<(?:main|div)\s+id=["']root["']\s*><\/(?:main|div)>/i,sl=n(e=>{let t=/<script>([\s\S]*?)<\/script>/g;for(let r of String(e||"").matchAll(t)){let o=r[1]||"";if(o.includes("createWebMcpLifecycleController")&&o.includes("toolDefinitions"))return o}return""},"extractWebMcpScript"),$a=n(()=>({"content-type":"text/html; charset=utf-8","cache-control":"no-store, no-cache, no-transform, must-revalidate, max-age=0","access-control-allow-origin":"*",link:Ma}),"rootHtmlHeaders"),il=n(()=>`<noscript><a href="${ae}/">Enter Knowgrph</a></noscript>`,"rootNoscriptFallbackMarkup"),Xr=n((e,t)=>String(e||"").includes("</head>")?String(e||"").replace("</head>",`${t}</head>`):`${String(e||"")}${t}`,"injectIntoHead"),cl=n(e=>{let t=String(e||"").replace(al,Ka);return/<meta\s+name=["']description["'][^>]*>/i.test(t)?t=t.replace(/<meta\s+name=["']description["'][^>]*>/i,`<meta name="description" content="${Qr}" />`):t=Xr(t,`    <meta name="description" content="${Qr}" />
`),/<link\s+rel=["']canonical["'][^>]*>/i.test(t)||(t=Xr(t,`    <link rel="canonical" href="${ae}/" />
`)),/<meta\s+name=["']x-knowgrph-root-alias["'][^>]*>/i.test(t)||(t=Xr(t,`    <meta name="x-knowgrph-root-alias" content="${ae}/" />
`)),t},"rewriteRootAppHtml"),ll=n(async e=>{let t=new URL(`${ae}/?agentReadyRootWebMcp=1`,e.url),r=await fetch(t,{headers:{accept:"text/html"}});return r.ok?sl(await r.text()):""},"loadWebMcpScript"),pl=n(async e=>{let t=new URL(`${ae}/?agentReadyRootAlias=1`,e.url),r=await fetch(t,{headers:{accept:"text/html"}});if(!r.ok)return null;let o=cl(await r.text());return!o.includes(Ka)||!o.includes(`${ae}/assets/`)?null:new Response(o,{status:200,headers:$a()})},"loadKnowgrphAppShell"),ul=n((e="")=>new Response(`<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
</html>`,{status:200,headers:$a()}),"rootHtmlResponse");async function Da(e){let{request:t}=e,r=String(t.method||"GET").toUpperCase();if(r!=="GET"&&r!=="HEAD")return e.next();if(La(t)){let s=Jr(Ua(Na),{owner:Yr,tag:"root-homepage-markdown"});return r==="HEAD"?new Response(null,s):s}let o=r==="HEAD"?null:await pl(t),a=Jr(o||ul(r==="HEAD"?"":await ll(t)),{owner:Yr,tag:"root-homepage-html"});return r==="HEAD"?new Response(null,a):a}n(Da,"onRequest");var _=[{routePath:"/api/llm/chat/completions",mountPath:"/api/llm/chat",method:"",middlewares:[],modules:[ro]},{routePath:"/api/payments/commerce/x402",mountPath:"/api/payments/commerce",method:"",middlewares:[],modules:[ko]},{routePath:"/api/llm/models",mountPath:"/api/llm",method:"",middlewares:[],modules:[xo]},{routePath:"/api/llm/responses",mountPath:"/api/llm",method:"",middlewares:[],modules:[Eo]},{routePath:"/knowgrph/doc-default/:path*",mountPath:"/knowgrph/doc-default",method:"",middlewares:[],modules:[Zn]},{routePath:"/knowgrph/doc/:path*",mountPath:"/knowgrph/doc",method:"",middlewares:[],modules:[ea]},{routePath:"/knowgrph/share/:path*",mountPath:"/knowgrph/share",method:"",middlewares:[],modules:[ta]},{routePath:"/api/link-preview",mountPath:"/api",method:"GET",middlewares:[],modules:[oa]},{routePath:"/api/link-proxy",mountPath:"/api",method:"GET",middlewares:[],modules:[aa]},{routePath:"/api/graph",mountPath:"/api",method:"",middlewares:[],modules:[ga]},{routePath:"/api/oembed",mountPath:"/api",method:"",middlewares:[],modules:[wa]},{routePath:"/__chat_proxy/:path*",mountPath:"/__chat_proxy",method:"",middlewares:[],modules:[_a]},{routePath:"/knowgrph/:path*",mountPath:"/knowgrph",method:"",middlewares:[],modules:[He]},{routePath:"/__video_frame",mountPath:"/",method:"",middlewares:[],modules:[ka]},{routePath:"/__youtube_transcript",mountPath:"/",method:"",middlewares:[],modules:[Ea]},{routePath:"/health",mountPath:"/",method:"",middlewares:[],modules:[Ca]},{routePath:"/",mountPath:"/",method:"",middlewares:[],modules:[Da]}];function dl(e){for(var t=[],r=0;r<e.length;){var o=e[r];if(o==="*"||o==="+"||o==="?"){t.push({type:"MODIFIER",index:r,value:e[r++]});continue}if(o==="\\"){t.push({type:"ESCAPED_CHAR",index:r++,value:e[r++]});continue}if(o==="{"){t.push({type:"OPEN",index:r,value:e[r++]});continue}if(o==="}"){t.push({type:"CLOSE",index:r,value:e[r++]});continue}if(o===":"){for(var a="",s=r+1;s<e.length;){var i=e.charCodeAt(s);if(i>=48&&i<=57||i>=65&&i<=90||i>=97&&i<=122||i===95){a+=e[s++];continue}break}if(!a)throw new TypeError("Missing parameter name at ".concat(r));t.push({type:"NAME",index:r,value:a}),r=s;continue}if(o==="("){var c=1,l="",s=r+1;if(e[s]==="?")throw new TypeError('Pattern cannot start with "?" at '.concat(s));for(;s<e.length;){if(e[s]==="\\"){l+=e[s++]+e[s++];continue}if(e[s]===")"){if(c--,c===0){s++;break}}else if(e[s]==="("&&(c++,e[s+1]!=="?"))throw new TypeError("Capturing groups are not allowed at ".concat(s));l+=e[s++]}if(c)throw new TypeError("Unbalanced pattern at ".concat(r));if(!l)throw new TypeError("Missing pattern at ".concat(r));t.push({type:"PATTERN",index:r,value:l}),r=s;continue}t.push({type:"CHAR",index:r,value:e[r++]})}return t.push({type:"END",index:r,value:""}),t}n(dl,"lexer");function ml(e,t){t===void 0&&(t={});for(var r=dl(e),o=t.prefixes,a=o===void 0?"./":o,s=t.delimiter,i=s===void 0?"/#?":s,c=[],l=0,p=0,m="",f=n(function(U){if(p<r.length&&r[p].type===U)return r[p++].value},"tryConsume"),A=n(function(U){var L=f(U);if(L!==void 0)return L;var V=r[p],se=V.type,Se=V.index;throw new TypeError("Unexpected ".concat(se," at ").concat(Se,", expected ").concat(U))},"mustConsume"),S=n(function(){for(var U="",L;L=f("CHAR")||f("ESCAPED_CHAR");)U+=L;return U},"consumeText"),O=n(function(U){for(var L=0,V=i;L<V.length;L++){var se=V[L];if(U.indexOf(se)>-1)return!0}return!1},"isSafe"),$=n(function(U){var L=c[c.length-1],V=U||(L&&typeof L=="string"?L:"");if(L&&!V)throw new TypeError('Must have text between two parameters, missing text after "'.concat(L.name,'"'));return!V||O(V)?"[^".concat(je(i),"]+?"):"(?:(?!".concat(je(V),")[^").concat(je(i),"])+?")},"safePattern");p<r.length;){var N=f("CHAR"),F=f("NAME"),Q=f("PATTERN");if(F||Q){var z=N||"";a.indexOf(z)===-1&&(m+=z,z=""),m&&(c.push(m),m=""),c.push({name:F||l++,prefix:z,suffix:"",pattern:Q||$(z),modifier:f("MODIFIER")||""});continue}var x=N||f("ESCAPED_CHAR");if(x){m+=x;continue}m&&(c.push(m),m="");var T=f("OPEN");if(T){var z=S(),te=f("NAME")||"",B=f("PATTERN")||"",le=S();A("CLOSE"),c.push({name:te||(B?l++:""),pattern:te&&!B?$(z):B,prefix:z,suffix:le,modifier:f("MODIFIER")||""});continue}A("END")}return c}n(ml,"parse");function _t(e,t){var r=[],o=ja(e,r,t);return hl(o,r,t)}n(_t,"match");function hl(e,t,r){r===void 0&&(r={});var o=r.decode,a=o===void 0?function(s){return s}:o;return function(s){var i=e.exec(s);if(!i)return!1;for(var c=i[0],l=i.index,p=Object.create(null),m=n(function(A){if(i[A]===void 0)return"continue";var S=t[A-1];S.modifier==="*"||S.modifier==="+"?p[S.name]=i[A].split(S.prefix+S.suffix).map(function(O){return a(O,S)}):p[S.name]=a(i[A],S)},"_loop_1"),f=1;f<i.length;f++)m(f);return{path:c,index:l,params:p}}}n(hl,"regexpToFunction");function je(e){return e.replace(/([.+*?=^!:${}()[\]|/\\])/g,"\\$1")}n(je,"escapeString");function Ha(e){return e&&e.sensitive?"":"i"}n(Ha,"flags");function gl(e,t){if(!t)return e;for(var r=/\((?:\?<(.*?)>)?(?!\?)/g,o=0,a=r.exec(e.source);a;)t.push({name:a[1]||o++,prefix:"",suffix:"",modifier:"",pattern:""}),a=r.exec(e.source);return e}n(gl,"regexpToRegexp");function fl(e,t,r){var o=e.map(function(a){return ja(a,t,r).source});return new RegExp("(?:".concat(o.join("|"),")"),Ha(r))}n(fl,"arrayToRegexp");function yl(e,t,r){return wl(ml(e,r),t,r)}n(yl,"stringToRegexp");function wl(e,t,r){r===void 0&&(r={});for(var o=r.strict,a=o===void 0?!1:o,s=r.start,i=s===void 0?!0:s,c=r.end,l=c===void 0?!0:c,p=r.encode,m=p===void 0?function(L){return L}:p,f=r.delimiter,A=f===void 0?"/#?":f,S=r.endsWith,O=S===void 0?"":S,$="[".concat(je(O),"]|$"),N="[".concat(je(A),"]"),F=i?"^":"",Q=0,z=e;Q<z.length;Q++){var x=z[Q];if(typeof x=="string")F+=je(m(x));else{var T=je(m(x.prefix)),te=je(m(x.suffix));if(x.pattern)if(t&&t.push(x),T||te)if(x.modifier==="+"||x.modifier==="*"){var B=x.modifier==="*"?"?":"";F+="(?:".concat(T,"((?:").concat(x.pattern,")(?:").concat(te).concat(T,"(?:").concat(x.pattern,"))*)").concat(te,")").concat(B)}else F+="(?:".concat(T,"(").concat(x.pattern,")").concat(te,")").concat(x.modifier);else{if(x.modifier==="+"||x.modifier==="*")throw new TypeError('Can not repeat "'.concat(x.name,'" without a prefix and suffix'));F+="(".concat(x.pattern,")").concat(x.modifier)}else F+="(?:".concat(T).concat(te,")").concat(x.modifier)}}if(l)a||(F+="".concat(N,"?")),F+=r.endsWith?"(?=".concat($,")"):"$";else{var le=e[e.length-1],U=typeof le=="string"?N.indexOf(le[le.length-1])>-1:le===void 0;a||(F+="(?:".concat(N,"(?=").concat($,"))?")),U||(F+="(?=".concat(N,"|").concat($,")"))}return new RegExp(F,Ha(r))}n(wl,"tokensToRegexp");function ja(e,t,r){return e instanceof RegExp?gl(e,t):Array.isArray(e)?fl(e,t,r):yl(e,t,r)}n(ja,"pathToRegexp");var er=/[.+?^${}()|[\]\\]/g;function*Sl(e){let t=new URL(e.url).pathname;for(let r of[..._].reverse()){if(r.method&&r.method!==e.method)continue;let o=_t(r.routePath.replace(er,"\\$&"),{end:!1}),a=_t(r.mountPath.replace(er,"\\$&"),{end:!1}),s=o(t),i=a(t);if(s&&i)for(let c of r.middlewares.flat())yield{handler:c,params:s.params,path:i.path}}for(let r of _){if(r.method&&r.method!==e.method)continue;let o=_t(r.routePath.replace(er,"\\$&"),{end:!0}),a=_t(r.mountPath.replace(er,"\\$&"),{end:!1}),s=o(t),i=a(t);if(s&&i&&r.modules.length){for(let c of r.modules.flat())yield{handler:c,params:s.params,path:s.path};break}}}n(Sl,"executeRequest");var Qm={async fetch(e,t,r){let o=e,a=Sl(o),s={},i=!1,c=n(async(l,p)=>{if(l!==void 0){let f=l;typeof l=="string"&&(f=new URL(l,o.url).toString()),o=new Request(f,p)}let m=a.next();if(m.done===!1){let{handler:f,params:A,path:S}=m.value,O={request:new Request(o.clone()),functionPath:S,next:c,params:A,get data(){return s},set data(N){if(typeof N!="object"||N===null)throw new Error("context.data must be an object");s=N},env:t,waitUntil:r.waitUntil.bind(r),passThroughOnException:n(()=>{i=!0},"passThroughOnException")},$=await f(O);if(!($ instanceof Response))throw new Error("Your Pages function should return a Response");return Zr($)}else{let f=await t.ASSETS.fetch(o);return Zr(f)}},"next");try{return await c()}catch(l){if(i){let p=await t.ASSETS.fetch(o);return Zr(p)}throw l}}},Zr=n(e=>new Response([101,204,205,304].includes(e.status)?null:e.body,e),"cloneResponse");export{Qm as default};
